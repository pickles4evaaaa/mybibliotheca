...
def run_regression_suite():
    ...
    resp = client.put(
        "/api/v1/books/book-1", json=update_payload, headers=_auth_headers()
    )
    ...
    resp = client.post(
        "/api/v1/reading-logs",
        json={"book_id": "book-1", "date": "2024-02-02"},
        headers=_auth_headers(),
    )
    results.append(("reading_log_create", resp.status_code == 201))

    resp = client.post(
        "/api/v1/reading-logs/check",
        json={"book_id": "book-existing", "date": "2024-02-02"},
        headers=_auth_headers(),
    )
    payload = resp.get_json() if resp.is_json else {}
    results.append(
        ("reading_log_check", resp.status_code == 200 and payload.get("exists") is True)
    )

    resp = client.get(
        "/api/v1/users/me", headers=_auth_headers(), follow_redirects=True
    )
    results.append(("users_me", resp.status_code == 200))

    resp = client.put(
        "/api/v1/users/me",
        json={"share_library": False},
        headers=_auth_headers(),
        follow_redirects=True,
    )
    results.append(
        ("users_update", resp.status_code == 200 and stubs["user_stub"].updated is True)
    )

    resp = client.get(
        "/api/v1/users/user-123", headers=_auth_headers(), follow_redirects=True
    )
