...
class Config:
    ...
    if not SECRET_KEY:
        ...
        if os.environ.get("FLASK_ENV") == "development" or os.environ.get(
            ...
            print(
                "⚠️  WARNING: Using temporary SECRET_KEY for development. Set SECRET_KEY in .env for production!"
            )
        else:
            ...
            raise ValueError(
                "No SECRET_KEY set for Flask application. Please set it in your .env file."
            )
    ...
    GRAPH_DATABASE_ENABLED = (
        os.environ.get("GRAPH_DATABASE_ENABLED", "true").lower() == "true"
    )
    ...
    MAIL_DEFAULT_SENDER = os.environ.get(
        "MAIL_DEFAULT_SENDER", "noreply@bibliotheca.local"
    )
    ...
    ADMIN_PASSWORD = os.environ.get(
        "ADMIN_PASSWORD"
    )  # No default - must be set via env var

    # API Authentication
    # For development, use a simple test token. In production, implement proper token management.
    API_TEST_TOKEN = os.environ.get(
        "API_TEST_TOKEN"
    )  # No default - must be set via env var

    # Debug settings (disabled by default for security)
    DEBUG_MODE = os.environ.get("MYBIBLIOTHECA_DEBUG", "false").lower() in [
        "true",
        "on",
        "1",
    ]
    DEBUG_CSRF = os.environ.get("MYBIBLIOTHECA_DEBUG_CSRF", "false").lower() in [
        "true",
        "on",
        "1",
    ]
    DEBUG_SESSION = os.environ.get("MYBIBLIOTHECA_DEBUG_SESSION", "false").lower() in [
        "true",
        "on",
        "1",
    ]
    DEBUG_AUTH = os.environ.get("MYBIBLIOTHECA_DEBUG_AUTH", "false").lower() in [
        "true",
        "on",
        "1",
    ]
    DEBUG_REQUESTS = os.environ.get(
        "MYBIBLIOTHECA_DEBUG_REQUESTS", "false"
    ).lower() in ["true", "on", "1"]
