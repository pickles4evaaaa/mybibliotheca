...
def _initialize_default_templates():
    ...
    try:
        ...
        global \
            _template_creation_disabled, \
            _template_creation_failures, \
            _template_creation_last_attempt

        # Check if verbose logging is enabled (default off)
        verbose_init = (
            os.getenv("MYBIBLIOTHECA_VERBOSE_INIT", "false").lower() == "true"
        )
        ...
        if _template_creation_disabled:
            if verbose_init:
                ...
                print(
                    "   The application will continue without default import templates"
                )
            return
        ...
        if _template_creation_last_attempt and _template_creation_failures >= 3:
            ...
            if time_since_last < 300:  # Don't retry for 5 minutes after 3 failures
                if verbose_init:
                    print(
                        f"⏳ Skipping template initialization - {_template_creation_failures} recent failures"
                    )
                return
            else:
        ...
        try:
            goodreads_template = import_mapping_service.get_template_by_id_sync(
                "default_goodreads"
            )
            storygraph_template = import_mapping_service.get_template_by_id_sync(
                "default_storygraph"
            )
            ...
            try:
                ...
                goodreads_template = ImportMappingTemplate(
                    ...
                    field_mappings={
                        ...
                        "Additional Authors": {
                            "action": "map_existing",
                            "target_field": "additional_authors",
                        },
                        ...
                        "Original Publication Year": {
                            "action": "map_existing",
                            "target_field": "original_publication_year",
                        },
                        "Date Read": {
                            "action": "map_existing",
                            "target_field": "date_read",
                        },
                        "Date Added": {
                            "action": "map_existing",
                        ...
                        "Bookshelves": {
                            "action": "map_existing",
                            "target_field": "custom_global_bookshelves",
                        },
                        "Exclusive Shelf": {
                            "action": "map_existing",
                            "target_field": "reading_status",
                        },
                        "My Review": {
                            "action": "map_existing",
                            "target_field": "notes",
            ...
            try:
                storygraph_template = ImportMappingTemplate(
                    ...
                    field_mappings={
                        ...
                        "Dates Read": {
                            "action": "map_existing",
                            "target_field": "date_ranges",
                        },
                        "Read Count": {
                            "action": "map_existing",
                            "target_field": "read_count",
                        },
                        "Star Rating": {
                            "action": "map_existing",
                            "target_field": "rating",
...
def create_app():
    ...
    with app.app_context():
        ...
        try:
            ...
            mig_summary = run_series_migration(
                verbose=os.getenv("MYBIBLIOTHECA_VERBOSE_INIT", "false").lower()
                == "true"
            )
        ...
        if "CSRF" in str(e) or "csrf" in str(e.description):
            ...
            if request.is_json or "application/json" in request.headers.get(
                ...
                if request.endpoint and request.endpoint.startswith("onboarding."):
                    flash(
                        "Security token expired. The page will be refreshed with a new token.",
                        "warning",
                    )
                    ...
                    if request.view_args and "step" in request.view_args:
                        return redirect(
                            url_for(
                                "onboarding.step", step_num=request.view_args["step"]
                            )
                        )
                    else:
    ...
    with app.app_context():
        ...
        try:
            ...
            if verbose_init:
                ...
                try:
                    user_result = safe_execute_query(
                        "MATCH (u:User) RETURN COUNT(u) AS count", {}
                    )
                    book_result = safe_execute_query(
                    ...
                    if verbose_init:
                        print(
                            f"📊 Database state: Users: {user_count}, Books: {book_count}"
                        )
            ...
            if "Could not set lock on file" in str(e):
                if verbose_init:
                    print(
                        "⏳ KuzuDB connection deferred - will initialize on first request"
                    )
                kuzu_init_attempted = (
                    False  # Mark as not attempted due to lock conflict
                )
            else:
        ...
        if kuzu_init_attempted:
            ...
            try:
                ...
                try:
                    ...
                    if backup_service.check_restart_required():
                        ...
                        if verbose_init:
                            print(
                                "🔄 Restart flag cleared after successful restoration"
                            )
            ...
            if verbose_init:
                print(
                    "⚠️ Services initialization deferred - will initialize on first request"
                )
    ...
    def check_setup_and_password_requirements():
        ...
        if request.endpoint == "auth.setup":
            ...
            debug_auth(
                f"Session keys before processing: {list(session.keys()) if 'session' in globals() else 'No session'}"
            )
            ...
            if request.method == "GET":
                if "csrf_token" not in session:
                    ...
                    debug_csrf(
                        "🔧 No CSRF token in session for setup GET. Generating one now."
                    )
                    generate_csrf()
                    # Explicitly mark the session as modified to ensure it's saved
                    session.modified = True
                    debug_csrf(
                        f"🔧 Session marked as modified. Keys: {list(session.keys())}"
                    )
        ...
        if request.endpoint and request.endpoint.startswith("onboarding."):
            ...
            if request.method == "GET":
                ...
                if "csrf_token" not in session:
                    ...
                    debug_csrf(
                        "🔧 No CSRF token in session for onboarding GET. Generating one now."
                    )
                    generate_csrf()
                    session.modified = True
                    debug_csrf(
                        f"🔧 Session marked as modified for onboarding. Keys: {list(session.keys())}"
                    )
                ...
                if "csrf_token" in request.form and "csrf_token" in session:
                    ...
                    debug_auth(
                        f"Form CSRF: {form_csrf[:10] if form_csrf else 'None'}..."
                    )
                    debug_auth(
                        f"Session CSRF: {session_csrf[:10] if session_csrf else 'None'}..."
                    )
                    from flask_wtf.csrf import generate_csrf
        ...
        if request.endpoint and request.endpoint.startswith("onboarding."):
            debug_auth(
                f"🔍 BEFORE_REQUEST: Onboarding route detected: {request.endpoint}"
            )
            if request.method == "GET":
                if "csrf_token" not in session:
                    from flask_wtf.csrf import generate_csrf
        ...
        try:
            ...
            debug_auth(
                f"Before request user count check: {user_count} for endpoint: {request.endpoint}"
            )
            ...
            if user_count == 0:
                ...
                if (
                    request.endpoint in allowed_when_no_users
                    or (
                        request.endpoint
                        and (
                            request.endpoint.startswith("static")
                            or request.endpoint.startswith("onboarding.")
                            or request.endpoint.startswith("genre_taxonomy.")
                        )
                    )
                    ...
                    debug_auth(
                        f"Skipping setup redirect for allowed endpoint: {request.endpoint}"
                    )
                    pass
                else:
                    # Redirect to setup page
                    debug_auth(
                        f"No users found, redirecting to setup from: {request.endpoint}"
                    )
                    return redirect(url_for("auth.setup"))
            else:
                debug_auth(
                    f"Users exist ({user_count}), allowing access to: {request.endpoint}"
                )
        except Exception as e:
            ...
            if not (
                ...
                debug_auth(
                    f"User count check failed, redirecting to setup from: {request.endpoint}"
                )
                return redirect(url_for("auth.setup"))
        ...
        if (
            hasattr(current_user, "password_must_change")
            and current_user.password_must_change
        ):
    ...
    def serve_static(filename):
        ...
        if os.path.exists(pkg_path):
            return _with_cache_headers(
                send_from_directory(package_static_dir, filename)
            )
    ...
    def shutdown_handler():
        ...
        try:
            print(
                f"🛑 [APP_SHUTDOWN] Application shutting down - Container: {os.getenv('HOSTNAME', 'unknown')}"
            )
            ...
            try:
                ...
                if is_safe_kuzu_initialized():
                    ...
                    try:
                        ...
                        user_count = (
                            user_result[0].get("count", 0) if user_result else 0
                        )
                        book_count = (
                            book_result[0].get("count", 0) if book_result else 0
                        )
                    except Exception:
            ...
            print("🛑 [APP_SHUTDOWN] Final database state:")
            ...
            if db_path.exists():
                ...
                print(
                    f"🛑 [APP_SHUTDOWN] Database files: {len(files)} files, {total_size} bytes total"
                )

            # 🔥 CRITICAL FIX: Close KuzuDB connections properly
            print(
                "🛑 [APP_SHUTDOWN] Closing KuzuDB connections to ensure data persistence..."
            )
            try:
                from .utils.safe_kuzu_manager import (
                    get_safe_kuzu_manager,
                    is_safe_kuzu_initialized,
                )

                if is_safe_kuzu_initialized():
                    manager = get_safe_kuzu_manager()
                    stale_count = manager.cleanup_stale_connections(
                        max_age_minutes=0
                    )  # Clean up all
                    print(
                        f"🛑 [APP_SHUTDOWN] ✅ KuzuDB connections closed ({stale_count} cleaned up)"
                    )
                else:
                    print(
                        "🛑 [APP_SHUTDOWN] Kuzu manager not initialized earlier; skipping connection cleanup"
                    )
            ...
            try:
                ...
                print("🛑 [APP_SHUTDOWN] ✅ KuzuDB connections closed after error")
            except Exception as close_error:
                print(
                    f"🛑 [APP_SHUTDOWN] ❌ Failed to close KuzuDB connection: {close_error}"
                )
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as _pool:
                ...
                try:
                    ...
                    if verbose_probe:
                        print(
                            "[APP] before_first_request: query executed, parsing result ..."
                        )
                    ok_val = None
                    ...
                    if verbose_probe:
                        print(
                            f"[APP] before_first_request: Readiness DB check returned: {ok_val} in {elapsed:.2f}s"
                        )
                except concurrent.futures.TimeoutError:
                    elapsed = time.time() - start_ts
                    if verbose_probe:
                        print(
                            f"[APP] before_first_request: Readiness DB check timed out after {elapsed:.2f}s — continuing without blocking"
                        )
        except Exception as e:
            if verbose_probe:
