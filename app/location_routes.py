...
def edit_location(location_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if updated_location:
                flash(
                    f'Location "{updated_location.name}" updated successfully.',
                    "success",
                )
            else:
...
def delete_location(location_id):
    ...
    try:
        if location_service.delete_location(location_id):
            ...
            flash(
                "Cannot delete location. You must have at least one active location.",
                "error",
            )
    except Exception as e:
...
def view_location(location_id):
    ...
    # Get book count
    book_count = location_service.get_location_book_count(
        location_id, str(current_user.id)
    )

    return render_template(
        "locations/view.html", location=location, books=books, book_count=book_count
    )
...
def api_set_book_location():
    ...
    try:
        ...
        result = location_service.set_book_location(
            book_id, location_id, str(current_user.id)
        )

        if result:
            return jsonify(
                {"success": True, "message": "Book location updated successfully"}
            )
        else:
            return jsonify(
                {"success": False, "error": "Failed to update book location"}
            ), 500
