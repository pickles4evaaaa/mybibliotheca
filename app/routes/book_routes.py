...
def _get_changed_fields(user_book, form):
    ...
    for field in all_fields:
        ...
        current_norm = (
            current_val.strip() if isinstance(current_val, str) else current_val
        )
...
def _handle_personal_only(uid, form):
    ...
    cleaned = {
        k: v for k, v in update_data.items() if v is not None or k == "user_rating"
    }
    try:
        ...
        if success:
            ...
            flash(
                f"Updated {', '.join(names)} successfully."
                if names
                else "Personal data updated successfully.",
                "success",
            )
        else:
...
def _handle_book_only(uid, form):
    ...
    try:
        success = book_service.update_book_sync(
            uid, str(current_user.id), **update_data
        )
        if success:
            label_map = {
                "publisher": "publisher",
            ...
            flash(
                f"Updated {', '.join(names)} successfully."
                if names
                else "Book metadata updated successfully.",
                "success",
            )
        else:
...
def _handle_mixed(uid, form):
    ...
    try:
        b_ok = True
        if book_update_data:
            b_ok = book_service.update_book_sync(
                uid, str(current_user.id), **book_update_data
            )
        p_ok = True
        if personal_update_data:
            p_ok = book_service.update_book_sync(
                uid, str(current_user.id), **personal_update_data
            )
        if b_ok and p_ok:
            updated = []
...
def _handle_edit_book_post(uid, user_book, form):
    ...
    has_category_fields = any(
        k == "categories" or k.startswith("categories[") for k in form_keys
    )
    ...
    if not collected:
        for key in potential_keys:
            ...
            if raw_value:
                fragments = [
                    fragment.strip()
                    for fragment in raw_value.replace(",", "\n").split("\n")
                    if fragment.strip()
                ]
                if fragments:
...
def _convert_published_date_to_date(published_date_str):
    ...
    try:
        ...
        current_app.logger.warning(
            f"Failed to parse published date '{published_date_str}': {e}"
        )
        return None
...
def fetch_book(isbn):
    ...
    try:
        ...
        if book_data and not book_data.get("isbn"):
            ...
            book_data["isbn"] = (
                book_data.get("isbn13") or book_data.get("isbn10") or isbn
            )
        ...
        if not book_data.get("author") and book_data.get("authors"):
            if isinstance(book_data["authors"], list):
                book_data["author"] = ", ".join(
                    [a for a in book_data["authors"] if isinstance(a, str)]
                )
            else:
        ...
        try:
            ...
            current_app.logger.debug(
                f"Best cover helper failed (unified path) for {isbn}: {ce}"
            )
        ...
        if not cover_url and (
            book_data.get("title")
            and (book_data.get("author") or book_data.get("authors"))
        ):
            try:
                ...
                search_results = fetch_unified_by_title(
                    str(title_for_search), max_results=4, author=author_for_search
                )
                for sr in search_results or []:
                    ...
                    if c:
                        ...
                        book_data["cover_source"] = (
                            sr.get("cover_source") or "TitleFallback"
                        )
                        cover_url = c
                        current_app.logger.info(
                            f"[COVER][ISBN_FALLBACK] Acquired via title/author search isbn={isbn}"
                        )
                        break
            except Exception as fb_err:
                current_app.logger.debug(
                    f"[COVER][ISBN_FALLBACK_FAIL] isbn={isbn} err={fb_err}"
                )
        ...
        if cover_url and not async_requested:
            try:
                ...
                current_app.logger.error(
                    f"[COVER][UNIFIED_FETCH] Failed cover service processing: {e}"
                )
            ...
            cand = cover_service.select_candidate(
                isbn=isbn, title=book_data.get("title"), author=book_data.get("author")
            )
            if cand:
                book_data["cover_candidate"] = cand
                book_data["cover_url"] = cand.get("url")
            job = cover_service.schedule_async_processing(
                isbn=isbn, title=book_data.get("title"), author=book_data.get("author")
            )
        ...
        if book_data.get("published_date"):
            ...
            if formatted_date:
                ...
                print(
                    f"📅 [FETCH_BOOK] Formatted published_date: '{original_date}' -> '{formatted_date}'"
                )

        # Debug logging
        print(
            f"🔍 [FETCH_BOOK] Unified data keys for ISBN {isbn}: {list(book_data.keys())}"
        )
        return jsonify(book_data), 200 if book_data else 404
    except Exception as e:
        # Fallback to legacy behavior if unified fails
        current_app.logger.warning(
            f"Unified fetch failed for {isbn}: {e}. Falling back to legacy."
        )
        ...
        try:
            ...
            current_app.logger.debug(
                f"Best cover helper failed (fallback path) for {isbn}: {ce}"
            )
            from app.utils.image_processing import process_image_from_url
                ...
                try:
                    ...
                    if processed_cover:
                        if processed_cover.startswith("/"):
                            book_data["cover"] = (
                                request.host_url.rstrip("/") + processed_cover
                            )
                        else:
                            book_data["cover"] = processed_cover
                except Exception as e:
                    current_app.logger.warning(
                        f"Failed to process cover image: {cover_url} ({e})"
                    )
    ...
    try:
        ...
        def _summary():
            ...
            for label, t in _marks:
                parts.append(
                    f"{label}+{(t - base) * 1000:.1f}ms(Δ{(t - last) * 1000:.1f}ms)"
                )
                last = t
        ...
        if not data.get("author") and data.get("authors"):
            if isinstance(data["authors"], list):
                data["author"] = ", ".join(
                    [a for a in data["authors"] if isinstance(a, str)]
                )
        ...
        if async_mode:
            cand = cover_service.select_candidate(
                isbn=isbn, title=data.get("title"), author=data.get("author")
            )
            if cand:
        ...
        try:
            current_app.logger.info(
                f"[BOOK][QUICK_ISBN][PERF] isbn={isbn} {_summary()}"
            )
        except Exception:
            pass
...
def add_book():
    ...
    if request.method == "GET":
        ...
        try:
            personal_fields = custom_field_service.get_available_fields_sync(
                current_user.id, is_global=False
            )
            global_fields = custom_field_service.get_available_fields_sync(
                current_user.id, is_global=True
            )
        except Exception as e:
    ...
    try:
        ...
        if raw_cat_val:
            try:
                ...
                raw_categories = [
                    s.strip() for s in raw_cat_val.split(",") if s.strip()
                ]
        ...
        try:
            ...
            if isbn13 or isbn10:
                ...
                results = book_service.search_user_books_sync(
                    current_user.id, search_isbn, limit=1
                )
                if results:
            ...
            current_app.logger.warning(
                f"Could not retrieve book ID after creation: {e}"
            )
            book_id = None
        ...
        return jsonify(
            {"success": True, "message": f'Added "{title}"', "book_id": book_id}
        )

    except Exception as e:
        current_app.logger.error(f"Fast add error: {type(e).__name__}")
        return jsonify(
            {"success": False, "message": "An error occurred while adding the book"}
        ), 500
    ...
    try:
        ...
        if book_data and "title" in book_data:
            ...
            try:
                ...
                elif book_data.get("publication_year") and not book_data.get(
                    "published_date"
                ):
                ...
                raw_isbn = (
                    book_data.get("isbn")
                    or book_data.get("isbn13")
                    or book_data.get("isbn_13")
                    or book_data.get("isbn10")
                    or book_data.get("isbn_10")
                )
                if raw_isbn:
    ...
    try:
        ...
        if ai_config.get("AI_BOOK_EXTRACTION_ENABLED") != "true":
            ...
            return jsonify(
                {
                    "success": False,
                    "error": "AI book extraction is not enabled. Please contact your administrator.",
                }
            )
        ...
        current_app.logger.info(
            f"File read successfully, size: {len(file_content)} bytes"
        )
        ...
        if book_data:
            ...
            return jsonify(
                {
                    "success": True,
                    "message": "AI extraction successful. Use the title search (magnifying glass) to select the correct edition.",
                    **book_data,
                }
            )
        else:
        ...
        return jsonify(
            {"success": False, "error": f"Failed to process image with AI: {str(e)}"}
        ), 500
...
def search_book_details():
    ...
    try:
        ...
        if not data:
            return jsonify(
                {"success": False, "message": "No search criteria provided"}
            ), 400
        ...
        if not title and not author:
            return jsonify(
                {
                    "success": False,
                    "message": "Please provide at least a title or author to search",
                }
            ), 400

        current_app.logger.info(
            f"Book search request: title='{title}', author='{author}'"
        )
        ...
        ol_query = " ".join(
            [q for q in [title or author, author if (title and author) else None] if q]
        )
        gb_parts = []
        if title:
        ...
        def _fetch_openlibrary():
            try:
                ...
                for doc in docs:
                    ...
                    doc_authors = (
                        doc.get("author_name", [])
                        if isinstance(doc.get("author_name"), list)
                        else [doc.get("author_name", "")]
                    )
                    ...
                    edition_keys = (
                        ...
                        if isinstance(doc.get("edition_key"), list)
                            ...
                            [doc.get("edition_key")] if doc.get("edition_key") else []
                        )
                    )
                    if (not edition_keys) and doc.get("key"):
                        work_path = doc.get("key")
                        try:
                            editions_resp = _req.get(
                                f"https://openlibrary.org{work_path}/editions.json?limit=3",
                                timeout=5,
                            )
                            ...
                            entries = (
                                ...
                                if isinstance(editions_data, dict)
                                else None
                            )
                                        ...
                                        for seq_key in (
                                            ...
                                            if isinstance(seq_val, list):
                                                for candidate in seq_val:
                                                    if (
                                                        candidate
                                                        and candidate not in doc_isbn
                                                    ):
                                                        doc_isbn.append(str(candidate))
                                            elif (
                                                isinstance(seq_val, str)
                                                and seq_val
                                                and seq_val not in doc_isbn
                                            ):
                                                doc_isbn.append(seq_val)
                        except Exception as editions_err:
                            current_app.logger.debug(
                                f"[SEARCH][OpenLibraryWork] Failed editions fetch {work_path}: {editions_err}"
                            )
                    # Fetch edition metadata if ISBN missing from search index
                    edition_payload = {}
                    if (
                        not doc_isbn or not any(len(str(x)) == 13 for x in doc_isbn)
                    ) and edition_keys:
                        for ed_key in edition_keys:
                            ...
                            try:
                                ed_resp = _req.get(
                                    f"https://openlibrary.org/books/{ed_key}.json",
                                    timeout=5,
                                )
                                ...
                                current_app.logger.debug(
                                    f"[SEARCH][OpenLibraryEdition] Failed {ed_key}: {ed_err}"
                                )
                                edition_payload = {}
                            if edition_payload:
                                ...
                                for seq in (doc_isbn, ed_isbn13, ed_isbn10):
                                    for candidate in (
                                        seq if isinstance(seq, list) else []
                                    ):
                                ...
                                break
                    best_isbn = next(
                        (i for i in doc_isbn if isinstance(i, str) and len(i) == 13),
                        (doc_isbn[0] if doc_isbn else None),
                    )
                    cleaned_isbn_list = []
                    ...
                    isbn13_candidate = next(
                        (digits for digits in cleaned_isbn_list if len(digits) == 13),
                        None,
                    )
                    isbn10_candidate = next(
                        (digits for digits in cleaned_isbn_list if len(digits) == 10),
                        None,
                    )
                    subjects_facet = (
                        doc.get("subject_facet")
                        if isinstance(doc.get("subject_facet"), list)
                        else []
                    )
                    subjects = (
                        doc.get("subject")
                    ...
                    combined_subjects = [
                        s for s in (subjects_facet or subjects) if isinstance(s, str)
                    ]
                    raw_category_paths = [
                        s
                        for s in (subjects if subjects else subjects_facet)
                        if isinstance(s, str)
                    ]
                    ...
                    if isinstance(ol_description, list):
                        ol_description = " ".join(
                            [str(item) for item in ol_description]
                        )
                    if not ol_description and isinstance(
                        edition_payload.get("description"), dict
                    ):
                        ol_description = edition_payload["description"].get("value")
                    elif not ol_description and isinstance(
                        edition_payload.get("description"), str
                    ):
                    ...
                    if not doc.get("publisher"):
                        publishers_seq = (
                            edition_payload.get("publishers")
                    ...
                    if doc.get("language"):
                        ...
                        isinstance(edition_payload.get("languages"), list)
                        and edition_payload["languages"]
                    ):
                    ...
                    res = {
                        ...
                        and doc.get("publisher")
                            ...
                            if ed_publishers
                                ...
                                if isinstance(doc.get("publisher"), str)
                                else ""
                            )
                        ),
                        "published_date": published_date,
                        ...
                        if isinstance(ol_description, str)
                        else "",
                    ...
                    if edition_payload:
                        if not res["page_count"] and edition_payload.get(
                            "number_of_pages"
                        ):
                            res["page_count"] = edition_payload.get("number_of_pages")
                        if (not res.get("subtitle")) and edition_payload.get(
                            "subtitle"
                        ):
                        ...
                        if not res.get("isbn13"):
                            ed_isbn13 = (
                                edition_payload.get("isbn_13")
                                if isinstance(edition_payload, dict)
                                else None
                            )
                            if isinstance(ed_isbn13, list) and ed_isbn13:
                                res["isbn13"] = re.sub(
                                    r"[^0-9Xx]", "", str(ed_isbn13[0])
                                )
                        if not res.get("isbn10"):
                            ed_isbn10 = (
                                edition_payload.get("isbn_10")
                                if isinstance(edition_payload, dict)
                                else None
                            )
                            if isinstance(ed_isbn10, list) and ed_isbn10:
                                res["isbn10"] = re.sub(
                                    r"[^0-9Xx]", "", str(ed_isbn10[0])
                                )
                    # Cover selection (non-blocking catch)
                    try:
                        from app.utils.book_utils import get_best_cover_for_book

                        cov = get_best_cover_for_book(
                            isbn=best_isbn,
                            title=doc_title,
                            author=", ".join(doc_authors) if doc_authors else None,
                        )
            ...
            try:
                ...
                for item in items:
                    ...
                    try:
                        ...
                        gb_cover = upgrade_google_cover_url(
                            select_highest_google_image(info.get("imageLinks", {}))
                        )
                    except Exception:
                        pass
                    out.append(
        ...
        provider_timeout = (
            float(current_app.config.get("BOOK_SEARCH_PROVIDER_TIMEOUT", 7.0))
            if current_app
            else 7.0
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as ex:
            ...
            try:
                ...
                current_app.logger.warning(
                    f"[SEARCH] OpenLibrary timed out after {provider_timeout:.1f}s"
                )
                fut_ol.cancel()
                ol_results = []
            except Exception as e:
            ...
            if len(results) >= 8:
                ...
                try:
                    ...
                    current_app.logger.warning(
                        f"[SEARCH] Google Books timed out after {provider_timeout:.1f}s"
                    )
                    fut_gb.cancel()
                except Exception as e:
                    ...
                    gb_results = []
            existing_keys = {
                (r.get("title", "").lower(), r.get("authors", "").lower())
                for r in results
            }
            for r in gb_results:
        ...
        message = (
            f"Found {len(results)} books matching your search"
            if results
            else "No books found matching your search criteria. Try different keywords or check spelling."
        )
        if results and timed_out_providers:
            formatted = ", ".join(
                _format_provider_label(name) for name in timed_out_providers
            )
...
def cover_candidates():
    ...
    try:
        ...
        if not (isbn or title or author):
            return jsonify(
                {"success": False, "message": "Provide isbn or title/author"}
            ), 400
        ...
        cands = (
            get_cover_candidates(
                isbn=isbn or None, title=title or None, author=author or None
            )
            or []
        )
        results = []
            ...
            results.append(
                {
                    ...
                    "authors_list": [
                        a.strip()
                        for a in (author.split(",") if author else [])
                        if a.strip()
                    ],
                    "cover_url": url,
        ...
        return jsonify(
            {
                "success": bool(results),
                "results": results,
                "message": f"Found {len(results)} cover option(s)"
                if results
                else "No cover options found.",
            }
        )
    except Exception as e:
        current_app.logger.error(f"[COVER][CANDIDATES] Failed: {e}")
        return jsonify(
            {"success": False, "message": "Error fetching cover candidates"}
        ), 500
...
def search_books():
    ...
    flash(
        "Search functionality has been moved to the Library page for a better experience.",
        "info",
    )
    return redirect(url_for("main.library"))
...
def month_wrapup():
    ...
    flash(
        "Month Wrap-up has been moved to the Stats page along with other reading statistics.",
        "info",
    )
...
def community_activity():
    ...
    flash(
        "Community activity has been moved to the Stats page along with your personal reading statistics.",
        "info",
    )
    return redirect(url_for("main.stats"))
...
def bulk_import():
    ...
    flash(
        "Book import has been upgraded! You can now map CSV fields and track progress in real-time.",
        "info",
    )
...
def toggle_finished(uid):
    ...
    finish_date = (
        user_book.get("finish_date")
        if isinstance(user_book, dict)
        else getattr(user_book, "finish_date", None)
    )
    if finish_date:
        ...
        book_service.update_book_sync(
            uid, str(current_user.id), finish_date=date.today()
        )
...
def start_reading(uid):
    ...
    start_date = (
        user_book.get("start_date")
        if isinstance(user_book, dict)
        else getattr(user_book, "start_date", None)
    )
    if not start_date:
    ...
    title = (
        user_book.get("title", "Unknown Title")
        if isinstance(user_book, dict)
        else getattr(user_book, "title", "Unknown Title")
    )
...
def update_status(uid):
    ...
    update_data = {"want_to_read": want_to_read, "library_only": library_only}
...
def library():
    ...
    sort_option = (
        raw_sort_option.strip().lower() if isinstance(raw_sort_option, str) else ""
    ) or default_sort
    ...
    has_filter = any(
        [
            ...
            bool(search_query),
            bool(category_filter.strip())
            if isinstance(category_filter, str)
            else False,
            bool(publisher_filter.strip())
            if isinstance(publisher_filter, str)
            else False,
            bool(language_filter.strip())
            if isinstance(language_filter, str)
            else False,
            bool(location_filter.strip())
            if isinstance(location_filter, str)
            else False,
            bool(media_type_filter.strip())
            if isinstance(media_type_filter, str)
            else False,
            bool(finished_after_raw.strip())
            if isinstance(finished_after_raw, str)
            else False,
            bool(finished_before_raw.strip())
            if isinstance(finished_before_raw, str)
            else False,
            sort_option
            != "title_asc",  # Treat non-default sort as requiring full fetch for proper ordering
        ]
    )
    if has_filter:
        try:
            # Use short-lived cache for expensive all-books call
            from app.utils.simple_cache import (
                cache_get,
                cache_set,
            ...
            if user_books is None:
                user_books = book_service.get_all_books_with_user_overlay_sync(
                    str(current_user.id)
                )
        ...
        try:
            ...
            user_books = cache_get(cache_key)
            if user_books is None:
                user_books = book_service.get_books_with_user_overlay_paginated_sync(
                    str(current_user.id), per_page, offset, sort_option
                )
    ...
    for book in user_books:
        ...
        book_title = (
            book.get("title")
            if isinstance(book, dict)
            else getattr(book, "title", "Unknown Title")
        )
        book_locations = (
            book.get("locations")
            if isinstance(book, dict)
            else getattr(book, "locations", None)
        )
        ...
        if book_locations:
            ...
            for location in book_locations:
                ...
                loc_id = (
                    ...
                    else getattr(location, "id", location)
                )
    ...
    def get_reading_status(book):
        ...
        if isinstance(book, dict):
            ...
            status = (
                ...
                or book.get("ownership", {}).get("reading_status")
                or book.get("status")
            )  # legacy field
        else:
    ...
    def get_ownership_status(book):
        if isinstance(book, dict):
            ...
            status = book.get("ownership_status") or book.get("ownership", {}).get(
                "ownership_status"
            )
            return status
    ...
    try:
        ...
        if global_counts is None:
            global_counts = book_service.get_library_status_counts_sync(
                str(current_user.id)
            )
    ...
    def _resolve_media_type_value(book_obj):
        value = (
            book_obj.get("media_type")
            if isinstance(book_obj, dict)
            else getattr(book_obj, "media_type", None)
        )
        if value is None:
            return None
    ...
    if status_filter and status_filter != "all":
        if status_filter == "wishlist":
            filtered_books = [
                ...
                if get_ownership_status(book) == "wishlist"
            ]
            ...
            filtered_books = [
                book
                for book in filtered_books
                if get_reading_status(book) in ["reading", "currently_reading"]
            ]
        else:
            filtered_books = [
                book
                for book in filtered_books
                if get_reading_status(book) == status_filter
            ]
                ...
                in (
                    (
                        (
                            ...
                            if isinstance(book, dict)
                            else getattr(book, "normalized_title", "")
                        )
                        or ""
                    ).casefold()
                )
    ...
    if publisher_filter:
        filtered_books = [
            ...
            if (
                book.get("publisher")
                if isinstance(book, dict)
                else getattr(book, "publisher", None)
            )
            and publisher_filter.lower()
            in (
                (
                    book.get("publisher", "")
                    if isinstance(book, dict)
                    else getattr(book, "publisher", "")
                )
                or ""
            ).lower()
        ]
    ...
    if language_filter:
        filtered_books = [
            ...
            if (
                book.get("language")
                if isinstance(book, dict)
                else getattr(book, "language", None)
            )
            == language_filter
        ]
            ...
            and any(
                ...
                in (
                    ...
                    if isinstance(loc, str)
                        ...
                        else getattr(loc, "name", "")
                    ).lower()
                )
                for loc in (
                    book.get("locations", [])
                    if isinstance(book, dict)
                    else getattr(book, "locations", [])
                )
            )
        ]
    ...
    if category_filter:
        filtered_books = [
            ...
            if (
                book.get("categories")
                if isinstance(book, dict)
                else getattr(book, "categories", None)
            )
            and any(
                ...
                in (
                    ...
                    else getattr(cat, "name", "")
                ).lower()
                for cat in (
                    book.get("categories", [])
                    if isinstance(book, dict)
                    else getattr(book, "categories", [])
                )
            )
        ]
    ...
    if finished_after or finished_before:
        ...
        def _book_finish_date(book_obj):
            val = (
                book_obj.get("finish_date")
                if isinstance(book_obj, dict)
                else getattr(book_obj, "finish_date", None)
            )
        ...
        filtered_books = [
            ...
            for book in filtered_books
            if (
                (fd := _book_finish_date(book)) is not None
                and (finished_after is None or fd >= finished_after)
                and (finished_before is None or fd <= finished_before)
            )
        ]
    ...
    def get_date_added_sort_key(book):
        ...
        date_value = (
            ...
            if isinstance(book, dict)
            else getattr(book, "added_at", None)
            or getattr(book, "created_at", None)
            or ""
        )
        title_value = (
            book.get("title", "")
    ...
    if sort_option == "title_asc":
        filtered_books.sort(
            key=lambda x: (
                x.get("title", "") if isinstance(x, dict) else getattr(x, "title", "")
            ).lower()
        )
        ...
        filtered_books.sort(
            key=lambda x: get_author_last_first(x).lower(), reverse=True
        )
        ...
        def get_pub_date(book):
            pub_date = (
                ...
                if isinstance(book, dict)
                else getattr(book, "published_date", None)
            )
            if not pub_date:
                return ""
        ...
        def get_finish_sort_desc(book):
            fd = _parse_finish_date(
                book.get("finish_date")
                if isinstance(book, dict)
                else getattr(book, "finish_date", None)
            )
    ...
    if has_filter:
        ...
        total_pages = (
            max(1, _math.ceil(filtered_total / per_page)) if per_page > 0 else 1
        )
        page = max(1, min(page, total_pages))
        offset = (page - 1) * per_page
    ...
    for book in books:
        if isinstance(book, dict):
                ...
                def __init__(self, data):
                    ...
                    try:
                        setattr(
                            ...
                            get_reading_status(data) or "",
                        )
                    except Exception:
                        setattr(
                            self,
                            "normalized_reading_status",
                            (getattr(self, "reading_status", None) or ""),
                        )
                    try:
                        owner = data.get("ownership_status") or data.get(
                            "ownership", {}
                        ).get("ownership_status")
                        owner = (
                            owner.strip().lower() if isinstance(owner, str) else owner
                        )
                        setattr(self, "normalized_ownership_status", owner or "owned")
                    except Exception:
                        setattr(
                            self,
                            "normalized_ownership_status",
                            (getattr(self, "ownership_status", None) or "owned"),
                        )
                ...
                def get_contributors_by_type(self, contribution_type):
                    ...
                    if hasattr(self, "contributors") and self.contributors:
                        return [
                            ...
                            if getattr(c, "contribution_type", None)
                            == contribution_type
                        ]
                    return []
            ...
            try:
                setattr(
                    book, "normalized_reading_status", get_reading_status(book) or ""
                )
            except Exception:
                pass
            try:
    ...
    media_type_options = [
        {"value": val, "label": _format_media_type_label(val)}
    ...
    reading_status_options = [
        {"value": status.value, "label": _humanize_status(status.value)}
    ...
    try:
        ...
        for location in all_locations or []:
            ...
            location_options.append(
                {"id": loc_id, "name": loc_name, "is_default": is_default}
            )

        if location_options:
            location_options.sort(
                key=lambda entry: (
                    not entry.get("is_default", False),
    ...
    try:
        ...
        current_app.logger.warning(
            f"Failed to load category list for bulk actions: {exc}"
        )
        all_categories = []
    ...
    for category in all_categories or []:
        if isinstance(category, dict):
            raw_name = (
                category.get("name") or category.get("normalized_name") or ""
            ).strip()
            display_label = (
                category.get("label") or category.get("display_name") or raw_name
            )
        else:
    ...
    for existing_category in categories:
        ...
        if key not in category_lookup:
            category_lookup[key] = {"name": name, "label": name}

    if category_lookup:
        category_options = sorted(
            category_lookup.values(), key=lambda entry: entry.get("label", "").lower()
        )
    ...
    if request.args.get("format") == "json":
        ...
        for b in books:
            bd = {
                ...
                if not isinstance(b, dict)
                else b.get("cover_url"),
                "average_rating": getattr(b, "average_rating", None)
                if not isinstance(b, dict)
                else b.get("average_rating"),
                "rating_count": getattr(b, "rating_count", None)
                if not isinstance(b, dict)
                else b.get("rating_count"),
                "normalized_reading_status": getattr(b, "normalized_reading_status", "")
                if not isinstance(b, dict)
                else (b.get("normalized_reading_status") or ""),
                "locations": getattr(b, "locations", [])
                if not isinstance(b, dict)
                else b.get("locations", []),
            }
            payload.append(bd)
    ...
    resp = make_response(
        render_template(
            ...
            categories=sorted(
                [cat for cat in categories if cat is not None and cat != ""]
            ),
            publishers=sorted(
                [pub for pub in publishers if pub is not None and pub != ""]
            ),
            languages=sorted(
                [lang for lang in languages if lang is not None and lang != ""]
            ),
            locations=sorted(
                [loc for loc in locations if loc is not None and loc != ""]
            ),
...
def public_library():
    ...
    return render_template(
        "public_library.html", books=books, filter_status=filter_status
    )
        ...
        try:
            ...
            if callable(get_column_names):
                try:
                    ...
                    column_names = (
                        ...
                        if isinstance(get_column_names, (list, tuple))
                        else []
                    )
            ...
            if rows:
                ...
                if isinstance(first_row, dict):
                    ...
                    raw_payload = {
                        column_names[i]: first_row[i]
                        for i in range(min(len(column_names), len(first_row)))
                    }
        except Exception as conversion_error:
            current_app.logger.error(
                "Failed to convert raw book payload: %s", conversion_error
            )
            raw_payload = {}
...
def edit_book(uid):
    try:
        ...
        current_app.logger.info(
            f"[CONTRIB_DEBUG] Received contributor data: {contributor_data}"
        )
            ...
            if cat_data.get("name"):
                ...
                if is_new:
                    try:
                        ...
                        created_category = book_service.create_category_sync(
                            new_category_data
                        )
                        if created_category:
                            current_app.logger.info(
                            ...
                            current_app.logger.warning(
                                f"Failed to create new category: {category_name}"
                            )
                    except Exception as e:
                        current_app.logger.error(
                            f"Error creating category {category_name}: {e}"
                        )
                        # Continue anyway - the raw_categories processing might still handle it

                categories.append(
                    category_name
                )  # Add to list for raw_categories processing
        ...
        try:
            ...
            current_app.logger.warning(
                f"[CATEGORIES] Failed to parse raw_categories JSON: {_e}"
            )
        ...
        for contrib_index, contrib in contributor_data.items():
            if contrib.get("name"):
                ...
                try:
                    ...
                    person_result = safe_manager.execute_query(
                        find_person_query, {"normalized_name": normalized_name}
                    )
                    ...
                    if person_data_list:
                        ...
                        person = Person(
                            ...
                            normalized_name=(person_data.get("normalized_name") or "")
                            .strip()
                            .lower(),
                        ...
                        try:
                            ...
                            if created_person:
                                ...
                                person = Person(
                                    ...
                                    normalized_name=created_person.get(
                                        "normalized_name", ""
                                    ),
                ...
                contrib_type = contrib_type_map.get(
                    contrib.get("type", "authored"), ContributionType.AUTHORED
                )

                current_app.logger.info(
                    f"[CONTRIB_DEBUG] Creating contribution: person={person.name}, type={contrib_type}"
                )
        ...
        update_data = {
            ...
            "published_date": _convert_published_date_to_date(
                request.form.get("published_date", "").strip()
            )
            if request.form.get("published_date", "").strip()
            else None,
            "page_count": int(page_count_str)
            if (page_count_str := request.form.get("page_count", "").strip())
            else None,
            ...
            if (series_order_str := request.form.get("series_order", "").strip())
            else None,
            "contributors": contributors,
            "raw_categories": (
                raw_categories_payload
                if raw_categories_payload is not None
                else (categories if categories else None)
            ),
            ...
            "average_rating": float(avg_rating_str)
            if (avg_rating_str := request.form.get("average_rating", "").strip())
            else None,
            "rating_count": int(rating_count_str)
            if (rating_count_str := request.form.get("rating_count", "").strip())
            else None,
        ...
        try:
            success = book_service.update_book_sync(
                uid, str(current_user.id), **filtered_data
            )
        except Exception as e:
            import traceback
        ...
        try:
            ...
            if series_id:
                ...
                get_series_service().attach_book(
                    uid, series_id, volume=vol, order_number=order_int
                )
            else:
            ...
            current_app.logger.warning(
                f"[SERIES][EDIT_ATTACH_FAIL] uid={uid} err={series_err}"
            )
        ...
        if location_id is not None:  # Allow empty string to clear location
            ...
            try:
                ...
                location_success = location_service.set_book_location(
                    uid, location_id if location_id else None, str(current_user.id)
                )
            ...
            class BookObj:
                ...
                def get_contributors_by_type(self, contribution_type):
                    ...
                    type_str = (
                        ...
                        else str(contribution_type).upper()
                    )
                    ...
                    for contributor in self.contributors:
                        if isinstance(contributor, dict):
                            ...
                            if role == type_str or role == type_str.replace(
                                "ED", ""
                            ):  # Handle past tense
        ...
        return render_template(
            "edit_book_enhanced.html", book=user_book, book_categories=book_categories
        )
...
def view_book_enhanced(uid):
    ...
    if isinstance(user_book, dict):
        ...
        class BookObj:
            def __init__(self, data):
                ...
                if not hasattr(self, "isbn") and (
                    hasattr(self, "isbn13") or hasattr(self, "isbn10")
                ):
                    self.isbn = getattr(self, "isbn13", None) or getattr(
                        self, "isbn10", None
                    )
                ...
                isbn_value = getattr(self, "isbn", None)
                if isbn_value and (
                    not hasattr(self, "isbn13") or not getattr(self, "isbn13")
                ):
                    import re
                ...
                if not hasattr(self, "categories") or not getattr(
                    ...
                    if hasattr(self, "genres") and getattr(self, "genres", None):
                        ...
                        self.categories = (
                            [genre_val] if isinstance(genre_val, str) else genre_val
                        )
                ...
                if (
                    hasattr(self, "location_id")
                    and location_id
                    and (
                        not hasattr(self, "locations")
                        or not getattr(self, "locations", None)
                    )
                ):
                    try:
                        from app.location_service import LocationService
                        ...
                        for location in all_locations:
                            if hasattr(location, "id") and str(location.id) == str(
                                location_id
                            ):
                                self.locations = [
                        ...
                        if not hasattr(self, "locations") or not self.locations:
                            for location in all_locations:
                                if hasattr(location, "name") and str(
                                    location.name
                                ) == str(location_id):
                                    self.locations = [
                                        {"id": location.id, "name": location.name}
        ...
        if not hasattr(user_book, "isbn"):
            user_book.isbn = (
                isbn13 or isbn10
            )  # Fallback for any other template expectations
    ...
    try:
        ...
        custom_metadata = custom_field_service.get_custom_metadata_sync(
        ...
        if custom_metadata:
            ...
            global_metadata_display = (
                custom_field_service.get_custom_metadata_for_display(global_metadata)
                or []
            )
            personal_metadata_display = (
                custom_field_service.get_custom_metadata_for_display(personal_metadata)
                or []
            )
        ...
        personal_fields = (
            custom_field_service.get_available_fields_sync(
                current_user.id, is_global=False
            )
            or []
        )
    ...
    template_data.update({"all_persons": all_persons})
...
def update_book_details(uid):
    ...
    if ownership_status == "borrowed":
        ...
        if borrowed_due_date:
            update_data["borrowed_due_date"] = datetime.strptime(
                borrowed_due_date, "%Y-%m-%d"
            ).date()
    ...
    if ownership_status == "loaned":
        ...
        if loaned_due_date:
            update_data["loaned_due_date"] = datetime.strptime(
                loaned_due_date, "%Y-%m-%d"
            ).date()
    ...
    try:
        updated_book = book_service.update_book_sync(
            uid, str(current_user.id), **update_data
        )
            ...
            if redirect_to_library:
                ...
                if reading_status:
                    if reading_status == "wishlist":
                        return redirect(
                            url_for("main.library", status_filter="wishlist")
                        )
                    else:
                        return redirect(
                            url_for("main.library", status_filter=reading_status)
                        )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            current_app.logger.info(
                f"[COVER][REPLACE] START uid={uid} src={new_cover_url}"
            )
            ...
            try:
                if old_cover_url and not allow_downgrade:
                    ...
                    if old_size and new_size:
                        ...
                        if (new_size < 12000 and old_size > 20000) or (
                            new_size / old_size
                        ) < 0.5:
                            downgrade = True
                    if downgrade:
                ...
                current_app.logger.debug(
                    f"[COVER][REPLACE][DOWNGRADE_CHECK_FAIL] uid={uid} err={downgrade_err}"
                )

            book_service.update_book_sync(
                user_book.uid, str(current_user.id), cover_url=abs_cover_url
            )
            current_app.logger.info(
                f"[COVER][REPLACE] STORED uid={uid} cached={new_cached_cover_url}"
            )
            ...
            try:
                ...
                current_app.logger.debug(
                    f"[COVER][REPLACE][CACHE_BUMP_FAIL] uid={uid} err={cache_err}"
                )

            # Clean up old cover file if it exists and is a local file
            if old_cover_url and (
                old_cover_url.startswith("/covers/")
                or old_cover_url.startswith("/static/covers/")
            ):
                try:
                    ...
                    if old_filepath.exists():
                        old_filepath.unlink()
                        current_app.logger.debug(
                            f"Cleaned up old cover file: {old_filename}"
                        )
                except Exception as cleanup_error:
                    current_app.logger.debug(
                        f"Failed to clean up old cover file: {cleanup_error}"
                    )

            current_app.logger.info(
                f"[COVER][REPLACE] DONE uid={uid} src={new_cover_url}"
            )
            ...
            current_app.logger.error(
                f"[COVER][REPLACE] REQUEST_FAIL uid={uid} src={new_cover_url} err={e}"
            )
            return jsonify(
                {"success": False, "error": "Failed to download new cover image"}
            ), 500
        except Exception as e:
            current_app.logger.error(
                f"[COVER][REPLACE] FAIL uid={uid} src={new_cover_url} err={e}"
            )
            return jsonify(
                {"success": False, "error": "Failed to process new cover image"}
            ), 500
...
def update_reading_dates(uid):
    ...
    if finish_date_str:
        update_data["finish_date"] = datetime.strptime(
            finish_date_str, "%Y-%m-%d"
        ).date()
...
def edit_book_custom_metadata(uid):
    ...
    from app.debug_system import debug_metadata_operation
    ...
    try:
        ...
        debug_metadata_operation(
            book_id, uid, str(current_user.id), existing_metadata, "LOAD"
        )
        ...
        if request.method == "POST":
            ...
            valid, errors = (
                True,
                [],
            )  # custom_field_service.validate_and_save_metadata(personal_metadata, current_user.id, is_global=False)
        ...
        existing_metadata = custom_field_service.get_custom_metadata_sync(
            uid, str(current_user.id)
        )
        ...
        print("   📤 [EDIT_META] Passing to template:")
        print(f"      🌐 global_fields: {[f.get('name', '') for f in global_fields]}")
        print(
            f"      👤 personal_fields: {[f.get('name', '') for f in personal_fields]}"
        )
...
def month_review(year, month):
    ...
    books = [
        book
        for book in user_books
        if (finish_date := getattr(book, "finish_date", None))
        and start_date <= finish_date < end_date
    ]
    ...
    return send_file(
        ...
        mimetype="image/png",
        as_attachment=True,
        download_name=f"month_review_{year}_{month}.png",
    )
    ...
    try:
        ...
        for book in user_books:
            ...
            if isinstance(book, dict):
                ...
                start_date = (
                    book.get("start_date", "").isoformat()
                    if book.get("start_date")
                    and hasattr(book.get("start_date"), "isoformat")
                    else ""
                )
                finish_date = (
                    book.get("finish_date", "").isoformat()
                    if book.get("finish_date")
                    and hasattr(book.get("finish_date"), "isoformat")
                    else ""
                )
                title = book.get("title", "")
            else:
                author_names = (
                    ", ".join([author.name for author in getattr(book, "authors", [])])
                    if hasattr(book, "authors") and getattr(book, "authors", [])
                    else ""
                )
                ...
                start_date = (
                    ...
                    if start_date_val and hasattr(start_date_val, "isoformat")
                    else ""
                )
                finish_date_val = getattr(book, "finish_date", None)
                finish_date = (
                    finish_date_val.isoformat()
                    if finish_date_val and hasattr(finish_date_val, "isoformat")
                    else ""
                )
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=bibliotheca_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
...
def user_profile(user_id):
    ...
    try:
        ...
        total_books = len(
            [book for book in all_user_books if get_finish_date_safe(book)]
        )
        ...
        books_this_year = len(
            [
                ...
                if (finish_date := get_finish_date_safe(book))
                and finish_date >= date(current_year, 1, 1)
            ]
        )
        ...
        currently_reading = (
            [
                book
                for book in all_user_books
                if getattr(book, "start_date", None)
                and not getattr(book, "finish_date", None)
            ]
            if user.share_current_reading
            else []
        )

        recent_finished = sorted(
            [book for book in all_user_books if get_finish_date_safe(book)],
            key=lambda x: get_finish_date_safe(x) or date.min,
            reverse=True,
        )[:10]
...
def assign_book(uid):
    try:
        ...
        flash(
            "Book assignment feature needs to be implemented for Kuzu backend.",
            "warning",
        )
        return redirect(url_for("main.library"))
    except Exception as e:
...
def bulk_delete_books():
    ...
    if not selected_uids:
        return _bulk_response(
            False, "Select at least one book to delete.", redirect_url, status_code=400
        )
...
def bulk_update_book_status():
    ...
    if not target_status:
        return _bulk_response(
            False, "Choose a valid reading status.", redirect_url, status_code=400
        )
    ...
    if not selected_uids:
        return _bulk_response(
            False,
            "Select at least one book to change status.",
            redirect_url,
            status_code=400,
        )
    ...
    for uid in selected_uids:
        try:
            ...
            if not user_book:
                failed_ids.append(uid)
                continue
            updated = book_service.update_book_sync(
                uid, user_id, reading_status=target_status
            )
            if updated:
                updated_count += 1
            else:
    ...
    if updated_count:
        message_bits.append(
            f"Set status to {_humanize_status(target_status)} for {updated_count} book(s)."
        )
    if failed_ids:
    ...
    return _bulk_response(
        updated_count > 0,
        " ".join(message_bits)
        or "Unable to update reading status for the selected books.",
        redirect_url,
        data=payload,
        status_code=status_code,
    )
...
def bulk_update_book_location():
    ...
    if not selected_uids:
        return _bulk_response(
            False,
            "Select at least one book to change location.",
            redirect_url,
            status_code=400,
        )
    ...
    for uid in selected_uids:
        try:
            ...
            current_app.logger.exception(
                f"Bulk location update failed for {uid}: {exc}"
            )
            failed_ids.append(uid)
    ...
    if updated_count:
        action_phrase = (
            f"Set location to {location_label}" if location_id else "Cleared location"
        )
...
def bulk_update_book_categories():
    ...
    if not selected_uids:
        return _bulk_response(
            False,
            "Select at least one book to change categories.",
            redirect_url,
            status_code=400,
        )
    ...
    selected_categories = [
        value for value in request.form.getlist("categories") if value
    ]
    additional_categories = _parse_additional_categories(
        request.form.get("additional_categories", "")
    )
    requested_categories = _dedupe_preserve_order(
        selected_categories + additional_categories
    )
    ...
    for uid in selected_uids:
        try:
            ...
            result = book_service.update_book_sync(
                uid, user_id, raw_categories=final_categories
            )
            ...
            current_app.logger.exception(
                f"Bulk category update failed for {uid}: {exc}"
            )
            failed_ids.append(uid)
    ...
    if updated_count:
        if clear_existing and not requested_categories:
            ...
            message_bits.append(
                f"Categories already up to date for {updated_count} book(s)."
            )
    if failed_ids:
        message_bits.append(f"Failed to update {len(failed_ids)} book(s).")
...
def search_books_in_library():
    ...
    declared_media_types = {mt.value.lower() for mt in MediaType}
    all_media_type_values = sorted(
        declared_media_types.union(media_types), key=lambda val: val.replace("_", " ")
    )
    ...
    stats = {
        ...
        "books_read": len(
            [b for b in user_books if getattr(b, "reading_status", None) == "read"]
        ),
        "currently_reading": len(
            [b for b in user_books if getattr(b, "reading_status", None) == "reading"]
        ),
        "want_to_read": len(
            [
                b
                for b in user_books
                if getattr(b, "reading_status", None) == "plan_to_read"
            ]
        ),
        "on_hold": len(
            [b for b in user_books if getattr(b, "reading_status", None) == "on_hold"]
        ),
        "wishlist": len(
            [
                b
                for b in user_books
                if getattr(b, "ownership_status", None) == "wishlist"
            ]
        ),
    }
...
def add_book_manual():
    ...
    contrib = {
        k: _load_json_list(f"contributors_{k}")
        for k in ["authored", "narrated", "edited", "translated", "illustrated"]
    }
    author = (
        contrib["authored"][0]["name"].strip()
        if contrib.get("authored")
        else (request.form.get("author") or "").strip()
    ) or "Unknown Author"
    additional_authors = (
        ", ".join([c["name"] for c in (contrib.get("authored") or [])[1:]]) or None
    )
    ...
    if "categories" in request.form:
        try:
            categories = [
                c.get("name")
                for c in json.loads(request.form["categories"])
                if c.get("name")
            ]
        except Exception:
            pass
    ...
    isbn10_form = (
        request.form.get("isbn_10") or request.form.get("isbn10") or ""
    ).upper()
    isbn13_form = (
        request.form.get("isbn_13") or request.form.get("isbn13") or ""
    ).upper()
    ...
    if created:
        ...
        if fd:
            updates["finish_date"] = fd
        if raw_categories:
            updates["raw_categories"] = raw_categories
        if updates:
            try:
                book_service.update_book_sync(
                    created.uid, str(current_user.id), **updates
                )
            except Exception:
                pass
    ...
    if wants_json:
        ...
        redirect_url = (
            ...
            else url_for("main.library")
        )
        return jsonify(
            {"success": True, "message": message, "redirect_url": redirect_url}
        )
    else:
...
def resolve_duplicate():
    ...
    try:
        ...
        if action == "increment_count":
            ...
            if not book_id:
                return jsonify(
                    {"success": False, "message": "Book ID is required for increment"}
                ), 400
            ...
            try:
                ...
                try:
                    ...
                    safe_execute_query(
                        "ALTER TABLE Book ADD quantity INT64",
                        {},
                        user_id=str(current_user.id),
                        operation="ensure_quantity_column",
                    )
                ...
                existing_book = book_service.get_book_by_uid_sync(
                ...
                success = book_service.update_book_sync(
                    book_id, str(current_user.id), quantity=new_quantity
                )
                if success:
                    # Invalidate cache
                    try:
                    ...
                    return jsonify(
                        {
                            ...
                            "redirect_url": url_for(
                                "book.view_book_enhanced", uid=book_id
                            ),
                        }
                    )
                else:
            ...
            try:
                ...
                if book_id:
                    ...
                    try:
                        ...
                        result = book_service.add_book_to_user_library_sync(
                            user_id=current_user.id,
                            book_id=book_id,
                            reading_status=reading_status_enum.value
                            if reading_status_enum
                            else reading_status or "library_only",
                        ...
                        if not result:
                            current_app.logger.error(
                                f"Failed to add book {book_id} to user library"
                            )
                            return jsonify(
                                {
                                    "success": False,
                        ...
                        current_app.logger.error(
                            f"Error adding book to user library: {e}"
                        )
                        import traceback
                        ...
                        return jsonify(
                            {
                                "success": False,
                                "message": f"Failed to add to library: {str(e)}",
                            }
                        ), 500
                ...
                traceback.print_exc()
                return jsonify(
                    {"success": False, "message": f"Error creating book: {str(e)}"}
                ), 500
    ...
    finish_date = (
        datetime.strptime(finish_date_str, "%Y-%m-%d").date()
        if finish_date_str
        else None
    )
    ...
    for key, value in request.form.items():
        if key.startswith("custom_field_") and value and value.strip():
            ...
            print(
                f"📋 [MANUAL] Selected existing custom field: {field_name} = {field_value}"
            )
    ...
    if normalized_isbn:
            ...
            try:
                ...
                for identifier in industry_identifiers:
                    ...
                    if id_type == "ISBN_10":
                        ...
                        if (
                            id_value
                            and len(id_value.strip()) == 10
                            and id_value.strip().isalnum()
                        ):
        ...
        def enhanced_openlibrary_lookup(isbn):
            ...
            try:
                ...
                return {
                    ...
                    "categories": categories,
                    "cover_url": cover_url,
                    "openlibrary_id": book.get("key", "").replace("/books/", "")
                    if book.get("key")
                    else "",
                    "source": "openlibrary",
                }
        ...
        if google_data or ol_data:
            ...
            if (
                api_data.get("cover_url") and not cover_url
            ):  # Only if no manual cover URL provided
    ...
    final_subtitle = (
        subtitle if subtitle else (api_data.get("subtitle") if api_data else None)
    )
    ...
    if api_data and api_data.get("categories"):
        ...
        if isinstance(api_categories, list):
            ...
            final_categories.extend(
                [cat.strip() for cat in api_categories.split(",") if cat.strip()]
            )
    ...
    try:
        ...
        domain_book = DomainBook(
            ...
            contributors=contributors,
            publisher=Publisher(id=str(uuid.uuid4()), name=final_publisher)
            if final_publisher
            else None,
        ...
        if update_data:
            book_service.update_book_sync(
                existing_book.uid, str(current_user.id), **update_data
            )
        ...
        if existing_book.id == domain_book.id:
            ...
            if custom_metadata:
                flash(
                    f'Book "{title}" added successfully with {len(custom_metadata)} custom fields.',
                    "success",
                )
            else:
            ...
            if custom_metadata:
                flash(
                    f'Book "{title}" already exists. Added to your collection with {len(custom_metadata)} custom fields.',
                    "info",
                )
            else:
                flash(
                    f'Book "{title}" already exists in the library. Added to your collection.',
                    "info",
                )
            return redirect(url_for("main.library"))
