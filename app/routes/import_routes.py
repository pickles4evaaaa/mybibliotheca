...
_IMPORT_VERBOSE = (
    ...
    _os_for_import_verbosity.getenv("IMPORT_VERBOSE") or "false"
).lower() == "true"
...
def _update_import_progress(
    ...
    *,
    ...
    if processed_book is not None:
        payload["processed_books"] = _bounded_list(
            snapshot.get("processed_books"), processed_book, MAX_PROGRESS_RECENT_BOOKS
        )
    if error_message is not None:
        payload["error_messages"] = _bounded_list(
            snapshot.get("error_messages"), error_message, MAX_PROGRESS_ERROR_MESSAGES
        )
...
async def _ensure_unassigned_reading_log_book(
    ...
    try:
        existing = book_service.search_books_sync(
            UNASSIGNED_READING_LOG_TITLE, user_id, limit=1
        )
        if existing:
    ...
    placeholder = SimplifiedBook(
        title=UNASSIGNED_READING_LOG_TITLE, author="System Generated"
    )
    try:
    ...
    try:
        if location_service is None:
            ...
            default_location_id = (
                getattr(default_location, "id", None) if default_location else None
            )
        if location_service and default_location_id:
            try:
                location_service.add_book_to_location(
                    book_id, default_location_id, user_id
                )
            except Exception as loc_error:
                logger.debug(
                    f"Unable to link unassigned book to default location: {loc_error}"
                )
...
def detect_csv_format(csv_file_path):
    ...
    try:
        with open(csv_file_path, "r", encoding="utf-8") as csvfile:
            ...
            storygraph_normalized = storygraph_score / sum(
                storygraph_indicators.values()
            )
            ...
            if (
                goodreads_normalized >= min_confidence
                and goodreads_normalized > storygraph_normalized
            ):
                return "goodreads", goodreads_normalized
            elif (
                storygraph_normalized >= min_confidence
                and storygraph_normalized > goodreads_normalized
            ):
...
def auto_create_custom_fields(field_mappings, user_id):
    ...
    FIELD_CONFIGS = {
        "goodreads_book_id": {
            "display_name": "Goodreads Book ID",
        ...
        "original_publication_year": {
            "display_name": "Original Publication Year",
            "type": CustomFieldType.NUMBER,
            "global": True,
        },
        "bookshelves": {
        ...
        "shelf_positions": {
            "display_name": "Bookshelves with Positions",
            "type": CustomFieldType.TEXTAREA,
            "global": True,
        },
        "spoiler_flag": {
            "display_name": "Spoiler Review",
        ...
        "read_count": {
            "display_name": "Number of Times Read",
            "type": CustomFieldType.NUMBER,
            "global": False,
        },
        "owned_copies": {
            "display_name": "Number of Owned Copies",
            "type": CustomFieldType.NUMBER,
            "global": False,
        },
        "format": {
        ...
        "pace": {
            "display_name": "Reading Pace",
            "type": CustomFieldType.TEXT,
            "global": True,
        },
        "character_plot_driven": {
            "display_name": "Character vs Plot Driven",
            "type": CustomFieldType.TEXT,
            "global": True,
        },
        "content_warnings": {
        ...
        "character_development": {
            "display_name": "Strong Character Development",
            "type": CustomFieldType.BOOLEAN,
            "global": False,
        },
        "loveable_characters": {
            "display_name": "Loveable Characters",
            "type": CustomFieldType.BOOLEAN,
            "global": False,
        },
        "diverse_characters": {
            "display_name": "Diverse Characters",
            "type": CustomFieldType.BOOLEAN,
            "global": False,
        },
        "flawed_characters": {
            "display_name": "Flawed Characters",
            "type": CustomFieldType.BOOLEAN,
            "global": False,
        },
    ...
    if request.method == "POST":
        ...
        if existing_csv_path and force_custom:
            ...
            if not file or not file.filename:
                flash(
                    "Please upload a valid delimited file (.csv, .tsv, or .txt).",
                    "danger",
                )
            ...
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=".csv", prefix=f"import_{current_user.id}_"
            )
        ...
        try:
            ...
            with open(temp_path, "r", encoding="utf-8") as csvfile:
                ...
                if _looks_like_reading_history(headers):
                    ...
                    return redirect(
                        url_for(
                            "import.import_reading_history", csv_file_path=temp_path
                        )
                    )
                ...
                if not headers or len(headers) == 1:
                    ...
                    if first_line.isdigit() or len(first_line) in [
                        10,
                        13,
                    ]:  # Looks like ISBN
                        headers = ["ISBN"]
            ...
            if not force_custom:
                goodreads_signatures = [
                    "Book Id",
                    "Author l-f",
                    "Bookshelves",
                    "Exclusive Shelf",
                ]
                storygraph_signatures = [
                    "Read Status",
                    "Moods",
                    "Pace",
                ...
                is_storygraph = any(
                    header in headers for header in storygraph_signatures
                )
                ...
                if is_goodreads or is_storygraph:
                    ...
                    flash(
                        f"Detected {import_type.title()} export file! We've pre-mapped common fields for you. Review and customize the mappings below.",
                        "info",
                    )
            ...
            try:
                ...
                shareable_fields = custom_field_service.get_available_fields_sync(
                ...
                if shareable_fields and hasattr(shareable_fields, "__iter__"):
                    ...
                    for field in shareable_fields_list:
                        field_id = (
                            field.get("id")
                            if isinstance(field, dict)
                            else getattr(field, "id", None)
                        )
                        if field_id and not any(
                            (
                                gf.get("id")
                                if isinstance(gf, dict)
                                else getattr(gf, "id", None)
                            )
                            == field_id
                ...
                print(
                    f"🔧 [IMPORT] Loaded {len(global_custom_fields)} global fields and {len(personal_custom_fields)} personal fields"
                )
            ...
            try:
                import_templates = import_mapping_service.get_user_templates_sync(
                    current_user.id
                )
                ...
                if force_custom:
                    ...
                    headers_list = (
                        list(headers) if not isinstance(headers, list) else headers
                    )
                    detected_template = import_mapping_service.detect_template_sync(
                        headers_list, current_user.id
                    )
                    detected_template_name = (
                        detected_template.name if detected_template else None
                    )

                # If a default system template was detected, auto-create fields but still show mapping UI for review
                if (
                    not force_custom
                    and detected_template
                    and detected_template.user_id == "__system__"
                    and detected_template.field_mappings
                ):
                    # Auto-create any custom fields referenced in the template BEFORE showing mapping screen
                    auto_create_custom_fields(
                        detected_template.field_mappings, current_user.id
                    )

                    # Reload custom fields after creating new ones so they appear in the mapping screen
                    user_fields = custom_field_service.get_user_fields_sync(
                        current_user.id
                    )
                    if user_fields and hasattr(user_fields, "__iter__"):
                        global_custom_fields = []
                    ...
                    print(
                        f"DEBUG: Using template mappings as suggestions: {suggested_mappings}"
                    )
                ...
                # If a custom template was detected, use its mappings
                    ...
                    if template_list and not any(
                        ...
                        template_list.append(template_dict)
                        print(
                            f"DEBUG: Added detected template to dropdown: {detected_template.name}"
                        )
                else:
                    print(
                        f"DEBUG: No template detected, using auto-detected mappings: {suggested_mappings}"
                    )
...
def import_books_execute():
    ...
    if use_template and skip_mapping:
        ...
        try:
            ...
            template = None
            all_templates_raw = import_mapping_service.get_user_templates_sync(
                current_user.id
            )
        ...
        for key, value in field_mapping.items():
            if key.startswith("field_mapping[") and value:
                ...
                if value in ["create_global_field", "create_personal_field"]:
                    ...
                    if not field_label:
                        flash(
                            f'Field label is required for CSV column "{csv_field}"',
                            "error",
                        )
                        return redirect(url_for("import.import_books"))
                    ...
                    try:
                        ...
                        if field_exists:
                            flash(
                                f'A custom field with name "{field_name}" already exists',
                                "error",
                            )
                            return redirect(url_for("import.import_books"))
                        ...
                        custom_field_service.create_field_sync(
                            current_user.id, field_data
                        )

                        # Update mapping to use the new field
                        mappings[csv_field] = (
                            f"custom_{'global' if is_global else 'personal'}_{field_name}"
                        )

                        flash(
                            f"Created new {'global' if is_global else 'personal'} field: {field_label}",
                            "success",
                        )
    ...
    if save_as_template:
        if not template_name:
            ...
            if not template_field_mappings:
                ...
                try:
                    existing_templates_raw = (
                        import_mapping_service.get_user_templates_sync(current_user.id)
                        or []
                    )
                    existing_templates = (
                        list(existing_templates_raw)
                        if hasattr(existing_templates_raw, "__iter__")
                        else []
                    )
                    ...
                    for existing in existing_templates:
                        ...
                        if (
                            existing_name
                            and existing_name.lower() == template_name_lower
                        ):
                            conflict_found = True
                            break
                    if conflict_found:
                        flash(
                            f'An import template named "{template_name}" already exists. Choose another name.',
                            "warning",
                        )
                    else:
                        ...
                        created_template = import_mapping_service.create_template_sync(
                            new_template
                        )
                        if created_template:
                            flash(
                                f'Saved import template "{template_name}".', "success"
                            )
                        else:
                            flash(
    ...
    default_import_media_type = request.form.get(
        "default_import_media_type", "physical"
    )
...
def import_books_progress(task_id):
    ...
    if (
        job.get("import_type") == "reading_history"
        and job.get("status") == "needs_book_matching"
    ):
        # Redirect to book matching page for reading history imports
        return redirect(
            url_for("import.reading_history_book_matching", task_id=task_id)
        )
            ...
            for b in books_for_matching:
                try:
                    ...
                    if dates:
                        ...
                        if (
                            len(dates_sorted) > 1
                            and dates_sorted[-1] != dates_sorted[0]
                        ):
                            date_span += f" → {dates_sorted[-1]}"
                    label = (
                        b.get("csv_name")
                        or b.get("matched_title")
                        or b.get("title")
                        or "Unknown"
                    )
                    if date_span:
...
def api_import_cancel(task_id):
    ...
    safe_update_import_job(
        current_user.id, task_id, {"cancelled": True, "status": "cancelling"}
    )
        ...
        for err in errors:
            if isinstance(err, dict):
                row = [
                    ...
                    (err.get("message") or err.get("error") or "")
                    .replace("\n", " ")
                    .replace("\r", " ")[:500],
                    err.get("raw_isbn") or "",
                    ...
                    json.dumps(err.get("raw_row"))
                    if isinstance(err.get("raw_row"), dict)
                    else (err.get("raw_row") or ""),
                ]
            else:
...
def debug_import_jobs():
    ...
    if is_admin:
        ...
        debug_info = safe_import_manager.get_jobs_for_admin_debug(
            current_user.id, include_user_data=True
        )

        # Migration is complete - no legacy jobs should exist
        debug_info["migration_status"] = "completed"
        debug_info["security_note"] = (
            "All import jobs now use SafeImportJobManager with user isolation"
        )
...
def direct_import():
    ...
    try:
        ...
        if use_suggested_file and session.get("direct_import_file"):
            ...
            filename = secure_filename(file.filename)
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=".csv", prefix=f"direct_import_{current_user.id}_"
            )
            file.save(temp_file.name)
            temp_path = temp_file.name
        ...
        def run_import():
            try:
                print(
                    f"🚀 [DIRECT_IMPORT] Starting import job {task_id} in background thread"
                )
...
def migrate_sqlite():
    ...
    try:
        ...
        if (
            ...
            or not file.filename.lower().endswith((".db", ".sqlite", ".sqlite3"))
        ):
            flash(
                "Please upload a valid SQLite database file (.db, .sqlite, .sqlite3)",
                "error",
            )
            return redirect(request.url)
...
def detect_sqlite():
    ...
    try:
        ...
        temp_path = os.path.join(
            tempfile.gettempdir(), f"detect_{secrets.token_hex(8)}_{filename}"
        )
        file.save(temp_path)
...
def process_import_job(task_id):
    ...
    print(
        f"⚠️ WARNING: process_import_job({task_id}) called - this function is deprecated and unsafe"
    )
    return
            ...
            try:
                ...
                if job:
                    ...
                    completion_updates = {
                        ...
                        + [f"Import completed! {job['total']} books imported"],
                    }
                    safe_update_import_job(current_user.id, task_id, completion_updates)
            except Exception as e:
                error_updates = {"status": "failed", "error_messages": [str(e)]}
...
async def process_simple_import(import_config):
    ...
    if _META_DEBUG_FLAG:
        logger.debug(
            f"[IMPORT][PROCESS_START] task={task_id} format={format_type} enrich={enable_api_enrichment} mappings={mappings}"
        )
    ...
    if default_import_media_type and default_import_media_type in [
        "physical",
        "ebook",
        "audiobook",
        "kindle",
    ...
    try:
        ...
        try:
            cf_ok, created_cf = await pre_analyze_and_create_custom_fields(
        ...
        with open(csv_file_path, "r", encoding="utf-8") as fh:
            ...
            for idx, scan_row in enumerate(reader, 1):
                ...
                raw_isbn = (
                    scan_row.get("ISBN13")
                    or scan_row.get("isbn13")
                    or scan_row.get("ISBN")
                    or scan_row.get("ISBN/UID")
                    or scan_row.get("isbn")
                )
                if not raw_isbn:
                    continue
                ...
                if _META_DEBUG_FLAG:
                    logger.debug(
                        f"[IMPORT][ISBN_COLLECT] row={idx} raw={raw_isbn!r} cleaned={cleaned!r}"
                    )
        ...
        if enable_api_enrichment and isbn_candidates:
            ...
            if _META_DEBUG_FLAG:
                logger.debug(
                    f"[IMPORT][ISBN_COLLECT] dedup_size={len(uniq)} values={uniq}"
                )
            if uniq:
                max_batch_size = 50
                aggregated_metadata = {}
                for start in range(0, len(uniq), max_batch_size):
                    ...
                    if _META_DEBUG_FLAG:
                        logger.debug(
                            f"[IMPORT][METADATA][BATCH_EXEC] offset={start} size={len(batch)} sample={batch[:3]}"
                        )
                ...
                if _META_DEBUG_FLAG:
                    logger.debug(
                        f"[IMPORT][POST_BATCH] requested={len(uniq)} fetched={len(book_metadata)} keys={list(book_metadata.keys())}"
                    )
        ...
        with open(csv_file_path, "r", encoding="utf-8") as fh:
            ...
            for row_num, row in enumerate(reader, 1):
                try:
                    simplified_book = simplified_service.build_book_data_from_row(
                        row, mappings
                    )
                    # Allow ISBN-only rows (title may be filled after enrichment). Skip only if missing both title and ISBN.
                    if not simplified_book or (
                        not simplified_book.title
                        and not (simplified_book.isbn13 or simplified_book.isbn10)
                    ):
                        processed_count += 1
                        skipped_count += 1
                        raw_title = (
                            ...
                            or row.get("Name")
                            or row.get("Book Name")
                            or "Untitled"
                        )
                        _update_import_progress(
                            user_id,
                            task_id,
                            processed_book={"title": raw_title, "status": "skipped"},
                        )
                        continue
                    ...
                    if enable_api_enrichment and book_metadata:
                        ...
                        if _META_DEBUG_FLAG:
                            logger.debug(
                                f"[IMPORT][ROW_ENRICHED] row={row_num} isbn13={simplified_book.isbn13} isbn10={simplified_book.isbn10} title={simplified_book.title!r}"
                            )
                    ...
                    if isbn_key and enable_api_enrichment:
                        ...
                        in_meta = (
                            isbn_key in book_metadata
                            or (
                                simplified_book.isbn13
                                and simplified_book.isbn13 in book_metadata
                            )
                            or (
                                simplified_book.isbn10
                                and simplified_book.isbn10 in book_metadata
                            )
                        )
                        # Failed if not in metadata OR title is still placeholder (exactly equals any isbn form)
                        placeholder_title = (
                            not simplified_book.title
                            or simplified_book.title.strip() == ""
                            or simplified_book.title
                            in {simplified_book.isbn13, simplified_book.isbn10}
                        )
                        ...
                        if lookup_failed and (not row.get(mappings.get("title", ""))):
                            try:
                                logger.error(
                                    f"[IMPORT][LOOKUP_FAIL] row={row_num} isbn={isbn_key} in_metadata={isbn_key in book_metadata} title_after={simplified_book.title!r}"
                                )
                            except Exception:
                                pass
                            processed_count += 1
                    ...
                    if not simplified_book.reading_status:
                        simplified_book.reading_status = import_config.get(
                            "default_reading_status", ""
                        )

                    # Extract personal metadata fields from SimplifiedBook
                    personal_metadata_for_import = (
                        getattr(simplified_book, "personal_custom_metadata", None) or {}
                    )

                    # Add standard personal fields if present in SimplifiedBook
                    if simplified_book.date_started:
                        personal_metadata_for_import["start_date"] = (
                            simplified_book.date_started
                        )
                    if simplified_book.date_read:
                        personal_metadata_for_import["finish_date"] = (
                            simplified_book.date_read
                        )
                    ...
                    try:
                        ...
                        if add_ex.__class__.__name__ == "BookAlreadyExistsError":
                            ...
                            try:
                                ...
                                from app.services.kuzu_book_service import (
                                    KuzuBookService,
                                )
                                from app.services.kuzu_custom_field_service import (
                                    KuzuCustomFieldService,
                                )

                                kbs = KuzuBookService(user_id=user_id)
                                existing = (
                                    await kbs.get_book_by_id(duplicate_existing_id)
                                ...
                                try:
                                    ...
                                    if gmeta or pmeta:
                                        cfs.ensure_custom_fields_exist(
                                            user_id, gmeta, pmeta
                                        )
                                        combined_meta = {}
                                        ...
                                        if combined_meta and duplicate_existing_id:
                                            cfs.save_custom_metadata_sync(
                                                book_id=duplicate_existing_id,
                                                user_id=user_id,
                                                custom_metadata=combined_meta,
                                            )
                                            merged_applied = True
                                except Exception as meta_ex:
                                    print(
                                        f"⚠️ [MERGE] Custom metadata merge failed: {meta_ex}"
                                    )
                    ...
                    if result and merged_applied:
                        ...
                        if result:
                            success_count += 1
                    progress_entry = {
                        "title": simplified_book.title or "Untitled",
                        "status": status_value,
                    }
                    progress_update = {
                        "processed": processed_count,
                    ...
                    if not result:
                        error_count += 1
                        message_txt = (
                            "Duplicate book detected but merge failed"
                            if duplicate_existing_id
                            else "Failed to add book to library (service returned no result)"
                        )
                        error_payload = {
                            "row_number": row_num,
                            "title": simplified_book.title or "",
                            "author": getattr(simplified_book, "authors", [""])[0]
                            if getattr(simplified_book, "authors", None)
                            else "",
                            "isbn": simplified_book.isbn13
                            or simplified_book.isbn10
                            or "",
                            "raw_isbn": row.get("ISBN")
                            or row.get("ISBN13")
                            or row.get("ISBN/UID")
                            or "",
                            "file_name": source_filename,
                            "error_type": "duplicate_merge_failed"
                            if duplicate_existing_id
                            else "add_failed",
                            "message": message_txt,
                            "raw_row": {k: v for k, v in row.items() if v},
        ...
        if pending_processed_entries:
            progress_update = {
                ...
                "errors": error_count,
                "skipped": skipped_count,
                "current_book": pending_processed_entries[-1].get("title")
                if pending_processed_entries
                else None,
            }
            _update_import_progress(
        ...
        print(
            f"🎉 [PROCESS_SIMPLE] Done: {success_count} success, {error_count} errors, {skipped_count} skipped"
        )
        if success_count > 0:
            try:
                ...
                def _run_backup():
                    try:
                        from app.services.simple_backup_service import (
                            get_simple_backup_service,
                        )

                        svc = get_simple_backup_service()
                        svc.create_backup(
                            description=f"Post-import backup: {success_count} books added",
                            reason="post_import_books",
                        )
                ...
                logger.warning(
                    f"Failed launching post-import backup thread: {outer_be}"
                )
    except Exception as e:
        traceback.print_exc()
...
def merge_api_data_into_simplified_book(simplified_book, book_metadata, extra_metadata):
    ...
    print(
        f"🔀 [MERGE_API] Starting merge for book with ISBN13: {simplified_book.isbn13}, ISBN10: {simplified_book.isbn10}"
    )
    ...
    if not api_data:
        print("❌ [MERGE_API] No API data found for this book")
    ...
    try:
        if not simplified_book.cover_url or not simplified_book.cover_url.startswith(
            "/covers/"
        ):
    ...
    if api_data.get("title") and api_data["title"] != simplified_book.title:
        print(
            f"🔀 [MERGE_API] Updating title from '{simplified_book.title}' to '{api_data['title']}'"
        )
    ...
    if api_data.get("description"):
        if not simplified_book.description or simplified_book.description.strip() == "":
            ...
            print(
                f"🔀 [MERGE_API] Updated description to: {api_data['description'][:100]}..."
            )
        else:
            print("🔀 [MERGE_API] Description already set, keeping CSV description")
    else:
        print("🔀 [MERGE_API] No description found in API data")

    # Merge publisher (prefer API if CSV doesn't have one) with normalization
    if api_data.get("publisher") and (
        not simplified_book.publisher or str(simplified_book.publisher).strip() == ""
    ):
        try:
    ...
    if api_data.get("published_date") and not simplified_book.published_date:
        ...
        print(
            f"🔀 [MERGE_API] Updated published_date to: '{api_data['published_date']}' (type: {type(api_data['published_date'])})"
        )
    ...
    if api_data.get("categories"):
        ...
        if isinstance(api_categories, list) and api_categories:
            ...
            print(
                f"🔀 [MERGE_API] Updated categories from string: {len(simplified_book.categories)} items"
            )
    else:
    ...
    try:
        ...
        cover_url = normalize_cover_url(
            best_cover.get("cover_url")
            or api_data.get("cover_url")
            or api_data.get("cover")
        )
        if cover_url and not simplified_book.cover_url:
            try:
                ...
                print(
                    f"🔀 [MERGE_API] Updated cover URL to: {processed_url} (source={best_cover.get('source')})"
                )
            except Exception as e:
                simplified_book.cover_url = cover_url
                print(
                    f"🔀 [MERGE_API] Failed to process cover, using raw URL: {cover_url} ({e})"
                )
        elif cover_url:
            print("🔀 [MERGE_API] Cover URL already set, not overriding")
        else:
            print("🔀 [MERGE_API] No cover URL found in API data or best-cover helper")
    except Exception as ce:
    ...
    if api_data.get("description") and not simplified_book.description:
        ...
        print("🔀 [MERGE_API] Description already set, not overriding")
    else:
        print("🔀 [MERGE_API] No description found in API data")
    ...
    if api_data.get("contributors"):
        ...
        illustrators = [
            c["name"] for c in contributors if c.get("role") == "illustrator"
        ]
        ...
        if authors and len(authors) > 1:
            ...
            additional_author_names = (
                authors[1:]
                if authors[0].lower() == simplified_book.author.lower()
                else authors
            )
            if additional_author_names:
                if simplified_book.additional_authors:
                    ...
                    simplified_book.additional_authors = ", ".join(
                        additional_author_names
                    )
                print(
                    f"🔀 [MERGE_API] Set additional_authors: {simplified_book.additional_authors}"
                )
        ...
        if translators:
            ...
            simplified_book.global_custom_metadata["translators"] = ", ".join(
                translators
            )
        ...
        if illustrators:
            ...
            simplified_book.global_custom_metadata["illustrators"] = ", ".join(
                illustrators
            )
        ...
        print(
            f"    Authors: {len(authors)}, Editors: {len(editors)}, Translators: {len(translators)}, Narrators: {len(narrators)}, Illustrators: {len(illustrators)}"
        )
    ...
    if api_data.get("google_books_id"):
        simplified_book.global_custom_metadata["google_books_id"] = api_data[
            "google_books_id"
        ]
    ...
    if api_data.get("openlibrary_id"):
        ...
        simplified_book.global_custom_metadata["openlibrary_id"] = (
            normalized_olid or api_data["openlibrary_id"]
        )
        # Ensure core field is set so templates and storage persist it
        if not getattr(simplified_book, "openlibrary_id", None):
            simplified_book.openlibrary_id = (
                normalized_olid or api_data["openlibrary_id"]
            )
    ...
    print(
        f"    Description: {simplified_book.description[:100] if simplified_book.description else None}..."
    )
...
def batch_fetch_book_metadata(isbns):
    ...
    for i, raw_isbn in enumerate(isbns):
        ...
        if not _is_valid_isbn(isbn):
            if _META_DEBUG_FLAG:
                logger.debug(
                    f"[IMPORT][METADATA][SKIP_INVALID] idx={i} isbn={isbn or raw_isbn}"
                )
            continue
    ...
    if not valid_entries:
        if _META_DEBUG_FLAG:
            logger.debug(
                "[IMPORT][METADATA][BATCH_COMPLETE] success=0 failed=0 rate=0.0% keys=[]"
            )
        return metadata
    ...
    env_override_workers = _read_int_env(
        "IMPORT_METADATA_CONCURRENCY", env_default_workers
    )
    admin_override_workers = _read_admin_concurrency()
    configured_workers = (
        admin_override_workers
        if admin_override_workers is not None
        else env_override_workers
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(_fetch_single, idx, isbn) for idx, isbn in valid_entries
        ]
        for future in as_completed(futures):
            ...
            if _META_DEBUG_FLAG:
                logger.debug(
                    f"[IMPORT][METADATA][PROVIDERS] isbn={isbn} providers={provider_errors}"
                )
            ...
            if data:
                ...
                try:
                    ...
                    if alt13 and alt13 != isbn and alt13 not in metadata:
                        ...
                        if _META_DEBUG_FLAG:
                            logger.debug(
                                f"[IMPORT][METADATA][CANON] promoted_isbn13={alt13} original_query={isbn}"
                            )
                    for alt in [alt10]:
                    ...
                    if alt13 and alt10 and alt13 != alt10 and _META_DEBUG_FLAG:
                        logger.debug(
                            f"[IMPORT][METADATA][ALT_INDEX] isbn_query={isbn} alt13={alt13} alt10={alt10}"
                        )
                except Exception:
                    pass
                if _META_DEBUG_FLAG:
                    logger.debug(
                        f"[IMPORT][METADATA][FETCH_OK] isbn={isbn} title={data.get('title')}"
                    )
            else:
    ...
    if _META_DEBUG_FLAG:
        ...
        logger.debug(
            f"[IMPORT][METADATA][BATCH_COMPLETE] success={len(metadata)} failed={len(failed_isbns)} rate={success_rate:.1f}% keys={list(metadata.keys())}"
        )
    if failed_isbns:
        print(f"⚠️ [UNIFIED_API] Failed to fetch: {failed_isbns}")
        logger.error(
            f"[IMPORT][METADATA] Batch fetch failures ({len(failed_isbns)}): {failed_isbns}"
        )
    else:
...
def simple_csv_import():
    ...
    try:
        ...
        # Save file temporarily
        temp_file = tempfile.NamedTemporaryFile(
            delete=False, suffix=".csv", prefix=f"simple_import_{current_user.id}_"
        )
        file.save(temp_file.name)
        temp_path = temp_file.name
        ...
        if confidence < 0.5:
            flash(
                "Could not automatically detect file format. Please use the manual import.",
                "warning",
            )
            try:
                import os
        ...
        if format_type == "goodreads":
            mappings = get_goodreads_field_mappings()
            print(
                f"📋 [SIMPLE_IMPORT] Using Goodreads mappings: {len(mappings)} fields"
            )
        elif format_type == "storygraph":
            mappings = get_storygraph_field_mappings()
            print(
                f"📋 [SIMPLE_IMPORT] Using StoryGraph mappings: {len(mappings)} fields"
            )
        else:
        ...
        # Initialize service
        simplified_service = SimplifiedBookService()
        print("🔧 [SIMPLE_IMPORT] Service initialized")
        default_media_type = get_default_book_format()
        ...
        try:
            with open(temp_path, "r", encoding="utf-8") as csvfile:
                ...
                for row_num, row in enumerate(rows, 1):
                    try:
                        print(
                            f"📖 [SIMPLE_IMPORT] Processing row {row_num}/{len(rows)}"
                        )
            ...
            try:
                ...
                os.unlink(temp_path)
                print("🗑️ [SIMPLE_IMPORT] Cleaned up temp file")
            except Exception as cleanup_error:
                print(f"Warning: Could not clean up temp file: {cleanup_error}")

        # Report results
        print(
            f"🎉 [SIMPLE_IMPORT] Completed: {success_count} success, {error_count} errors, {processed_count} total"
        )

        if success_count > 0:
            if error_count > 0:
                flash(
                    f"Import completed! {success_count} books imported, {error_count} errors occurred.",
                    "warning",
                )
            else:
                flash(
                    f"Import completed successfully! {success_count} books imported.",
                    "success",
                )
        else:
            flash(
                f"Import failed. {error_count} errors occurred. No books were imported.",
                "error",
            )
...
def simple_upload():
    ...
    try:
        ...
        # Save file temporarily
        temp_file = tempfile.NamedTemporaryFile(
            delete=False, suffix=".csv", prefix=f"upload_{current_user.id}_"
        )
        file.save(temp_file.name)
        temp_path = temp_file.name
        ...
        if format_type == "goodreads":
            ...
            print(
                f"📋 [SIMPLE_UPLOAD] Using Goodreads mappings: {len(field_mappings)} fields"
            )
        elif format_type == "storygraph":
            field_mappings = get_storygraph_field_mappings()
            print(
                f"📋 [SIMPLE_UPLOAD] Using StoryGraph mappings: {len(field_mappings)} fields"
            )
        else:
            ...
            try:
                with open(temp_path, "r", encoding="utf-8") as csvfile:
                    ...
                    print(
                        f"📋 [SIMPLE_UPLOAD] Auto-detected {len(field_mappings)} field mappings"
                    )
            except Exception as e:
        ...
        import_config = {
            ...
            "field_mappings": field_mappings,
            "default_reading_status": "",
            "enable_api_enrichment": request.form.get(
                "enable_api_enrichment", "true"
            ).lower()
            == "true",
        }
        ...
        job_data = {
            ...
            "recent_activity": [
                f"Started import of {file.filename} ({total_books} books)"
            ],
        }
        # Store job data with user isolation
        safe_create_import_job(current_user.id, task_id, job_data)
        ...
        def process_upload():
            try:
                print(
                    f"🚀 [SIMPLE_UPLOAD] Starting background processing for {task_id}"
                )
        ...
        print(
            f"🚀 [SIMPLE_UPLOAD] Background processing started for {import_config['task_id']}"
        )
...
async def pre_analyze_and_create_custom_fields(csv_file_path, field_mappings, user_id):
    ...
    try:
            ...
            if csv_field in csv_headers and mapped_field.startswith("custom_"):
                ...
                if mapped_field.startswith("custom_global_"):
                    ...
                    field_name = mapped_field[
                        7:
                    ]  # Remove 'custom_' prefix - default to global
                    is_global = True
        ...
        for field in custom_fields_to_create:
            print(
                f"   - {field['name']} ({field['type']}, {'global' if field['is_global'] else 'personal'})"
            )
        ...
        for field_info in custom_fields_to_create:
            try:
                ...
                field_def = custom_field_service.create_field_sync(
                    user_id,
                    {
...
def infer_field_type_from_samples(samples):
    ...
    # Clean samples
    clean_samples = [
        str(s).strip() for s in samples if s is not None and str(s).strip()
    ]
...
def import_reading_history():
    ...
    if request.method == "GET":
        ...
        if csv_file_path and os.path.exists(csv_file_path):
            try:
                ...
                return render_template(
                    "import_reading_history_mapping.html",
                    csv_file_path=csv_file_path,
                    csv_headers=headers,
                    csv_preview=preview_rows,
                    total_rows=total_rows,
                ...
                current_app.logger.error(
                    f"Error preparing mapping for handed-off reading history file: {e}"
                )
    ...
    try:
        ...
        temp_file = tempfile.NamedTemporaryFile(
            delete=False, suffix=".csv", prefix=f"reading_history_{current_user.id}_"
        )
        ...
        if missing_columns:
            flash(
                f'Missing required columns: {", ".join(missing_columns)}. Please ensure your CSV has a "Date" column.',
                "error",
            )
            os.unlink(temp_path)
            return redirect(url_for("import.import_reading_history"))
    ...
    try:
        ...
        if current_app.static_folder and os.path.exists(current_app.static_folder):
            template_path = os.path.join(
                current_app.static_folder, "templates", "reading_history_template.csv"
            )
            ...
            template_path = os.path.join(
                app_root, "static", "templates", "reading_history_template.csv"
            )
        ...
        if not os.path.exists(template_path):
            ...
            fallback_paths = [
                ...
                os.path.join(
                    os.getcwd(), "static", "templates", "reading_history_template.csv"
                ),
            ]
            ...
            for fallback_path in fallback_paths:
                ...
                current_app.logger.error(
                    "Template file not found at any expected location"
                )
                current_app.logger.error(
                    f"Tried paths: {[template_path] + fallback_paths}"
                )
...
def quick_add_reading_history_preview():
    ...
    today = date.today()
    target_dates = (
        [today - timedelta(days=i) for i in range(1, days + 1)] if days > 0 else []
    )
    target_dates.sort()
    if target_dates:
    ...
    if _is_ajax_request():
        preview_html = render_template(
            "partials/_quick_add_preview_summary.html", **context
        )
...
def quick_add_reading_history_execute_bulk():
    ...
    if errors:
        if _is_ajax_request():
            result_html = render_template(
                "partials/_quick_add_result_summary.html", **base_context
            )
            return jsonify(
                {
                    "status": "error",
    ...
    try:
        ...
        current_app.logger.error(
            f"Quick add placeholder resolution failed: {helper_error}"
        )
        unassigned_book_id = None

    if not unassigned_book_id:
        errors.append(
            "Unable to locate or create the placeholder book used for quick add logs."
        )
    ...
    for offset in range(1, days + 1):
        ...
        try:
            ...
            failed_dates.append(log_date)
            current_app.logger.error(
                f"Quick add log creation failed for {log_date}: {create_error}"
            )
        ...
        return jsonify(
            {
                "status": status,
                "html": result_html,
                "modal_title": "Quick Add Results"
                if status != "error"
                else "Quick Add Issues",
                "created_count": created_count,
...
def reading_history_book_matching(task_id):
    ...
    try:
        ...
        if isinstance(books_for_matching, list):
            books_for_matching = [
                b for b in books_for_matching if not b.get("is_bookless", False)
            ]
        ...
        if not isinstance(books_for_matching, list):
            current_app.logger.error(
                f"books_for_matching is not a list: {type(books_for_matching)} = {books_for_matching}"
            )
            books_for_matching = []

        if not isinstance(validation_errors, list):
            current_app.logger.error(
                f"validation_errors is not a list: {type(validation_errors)} = {validation_errors}"
            )
            validation_errors = []
        ...
        user_books = book_service.get_user_books_sync(
            current_user.id
        ...
        if not isinstance(user_books, list):
            current_app.logger.error(
                f"user_books is not a list: {type(user_books)} = {user_books}"
            )
            user_books = []
        ...
        return render_template(
            "import_reading_history_book_matching.html",
            task_id=task_id,
...
def api_search_book_details():
    ...
    try:
        ...
        current_app.logger.info(
            f"[IMPORT_SEARCH] Unified search request: query='{query}'"
        )
            ...
            results.append(
                {
                    ...
                    "cover_url": process_image_from_url(
                        item.get("cover_url") or item.get("cover") or ""
                    )
                    if (item.get("cover_url") or item.get("cover"))
                    else "",
        ...
        current_app.logger.info(
            f"[IMPORT_SEARCH] Returning {len(results)} unified results"
        )
        return jsonify(
...
def resolve_reading_history_books(task_id):
    ...
    try:
        ...
        for book_data in books_for_matching:
            try:
                if (
                    book_data.get("is_bookless")
                    or book_data.get("csv_name") == "[BOOKLESS_ENTRY]"
        ...
        safe_update_import_job(
            ...
            task_id,
            {
                ...
                "recent_activity": [
                    "Processing book matches and creating reading logs..."
                ],
            },
        )
        ...
        def process_reading_logs():
            try:
                asyncio.run(
                    _process_final_reading_history_import(
                        task_id, job_data, book_resolutions
                    )
                )
            except Exception as e:
                ...
                safe_update_import_job(
                    job_data["user_id"],
                    task_id,
                    {"status": "failed", "error_messages": [str(e)]},
                )
        ...
        flash(
            "Processing book matches. Please check progress page for updates.", "info"
        )
        return redirect(url_for("import.import_books_progress", task_id=task_id))
...
async def _process_final_reading_history_import(task_id, job_data, book_resolutions):
    ...
    try:
        ...
        default_location_id = (
            getattr(default_location_obj, "id", None) if default_location_obj else None
        )
    except Exception:
    ...
    safe_update_import_job(
        ...
        task_id,
        {
            ...
            "books_created": 0,
            "recent_activity": [
                f"Starting import of {total_entries_in_csv} reading log entries..."
            ],
        },
    )
    try:
        ...
        if isinstance(book_resolutions, dict):
            for csv_name, resolution in book_resolutions.items():
                ...
                try:
                    ...
                    use_isbn_lookup = bool(
                        api_meta.get("_use_isbn_lookup") and selected_isbn
                    )
                    ...
                    if use_isbn_lookup and selected_isbn:
                        try:
                            from app.utils.metadata_aggregator import (
                        ...
                        author = (
                            ...
                            or (unified.get("authors") or ["Unknown Author"])[0]
                        )
                        new_book = SimplifiedBook(title=title, author=author)
                        new_book.isbn13 = (
                            selected_isbn
                            if len(selected_isbn) == 13
                            else unified.get("isbn_13")
                        )
                        new_book.isbn10 = (
                            selected_isbn
                            if len(selected_isbn) == 10
                            else unified.get("isbn_10")
                        )
                        ...
                        new_book.cover_url = unified.get("cover_url") or unified.get(
                            "cover"
                        )
                    elif manual_creation:
                        new_book = SimplifiedBook(
                            title=resolution.get("title") or "Unknown Title",
                            author=resolution.get("author") or "Unknown Author",
                        )
                    if new_book:
                        try:
                            from app.utils.book_utils import get_best_cover_for_book

                            best = get_best_cover_for_book(
                                isbn=selected_isbn,
                        ...
                        if created_id:
                            ...
                            try:
                                if (
                                    new_book.title
                                    and new_book.title not in created_book_titles
                                ):
                                ...
                                if default_location_id and location_service:
                                    try:
                                        location_service.add_book_to_location(
                                            created_id, default_location_id, user_id
                                        )
                                    except Exception:
                                        pass
                            except Exception:
            ...
            for entry in entries:
                try:
                    ...
                    if created_log:
                        ...
                        if act == "match":
                            try:
                                matched_book_obj = book_service.get_book_sync(
                                    target_book_id
                                )
                                if matched_book_obj:
                                    display_name = (
                                        getattr(matched_book_obj, "title", None)
                                        or getattr(matched_book_obj, "name", None)
                                        or (
                                            matched_book_obj.get("title")
                                            if isinstance(matched_book_obj, dict)
                                            else csv_name
                                        )
                                    )
                            except Exception:
                    ...
                    if (
                        total_entries_processed % 5 == 0
                        or total_entries_processed == total_entries_in_csv
                    ):
                        safe_update_import_job(
                            user_id,
                            task_id,
                            {
                                ...
                                "recent_activity": [
                                    f"Processing reading logs: {total_entries_processed}/{total_entries_in_csv} entries completed"
                                ],
                            },
                        )
        matched_existing = 0
        ...
        if isinstance(book_resolutions, dict):
            ...
            for bid, csv_name in matched_ids.items():
                try:
                    ...
                    title = None
                    if bobj:
                        title = getattr(bobj, "title", None) or getattr(
                            bobj, "name", None
                        )
        ...
        distinct_books_total = len(
            [b for b in books_for_matching if b.get("csv_name") != "[BOOKLESS_ENTRY]"]
        )
            ...
            try:
                ...
                def _run_backup():
                    try:
                        ...
                        svc = get_simple_backup_service()
                        svc.create_backup(
                            description=f"Post-reading-history import backup: {success_count} logs",
                            reason="post_import_reading_history",
                        )
                    except Exception as be:
                ...
                logger.warning(
                    f"Failed launching post-reading-history backup thread: {outer_be}"
                )
    except Exception as e:
...
async def process_reading_history_import(import_config):
    ...
    try:
        ...
        with open(csv_file_path, "r", encoding="utf-8") as csvfile:
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                if processed_entries % 10 == 0:
                    safe_update_import_job(
                        ...
                        task_id,
                        {
                            ...
                            "recent_activity": [
                                f"Analyzing entry {processed_entries}..."
                            ],
                        },
                    )
                ...
                try:
                    ...
                    minutes_read_str = row.get(
                        mappings.get("Minutes Read", ""), ""
                    ).strip()
                    ...
                    if not book_name or book_name.strip() == "":
                        ...
                        print(
                            f"ℹ️ [READING_HISTORY] Row {row_num}: Empty book name, will create bookless reading log"
                        )
                    ...
                    if pages_read <= 0 and minutes_read <= 0:
                        validation_errors.append(
                            f"Row {row_num}: Must have either pages read or minutes read"
                        )
                        continue
        ...
        print(
            f"📚 [READING_HISTORY] Found {len(distinct_books)} distinct books in {total_entries} entries"
        )
        ...
        safe_update_import_job(
            ...
            task_id,
            {
                ...
                "total_entries": total_entries,
                "recent_activity": [
                    f"Analysis complete: {len(distinct_books)} distinct books found from {total_entries} entries"
                ],
            },
        )
        ...
        for book_name, entries in distinct_books.items():
            books_processed += 1
            print(
                f"🔍 [READING_HISTORY] Searching for book: '{book_name}' ({books_processed}/{len(distinct_books)})"
            )
            ...
            if books_processed % 5 == 0 or books_processed == len(distinct_books):
                auto_matched_so_far = len(
                    [b for b in books_for_matching if b.get("matched_book")]
                )
                bookless_so_far = len(
                    [b for b in books_for_matching if b.get("is_bookless", False)]
                )
                need_review_count = (
                    len(distinct_books) - auto_matched_so_far - bookless_so_far
                )
                ...
                safe_update_import_job(
                    ...
                    task_id,
                    {
                        ...
                        "total_entries": total_entries,
                        "recent_activity": [
                            f"Matching books: {books_processed}/{len(distinct_books)} books processed"
                        ],
                    },
                )
            ...
            if book_name == "[BOOKLESS_ENTRY]":
                print("🔍 [READING_HISTORY] Skipping search for bookless entry")
            ...
            print(
                f"🔍 [READING_HISTORY] Found {len(search_results)} search results for '{book_name}'"
            )
            ...
            if search_results:
                ...
                for result in search_results:
                    result_title = (
                        ...
                        else result.get("title", "").lower().strip()
                    )
                    if result_title == book_name_lower:
                        matched_book = {
                            ...
                            else result.get("author"),
                        }
                        print(
                            f"✅ [READING_HISTORY] Auto-matched: '{book_name}' -> '{matched_book['title']}'"
                        )
                        break

                if not matched_book:
                    print(
                        f"⚠️ [READING_HISTORY] No exact match found for '{book_name}', will need user selection"
                    )
            else:
        ...
        needs_user_matching = auto_matched_count + bookless_count < len(
            books_for_matching
        )
        ...
        if needs_user_matching:
            ...
            safe_update_import_job(
                ...
                task_id,
                {
                    ...
                    "need_review": len(distinct_books)
                    - auto_matched_count
                    - bookless_count,  # Books needing manual review
                    "validation_errors_count": len(
                        validation_errors
                    ),  # Use different key for count
            ...
            print(
                "🎯 [READING_HISTORY] Analysis complete. Ready for user book matching."
            )
        else:
            # All books are auto-matched, proceed directly to final processing
            print(
                "🎉 [READING_HISTORY] All books auto-matched! Proceeding to final processing..."
            )
            ...
            safe_update_import_job(
                ...
                task_id,
                {
                    ...
                    "import_type": "reading_history",
                    "recent_activity": [
                        "All books auto-matched, creating reading logs..."
                    ],
                },
            )
        ...
        safe_update_import_job(
            user_id, task_id, {"status": "failed", "error_messages": [str(e)]}
        )
        raise
