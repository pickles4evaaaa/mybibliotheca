        ...
        for s in series_list[:25]:  # limit log volume
            try:
                _ca.logger.debug(
                    ...
                    getattr(s, "id", None),
                    getattr(s, "name", None),
                    getattr(s, "cover_url", None),
                    getattr(s, "custom_cover", None),
                )
            except Exception:
                pass
    except Exception:
...
def upload_series_cover(series_id: str):
    ...
    try:
        ...
        svc.update_series_cover(
            series_id, abs_url, custom=True, generated_placeholder=False
        )
...
def clear_series_cover(series_id: str):
    ...
    t0 = time.time()
    logger.info(
        f"[SERIES][CLEAR_COVER][{trace_id}] START series_id=%s user_id=%s",
        series_id,
    ...
    if not series_obj:
        ...
        return jsonify(
            {"success": False, "error": "Series not found", "trace_id": trace_id}
        ), 404
    ...
    try:
        ...
        logger.exception(
            f"[SERIES][CLEAR_COVER][{trace_id}] Failed logging series snapshot"
        )
    ...
    params = {"id": series_id}
    logger.debug(
        f"[SERIES][CLEAR_COVER][{trace_id}] Executing query=%s params=%s", q, params
    )
    ...
    try:
        ...
        logger.debug(
            f"[SERIES][CLEAR_COVER][{trace_id}] safe_execute_kuzu_query returned type=%s",
            type(res).__name__,
        )
            ...
            while True:
                try:
                    ...
                    if first_row is None:
                        first_row = (
                            list(row) if not isinstance(row, (list, tuple)) else row
                        )
                    row_count += 1
                except Exception as iter_e:
                    logger.error(
                        f"[SERIES][CLEAR_COVER][{trace_id}] Iteration error: %s", iter_e
                    )
                    break
        else:
            logger.warning(
                f"[SERIES][CLEAR_COVER][{trace_id}] Result object has no has_next attribute or is None"
            )
    except Exception as qe:
        logger.exception(
            f"[SERIES][CLEAR_COVER][{trace_id}] Query execution failed: {qe}"
        )
        return jsonify({"success": False, "error": str(qe), "trace_id": trace_id}), 500

    logger.debug(
        f"[SERIES][CLEAR_COVER][{trace_id}] Query rows=%d first_row=%r",
        row_count,
        first_row,
    )
    ok = row_count > 0
    ...
    try:
        ...
        logger.exception(
            f"[SERIES][CLEAR_COVER][{trace_id}] Failed extracting fallback_cover"
        )
    ...
    if old_custom and isinstance(old_custom, str) and old_custom.startswith("/covers/"):
        try:
            ...
            if p.exists():
                ...
                file_deleted = True
                logger.debug(
                    f"[SERIES][CLEAR_COVER][{trace_id}] Deleted previous custom cover file path=%s",
                    p,
                )
            else:
                logger.debug(
                    f"[SERIES][CLEAR_COVER][{trace_id}] Previous custom cover file not found path=%s",
                    p,
                )
        except Exception as fe:
            file_delete_error = str(fe)
            logger.error(
                f"[SERIES][CLEAR_COVER][{trace_id}] Failed deleting old cover: %s", fe
            )
            logger.debug(
                "[SERIES][CLEAR_COVER][%s] Traceback:\n%s",
                trace_id,
                traceback.format_exc(),
            )
    else:
        logger.debug(
            f"[SERIES][CLEAR_COVER][{trace_id}] No old_custom file deletion needed value=%r",
            old_custom,
        )

    dt = (time.time() - t0) * 1000.0
    logger.info(
        f"[SERIES][CLEAR_COVER][{trace_id}] END success=%s ms=%.2f rows=%d file_deleted=%s",
        ok,
        dt,
        row_count,
        file_deleted,
    ...
    logger.debug(
        f"[SERIES][CLEAR_COVER][{trace_id}] effective_cover=%r", effective_cover
    )
    return jsonify(
        {
            "success": ok,
...
def delete_series(series_id: str):
    ...
    try:
        ...
        q = "MATCH (s:Series {id:$id}) DETACH DELETE s"
...
def attach_book_api(series_id: str):
    ...
    try:
        ok = svc.attach_book(
            book_id, series_id, volume=volume, order_number=order_number
        )
        if not ok:
