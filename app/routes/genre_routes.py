    ...
    try:
        ...
        def calculate_book_counts(categories):
            for cat in categories:
                ...
                if cat_id:
                    ...
                    category_books = book_service.get_books_by_category_sync(
                        cat_id, str(current_user.id)
                    )
        ...
        def get_children_for_categories(categories):
            for cat in categories:
                cat_id = get_attr(cat, "id")
                if cat_id:
                    children = book_service.get_category_children_sync(
                        cat_id, str(current_user.id)
                    )
                    if isinstance(cat, dict):
        ...
        root_categories = (
            book_service.get_root_categories_sync(str(current_user.id)) or []
        )
        ...
        for category in all_categories:
            ...
            if category_id:
                category_books = (
                    book_service.get_books_by_category_sync(
                        category_id, str(current_user.id)
                    )
                    or []
                )
...
def hierarchy_view():
    ...
    try:
        ...
        current_app.logger.info(
            f"Got {len(root_categories) if root_categories else 0} root categories"
        )
...
def merge_categories():
    ...
    if request.method == "GET":
        try:
            ...
            current_app.logger.info(
                f"Rendering merge page with form and {len(all_categories)} categories"
            )
            return render_template(
                "genres/merge.html", categories=all_categories, form=form
            )
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            primary_category = book_service.get_category_by_id_sync(
                primary_category_id, None
            )
            merge_categories = [
                book_service.get_category_by_id_sync(cat_id, None)
                for cat_id in merge_category_ids
            ]
            merge_categories = [
                cat for cat in merge_categories if cat
            ...
            # Perform merge
            success = book_service.merge_categories_sync(
                primary_category_id, merge_category_ids
            )

            if success:
                merge_names = [
                    get_attr(cat, "name", "Unknown") for cat in merge_categories
                ]
                primary_name = get_attr(primary_category, "name", "Unknown")
                flash(
                    f'Successfully merged "{merge_names[0]}" into "{primary_name}".',
                    "success",
                )
                return redirect(
                    url_for("genres.category_details", category_id=primary_category_id)
                )
            else:
...
def category_details(category_id):
    ...
    try:
        category = book_service.get_category_by_id_sync(
            category_id, str(current_user.id)
        )
        if not category:
        ...
        # Get subcategories
        subcategories = book_service.get_category_children_sync(
            category_id, str(current_user.id)
        )
        ...
        if hasattr(category, "parent_id") and category.parent_id:
            parent_category = book_service.get_category_by_id_sync(
                category.parent_id, str(current_user.id)
            )
        ...
        if subcategories:
            for subcat in subcategories:
                subcat_id = (
                    ...
                    else getattr(subcat, "id", None)
                )
                if subcat_id:
                    subcat_books = book_service.get_books_by_category_sync(
                        subcat_id, str(current_user.id), include_subcategories=True
                    )
                    if subcat_books:
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            category_data = {
                ...
                if form.color.data
                else generate_random_color(),
                "icon": form.icon.data if form.icon.data else None,
                "aliases": [
                    alias.strip()
                    for alias in form.aliases.data.split("\n")
                    if alias.strip()
                ]
            ...
            if new_category:
                ...
                return redirect(
                    url_for("genres.category_details", category_id=new_category.id)
                )
            else:
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            updated_category_data = {
                ...
                "color": form.color.data
                if form.color.data
                else getattr(category, "color", None),
                "icon": form.icon.data
                if form.icon.data
                else getattr(category, "icon", None),
                "aliases": [
                    alias.strip()
                    for alias in form.aliases.data.split("\n")
                    if alias.strip()
            ...
            if success:
                ...
                return redirect(
                    url_for("genres.category_details", category_id=category_id)
                )
            else:
                flash("Error updating category.", "error")
                return render_template(
                    "genres/add_edit.html", form=form, category=category
                )
        else:
            return render_template("genres/add_edit.html", form=form, category=category)
...
def delete_category(category_id):
    ...
    try:
        ...
        if subcategories and len(subcategories) > 0:
            flash(
                f'Cannot delete category "{category_name}" because it contains {len(subcategories)} subcategories.',
                "error",
            )
        ...
        if success:
            flash(
                f'Category "{category_name}" deleted successfully. All book associations have been removed.',
                "success",
            )
            return redirect(url_for("genres.index"))
        else:
...
def bulk_delete_categories():
    ...
    try:
        ...
        current_app.logger.info(
            f"Bulk deleting {len(selected_categories)} categories, force_delete={force_delete}"
        )
            ...
            try:
                ...
                if not category:
                    current_app.logger.warning(
                        f"Category {category_id} not found, skipping"
                    )
                    skipped_count += 1
                    continue
                ...
                if not force_delete:
                    ...
                    if subcategories and len(subcategories) > 0:
                        current_app.logger.warning(
                            f"Skipping category {category_name} - has subcategories"
                        )
                        skipped_count += 1
                        continue
        ...
        if deleted_count > 0:
            flash(
                f"Successfully deleted {deleted_count} categories and removed their book associations.",
                "success",
            )
        if skipped_count > 0:
...
def search_categories():
    ...
    try:
        ...
        # Search categories
        results = book_service.search_categories_sync(
            query, limit=50, user_id=str(current_user.id)
        )
        if results is None:
            results = []
...
def api_search_categories():
    ...
    try:
        ...
        # Search categories
        results = book_service.search_categories_sync(
            query, limit=limit, user_id=str(current_user.id)
        )
        if results is None:
            results = []
...
def api_create_category():
    ...
    try:
        ...
        existing = book_service.search_categories_sync(
            data["name"], limit=1, user_id=str(current_user.id)
        )
