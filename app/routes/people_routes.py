...
def people():
    ...
    try:
        ...
        all_persons = safe_call_sync_method(
            book_service.list_all_persons_sync, str(current_user.id)
        )
        ...
        for i, person in enumerate(all_persons):
            ...
            try:
                ...
                if person_id:
                    ...
                    all_books_by_person = safe_call_sync_method(
                        person_service.get_books_by_person_sync, person_id
                    )
        ...
        try:
            total_with_books = sum(
                1 for p in processed_persons if getattr(p, "book_count", 0) > 0
            )
        except Exception:
            pass
        ...
        try:
            contribution_counts = safe_call_sync_method(
                person_service.get_contribution_type_counts_sync
            )
        except Exception:
            contribution_counts = {}
...
def person_details(person_id):
    ...
    try:
        ...
        person_name = getattr(person, "name", None) or (
            person.get("name") if isinstance(person, dict) else "Unknown"
        )
        person_id_val = getattr(person, "id", None) or (
            person.get("id") if isinstance(person, dict) else person_id
        )

        # Get books by this person for current user
        books_by_type = book_service.get_books_by_person_sync(
    ...
    if request.method == "POST":
        try:
            ...
            if not name:
                ...
                return render_template(
                    "add_person.html", current_year=datetime.now().year
                )
            ...
            # Validate year range
            if birth_year_int and (
                birth_year_int < 0 or birth_year_int > datetime.now().year
            ):
            ...
            if death_year_int and (
                death_year_int < 0 or death_year_int > datetime.now().year
            ):
            ...
            try:
                ...
                if created_person:
                    flash(
                        f'Person "{name}" added successfully with OpenLibrary metadata!',
                        "success",
                    )
                    return redirect(
                        url_for("people.person_details", person_id=person.id)
                    )
                else:
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not name:
                flash("Name is required.", "error")
                return render_template(
                    "edit_person.html", person=person, current_year=datetime.now().year
                )
            ...
            if alternate_names_str:
                try:
                    ...
                    if alternate_names_str.startswith(
                        "["
                    ) and alternate_names_str.endswith("]"):
                        ...
                        if "\n" in alternate_names_str:
                            alternate_names_list = [
                                name.strip()
                                for name in alternate_names_str.split("\n")
                                if name.strip()
                            ]
                        else:
                            alternate_names_list = [
            ...
            if official_links_str:
                try:
                    # Try to parse as existing JSON first
                    if official_links_str.startswith(
                        "["
                    ) and official_links_str.endswith("]"):
                        links_data = json.loads(official_links_str)
                    else:
                        # Split by newlines and create simple link objects
                        urls = [
                            url.strip()
                            for url in official_links_str.split("\n")
                            if url.strip() and url.strip().startswith("http")
                        ]
                        links_data = []
                        for url in urls:
                            links_data.append(
                                {"title": "Official Link", "url": url, "type": ""}
                            )
            ...
            person_name = getattr(person, "name", None) or (
                person.get("name") if isinstance(person, dict) else name
            )
            normalized_name = Person._normalize_name(
                person_name or name
            ...
            person_id_for_storage = getattr(person, "id", None) or (
                person.get("id") if isinstance(person, dict) else person_id
            )

            # Create person data using safe attribute access and the updated values
            created_at_val = getattr(person, "created_at", None) or (
                person.get("created_at") if isinstance(person, dict) else None
            )
            ...
            if not person_id_for_storage:
                person_id_for_storage = (
                    person_id  # Fallback to the original person_id parameter
                )
            ...
            return redirect(
                url_for("people.person_details", person_id=person_id_for_storage)
            )
...
def delete_person(person_id):
    ...
    try:
        ...
        user_books = safe_call_sync_method(
            book_service.get_all_books_with_user_overlay_sync, str(current_user.id)
        )
        ...
        for book_data in all_book_nodes:
            ...
            rel_result = safe_manager.execute_query(
                book_relationships_query, {"book_id": book_id, "person_id": person_id}
            )
            ...
            for rel_data in all_relationships:
                ...
                if not book_exists_in_user_library:
                    ...
                    DELETE r
                    """
                    safe_manager.execute_query(
                        delete_rel_query, {"book_id": book_id, "person_id": person_id}
                    )
        ...
        if total_associated_books > 0:
            flash(
                f'Cannot delete "{person_name}" because they are associated with {total_associated_books} books. Please consider merging with another person instead.',
                "error",
            )
        ...
        person_result = safe_manager.execute_query(
            person_check_query, {"person_id": person_id}
        )
        ...
        try:
            if person_exists:
                ...
                safe_manager.execute_query(
                    delete_person_query, {"person_id": person_id}
                )
...
def parse_comprehensive_openlibrary_data(author_data):
    ...
    if author_data.get("fuller_name") or author_data.get("personal_name"):
        updates["fuller_name"] = author_data.get("fuller_name") or author_data.get(
            "personal_name"
        )
    ...
    if author_data.get("links"):
        ...
        for link in author_data["links"]:
            if isinstance(link, dict) and link.get("url"):
                if (
                    "official" in link.get("title", "").lower()
                    or "website" in link.get("title", "").lower()
                ):
...
def refresh_person_metadata(person_id):
    ...
    try:
        ...
        current_app.logger.info(
            f"Person data retrieved: name='{person_name}', openlibrary_id='{current_openlibrary_id}'"
        )
        ...
        if current_openlibrary_id and current_openlibrary_id.strip():
            current_app.logger.info(
                f"✅ Person has OpenLibrary ID - fetching fresh metadata for person '{person_name}' with OpenLibrary ID: {current_openlibrary_id}"
            )
            ...
            if author_data:
                ...
                current_app.logger.info(
                    f"Updating person {person_id} with comprehensive data: {updates}"
                )
                ...
                if updated_person:
                    ...
                    current_app.logger.error(
                        f"Failed to update person {person_id} in database"
                    )
                    flash(f'Error updating metadata for "{person_name}".', "error")
            else:
                current_app.logger.warning(
                    f"No author data returned from OpenLibrary for ID: {current_openlibrary_id}"
                )
                flash(f'Could not refresh metadata for "{person_name}".', "warning")
        else:
            # Search for the person by name
            current_app.logger.info(
                f"❌ No OpenLibrary ID found (ID='{current_openlibrary_id}') - searching OpenLibrary for person by name: '{person_name}'"
            )
            if not person_name or not person_name.strip():
                current_app.logger.error(
                    f"❌ Person name is empty! Cannot search OpenLibrary. Person ID: {person_id}"
                )
                flash("Person name is missing - cannot search for metadata.", "error")
                return redirect(url_for("people.person_details", person_id=person_id))
            ...
            if search_result:
                ...
                if author_id:
                    ...
                    current_app.logger.info(
                        f"Detailed author data from OpenLibrary: {author_data}"
                    )
                    if author_data:
                        ...
                        updated_person = person_service.update_person_sync(
                            person_id, updates
                        )
                        if updated_person:
                            flash(
                                f'Metadata found and added for "{person_name}".',
                                "success",
                            )
                            metadata_updated = True
                        else:
                            ...
                            flash(
                                f'Error updating metadata for "{person_name}".', "error"
                            )
                    else:
                        current_app.logger.warning(
                            f"No detailed author data returned for OpenLibrary ID: {author_id}"
                        )
                        flash(
                            f'Found author but could not fetch metadata for "{person_name}".',
                            "warning",
                        )
                else:
                    current_app.logger.warning(
                        f"No OpenLibrary ID found in search result: {search_result}"
                    )
                    flash(f'No valid author ID found for "{person_name}".', "warning")
            else:
                current_app.logger.warning(
                    f"No search results returned from OpenLibrary for: '{person_name}'"
                )
        ...
        current_app.logger.error(
            f"Error refreshing metadata for person {person_id}: {e}"
        )
...
def bulk_delete_persons():
    ...
    for person_id in selected_person_ids:
        try:
            ...
            person = book_service.get_person_by_id_sync(person_id)
            person_name = (
                getattr(person, "name", "Unknown Person")
                if person
                else f"Person {person_id[:8]}..."
            )
            ...
            if person:
                ...
                total_books = (
                    sum(len(books) for books in books_by_type.values())
                    if books_by_type
                    else 0
                )
            ...
            try:
                ...
                """
                delete_result = safe_manager.execute_query(
                    delete_person_query, {"person_id": person_id}
                )
                ...
                current_app.logger.error(
                    f"Error deleting person {person_id}: {delete_error}"
                )
                pass

            if (
                deletion_success or person
            ):  # Count as success if we deleted something OR if person was found
                deleted_count += 1
            else:
                failed_count += 1
    ...
    if failed_count > 0:
        if len(failed_persons) <= 5:
            ...
            flash(
                f"Failed to delete {failed_count} person(s): {failed_list}", "warning"
            )
        else:
            flash(
                f"Failed to delete {failed_count} person(s). Some have associated books or encountered errors.",
                "warning",
            )
...
def api_search_persons():
    ...
    try:
        ...
        for person in all_persons:
            person_name = (
                getattr(person, "name", "")
                or (person.get("name") if isinstance(person, dict) else "")
                or ""
            )
            ...
            if query_lower in person_name_lower:
                person_id = getattr(person, "id", "") or (
                    person.get("id") if isinstance(person, dict) else ""
                )
                matching_persons.append({"id": person_id, "name": person_name})
...
def merge_persons():
    ...
    if request.method == "POST":
        try:
            ...
            # Get persons
            primary_person = safe_call_sync_method_merge(
                book_service.get_person_by_id_sync, primary_person_id
            )
            ...
            primary_person_name = (
                primary_person.get("name", "Unknown Person")
                if isinstance(primary_person, dict)
                else getattr(primary_person, "name", "Unknown Person")
            )
            ...
            """
            primary_check_result = safe_manager.execute_query(
                primary_check_query, {"person_id": primary_person_id}
            )
            primary_check_data = _convert_query_result_to_list(primary_check_result)
            if not primary_check_data:
                flash(
                    f'Primary person "{primary_person_name}" not found in database.',
                    "error",
                )
                return redirect(url_for("people.merge_persons"))

            current_app.logger.info(
                f"Primary person validated: {primary_person_name} (ID: {primary_person_id})"
            )

            merge_persons = []
            for person_id in merge_person_ids:
                person = safe_call_sync_method_merge(
                    book_service.get_person_by_id_sync, person_id
                )
                if person:
                    # Also validate this person exists in KuzuDB
                    person_check_result = safe_manager.execute_query(
                        primary_check_query, {"person_id": person_id}
                    )
                    person_check_data = _convert_query_result_to_list(
                        person_check_result
                    )
                    if person_check_data:
                        merge_persons.append(person)
                        current_app.logger.info(
                            f"Merge person validated: {person.get('name' if isinstance(person, dict) else 'name', 'Unknown')} (ID: {person_id})"
                        )
                    else:
                        current_app.logger.warning(
                            f"Person {person_id} not found in KuzuDB, skipping"
                        )
                else:
                    current_app.logger.warning(
                        f"Person {person_id} not found in service layer, skipping"
                    )
            ...
            for merge_person in merge_persons:
                ...
                try:
                    # Handle both dict and object formats
                    merge_person_name = (
                        merge_person.get("name", "Unknown")
                        if isinstance(merge_person, dict)
                        else getattr(merge_person, "name", "Unknown")
                    )
                    merge_person_id = (
                        merge_person.get("id")
                        if isinstance(merge_person, dict)
                        else getattr(merge_person, "id", None)
                    )

                    current_app.logger.info(
                        f"Merging person {merge_person_name} (ID: {merge_person_id}) into {primary_person_name}"
                    )
                    ...
                    check_result = safe_manager.execute_query(
                        check_query, {"merge_person_id": merge_person_id}
                    )
                    check_data = _convert_query_result_to_list(check_result)
                    total_rels = (
                        check_data[0]["total_relationships"] if check_data else 0
                    )
                    current_app.logger.info(
                        f"Person {merge_person_name} has {total_rels} total relationships"
                    )
                    ...
                    transfer_result = safe_manager.execute_query(
                        transfer_query,
                        {
                    ...
                    transferred_count = (
                        transfer_data[0]["transferred_count"] if transfer_data else 0
                    )

                    current_app.logger.info(
                        f"Transferred {transferred_count} AUTHORED relationships from {merge_person_name} to {primary_person_name}"
                    )
                    ...
                    for rel_type in other_relationships:
                        ...
                        other_result = safe_manager.execute_query(
                            other_transfer_query,
                            {
                                "merge_person_id": merge_person_id,
                                "primary_person_id": primary_person_id,
                            },
                        ...
                        other_count = (
                            other_data[0]["transferred_count"] if other_data else 0
                        )
                        if other_count > 0:
                            current_app.logger.info(
                                f"Transferred {other_count} {rel_type} relationships from {merge_person_name} to {primary_person_name}"
                            )
                    ...
                    delete_result = safe_manager.execute_query(
                        delete_query, {"person_id": merge_person_id}
                    )
                    current_app.logger.info(
                        f"Delete query completed for person {merge_person_name}"
                    )

                    merged_count += 1
                    current_app.logger.info(
                        f"Successfully merged person {merge_person_name}"
                    )

                except Exception as e:
                    current_app.logger.error(
                        f"Error merging person {merge_person_name} (ID: {merge_person_id}): {e}"
                    )
                    continue
            ...
            if merged_count > 0:
                ...
                flash(
                    f'Successfully merged {merged_count} person(s) ({", ".join(person_names)}) into "{primary_person_name}".',
                    "success",
                )
            else:
            ...
            return redirect(
                url_for("people.person_details", person_id=primary_person_id)
            )
    ...
    try:
        ...
        for person in all_persons:
            ...
            try:
                ...
                person_id = getattr(person_obj, "id", None)
                if person_id:
                    books_by_type = safe_call_sync_method(
                        person_service.get_books_by_person_for_user_sync,
                        person_id,
                        str(current_user.id),
                    )
                    if books_by_type:
                        ...
                        if isinstance(books_by_type, dict):
                            total_books = sum(
                                len(books) for books in books_by_type.values()
                            )
                ...
                current_app.logger.warning(
                    f"Error getting book count for person {getattr(person_obj, 'name', 'Unknown')}: {person_error}"
                )
                person_obj.book_count = 0
                processed_persons.append(person_obj)
...
def toggle_theme():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Failed to toggle theme"}), 500
