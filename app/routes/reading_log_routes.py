...
def create_reading_log_entry():
    ...
    try:
        ...
        if not book_id:
            return jsonify(
                {"status": "error", "message": "Book selection is required"}
            ), 400
        ...
        try:
            ...
            return jsonify(
                {"status": "error", "message": "Invalid page or minute values"}
            ), 400
        ...
        if created_log:
            ...
            try:
                ...
                if (
                    normalized_status not in terminal_statuses
                    and normalized_status != ReadingStatus.READING.value
                ):
                ...
                logger.warning(
                    f"Unable to ensure reading metadata for book {book_id}: {ensure_exc}"
                )

            return jsonify(
                {
                    "status": "success",
                    "message": f"Logged {pages_read_int} pages"
                    + (
                        f" and {minutes_read_int} minutes"
                        if minutes_read_int > 0
                        else ""
                    )
                    + " of reading!",
                }
            ...
            return jsonify(
                {"status": "error", "message": "Failed to save reading log entry"}
            ), 500
...
def my_reading_logs():
    ...
    try:
        ...
        logs = reading_log_service.get_user_reading_logs_sync(
            current_user.id, days_back=30
        )

        return render_template(
            "reading_logs/my_logs.html", logs=logs, title="My Reading Logs"
        )
    ...
    try:
        ...
        def authors_string(book) -> str:
            ...
            if hasattr(book, "contributors") and getattr(book, "contributors"):
                names = [
                    ...
                    and getattr(c.contribution_type, "value", None)
                    in ["authored", "co_authored"]
                ]
                if names:
        ...
        def qualifies_for_in_progress_prefill(
            ...
            if isinstance(meta, dict):
                meta_status_raw = (
                    meta.get("reading_status")
                    or meta.get("status")
                    or meta.get("relationship_status")
                )
                meta_finish_raw = meta.get("finish_date") or meta.get("finishDate")
                latest_log_raw = (
                    meta.get("latest_log_date")
                    or meta.get("latestLogDate")
                    or meta.get("last_logged_at")
                )

            if _has_finish(meta_finish_raw) or _has_finish(
                getattr(book_obj, "finish_date", None)
            ):
                return False
            ...
            if (
                ...
                or meta_status_norm in terminal_statuses
            ):
                return False

            if (
                normalized_status in in_progress_statuses
                or meta_status_norm in in_progress_statuses
            ):
                return True
                ...
                if isinstance(latest_log_raw, datetime):
                    ...
                    if candidate:
                        ...
                        try:
                            ...
                            try:
                                latest_log_date = date.fromisoformat(
                                    candidate.split("T")[0]
                                )
                            except ValueError:
        ...
        if query:
            matched_books = book_service.search_books_sync(
                query, current_user.id, limit=limit
            )
            results = [make_payload(book) for book in matched_books if book]
            return jsonify(results)

        # 1) Recently read (prioritized)
        recently_read = reading_log_service.get_recently_read_books_sync(
        ...
        for bdict in recently_read or []:
            ...
            try:
                ...
                logger.debug(
                    f"Unable to load personal metadata for book {bid}: {meta_exc}"
                )

            if metadata_snapshot.get("reading_status") and not getattr(
                b, "reading_status", None
            ):
                try:
                    setattr(
                        b, "reading_status", metadata_snapshot.get("reading_status")
                    )
                except Exception:
                    pass
            if metadata_snapshot.get("finish_date") and not getattr(
                b, "finish_date", None
            ):
                try:
            ...
            if suggested_payload is None and qualifies_for_in_progress_prefill(
                b, meta_context
            ):
        ...
        if len(books) < limit:
            recently_added = book_service.get_recently_added_books_sync(
                limit=min(5, limit - len(books))
            )
        ...
        if len(books) < limit:
            ...
            more = book_service.get_books_for_user(
                current_user.id, limit=remaining + 50
            )
            for b in more:
                if len(books) >= limit:
                    break
...
def reading_stats():
    ...
    try:
        ...
        return render_template(
            "reading_logs/stats.html", stats=stats, title="Reading Statistics"
        )
...
def edit_reading_log(log_id):
    ...
    try:
        ...
        existing_log = reading_log_service.get_reading_log_by_id_sync(
        ...
        log_date_str = request.form.get("date") or str(
            existing_log.get("date") or date.today().isoformat()
        )
...
def quick_add_bookless_log():
    ...
    try:
        ...
        if result:
            ...
            return jsonify(
                {"status": "error", "message": "Failed to create reading log"}
            ), 500

    except Exception as e:
        logger.error(f"Error creating quick reading log: {e}")
        return jsonify(
            {"status": "error", "message": "Error creating reading log"}
        ), 500
