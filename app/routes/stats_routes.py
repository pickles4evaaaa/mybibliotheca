...
def network_explorer():
    ...
    try:
        ...
        return render_template(
            "stats/network_explorer.html", network_data=network_data, stats=stats
        )
...
def library_journey():
    ...
    try:
        ...
        logger.info(
            f"Processing {len(user_books)} total books with filters: date_type={date_type}, status={status_filter}"
        )
        ...
        for book in user_books:
            try:
                ...
                try:
                    book_data["authors_text"] = (
                        ", ".join([a for a in book_data["authors"] if a])
                        if isinstance(book_data["authors"], list)
                        else (book_data["authors"] or "Unknown Author")
                    )
                except Exception:
                ...
                try:
                    book_data["categories_text"] = (
                        ", ".join(book_data["categories"])
                        if isinstance(book_data["categories"], list)
                        else (book_data["categories"] or "")
                    )
                except Exception:
                    book_data["categories_text"] = ""
                ...
                book_data["status_color"] = _get_status_color(
                    book_data.get("reading_status")
                )
                book_data["display_title"] = (
                    (
                        book_data["title"][:10]
                        + ("" if len(book_data["title"]) <= 10 else "")
                    )
                    if isinstance(book_data.get("title"), str)
                    else "Unknown"
                )
                ...
                seen = set()
                fallback_order = [
                    f for f in fallback_order if not (f in seen or seen.add(f))
                ]
                target_date = None
                source_field = None
                ...
                logger.warning(
                    f"Error processing book {_get_value(book, 'title', 'Unknown')}: {book_error}"
                )
                continue
        ...
        if not processed_books:
            ...
            return render_template(
                "stats/library_journey.html", books=[], stats=stats, filters=filters
            )
        ...
        logger.info(
            f"Successfully processed {len(timeline_books)} books for Library Journey timeline using {date_type}"
        )
...
def reading_history():
    ...
    domain_books = book_service.get_all_books_with_user_overlay_sync(
        str(current_user.id)
    )
    books_data = []
    for user_book in domain_books:
        book_dict = {
            ...
            else getattr(user_book, "id", ""),
            "uid": user_book.get("uid")
            if isinstance(user_book, dict)
            else getattr(user_book, "uid", ""),
            "title": user_book.get("title")
            if isinstance(user_book, dict)
            else getattr(user_book, "title", ""),
            "author": user_book.get("author")
            if isinstance(user_book, dict)
            else getattr(user_book, "author", ""),
            "isbn": user_book.get("isbn")
            if isinstance(user_book, dict)
            else getattr(user_book, "isbn", ""),
            "description": user_book.get("description")
            if isinstance(user_book, dict)
            else getattr(user_book, "description", ""),
            "start_date": _safe_date_to_isoformat(
                user_book.get("start_date")
                if isinstance(user_book, dict)
                else getattr(user_book, "start_date", None)
            ),
            "finish_date": _safe_date_to_isoformat(
                user_book.get("finish_date")
                if isinstance(user_book, dict)
                else getattr(user_book, "finish_date", None)
            ),
            "want_to_read": user_book.get("want_to_read")
            if isinstance(user_book, dict)
            else getattr(user_book, "want_to_read", False),
            "library_only": user_book.get("library_only")
            if isinstance(user_book, dict)
            else getattr(user_book, "library_only", False),
            "cover_url": user_book.get("cover_url")
            if isinstance(user_book, dict)
            else getattr(user_book, "cover_url", None),
            "user_rating": user_book.get("user_rating")
            if isinstance(user_book, dict)
            else getattr(user_book, "user_rating", None),
            "personal_notes": user_book.get("personal_notes")
            if isinstance(user_book, dict)
            else getattr(user_book, "personal_notes", None),
            "status": user_book.get("status")
            if isinstance(user_book, dict)
            else getattr(user_book, "status", None),
            "date_added": _safe_date_to_isoformat(
                user_book.get("date_added")
                if isinstance(user_book, dict)
                else getattr(user_book, "date_added", None)
            ),
        }
...
def index():
            ...
            if custom_blob:
                try:
                    ...
                    if isinstance(parsed, dict):
                        raw_status = parsed.get("reading_status") or parsed.get(
                            "status"
                        )
                except Exception:
                    raw_status = None
        if raw_status in (None, "", "null"):
            ...
            if book_id:
                try:
                    ...
                    metadata = personal_metadata_service.get_personal_metadata(
                        user_id, str(book_id)
                    )
                    raw_status = metadata.get("reading_status") or metadata.get(
                        "status"
                    )
                except Exception as pm_error:
                    current_app.logger.debug(
                        f"Stats status fallback failed for book {book_id}: {pm_error}"
                    )
        if (
            not isinstance(raw_status, str)
            and raw_status is not None
            and hasattr(raw_status, "value")
        ):
    ...
    try:
        reading_logs_recent = (
            reading_log_service.get_user_reading_logs_sync(
                user_id, days_back=365, limit=2000
            )
            or []
        )
    except Exception as log_error:
        current_app.logger.error(
            f"Error loading reading logs for stats overview: {log_error}"
        )
        reading_logs_recent = []
    ...
    for idx, book in enumerate(user_books):
        ...
        finished_flag, activity_date = determine_finished_activity(
            status_normalized, finish_date, fallback_log_date
        )
        if finished_flag:
            if activity_date:
    ...
    try:
        ...
        sharing_users = [
            user for user in all_users if getattr(user, "share_reading_activity", False)
        ]
        ...
        for user in sharing_users:
            ...
            for book in user_books:
                ...
                elif getattr(user, "share_current_reading", False) and getattr(
                    book, "reading_status", None
                ) in ["reading", "currently_reading"]:
                    book_data = {
                        "title": getattr(book, "title", "Unknown"),
                        "author": getattr(book, "author", "Unknown"),
                        "cover_url": getattr(book, "cover_url", None),
        ...
        if hasattr(reading_log_service, "get_recent_shared_logs_sync"):
            recent_logs_result = reading_log_service.get_recent_shared_logs_sync(
    ...
    try:
        if hasattr(reading_log_service, "get_user_all_time_reading_stats_sync"):
            reading_log_stats = (
                reading_log_service.get_user_all_time_reading_stats_sync(
                    str(current_user.id)
                )
            )
    except Exception as e:
...
def community_active_readers():
    ...
    try:
            ...
            for book in user_books:
                ...
                if finish_date:
                    ...
                    if finish_date >= month_start:
                        books_this_month += 1
                elif getattr(book, "reading_status", None) in [
                    "reading",
                    "currently_reading",
                ]:
                    currently_reading += 1
        ...
        user_stats.sort(
            key=lambda x: (x["books_this_month"], x["total_books"]), reverse=True
        )

        return render_template(
            "community_stats/active_readers.html", user_stats=user_stats
        )
    except Exception as e:
...
def community_books_this_month():
    ...
    try:
        ...
        books_this_month.sort(
            key=lambda b: b.get("finish_date", date.min), reverse=True
        )

        return render_template(
            "community_stats/books_this_month.html", books=books_this_month
        )
    except Exception as e:
...
def community_currently_reading():
    ...
    try:
        ...
        sharing_users = [
            ...
            if getattr(user, "share_reading_activity", False)
            and getattr(user, "share_current_reading", False)
        ]
        ...
        for user in sharing_users:
            ...
            for book in user_books:
                ...
                if getattr(book, "reading_status", None) in [
                    "reading",
                    "currently_reading",
                ]:
                    # Create book dict with user info for template
        ...
        currently_reading.sort(
            key=lambda b: b.get("start_date", date.min), reverse=True
        )

        return render_template(
            "community_stats/currently_reading.html", books=currently_reading
        )
    except Exception as e:
...
def reading_logs():
    ...
    try:
        ...
        all_time_stats = reading_log_service.get_user_all_time_reading_stats_sync(
            str(current_user.id)
        )

        return render_template(
            "reading_logs/user_logs.html",
            logs=result["logs"],
...
def community_recent_activity():
    ...
    try:
        # Get recent shared reading logs
        if hasattr(reading_log_service, "get_recent_shared_logs_sync"):
            recent_logs = reading_log_service.get_recent_shared_logs_sync(
                days_back=7, limit=50
            )
        ...
        try:
            ...
            if "author" in contrib_type_str or contrib_type_str == "authored":
                ...
                if person:
                    ...
                    name = _get_value(contributor, "name", None) or _get_value(
                        contributor, "author_name", None
                    )
                    if name:
                        author_names.append(name)
        except Exception:
...
def _extract_date(book, field_name):
    ...
    if field_name == "publication_date":
        date_value = _get_value(book, "publication_date", None) or _get_value(
...
def _calculate_timeline_positions(books, date_type):
    ...
    for book in books:
        ...
        if date_value:
            ...
            book_positions.append(
                {"book": book, "timestamp": timestamp, "year": date_value.year}
            )
    ...
    for i in range(1, len(book_positions)):
        time_diff = (
            book_positions[i]["timestamp"] - current_cluster_books[-1]["timestamp"]
        )
        if time_diff <= cluster_threshold:
            ...
            if len(current_cluster_books) >= 3:  # Cluster if 3 or more books
                avg_timestamp = sum(
                    item["timestamp"] for item in current_cluster_books
                ) / len(current_cluster_books)
                avg_year = sum(item["year"] for item in current_cluster_books) / len(
                    current_cluster_books
                )
                clusters.append(
                    {
                        "type": "cluster",
    ...
    if len(current_cluster_books) >= 3:
        avg_timestamp = sum(item["timestamp"] for item in current_cluster_books) / len(
            current_cluster_books
        )
        avg_year = sum(item["year"] for item in current_cluster_books) / len(
            current_cluster_books
...
def _calculate_timeline_stats(timeline_items):
    ...
    ratings = [
        ...
        if book.get("user_rating") and book.get("user_rating") > 0
    ]
    ...
    pages = [
        book.get("page_count")
        for book in all_books
        if book.get("page_count") and book.get("page_count") > 0
    ]
...
def reading_journey():
    ...
    try:
        ...
        result = reading_log_service.get_user_reading_logs_paginated_sync(
            user_id, page=1, per_page=1000
        )
        all_logs = result.get("logs", []) if result else []
        logger.info(
            f"Retrieved {len(all_logs) if all_logs else 0} reading logs for user {current_user.id}"
        )
        ...
        books_by_id = (
            {
                book.id if hasattr(book, "id") else _get_value(book, "id"): book
                for book in user_books
            }
            if user_books
            else {}
        ...
        logger.info(
            f"Retrieved {len(books_by_id)} full book objects for author extraction"
        )
        ...
        for log in all_logs:
            try:
                ...
                reading_status = (
                    log.get("reading_status")
                    or book_data.get("reading_status")
                    or "read"
                )
                ...
                processed_log = {
                    ...
                    "log_date": log_date,
                    "day": log_date.day,
                    "display_title": book_data.get("title", "Unknown")[:10]
        ...
        active_days = [
            ...
            if day.get("logs") or day.get("clusters")
        ]
        activity_counts = [
            day.get("activity_count", 0)
            for day in calendar_data["days"]
            if day.get("is_current_month")
        ]
        ...
        logger.info(
            f"Successfully generated calendar for {calendar.month_name[month]} {year} with {len(processed_logs)} reading logs"
        )
...
def _generate_calendar_with_logs(year, month, logs):
    ...
    avg_activity = (
        sum(all_activity_counts) / len(all_activity_counts)
        if all_activity_counts
        else 0
    )
    ...
    for book in user_books:
        ...
        if contributors:
            for contributor in contributors:
                ...
                if person and contribution_type:
                    ...
                    contributor_id = (
                        f"contributor_{person_id}_{contrib_type}"
                        if person_id
                        else f"contributor_{person_name.replace(' ', '_').lower()}_{contrib_type}"
                    )
                    ...
                    network_data["contributors"][contributor_id]["books"].append(
                        book_id
                    )
        ...
        if not contributors:
            ...
            for author in authors:
                contributor_id = (
                    f"contributor_{author.replace(' ', '_').lower()}_authored"
                )
                if contributor_id not in network_data["contributors"]:
                    network_data["contributors"][contributor_id] = {
                        "id": contributor_id,
            ...
            if isinstance(custom_metadata, dict):
                for field_name, field_value in custom_metadata.items():
                    if field_value and str(field_value).strip():
                        ...
                        network_data["custom_fields"][custom_field_id]["books"].append(
                            book_id
                        )
