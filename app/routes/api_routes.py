    ...
    try:
        ...
        for book in user_books:
            try:
                ...
                if series:
                    book_data["series"] = (
                        _get_value(series, "name", str(series))
        ...
        logger.info(
            f"Successfully processed {len(timeline_books)} books for user {current_user.id}"
        )

        return jsonify({"books": timeline_books, "total_count": len(timeline_books)})
        ...
        if books_by_finish_date:
            ...
            for book in books_by_finish_date:
                ...
                if current_month is None:
                    ...
                    reading_pace.append(
                        {"month": current_month.isoformat(), "books_read": month_count}
                    )
                    current_month = book_month
                    month_count = 1
