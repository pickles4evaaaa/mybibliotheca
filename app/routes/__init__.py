            ...
            for book in books:
                ...
                if hasattr(book, "contributors") and book.contributors:
                    author_names = [
                        ...
                        if contrib.contribution_type.value
                    ...
                    authors_str = ", ".join(
                        [author.name for author in book.authors[:3]]
                    )
                    if len(book.authors) > 3:
                        authors_str += " et al."

                book_data.append(
                    {"id": book.id, "title": book.title, "authors": authors_str}
                )

            # Sort by title
            book_data.sort(key=lambda x: x["title"].lower())

            return jsonify({"status": "success", "books": book_data})
...
def bulk_delete_books():
    ...
    if not selected_uids:
        single_value = request.form.get("selected_books") or request.form.get(
            "book_ids"
        )
        if single_value:
            ...
            selected_uids = [
                uid.strip()
                for uid in single_value.replace(",", "\n").split("\n")
                if uid.strip()
            ]
    ...
    if deleted_count > 0:
        flash(
            f"Successfully deleted {deleted_count} book(s) from your library.",
            "success",
        )
...
def refresh_person_metadata(person_id):
    ...
    return redirect(
        url_for("people.refresh_person_metadata", person_id=person_id), code=307
    )
