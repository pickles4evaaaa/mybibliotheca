...
simple_backup_bp = Blueprint(
    "simple_backup", __name__, url_prefix="/auth/simple-backup"
)
...
def create_backup():
    ...
    try:
        ...
        backup_info = backup_service.create_backup(
            name=custom_name if custom_name else None, description=description
        )

        if backup_info:
            flash(
                f'✅ Backup "{backup_info.name}" created successfully! ({backup_info.file_size / 1024 / 1024:.2f} MB)',
                "success",
            )
        else:
    ...
    try:
        ...
        if success:
            ...
            if backup_service.check_restart_required():
                flash(
                    f'✅ Successfully restored from backup "{backup_info.name}"! You will be logged out while the application restarts to complete the restore process.',
                    "success",
                )
                ...
                flash(
                    f'✅ Successfully restored from backup "{backup_info.name}"! Your data has been restored.',
                    "success",
                )
...
def restore_complete():
    ...
    def delayed_shutdown():
        ...
        logger.info(
            "Initiating graceful shutdown for container restart after backup restore"
        )
...
def api_backup_settings():
    ...
    try:
        ...
        if request.method == "GET":
            return jsonify(
                backup_service._settings if hasattr(backup_service, "_settings") else {}
            )
        # POST update
            ...
            if key in data:
                ...
                if key == "enabled":
                    ...
                    if freq_value not in VALID_BACKUP_FREQUENCIES:
                        ...
                        return jsonify(
                            {
                                "error": f"Invalid frequency value. Must be one of: {', '.join(VALID_BACKUP_FREQUENCIES)}"
                            }
                        ), 400
        ...
        if not backup_service._save_settings():
            current_app.logger.error(
                f"Failed to save backup settings. Attempted changes: {changed}"
            )
            return jsonify(
                {
        ...
        if settings.get("enabled"):
            try:
                ...
                current_app.logger.info(
                    "Backup scheduler started/ensured after settings update"
                )
            except Exception as e:
            ...
            try:
                ...
                current_app.logger.info(
                    "Backup scheduler stopped after settings update"
                )
            except Exception as e:
