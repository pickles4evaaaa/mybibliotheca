    ...
    with manager.get_connection(operation="quantity_migration") as conn:
        ...
        if dry_run:
            ...
            try:
                ...
                if result.has_next():
                    ...
                    logger.info(
                        f"Migration (dry-run): Would update {rows_to_update} books to quantity = 1"
                    )
                    return {
                        "status": "pending",
                ...
                logger.warning(
                    f"Migration (dry-run): Could not count books to update: {e}"
                )
                return {
                    "status": "pending",
                    "column_added": column_added,
        ...
        try:
            ...
            if result.has_next():
                ...
                logger.info(
                    f"Migration: updated {rows_updated} books with quantity = 1"
                )
        except Exception as e:
