...
def login():
    ...
    form = LoginForm()
    debug_auth("Form created, CSRF token should be generated")
        ...
        if user:
            ...
            if user.check_password(password):
                ...
                if user.password_must_change:
                    debug_auth(
                        "User must change password - redirecting to forced password change"
                    )
                ...
                if attempts_left > 0:
                    flash(
                        f"Invalid password. You have {attempts_left} attempts remaining.",
                        "error",
                    )
                else:
                    flash(
...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            if is_first_user:
                flash(
                    "Congratulations! As the first user, you have been granted admin privileges. You must change your password on first login.",
                    "info",
                )
            else:
                if domain_user:
                    role_label = (
                        "administrator" if domain_user.is_admin else "standard user"
                    )
                    article = (
                        "an"
                        if role_label[0].lower() in ("a", "e", "i", "o", "u")
                        else "a"
                    )
                    flash(
                        f"User {domain_user.username} has been created successfully as {article} {role_label}. They will be required to change their password on first login.",
                        "success",
                    )
                else:
                    fallback_role = (
                        "administrator" if should_be_admin else "standard user"
                    )
                    fallback_article = (
                        "an"
                        if fallback_role[0].lower() in ("a", "e", "i", "o", "u")
                        else "a"
                    )
                    flash(
                        f"User has been created successfully as {fallback_article} {fallback_role}. They will be required to change their password on first login.",
                        "success",
                    )
            ...
            try:
                ...
                if user_from_kuzu:
                    ...
                    if updated_user:
                        ...
                        current_user.password_must_change = (
                            updated_user.password_must_change
                        )
...
def forced_password_change():
    ...
    if form.validate_on_submit():
        ...
        if not new_password:
            ...
            return render_template(
                "auth/forced_password_change.html",
                title="Change Required Password",
                form=form,
            )
        ...
        try:
            ...
            if user_from_kuzu:
                ...
                if updated_user:
                    ...
                    flash(
                        "Your password has been changed successfully. You can now continue using the application.",
                        "success",
                    )
                    return redirect(url_for("main.index"))
                else:
    ...
    return render_template(
        "auth/forced_password_change.html", title="Change Required Password", form=form
    )
...
def settings():
    ...
    try:
        ...
        stats["users"] = (
            user_service.get_user_count_sync()
            if hasattr(user_service, "get_user_count_sync")
            else 0
        )  # type: ignore
        stats["active_users"] = (
            user_service.get_active_user_count_sync()
            if hasattr(user_service, "get_active_user_count_sync")
            else 0
        )  # type: ignore
        stats["admins"] = (
            user_service.get_admin_count_sync()
            if hasattr(user_service, "get_admin_count_sync")
            else 0
        )  # type: ignore
    except Exception as e:
    ...
    if stats["admins"] == 0:
        _simple_count(
            "MATCH (u:User) WHERE u.is_admin = true RETURN COUNT(u)", "admins"
        )
    if stats["active_users"] == 0:
        # Assume is_active flag; default to treating missing flag as active
        _simple_count(
            "MATCH (u:User) WHERE coalesce(u.is_active, true) = true RETURN COUNT(u)",
            "active_users",
        )
    try:
        if stats["users"]:
    ...
    try:
        ...
        if os.path.exists(pyproject_path):
            with open(pyproject_path, "rb") as f:
                data = tomllib.load(f)
                stats["app_version"] = data.get("project", {}).get(
                    "version", stats["app_version"]
                )
    except Exception as e:
        current_app.logger.debug(f"Version load failed: {e}")
    return render_template(
        "settings.html", title="Settings", site_name=site_name, stats=stats
    )
        ...
        try:
            ...
            if not current_user.check_password(form.current_password.data):  # type: ignore
                ...
                if (
                    ...
                    return render_template(
                        "settings/partials/password_form.html", form=form
                    )
                return redirect(url_for("auth.settings"))
        except Exception as e:
...
def settings_privacy_partial():
    ...
    if p_form.validate_on_submit():
        try:
            ...
            user_obj = None
            try:
                user_obj = user_service.get_user_by_id_sync(
                    getattr(current_user, "id", None)
                )
            except Exception as fe:
                current_app.logger.warning(
                    f"Failed to refetch user for privacy update, falling back to current_user: {fe}"
                )
            if user_obj is None:
            ...
            # Apply changes
            setattr(
                user_obj, "share_current_reading", p_form.share_current_reading.data
            )
            setattr(
                user_obj, "share_reading_activity", p_form.share_reading_activity.data
            )
            ...
            if (
                ...
                return render_template(
                    "settings/partials/privacy_form.html",
                    form=p_form,
        ...
        try:
            p_form.share_current_reading.data = getattr(
                current_user, "share_current_reading", False
            )
            p_form.share_reading_activity.data = getattr(
                current_user, "share_reading_activity", False
            )
        except Exception:
            pass
    return render_template(
        "settings/partials/privacy_form.html", form=p_form, streak_form=streak_form
    )
...
def settings_personal_abs_partial():
    ...
    if request.method == "POST":
        ...
        abs_sync_books = (
            True if request.form.get("abs_sync_books") in ("on", "true", "1") else False
        )
        abs_sync_listening = (
            True
            if request.form.get("abs_sync_listening") in ("on", "true", "1")
            else False
        )
        payload = {
    ...
    settings = load_user_settings(getattr(current_user, "id", None))
    return render_template(
        "settings/partials/personal_abs.html",
        settings=settings,
...
def settings_data_partial(panel: str):
    ...
    if panel == "backup":
        ...
        try:
            ...
            for b in backups:
                ...
                if age_delta.days > 0:
                    age = (
                        f"{age_delta.days} day{'s' if age_delta.days != 1 else ''} ago"
                    )
            ...
            return render_template(
                "settings/partials/data_backup_manager.html",
                backups=enhanced,
                backup_stats=stats,
    ...
    if panel == "export_logs":
        return (
            '<div class="card p-3"><h5 class="mb-2">Export Reading Logs</h5><p class="text-muted small mb-2">Download your reading activity as CSV.</p><a class="btn btn-sm btn-outline-secondary" href="'
            + url_for("reading_logs.export_my_logs")
            + '">Export</a></div>'
        )
...
def settings_server_partial(panel: str):
    ...
    if panel == "users":
                ...
                for row in raw_list:
                    try:
                        ...
                        username_val = (
                            row.get("username")
                            if isinstance(row, dict)
                            else getattr(row, "username", "")
                        ) or ""
                        email_val = (
                            row.get("email")
                            if isinstance(row, dict)
                            else getattr(row, "email", "")
                        ) or ""
                        user_obj = DomainUser(
                            id=uid,
                            ...
                            is_admin=bool(
                                ...
                                if isinstance(row, dict)
                                else getattr(row, "is_admin", False)
                            ),
                            is_active=bool(
                                row.get("is_active", True)
                                if isinstance(row, dict)
                                else getattr(row, "is_active", True)
                            ),
                            password_hash=(
                                row.get("password_hash", "")
                                if isinstance(row, dict)
                                else getattr(row, "password_hash", "")
                            )
                            or "",
                        )
        ...
        if search:
            ...
            users_render = [
                u
                for u in users_render
                if s in (u.username or "").lower() or s in (u.email or "").lower()
            ]
        # Sort by created_at desc for consistency
        try:
            users_render.sort(
                key=lambda u: getattr(u, "created_at", _now()), reverse=True
            )
        except Exception:
            pass
        return render_template(
            "settings/partials/server_users_full.html",
            users=users_render,
            search=search,
        )
    if panel == "user_edit":
        ...
        if request.method == "POST":
            ...
            try:
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] POST entry action={action} user_id={user_id} form_keys={list(request.form.keys())} headers={{'HX':request.headers.get('HX-Request'), 'XHR': request.headers.get('X-Requested-With')}}"
                )
            except Exception:
                pass
            from app.services.simple_backup_service import get_simple_backup_service
            ...
            def _trigger_backup(reason: str):
                try:
                    ...
                    current_app.logger.warning(
                        f"Auto-backup failed after user change: {be}"
                    )

            if action == "update_profile":
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] update_profile path entered for {user_id}"
                )
                ...
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] reset_password path entered for {user_id}"
                )
                ...
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] update_role path entered for {user_id}"
                )
                ...
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] toggle_active path entered for {user_id}"
                )
                ...
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] delete_user path entered for {user_id}"
                )
                ...
                if current_user.check_password(admin_pwd):  # type: ignore
                    try:
                        ...
                        if target_user.id == current_user.id:  # type: ignore
                            ...
                            current_app.logger.error(
                                f"[USER_DELETE_DEBUG] Attempt to delete self blocked: current_user={current_user.id} target={target_user.id}"
                            )
                        else:
                            ...
                            if getattr(target_user, "is_admin", False):  # type: ignore
                                ...
                                if admin_count <= 1:
                                    ...
                                    current_app.logger.error(
                                        f"[USER_DELETE_DEBUG] Last admin delete blocked admin_count={admin_count}"
                                    )
                                else:
                                    deleted = user_service.delete_user_sync(
                                        target_user.id
                                    )  # type: ignore
                                    current_app.logger.error(
                                        f"[USER_DELETE_DEBUG] delete attempt admin user deleted={deleted}"
                                    )
                                    if deleted:
                                        _trigger_backup("user_deleted")
                                        flash(
                                            "User deleted & backup created.", "success"
                                        )
                                    else:
                                        ...
                                        try:
                                            ...
                                            if repo and hasattr(repo, "safe_manager"):
                                                ...
                                                res = repo.safe_manager.execute_query(
                                                    check_q, {"uid": target_user.id}
                                                )
                                                from app.services.kuzu_service_facade import (
                                                    _convert_query_result_to_list as _cvt,
                                                )

                                                data = _cvt(res)
                                                if (
                                                    data
                                                    and int(data[0].get("c", 0)) > 0
                                                ):
                                                    exists_flag = True
                                            current_app.logger.error(
                                                f"[USER_DELETE_DEBUG] delete admin failed exists_flag={exists_flag}"
                                            )
                                            flash(
                                                f"Delete failed (diagnostic: exists={exists_flag}).",
                                                "error",
                                            )
                                        except Exception as de_diag:
                                            current_app.logger.error(
                                                f"[USER_DELETE_DEBUG] diagnostic error admin delete: {de_diag}"
                                            )
                                            flash(
                                                f"Delete failed (diag error: {de_diag}).",
                                                "error",
                                            )
                                ...
                                current_app.logger.error(
                                    f"[USER_DELETE_DEBUG] delete attempt non-admin deleted={deleted}"
                                )
                                if deleted:
                                    _trigger_backup("user_deleted")
                                    ...
                                    try:
                                        ...
                                        if repo and hasattr(repo, "safe_manager"):
                                            ...
                                            if data and int(data[0].get("c", 0)) > 0:
                                                exists_flag = True
                                        current_app.logger.error(
                                            f"[USER_DELETE_DEBUG] delete non-admin failed exists_flag={exists_flag}"
                                        )
                                        flash(
                                            f"Delete failed (diagnostic: exists={exists_flag}).",
                                            "error",
                                        ...
                                        current_app.logger.error(
                                            f"[USER_DELETE_DEBUG] diagnostic error non-admin delete: {de_diag}"
                                        )
                                        flash(
                                            f"Delete failed (diag error: {de_diag}).",
                                            "error",
                        ...
                        try:
                            current_app.logger.error(
                                "[USER_DELETE_DEBUG] Fetching updated user list after delete attempt"
                            )
                        except Exception:
                            pass
                        updated_users = [
                            u
                            for u in user_service.get_all_users_sync()
                        ...
                        try:
                            current_app.logger.error(
                                f"[USER_DELETE_DEBUG] Messages after delete attempt: {msgs}"
                            )
                        except Exception:
                            pass
                        return render_template(
                            ...
                            users=updated_users,
                            search="",
                            inline_messages=msgs,
                        )
                    except Exception as de:
                        current_app.logger.error(
                            f"[USER_DELETE_DEBUG] Exception during delete flow: {de}"
                        )
                        flash("Delete failed.", "error")
                else:
                    current_app.logger.error(
                        "[USER_DELETE_DEBUG] Admin password incorrect for delete"
                    )
                    flash("Admin password incorrect.", "error")
            # reload updated user
            try:
                current_app.logger.error(
                    f"[USER_DELETE_DEBUG] Reloading user editor for user_id={user_id}"
                )
            except Exception:
                pass
            target_user = user_service.get_user_by_id_sync(user_id)
        return render_template(
            "settings/partials/server_user_edit.html", user=target_user
        )
    if panel == "debug":
        try:
            ...
            if request.method == "POST":
                ...
                if action == "update_debug_env":
                    try:
                        ...
                        if not data_dir:
                            try:
                                ...
                                data_dir = str(
                                    pathlib.Path(current_app.root_path).parent / "data"
                                )
                            except Exception:
                                data_dir = "data"
                        ...
                        manage_keys = [
                            "MYBIBLIOTHECA_DEBUG",
                            "MYBIBLIOTHECA_DEBUG_AUTH",
                            "MYBIBLIOTHECA_DEBUG_CSRF",
                            "MYBIBLIOTHECA_DEBUG_SESSION",
                            "MYBIBLIOTHECA_DEBUG_REQUESTS",
                            "MYBIBLIOTHECA_VERBOSE_INIT",
                            "ABS_LISTENING_DEBUG",
                            "KUZU_DEBUG",
                            "LOG_LEVEL",
                        ]
                        ...
                        def _to_str_bool(name: str) -> str:
                            ...
                            return (
                                "true" if (v in ("on", "true", "1", "yes")) else "false"
                            )

                        updates = {
                            "MYBIBLIOTHECA_DEBUG": _to_str_bool("MYBIBLIOTHECA_DEBUG"),
                            "MYBIBLIOTHECA_DEBUG_AUTH": _to_str_bool(
                                "MYBIBLIOTHECA_DEBUG_AUTH"
                            ),
                            "MYBIBLIOTHECA_DEBUG_CSRF": _to_str_bool(
                                "MYBIBLIOTHECA_DEBUG_CSRF"
                            ),
                            "MYBIBLIOTHECA_DEBUG_SESSION": _to_str_bool(
                                "MYBIBLIOTHECA_DEBUG_SESSION"
                            ),
                            "MYBIBLIOTHECA_DEBUG_REQUESTS": _to_str_bool(
                                "MYBIBLIOTHECA_DEBUG_REQUESTS"
                            ),
                            "MYBIBLIOTHECA_VERBOSE_INIT": _to_str_bool(
                                "MYBIBLIOTHECA_VERBOSE_INIT"
                            ),
                            "ABS_LISTENING_DEBUG": _to_str_bool("ABS_LISTENING_DEBUG"),
                            "KUZU_DEBUG": _to_str_bool("KUZU_DEBUG"),
                            "LOG_LEVEL": (
                                request.form.get("LOG_LEVEL")
                        ...
                        try:
                            ...
                            with open(tmp_path, "w") as wf:
                                ...
                                for k in manage_keys:
                                    wf.write(
                                        f"{k}={existing.get(k, updates.get(k, ''))}\n"
                                    )
                        ...
                        try:
                            ...
                            pass
                        flash(
                            "Debug environment settings updated. Restart may be required.",
                            "success",
                        )
                    ...
                    try:
                        ...
                        toggle_to = (
                            (request.form.get("toggle_to") or "").strip().lower()
                        )
                        enabled = True if toggle_to == "enable" else False
                        save_abs_settings({"debug_listening_sync": enabled})
                        flash(
                            f"ABS listening debug {'enabled' if enabled else 'disabled'}.",
                            "success",
                        )
                    ...
                    try:
                        from app.services.audiobookshelf_sync_runner import (
                            get_abs_sync_runner,
                        )
                        ...
                        runner.enqueue_listening_sync(
                            str(current_user.id), page_size=ps
                        )
                        flash(
                            "Listening sync started. Check Import Progress for updates.",
                            "info",
                        )
            ...
            env_flags = {
                ...
                "MYBIBLIOTHECA_DEBUG_REQUESTS": _env_bool(
                    "MYBIBLIOTHECA_DEBUG_REQUESTS"
                ),
            ...
            tpl = render_template(
                "settings/partials/server_debug.html",
                env_flags=env_flags,
                log_level=log_level,
    ...
    if panel == "config":
        ...
        if request.method == "POST":
            ...
            metadata_concurrency_raw = (
                request.form.get("metadata_concurrency") or ""
            ).strip()
            try:
            ...
            raw_default_book_format = (
                (request.form.get("default_book_format") or "").strip().lower()
            )
                    ...
                    if (
                        ...
                        try:
                            ...
                            if data_dir:
                                ...
                                upload_dir = os.path.join(
                                    base_dir, "data", "uploads", "backgrounds"
                                )
                            os.makedirs(upload_dir, exist_ok=True)
                            upload_path = os.path.join(upload_dir, unique_filename)
                            file.save(upload_path)
                            background_config["image_url"] = (
                                f"/uploads/backgrounds/{unique_filename}"
                            )
                            background_config["type"] = "image"
                            flash(
                                f"Background image uploaded successfully: {file.filename}",
                                "success",
                            )
                        except Exception as e:
                            current_app.logger.error(
                                f"Inline background upload error: {e}"
                            )
            ...
            config = {
                ...
                "import_settings": {"metadata_concurrency": metadata_concurrency},
            }
    ...
    if panel == "opds":
        ...
        field_inventory = (
            session.get("opds_field_inventory")
            or settings.get("last_field_inventory")
            or {}
        )
        ...
        def _build_pending_job(
            kind: str, status_value: Any, task_id: Any, api_url: Any, progress_url: Any
        ) -> dict[str, Any] | None:
        ...
        if request.method == "POST":
            ...
            user_agent = (
                form.get("user_agent") or settings.get("user_agent") or ""
            ).strip()
            ...
            inventory_for_clean = (
                field_inventory or settings.get("last_field_inventory") or {}
            )
            ...
            if auto_sync_flag is not None:
                update_payload["auto_sync_enabled"] = str(
                    auto_sync_flag
            ...
            if action == "save-settings":
                try:
                    if (
                        ...
                        if save_ok:
                            ...
                            _refresh_settings()
                            field_inventory = (
                                settings.get("last_field_inventory") or field_inventory
                            )
                            session["opds_field_inventory"] = field_inventory
                            has_password = bool(
                                settings.get("password_present")
                            ) or bool(settings.get("password"))
                        else:
                ...
                if not base_url:
                    ...
                    try:
                        ...
                        _refresh_settings()
                        has_password = bool(settings.get("password_present")) or bool(
                ...
                if not base_url:
                    ...
                    error_message = (
                        "At least one field mapping is required before syncing."
                    )
                else:
                    try:
                        if update_payload:
                        ...
                        if isinstance(job_info, dict):
                            ...
                            message_text = (
                                f"Sync job queued as task <code>{task_id_text}</code>."
                            )
                        ...
                        if limit_value:
                            save_payload["last_sync_summary"] = {
                                "status": "queued",
                                "limit": limit_value,
                                "timestamp": now_iso,
                            }
                ...
                if not base_url:
                    ...
                    try:
                        ...
                        try:
                            ...
                            limit_value = 10
                        job_info = runner.enqueue_test_sync(
                            str(current_user.id), limit=limit_value
                        )
                        ...
                        current_app.logger.error(
                            f"OPDS test sync enqueue failed: {err}"
                        )
        ...
        if not preview_columns and last_test_preview:
            ...
            prioritized = [
                field for field in fallback_priority if field in discovered_fields
            ]
            remaining = [
                field for field in discovered_fields if field not in prioritized
            ]
        ...
        def _build_preview_cell(
            entry: dict[str, Any], field_name: str
        ) -> dict[str, Any]:
            entry_payload = (
                entry.get("entry") if isinstance(entry.get("entry"), dict) else {}
            )
            raw_value: Any
        ...
        if preview_columns and last_test_preview:
            for entry in last_test_preview:
                if not isinstance(entry, dict):
                    continue
                row_values = {
                    field: _build_preview_cell(entry, field)
                    for field in preview_columns
                }
                raw_inspect_payload = entry.get("entry")
                inspect_payload: dict[str, Any] = (
                    raw_inspect_payload if isinstance(raw_inspect_payload, dict) else {}
                )
                entry_link = _resolve_entry_link(inspect_payload)
                opds_identifier = (
                    inspect_payload.get("opds_source_id") if inspect_payload else None
                )
    ...
    if panel == "audiobookshelf":
        ...
        expects_partial = bool(
            request.headers.get("HX-Request")
            or (request.headers.get("X-Requested-With") or "").lower()
            in {"xmlhttprequest", "fetch"}
        )
        ...
        if request.method == "POST":
            ...
            if (
                    ...
                    try:
                        ...
                        if libs_raw.startswith("["):
                            ...
                            if isinstance(arr, list):
                                payload["library_ids"] = ",".join(
                                    [str(s).strip() for s in arr if str(s).strip()]
                                )
                            else:
                ...
                if (
                    "auto_sync_enabled" in request.form
                    or "library_sync_every_hours" in request.form
                    or "listening_sync_every_hours" in request.form
                ):
                    payload["auto_sync_enabled"] = (
                        True if request.form.get("auto_sync_enabled") else False
                    )
                    try:
                        payload["library_sync_every_hours"] = int(
                            request.form.get("library_sync_every_hours") or 24
                        )
                    except Exception:
                        payload["library_sync_every_hours"] = 24
                    try:
                        payload["listening_sync_every_hours"] = int(
                            request.form.get("listening_sync_every_hours") or 12
                        )
                    except Exception:
                        payload["listening_sync_every_hours"] = 12
                # Enforce order field (treat absence as False)
                payload["enforce_book_first"] = (
                    True if request.form.get("enforce_book_first") else False
                )
            ...
            if not expects_partial and not request.is_json:
                flash(
                    "Audiobookshelf settings saved."
                    if ok
                    else "Failed to save Audiobookshelf settings.",
                    "success" if ok else "error",
                )
                return redirect(
                    url_for("auth.settings", section="server", panel="audiobookshelf")
                )
        ...
        try:
            if request.args.get("test") == "1":
                ...
                connection_test = (
                    ...
                    if client
                    else {"ok": False, "message": "Missing base_url or api_key"}
                )
        except Exception:
            connection_test = {"ok": False, "message": "Connection test failed"}
        return render_template(
            "settings/partials/server_audiobookshelf.html",
            settings=settings,
    ...
    if panel == "metadata":
        ...
        if request.method == "POST":
            ...
            try:
                current_app.logger.error(
                    "[METADATA_SAVE][BEGIN] ct=%s is_json=%s content_length=%s",
                    request.content_type,
                    request.is_json,
                ...
                current_app.logger.error(
                    "[METADATA_SAVE][RAW_BODY]%s", (raw_body or "")[:2000]
                )
                import json as _json
                ...
                if (
                    ...
                    try:
                        ...
                        current_app.logger.error(
                            f"[METADATA_SAVE][JSON_DECODE_ERR] {je}"
                        )
                        data = {}
                else:
                    ...
                    if raw:
                        try:
                            ...
                            current_app.logger.error(
                                f"[METADATA_SAVE][FORM_JSON_ERR] {fe}"
                            )
                    ...
                    if not data and raw_body and raw_body.strip().startswith("{"):
                        try:
                            ...
                            current_app.logger.error(
                                "[METADATA_SAVE][FALLBACK_PARSE_OK]"
                            )
                        except Exception as fe2:
                            current_app.logger.error(
                                f"[METADATA_SAVE][FALLBACK_PARSE_ERR] {fe2}"
                            )
                has_books = isinstance(data.get("books"), dict)
                has_people = isinstance(data.get("people"), dict)
                current_app.logger.error(
                    "[METADATA_SAVE][PARSED_KEYS] keys=%s books=%s people=%s",
                    list(data.keys())[:8],
                    has_books,
                    has_people,
                )
                if not (has_books or has_people):
                    current_app.logger.error(
                        "[METADATA_SAVE][WARN] No valid books/people objects found in payload; aborting save."
                    )
                    return jsonify({"ok": False, "error": "no_valid_payload"}), 400
                # sanity: ensure payload not unexpectedly huge
                if len(str(data)) > 20000:
                    current_app.logger.error(
                        "[METADATA_SAVE][WARN] Payload unusually large size=%s",
                        len(str(data)),
                    )
                ...
                return jsonify(
                    {"ok": True, "metadata_settings": get_metadata_settings()}
                )
            except Exception as e:
    ...
    if panel == "repairs":
        ...
        def _run_query(
            rows_query: str, params: dict[str, Any] | None = None, op: str = "repairs"
        ) -> list[dict[str, Any]]:
            try:
        ...
        action_taken = (
            (request.form.get("action") or "").strip().lower()
            if request.method == "POST"
            else ""
        )
        ...
        if request.method == "POST" and action_taken:
            if action_taken == "backfill_media_type":
                try:
                    ...
                    current_app.logger.error(
                        f"Repair action backfill_media_type failed: {err}"
                    )
                    flash(
                        "Failed to backfill media types. Check logs for details.",
                        "error",
                    )
                ...
                try:
                    ...
                    if not default_location or not getattr(
                        default_location, "id", None
                    ):
                        flash(
                            "No default location is available. Create a location first.",
                            "warning",
                        )
                    else:
                        updated_rows = _run_query(
                        ...
                        if updated:
                            flash(
                                f"Default location assigned to {updated} book(s).",
                                "success",
                            )
                        else:
                    ...
                    current_app.logger.error(
                        f"Repair action assign_default_location failed: {err}"
                    )
                    flash(
                        "Failed to assign default locations. Check logs for details.",
                        "error",
                    )
            else:
        ...
        total_books_rows = _run_query(
            "MATCH (b:Book) RETURN COUNT(b) AS total", op="repairs_total_books"
        )
        total_books = 0
        if total_books_rows:
            total_books = int(
    ...
    if panel == "jobs":
        ...
        try:
            ...
            debug_map = safe_import_manager.get_jobs_for_admin_debug(
            ...
            stats = {
                "total_active_jobs": 0,
                "total_users_with_jobs": 0,
                "operation_stats": {},
                "abs_runner_alive": False,
            }
        return render_template(
            "settings/partials/server_jobs.html", jobs=jobs, manager_stats=stats
        )
...
def save_ai_settings():
    ...
    try:
        config = {
            ...
            "OPENAI_BASE_URL": request.form.get(
            ...
            "AI_BOOK_EXTRACTION_ENABLED": "true"
            if request.form.get("ai_book_extraction_enabled")
            else "false",
            "AI_BOOK_EXTRACTION_AUTO_SEARCH": "true"
            if request.form.get("ai_book_extraction_auto_search")
            else "false",
        }
        ollama_manual = (request.form.get("ollama_model_manual") or "").strip()
        ollama_selected = (request.form.get("ollama_model") or "").strip()
        config["OLLAMA_MODEL"] = (
            ollama_manual or ollama_selected or "llama3.2-vision:11b"
        )
...
def test_ai_connection():
    ...
    try:
        config = {
            ...
            "OLLAMA_MODEL": request.form.get("ollama_model_manual")
            or request.form.get("ollama_model", "llama3.2-vision:11b"),
            "AI_TIMEOUT": request.form.get("ai_timeout", "30"),
            "AI_MAX_TOKENS": request.form.get("ai_max_tokens", "1000"),
            "AI_TEMPERATURE": request.form.get("ai_temperature", "0.1"),
        }
...
def test_ollama_connection():
    ...
    try:
        ...
        return jsonify(
            {
                "success": False,
                "message": "Ollama connection test failed. Please check your settings.",
            }
        ), 500
...
def save_smtp_settings():
    ...
    try:
        config = {
            ...
            "smtp_from_name": (
                request.form.get("smtp_from_name") or "MyBibliotheca"
            ).strip(),
        }
        ...
        _log(
            "error",
            f"Error updating SMTP settings: {exc}",
            extra_secrets=[request.form.get("smtp_password", "")],
        )
        flash("Error saving SMTP settings. Please try again.", "danger")
    ...
    if expects_partial:
        ...
        ctx["site_name"] = (load_system_config() or {}).get(
...
def test_smtp_connection():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": "SMTP port must be a number."}
        ), 400

    secret_values = [smtp_password] if smtp_password else []
    _log_force(
        "info",
        f"[SMTP] Testing connection to {smtp_server}:{smtp_port}",
        extra_secrets=secret_values,
    )
    _log_force(
        "info",
        f"[SMTP] Configuration - Security: {raw_security.upper()}, Username: {smtp_username or '(none)'}",
        extra_secrets=secret_values,
    )
    ...
    if not smtp_server:
        _log_force(
            "warning",
            "[SMTP] Test aborted - no server specified",
            extra_secrets=secret_values,
        )
    ...
    try:
        _log_force(
            "info",
            f"[SMTP] Step 1/4: Resolving DNS for {smtp_server}...",
            extra_secrets=secret_values,
        )
        try:
            ...
            _log_force(
                "info",
                f"[SMTP] DNS resolved: {smtp_server} -> {resolved_ip}",
                extra_secrets=secret_values,
            )
        except socket.gaierror as dns_err:
            _log_force(
                "error",
                f"[SMTP] DNS resolution failed for {smtp_server}: {dns_err}",
                extra_secrets=secret_values,
            )
            return jsonify(
                {
        ...
        if raw_security == "ssl":
            _log_force(
                "info",
                f"[SMTP] Step 2/4: Connecting with implicit SSL to {smtp_server}:{smtp_port} (timeout: 30s)...",
                extra_secrets=secret_values,
            )
            try:
                ...
                _log_force(
                    "error",
                    f"[SMTP] Connection refused by {smtp_server}:{smtp_port}",
                    extra_secrets=secret_values,
                )
                return jsonify(
                    {
            ...
            _log_force(
                "info",
                f"[SMTP] Step 2/4: Connecting to {smtp_server}:{smtp_port} (timeout: 30s)...",
                extra_secrets=secret_values,
            )
            try:
        ...
        if raw_security == "starttls":
            _log_force(
                "info",
                "[SMTP] Step 3/4: Initiating STARTTLS...",
                extra_secrets=secret_values,
            )
            try:
                context = ssl.create_default_context()
                ...
                server.ehlo()
                _log_force(
                    "info", "[SMTP] STARTTLS successful", extra_secrets=secret_values
                )
            except smtplib.SMTPException as tls_err:
                _log_force(
                    "error",
                    f"[SMTP] STARTTLS failed: {tls_err}",
                    extra_secrets=secret_values,
                )
                return jsonify(
                    {"success": False, "message": f"TLS negotiation failed: {tls_err}"}
                ), 500
        elif raw_security == "ssl":
            _log_force(
                "info",
                "[SMTP] Step 3/4: SSL negotiation completed during connection",
                extra_secrets=secret_values,
            )
        else:
            _log_force(
                "info", "[SMTP] Step 3/4: No TLS requested", extra_secrets=secret_values
            )

        if smtp_username and smtp_password:
            _log_force(
                "info",
                f"[SMTP] Step 4/4: Authenticating as {smtp_username}...",
                extra_secrets=secret_values,
            )
            try:
                ...
                _log_force(
                    "info",
                    f"[SMTP] Authentication successful for {smtp_username}",
                    extra_secrets=secret_values,
                )
            except smtplib.SMTPAuthenticationError as auth_err:
                _log_force(
                    "error",
                    f"[SMTP] Authentication failed for {smtp_username}: {auth_err}",
                    extra_secrets=secret_values,
                )
                return jsonify(
                    {
                ...
                _log_force(
                    "error",
                    f"[SMTP] SMTP error during authentication: {smtp_err}",
                    extra_secrets=secret_values,
                )
                return jsonify(
                    {
            ...
            _log_force(
                "info",
                "[SMTP] Step 4/4: No authentication (username/password not provided)",
                extra_secrets=secret_values,
            )

        _log_force(
            "info",
            "[SMTP] All steps completed successfully. Closing connection...",
            extra_secrets=secret_values,
        )
        server.quit()
        _log_force(
            "info",
            f"[SMTP] Test completed successfully for {smtp_server}:{smtp_port}",
            extra_secrets=secret_values,
        )
        return jsonify(
            {
        ...
        _log_force(
            "error",
            f"[SMTP] SMTP error: {type(exc).__name__}: {exc}",
            extra_secrets=secret_values,
        )
        ...
        return jsonify(
            {
                "success": False,
                "message": "Operation timeout. The server may be slow or unreachable.",
            }
        ), 500
    except Exception as exc:
        _log_force(
...
def save_backup_settings():
    ...
    try:
        backup_directory = (
            request.form.get("backup_directory") or "data/backups"
        ).strip()
    ...
    if expects_partial:
        return render_template(
            "settings/partials/server_backup.html", backup_config=load_backup_config()
        )
...
def opds_preview_detail(row_index: int):
    ...
    if (
        not isinstance(preview_list, list)
        or row_index < 0
        or row_index >= len(preview_list)
    ):
        abort(404)
    ...
    mapped_payload = (
        entry_payload.get("entry") if isinstance(entry_payload, dict) else {}
    )
    ...
    if isinstance(entry_payload, dict):
        opds_id = entry_payload.get("opds_source_id") or mapped_payload.get(
            "opds_source_id"
        )
    raw_links = (
        mapped_payload.get("raw_links") if isinstance(mapped_payload, dict) else None
    )
    inspect_link = None
    ...
    column_map_obj = (
        entry_payload.get("columns") if isinstance(entry_payload, dict) else None
    )
    ...
    for field_name in mapped_field_order:
        ...
        if isinstance(cell, dict):
            ...
            if field_name.startswith("contributors.") and isinstance(
                mapped_payload.get("contributors"), list
            ):
                role = field_name.split(".", 1)[1].upper() if "." in field_name else ""
                contributor_names = []
        ...
        if not friendly_label:
            friendly_label = (
                field_name.replace("contributors.", "Contributor · ")
                .replace("_", " ")
                .title()
            )
...
def audiobookshelf_test_sync():
    ...
    try:
        ...
        try:
            ...
            limit = 5
        runner = get_abs_sync_runner()
        task_id = runner.enqueue_test_sync(
            str(current_user.id), library_ids, limit=limit
        )
...
def audiobookshelf_full_sync():
    ...
    try:
        ...
        if not settings.get("enabled"):
            return jsonify(
                {"ok": False, "message": "Audiobookshelf is disabled in settings"}
            ), 400
        ...
        progress_url = (
            url_for("import.import_books_progress", task_id=task_ids[0])
            if task_ids
            else None
        )
        api_progress_url = (
            url_for("import.api_import_progress", task_id=task_ids[0])
            if task_ids
            else None
        )
        return jsonify(
...
def audiobookshelf_listen_test():
    ...
    try:
        ...
        runner = get_abs_sync_runner()
        task_id = runner.enqueue_listening_sync(
            str(current_user.id), page_size=page_size
        )
        try:
            current_app.logger.info(
                f"[ABS Listen] Enqueued listening-only test task={task_id} user={current_user.id} page_size={page_size}"
            )
        except Exception:
            pass
    ...
    if form.validate_on_submit():
        try:
            ...
            if user_from_kuzu:
                ...
                if updated_user:
                    # Update current_user object for immediate reflection
                    current_user.share_current_reading = (
                        updated_user.share_current_reading
                    )
                    current_user.share_reading_activity = (
                        updated_user.share_reading_activity
                    )
...
def my_activity():
    try:
        ...
        books_this_year = sum(
            ...
            and isinstance(book.get("created_at"), datetime)
    ...
    if form.validate_on_submit():
        try:
            ...
            if user_from_kuzu:
                ...
                user_from_kuzu.reading_streak_offset = (
                    form.reading_streak_offset.data or 0
                )
                ...
                if updated_user:
                    # Update current_user object for immediate reflection
                    current_user.reading_streak_offset = (
                        updated_user.reading_streak_offset
                    )
...
def debug_user_count():
    ...
    try:
        ...
        try:
            ...
            query_result = safe_manager.execute_query(
                "MATCH (u:User) RETURN COUNT(u) as count"
            )

            if (
                query_result
                and hasattr(query_result, "get_next")
                and query_result.has_next()
            ):
