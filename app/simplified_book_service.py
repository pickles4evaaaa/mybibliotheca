...
class SimplifiedBookService:
    ...
    async def create_standalone_book(self, book_data: SimplifiedBook) -> str | None:
        ...
        try:
            ...
            if description:
                print(
                    f"📖 Creating: {book_node_data.get('title')} by {book_data.author}"
                )
            ...
            if book_data.cover_url and book_data.cover_url.startswith("http"):
                try:
                    print(
                        f"🖼️ [COVER_DOWNLOAD] Downloading cover for '{book_data.title}': {book_data.cover_url}"
                    )
                    ...
                    response = requests.get(
                        ...
                        stream=True,
                        headers={
                            "User-Agent": "Mozilla/5.0 (compatible; BookLibrary/1.0)"
                        },
                    )
                    ...
                    if update_cover_result:
                        print(
                            f"✅ [COVER_DOWNLOAD] Successfully downloaded and cached cover: {final_cover_url}"
                        )
                    else:
                        print(
                            "❌ [COVER_DOWNLOAD] Failed to update book record with local cover URL"
                        )

                except Exception as cover_error:
                    print(
                        f"⚠️ [COVER_DOWNLOAD] Failed to download cover for '{book_data.title}': {cover_error}"
                    )
            ...
            def create_person_data(name, enhance_with_api=True):
                ...
                if enhance_with_api and name and name.strip():
                    try:
                        ...
                        if author_search_result and author_search_result.get(
                            "openlibrary_id"
                        ):
                            author_id = author_search_result["openlibrary_id"]
                            print(
                                f"✅ [PERSON_API] Found OpenLibrary ID for {name}: {author_id}"
                            )
                            ...
                            if author_data:
                                print(
                                    f"✅ [PERSON_API] Retrieved detailed metadata for {name}"
                                )

                                # Update person data with API metadata
                                person_data.bio = author_data.get("bio", "") or ""
                                person_data.openlibrary_id = (
                                    author_data.get("openlibrary_id", "") or ""
                                )
                                person_data.image_url = (
                                    author_data.get("photo_url", "") or ""
                                )
                                person_data.website = (
                                    author_data.get("wikipedia_url", "") or ""
                                )
                                ...
                                if birth_date:
                                    try:
                                        ...
                                        if year_match:
                                            person_data.birth_year = int(
                                                year_match.group(1)
                                            )
                                            print(
                                                f"📅 [PERSON_API] Set birth year for {name}: {person_data.birth_year}"
                                            )
                                ...
                                if death_date:
                                    try:
                                        ...
                                        if year_match:
                                            person_data.death_year = int(
                                                year_match.group(1)
                                            )
                                            print(
                                                f"📅 [PERSON_API] Set death year for {name}: {person_data.death_year}"
                                            )
                                    except (ValueError, TypeError):
                                        pass

                                print(
                                    f"🎉 [PERSON_API] Enhanced {name} with: bio={bool(person_data.bio)}, image={bool(person_data.image_url)}, birth_year={person_data.birth_year}"
                                )
                            else:
                                print(
                                    f"❌ [PERSON_API] No detailed data found for OpenLibrary ID: {author_id}"
                                )
                        else:
                            print(
                                f"❌ [PERSON_API] No OpenLibrary ID found for author: {name}"
                            )
                ...
                try:
                    ...
                    if author_id:
                        ...
                        if not authored_result:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create AUTHORED relationship for main author: {book_data.author}"
                            )
                    else:
                        print(
                            f"❌ [SIMPLIFIED] Failed to create main author person: {book_data.author}"
                        )
                except Exception:
                    import traceback
            ...
            if book_data.additional_authors:
                try:
                    additional_authors_list = [
                        name.strip()
                        for name in book_data.additional_authors.split(",")
                        if name.strip()
                    ]
                    for index, author_name in enumerate(additional_authors_list):
                        ...
                        if author_id:
                            ...
                            if authored_result:
                                print(
                                    f"✅ [SIMPLIFIED] Created AUTHORED relationship for additional author: {author_name}"
                                )
                            else:
                                print(
                                    f"❌ [SIMPLIFIED] Failed to create AUTHORED relationship for additional author: {author_name}"
                                )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create additional author person: {author_name}"
                            )
                except Exception as e:
                    print(
                        f"❌ [SIMPLIFIED] Error creating additional author relationship: {e}"
                    )
            ...
            if book_data.narrator:
                try:
                    narrator_list = [
                        name.strip()
                        for name in book_data.narrator.split(",")
                        if name.strip()
                    ]
                    for index, narrator_name in enumerate(narrator_list):
                        if (
                            not narrator_name
                            or narrator_name.strip().lower() == "unknown"
                        ):
                            continue
                        ...
                        if narrator_id:
                            ...
                            if narrated_result:
                                print(
                                    f"✅ [SIMPLIFIED] Created AUTHORED (narrated) relationship for {narrator_name}"
                                )
                            else:
                                print(
                                    f"❌ [SIMPLIFIED] Failed to create AUTHORED (narrated) relationship for {narrator_name}"
                                )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create narrator person: {narrator_name}"
                            )
                except Exception as e:
            ...
            if book_data.editor:
                try:
                    editor_list = [
                        name.strip()
                        for name in book_data.editor.split(",")
                        if name.strip()
                    ]
                    for index, editor_name in enumerate(editor_list):
                        ...
                        if editor_id:
                            ...
                            if edited_result:
                                print(
                                    f"✅ [SIMPLIFIED] Created AUTHORED (edited) relationship for {editor_name}"
                                )
                            else:
                                print(
                                    f"❌ [SIMPLIFIED] Failed to create AUTHORED (edited) relationship for {editor_name}"
                                )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create editor person: {editor_name}"
                            )
                except Exception as e:
                    ...
                    for index, translator_name in enumerate(translator_list):
                        ...
                        if translator_id:
                            ...
                            if translated_result:
                                print(
                                    f"✅ [SIMPLIFIED] Created AUTHORED (translated) relationship for {translator_name}"
                                )
                            else:
                                print(
                                    f"❌ [SIMPLIFIED] Failed to create AUTHORED (translated) relationship for {translator_name}"
                                )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create translator person: {translator_name}"
                            )
                except Exception as e:
                    print(
                        f"❌ [SIMPLIFIED] Error creating translator relationship: {e}"
                    )
                    ...
                    for index, illustrator_name in enumerate(illustrator_list):
                        ...
                        if illustrator_id:
                            ...
                            if illustrated_result:
                                print(
                                    f"✅ [SIMPLIFIED] Created AUTHORED (illustrated) relationship for {illustrator_name}"
                                )
                            else:
                                print(
                                    f"❌ [SIMPLIFIED] Failed to create AUTHORED (illustrated) relationship for {illustrator_name}"
                                )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create illustrator person: {illustrator_name}"
                            )
                except Exception as e:
                    print(
                        f"❌ [SIMPLIFIED] Error creating illustrator relationship: {e}"
                    )

            # 3. Create publisher relationship using clean repository
            if book_data.publisher:
                try:
                    publisher_id = await book_repo._ensure_publisher_exists(
                        book_data.publisher
                    )
                    if publisher_id:
                        ...
                        if published_result:
                            print(
                                f"✅ [SIMPLIFIED] Created PUBLISHED_BY relationship for {book_data.publisher}"
                            )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create PUBLISHED_BY relationship for {book_data.publisher}"
                            )
                    else:
                        print(
                            f"❌ [SIMPLIFIED] Failed to create publisher: {book_data.publisher}"
                        )
                except Exception as e:
            ...
            try:
                ...
                if getattr(book_data, "raw_categories", None):
                    await book_repo._create_category_relationships_from_raw(
                        book_id, book_data.raw_categories
                    )
                    ...
                    for category_name in book_data.categories:
                        if not category_name.strip():
                            continue
                        category_id = await book_repo._ensure_category_exists(
                            category_name.strip()
                        )
                        if category_id:
                            ...
                            if not categorized_result:
                                print(
                                    f"❌ [SIMPLIFIED] Failed to create CATEGORIZED_AS relationship for {category_name}"
                                )
                        else:
                            print(
                                f"❌ [SIMPLIFIED] Failed to create category: {category_name}"
                            )
            except Exception as e:
            ...
            if book_data.global_custom_metadata:
                try:
                    print(
                        f"📝 [SIMPLIFIED] Processing {len(book_data.global_custom_metadata)} global custom fields"
                    )
                    ...
                    fields_ensured = (
                        self.custom_field_service.ensure_custom_fields_exist(
                    ...
                    if fields_ensured:
                        ...
                        global_saved = (
                            self.custom_field_service.save_custom_metadata_sync(
                                book_id,
                                system_user_id,
                                book_data.global_custom_metadata,
                            )
                        )
        ...
        try:
            ...
            if result:
                ...
                if annotation.custom_metadata:
                    try:
                        print(
                            f"📝 [ANNOTATION] Processing {len(annotation.custom_metadata)} personal custom fields"
                        )
                        ...
                        if fields_ensured:
                            ...
                            if not personal_saved:
                                print(
                                    "⚠️ [ANNOTATION] Failed to save personal custom metadata"
                                )
                        else:
                            print(
                                "⚠️ [ANNOTATION] Failed to ensure custom field definitions exist"
                            )

                    except Exception as e:
                        print(
                            f"❌ [ANNOTATION] Error saving personal custom metadata: {e}"
                        )
    ...
    def find_book_by_isbn(self, isbn: str) -> str | None:
        ...
        try:
            ...
            normalized_isbn = "".join(
                c for c in str(isbn).strip().upper() if c.isdigit() or c == "X"
            )
    ...
    def find_book_by_title_author(self, title: str, author: str) -> str | None:
        ...
        try:
            ...
            title_results = safe_execute_kuzu_query(
                title_query, {"title": normalized_title}
            )
            ...
            if title_results_list:
                ...
                for result in title_results_list:
                    ...
                    author_results = safe_execute_kuzu_query(
                        author_query, {"book_id": book_id, "author": normalized_author}
                    )
                    author_results_list = self._convert_query_result_to_list(
                        author_results
                    )

                    if author_results_list:
                        print(
                            f"🔍 [DUPLICATE] Found matching book: '{title}' by author containing '{author}'"
                        )
                        return book_id
    ...
    async def find_or_create_book(self, book_data: SimplifiedBook) -> str | None:
        ...
        try:
            ...
            if book_data.isbn13:
                ...
                if existing_id:
                    print(
                        f"🔍 [DUPLICATE] Found existing book by ISBN13: {book_data.isbn13}"
                    )
                    raise BookAlreadyExistsError(
                        existing_id,
                        f"Book already exists with ISBN13: {book_data.isbn13}",
                    )
            ...
            if book_data.isbn10 and not existing_id:
                ...
                if existing_id:
                    print(
                        f"🔍 [DUPLICATE] Found existing book by ISBN10: {book_data.isbn10}"
                    )
                    raise BookAlreadyExistsError(
                        existing_id,
                        f"Book already exists with ISBN10: {book_data.isbn10}",
                    )

            # If no ISBN or ISBN not found, check by title and author
            if not existing_id and book_data.title and book_data.author:
                existing_id = self.find_book_by_title_author(
                    book_data.title, book_data.author
                )
                if existing_id:
                    print(
                        f"🔍 [DUPLICATE] Found existing book by title/author: '{book_data.title}' by '{book_data.author}'"
                    )
                    raise BookAlreadyExistsError(
                        existing_id,
                        f"Book already exists: '{book_data.title}' by '{book_data.author}'",
                    )
    ...
    def build_book_data_from_row(
        self, row, mappings, book_meta_map=None, author_meta_map=None
    ):
        """
        ...
        for csv_field, book_field in mappings.items():
            if csv_field in row and row[csv_field]:
                ...
                if book_field == "title":
                    ...
                    for p in parts:
                        ...
                        if "(" in p and ")" in p and p.index("(") < p.index(")"):
                            ...
                            if "editor" in role_text:
                                ...
                                "illustrator" in role_text
                                or "illustration" in role_text
                            ):
                                role = "illustrator"
                        ...
                        if role == "editor":
                            ...
                            if (
                                not book_data.get("author")
                                or name.lower() != str(book_data["author"]).lower()
                            ):
                    ...
                    clean_isbn = "".join(
                        c for c in str(value).strip().upper() if c.isdigit() or c == "X"
                    )
                    ...
                    if normalized_isbn:
                        clean_isbn = "".join(
                            c
                            for c in str(normalized_isbn).strip().upper()
                            if c.isdigit() or c == "X"
                        )
                        if len(clean_isbn) == 13:
                            book_data["isbn13"] = clean_isbn
                    ...
                    if value and value.strip():
                        ...
                        print(
                            f"📅 [CSV] Set published_date from CSV: '{value.strip()}'"
                        )
    ...
    async def add_book_to_user_library(
        ...
        try:
            ...
            print(
                f"📚 [UNIVERSAL_LIBRARY] Book {book_id} created in universal library (no ownership)"
            )
            ...
            if location_id:
                try:
                    ...
                    location_success = location_service.add_book_to_location(
                        book_id, location_id, user_id
                    )
                    if location_success:
                        print(
                            f"✅ [LOCATION_SERVICE] Successfully added book {book_id} to location {location_id}"
                        )
                    else:
                        print(
                            f"❌ [LOCATION_SERVICE] Failed to add book {book_id} to location {location_id}"
                        )

                except Exception as e:
                    print(
                        f"❌ [LOCATION_SERVICE] Exception adding book to location: {e}"
                    )
                    import traceback
                ...
                try:
                    ...
                    if default_location and default_location.id:
                        location_success = location_service.add_book_to_location(
                            book_id, default_location.id, user_id
                        )
                        if location_success:
                            print(
                                f"✅ [DEFAULT_LOCATION] Successfully added book {book_id} to default location {default_location.id}"
                            )
                        else:
                            print(
                                f"❌ [DEFAULT_LOCATION] Failed to add book {book_id} to default location"
                            )
                    else:
                        print(
                            f"❌ [DEFAULT_LOCATION] No default location available for book {book_id}"
                        )

                except Exception as e:
                    print(
                        f"❌ [DEFAULT_LOCATION] Exception assigning book to default location: {e}"
                    )
            ...
            try:
                ...
                if getattr(book_data, "personal_custom_metadata", None):
                    ...
                    if pcm:
                        print(
                            f"📝 [UNIVERSAL_LIBRARY] Including {len(pcm)} personal custom fields"
                        )
                        ...
                        if sd_raw:
                            try:
                                if isinstance(sd_raw, dt):
                                    ...
                                    start_date_value = dt.fromisoformat(
                        ...
                        if fd_raw:
                            try:
                                if isinstance(fd_raw, dt):
                                    ...
                                    finish_date_value = dt.fromisoformat(
                                        fd_raw.replace("Z", "+00:00")
                                    )
                            except Exception:
                                pass
                ...
                if (
                    ...
                    or personal_notes
                    or start_date_value
                    or finish_date_value
                ):
                    print(
                        f"📝 [UNIVERSAL_LIBRARY] Saving personal metadata for user {user_id}"
                    )
                    ...
                    print("✅ [UNIVERSAL_LIBRARY] Personal metadata saved successfully")
    ...
    def add_book_to_user_library_sync(
        self,
        book_data: SimplifiedBook,
        user_id: str,
        reading_status: str = "",
