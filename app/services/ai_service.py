...
class AIService:
    ...
    def extract_book_info_from_image(
        ...
        try:
            ...
            fallback_enabled = (
                self.config.get("AI_FALLBACK_ENABLED", "true").lower() == "true"
            )
            other_configured = (
                secondary == "openai" and bool(self.config.get("OPENAI_API_KEY"))
            ) or (
                secondary == "ollama"
                and bool(self.config.get("OLLAMA_BASE_URL", "http://localhost:11434"))
            )
            ...
            for prov in providers_to_try:
                try:
                    current_app.logger.info(
                        f"AI extraction attempting provider: {prov}"
                    )
                    if prov == "openai":
                    ...
                    if result:
                        if prov != primary:
                            current_app.logger.info(
                                f"AI extraction succeeded with fallback provider: {prov}"
                            )
                        return result
            ...
            if last_error:
                current_app.logger.error(
                    f"AI extraction failed after trying providers {providers_to_try}: {last_error}"
                )
            return None
        ...
        try:
            ...
            if os.path.exists(prompt_path):
                ...
                try:
                    ...
                    current_app.logger.warning(
                        "pystache not available, using template without processing"
                    )
    ...
    def _extract_with_openai(
        ...
        try:
            ...
            current_app.logger.info(
                f"OpenAI API Request Payload: {json.dumps(payload, indent=2)}"
            )
            ...
            current_app.logger.info(
                f"OpenAI API Response Headers: {dict(response.headers)}"
            )
            ...
            current_app.logger.error("OpenAI API error traceback: ", exc_info=True)
            return None
    ...
    def _extract_with_ollama(
        ...
        try:
            ...
            current_app.logger.info(
                f"Ollama API Image size: {len(image_base64)} base64 characters"
            )
            ...
            if response.status_code == 200:
                result = response.json()
                current_app.logger.info(
                    f"Ollama parsed response: {json.dumps(result, indent=2)}"
                )
    ...
    def _test_ollama_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            current_app.logger.info(
                f"Ollama connection test response: {response.status_code} - {response.text}"
            )
            ...
            if response.status_code == 200:
                ...
                message = (
                    f"Ollama connection successful! Available models: {model_list}"
                )
                if models:
                    message += "\n\nDetailed model info:\n"
                    for model in models:
            ...
            current_app.logger.error(
                f"Ollama connection test error: {e}", exc_info=True
            )
            return {"success": False, "message": f"Connection error: {str(e)}"}
