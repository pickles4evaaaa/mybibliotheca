...
class AudiobookshelfListeningSync:
    def __init__(self, user_id: str, client: AudiobookShelfClient):
        ...
        env_debug = (os.getenv("ABS_LISTENING_DEBUG") or "").lower() in (
            "1",
            "true",
            "yes",
            "on",
    ...
    def _find_book_id_by_abs_item(self, item: dict[str, Any]) -> str | None:
        ...
        if title:
            try:
                ...
                pass
        self._dbg(
            "No book match for item",
            {"abs_id": abs_id, "title": title, "author": author},
        )
        return None
    ...
    def _upsert_progress(
        ...
        *,
        ...
        try:
            meta = (
                personal_metadata_service.get_personal_metadata(self.user_id, book_id)
                or {}
            )
        except Exception:
            meta = {}
        ...
        if finished_flag or (
            ...
            rs = (
                (meta.get("reading_status") or "").lower()
                if isinstance(meta.get("reading_status"), str)
                else meta.get("reading_status")
            )
            ...
            if isinstance(progress_pct, (int, float)) and float(progress_pct) == 0.0:
                ...
                if (
                    isinstance(progress_pct, (int, float)) and float(progress_pct) > 0.0
                ) or (
                    has_listening_activity
                    and not isinstance(progress_pct, (int, float))
                ):
        ...
        self._dbg(
            "Upsert progress",
            {
                ...
                **{
                    k: updates.get(k)
                    for k in ("progress_ms", "last_listened_at", "reading_status")
                },
            },
        )
    ...
    def sync(
        ...
        while True:
            res = self.client.list_user_sessions(
                user_id=abs_user_id,
                updated_after=updated_after,
                limit=page_size,
                page=page,
            ...
            if not sessions:
                ...
                if not first_page_checked and page == 0 and updated_after:
                    self._dbg(
                        "Empty first page with updated_after; retrying without filter",
                        {"updated_after": updated_after},
                    )
                        ...
                        for sid in seed_item_ids:
                            try:
                                ...
                                updated_at = _parse_iso(
                                    prog.get("updatedAt")
                                    or prog.get("updated_at")
                                    or prog.get("lastUpdate")
                                ...
                                for dk in ("durationMs", "duration_ms", "duration"):
                                    ...
                                    if isinstance(dv, (int, float)):
                                        dur_ms = int(
                                            dv if dv > 10_000_000 else dv * 1000
                                        )
                                        break
                                ...
                                if isinstance(pos_ms, int) and pos_ms > 0:
                                    finished_flag = False
                                    try:
                                        finished_flag = self._is_finished_from_payload(
                                            prog, position_ms=pos_ms, duration_ms=dur_ms
                                        )
                                    except Exception:
                                        finished_flag = False
                                    # Coalesced single write
                                    self._upsert_progress(
                                        book_id, pos_ms, updated_at, finished_flag
                                    )
                ...
                try:
                    ...
                    if pos_ms is None:
                        try:
                            ...
                            if item_id:
                                gp = self.client.get_user_progress_for_item(
                                    str(item_id)
                                )
                                prog = gp.get("progress") or {}
                                # fill pos
                    ...
                    if duration_ms is None:
                        md = (item.get("media") or {}).get("metadata") or {}
                        dm = (item.get("media") or {}).get("duration") or md.get(
                            "duration"
                        )
                    ...
                    if duration_ms is None:
                        if book_id not in duration_ms_cache:
                            try:
                                qr = safe_execute_kuzu_query(
                                    "MATCH (b:Book) WHERE b.id = $id RETURN b.audio_duration_ms LIMIT 1",
                                    {"id": book_id},
                                )
                                val = None
                                if isinstance(qr, list) and qr:
                                    ...
                                    if isinstance(first, dict):
                                        val = (
                                            first.get("result")
                                            or first.get("b.audio_duration_ms")
                                            or first.get("col_0")
                                        )
                                    elif isinstance(first, (list, tuple)) and first:
                                        val = first[0]
                                duration_ms_cache[book_id] = (
                                    int(val) if isinstance(val, (int, float)) else None
                                )
                            except Exception:
                    ...
                    try:
                        finished_from_session = self._is_finished_from_payload(
                            s, position_ms=pos_ms, duration_ms=duration_ms
                        )
                    ...
                    try:
                        ...
                        if (
                            ...
                            progress_pct = max(
                                0.0,
                                min(
                                    100.0, (float(pos_ms) / float(duration_ms)) * 100.0
                                ),
                            )
                    except Exception:
                        progress_pct = None
                    ...
                    if minutes_from_session is None and isinstance(pos_ms, int):
                        ...
                        if isinstance(prev, int):
                            ...
                            if delta > 15_000:
                                minutes_from_session = max(
                                    1, min(int(delta // 60000), 240)
                                )
                        # Update last position regardless
                        last_pos_ms_by_book[book_id] = (
                            pos_ms
                            if isinstance(pos_ms, int)
                            else last_pos_ms_by_book.get(book_id, 0)
                        )
                    if (
                        minutes_from_session is None
                        and finished_flag
                        and isinstance(duration_ms, int)
                        and duration_ms > 0
                    ):
                    ...
                    try:
                        if (
                            ...
                            if book_id not in page_count_cache:
                                try:
                                    qr = safe_execute_kuzu_query(
                                        "MATCH (b:Book) WHERE b.id = $id RETURN b.page_count LIMIT 1",
                                        {"id": book_id},
                                    )
                                    pc = None
                                    if isinstance(qr, list) and qr:
                                        ...
                                        if isinstance(first, dict):
                                            pc = (
                                                first.get("result")
                                                or first.get("b.page_count")
                                                or first.get("col_0")
                                            )
                                        elif isinstance(first, (list, tuple)) and first:
                                            pc = first[0]
                                    page_count_cache[book_id] = (
                                        int(pc)
                            ...
                            if isinstance(pcache, int) and pcache > 0:
                                ...
                                pages_from_session = max(
                                    0,
                                    int(
                                        round(
                                            (minutes_from_session / total_min) * pcache
                                        )
                                    ),
                                )
                    except Exception:
                    ...
                    if pos_ms is not None:
                        ...
                        self._upsert_progress(
                            ...
                            has_listening_activity=bool(
                                minutes_from_session and minutes_from_session > 0
                            ),
                        )
                        if not minutes_from_session:
                            self._dbg(
                                "No minutes derived from session (skipped logging)",
                                {"book_id": book_id},
                            )
                except Exception as e:
    ...
    def sync_item_progress(self, item_or_id: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(item_or_id, dict):
                item_id = (
                    item_or_id.get("id")
                    or item_or_id.get("_id")
                    or item_or_id.get("itemId")
                )
            else:
            ...
            try:
                finished_probe = self._is_finished_from_payload(
                    prog, position_ms=pos_ms, duration_ms=dur_ms
                )
            except Exception:
                finished_probe = False
            ...
            try:
                ...
                if (
                    ...
                    progress_pct = max(
                        0.0, min(100.0, (float(pos_ms) / float(dur_ms)) * 100.0)
                    )
            except Exception:
                progress_pct = None
            ...
            if isinstance(pos_ms, int):
                finished_once = False
                try:
                    finished_once = self._is_finished_from_payload(
                        prog, position_ms=pos_ms, duration_ms=dur_ms
                    )
                except Exception:
                    finished_once = False
                self._upsert_progress(
                    ...
                    finished_once,
                    progress_pct=progress_pct,
                    has_listening_activity=bool(minutes and minutes > 0),
                )
            if isinstance(minutes, int) and minutes > 0:
                self._log_minutes(book_id, minutes, updated_at)
            self._dbg(
                "sync_item_progress: applied",
                {
                    "item_id": item_id,
                    "book_id": book_id,
                    "pos_ms": pos_ms,
            ...
            finished = False
            try:
                finished = self._is_finished_from_payload(
                    prog, position_ms=pos_ms, duration_ms=dur_ms
                )
            except Exception:
                finished = False
