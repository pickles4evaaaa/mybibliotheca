...
class _AbsSyncRunner:
    ...
    def ensure_started(self) -> None:
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._run_loop, name="abs-sync-runner", daemon=True
            )
            self._thread.start()

    def enqueue_test_sync(
        self, user_id: str, library_ids: list[str], limit: int = 5
    ) -> str:
    ...
    def enqueue_user_composite_sync(
        self,
        user_id: str,
        page_size: int = 50,
        *,
        force_books: bool = False,
        force_listening: bool = False,
    ) -> str:
            ...
            self._queue.append(
                (
                    task_id,
                    {
                        ...
                        "user_id": user_id,
                        "page_size": page_size,
                        "force_books": bool(force_books),
                        "force_listening": bool(force_listening),
                    },
                )
            )
        return task_id
    ...
    def enqueue_full_sync(
        self, user_id: str, library_ids: list[str], page_size: int = 50
    ) -> str:
    ...
    def _create_job(
        self, user_id: str, task_id: str, job_type: str, total: int
    ) -> None:
            ...
            while self._running:
                ...
                try:
                        ...
                        try:
                            safe_update_import_job(
                                ...
                                task_id,
                                {
                                    "status": "failed",
                                    "error_messages": [
                                        "ABS not configured: base_url or api_key missing"
                                    ],
                                },
                            )
                        except Exception:
                    ...
                    if kind == "listen":
                        ...
                        do_books = force_books or (
                            bool(u.get("abs_sync_books"))
                            if isinstance(u, dict)
                            else False
                        )
                        do_listen = force_listening or (
                            bool(u.get("abs_sync_listening"))
                            if isinstance(u, dict)
                            else False
                        )
                        ...
                        if do_books:
                            svc = AudiobookshelfImportService(
                                payload["user_id"], eff_client
                            )
                            svc._run_full_sync_job(
                                task_id,
                                settings.get("library_ids") or [],
                                int(payload.get("page_size") or 50),
                            )
                        if do_listen:
                            listener = AudiobookshelfListeningSync(
                                payload["user_id"], eff_client
                        ...
                        try:
                            safe_update_import_job(
                                payload["user_id"], task_id, {"status": "completed"}
                            )
                        except Exception:
                            pass
                    else:
                        ...
                        if kind == "test":
                            svc._run_test_sync_job(
                                task_id,
                                payload.get("library_ids") or [],
        ...
        if users:
            for u in users:
                try:
                    ...
                    self.enqueue_user_composite_sync(
                        str(getattr(u, "id")), force_books=True, force_listening=True
                    )
                    queued += 1
                except Exception:
                    continue
