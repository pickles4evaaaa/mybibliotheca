...
class KuzuRelationshipService:
    ...
    def _create_enriched_book(
        ...
        if personal_meta:
            ...
            custom_blob = (
                personal_meta.get("personal_custom_fields")
        ...
        try:
            if not any(
                ...
                pm = personal_metadata_service.get_personal_metadata(
                    self.user_id, str(getattr(book, "id", ""))
                )
                for k in [
                    "personal_notes",
                    "user_review",
        ...
        setattr(
            book, "review", combined.get("user_review") or combined.get("review")
        )  # Map user_review back to review
        ...
        rs_val = (
            combined.get("reading_status")
            if combined.get("reading_status") is not None
            else (
                relationship_data.get("reading_status")
        ...
        debug_log(
            f"Processing locations for book: filtered_locations={locations}, relationship_data location_id={relationship_data.get('location_id')}",
            "RELATIONSHIP",
        )

        # If we have locations from STORED_AT relationships, use them
        if locations:
            location_id = locations[0].get(
                "id"
            ...
            if location_id:
                ...
                try:
                    ...
                    if location:
                        ...
                        debug_log(
                            f"Resolved legacy location {location_id} to name '{location_name}'",
                            "RELATIONSHIP",
                        )
                    else:
                        debug_log(
                            f"Legacy location {location_id} not found, using ID as name",
                            "RELATIONSHIP",
                        )
                except Exception as e:
                    debug_log(
                        f"Error resolving legacy location name for {location_id}: {e}",
                        "RELATIONSHIP",
                    )
        ...
        debug_log(
            f"Final book location attributes: location_id={location_id}, locations={locations}",
            "RELATIONSHIP",
        )
    ...
    async def get_books_for_user(
        ...
        try:
            ...
            result = safe_execute_kuzu_query(query, {"offset": offset, "limit": limit})
            results = _convert_query_result_to_list(result)
    ...
    def add_book_to_user_library_sync(
        ...
        logger.warning(
            "[UNIVERSAL_LIBRARY] add_book_to_user_library_sync called but deprecated in universal library mode"
        )
    ...
    async def update_user_book_relationship(
        ...
        try:
            ...
            direct_fields = {
                ...
                if k in {"personal_notes", "user_review", "start_date", "finish_date"}
            }
            ...
            start_dt = (
                _parse_date(direct_fields.get("start_date"))
                if "start_date" in direct_fields
                else None
            )
            finish_dt = (
                _parse_date(direct_fields.get("finish_date"))
    ...
    async def remove_book_from_user_library(self, user_id: str, book_id: str) -> bool:
        ...
        try:
            ...
            try:
                safe_execute_kuzu_query(
                    "MATCH (u:User {id: $uid})-[o:OWNS]->(b:Book {id: $bid}) DELETE o",
    ...
    async def get_all_books_with_user_overlay(
        ...
        try:
            ...
            logger.info(
                f"[RELATIONSHIP_SERVICE] Raw query returned {len(results)} results with optional user overlay"
            )
            ...
            for i, result_row in enumerate(results):
                try:
                    if "col_0" not in result_row:
                        logger.warning(
                            f"[RELATIONSHIP_SERVICE] Row {i} missing col_0 (book data)"
                        )
                        continue
                    ...
                    book = self._create_enriched_book(
                        book_data, relationship_data, locations_data, personal_meta
                    )
                    ...
                    logger.error(
                        f"[RELATIONSHIP_SERVICE] Error processing book {i}: {e}"
                    )
                    continue

            logger.info(
                f"[RELATIONSHIP_SERVICE] Successfully processed {len(book_dicts)} books with personal metadata overlay"
            )
            return book_dicts

        except Exception as e:
            logger.error(
                f"[RELATIONSHIP_SERVICE] Error in get_all_books_with_user_overlay: {e}"
            )
            traceback.print_exc()
            return []
    ...
    def get_book_by_uid_sync(self, uid: str, user_id: str) -> Optional[Book]:
        ...
        logger.debug(
            f"[RELATIONSHIP_SERVICE][BOOK_LOOKUP] Raw rows for id={uid}: {results}"
        )
        if not results:
            logger.debug(
                f"[RELATIONSHIP_SERVICE][BOOK_LOOKUP] No results for book id={uid} user={user_id} db_path={os.getenv('KUZU_DB_PATH')}"
            )
            return None
        row = results[0]
        ...
        if "col_0" not in row:
            ...
            if possible_keys:
                ...
                logger.debug(
                    f"[RELATIONSHIP_SERVICE][BOOK_LOOKUP] Missing col_0 key; recovered using {possible_keys[0]} for book id={uid}"
                )
            elif "result" in row:
                book_data = row["result"]
                logger.debug(
                    f"[RELATIONSHIP_SERVICE][BOOK_LOOKUP] Used 'result' key fallback for book id={uid}"
                )
            else:
                logger.debug(
                    f"[RELATIONSHIP_SERVICE][BOOK_LOOKUP] Row lacked expected keys for book id={uid}: keys={list(row.keys())}"
                )
                return None
        else:
            book_data = row["col_0"]
        ...
        valid_locations = [
            loc for loc in locations_data if loc and loc.get("id") and loc.get("name")
        ]
        debug_log(
            f"Book {uid} locations from STORED_AT: {valid_locations}", "RELATIONSHIP"
        )
        ...
        if isinstance(book_data, dict) and "title" not in book_data:
            ...
            try:
                ...
                logger.debug(
                    f"[RELATIONSHIP_SERVICE][BOOK_LOOKUP] Fallback field fetch failed for id={uid}: {_fb_err}"
                )
        book = self._create_enriched_book(
            book_data, relationship_data, valid_locations, personal_meta
        )
        setattr(book, "custom_metadata", {})
        return book
    ...
    async def get_recently_added_want_to_read_books(
        ...
        # This method is deprecated but kept for backward compatibility
        logger.warning(
            "[UNIVERSAL_LIBRARY] get_recently_added_want_to_read_books called but deprecated in universal library mode"
        )
    ...
    async def get_books_with_user_overlay_paginated(
        ...
        try:
            ...
            logger.error(
                f"[RELATIONSHIP_SERVICE] get_books_with_user_overlay_paginated error: {e}"
            )
            return []
        ...
        try:
            ...
            for book in books or []:
                ...
                rs = (
                    ...
                    else getattr(book, "reading_status", None)
                )
                owner = (
                    book.get("ownership_status")
                    if isinstance(book, dict)
                    else getattr(book, "ownership_status", None)
                )
            ...
            logger.error(
                f"[RELATIONSHIP_SERVICE] get_library_status_counts_sync error: {e}"
            )
        return counts
