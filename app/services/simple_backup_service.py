...
class SimpleBackupService:
        ...
        try:
            ...
            for key in [
                ...
                if key in self._settings and verified.get(key) != self._settings[key]:
                    raise ValueError(
                        f"Setting '{key}' verification failed: expected {self._settings[key]}, got {verified.get(key)}"
                    )

            # If verification succeeds, rename temp file to actual file
            temp_file.replace(self.backup_settings_file)
            logger.info(
                f"Backup settings saved and verified successfully: {self._settings}"
            )
            return True
        except Exception as e:
    ...
    def _maybe_run_scheduled_backup(self):
        ...
        if freq == "daily":
            if not last_run or (
                ...
                and now.hour >= self._settings.get("scheduled_hour", 2)
                and now.minute >= self._settings.get("scheduled_minute", 30)
            ):
                due = True
        ...
        if due:
            ...
            self.create_backup(
                description="Scheduled daily backup", reason="scheduled_daily"
            )
    ...
    def _load_backup_index(self) -> dict[str, SimpleBackupInfo]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Backup index JSON corrupted at position {parse_error.pos}, attempting recovery..."
                )
                ...
                if content.startswith("{"):
                    ...
                    if last_valid_pos > 0:
                        try:
                            ...
                            logger.info(
                                f"Successfully recovered backup index by truncating at position {last_valid_pos}"
                            )
                            # Save the corrected version
                            self._backup_index = {}
                            for backup_id, backup_data in data.items():
                                try:
                                    self._backup_index[backup_id] = (
                                        SimpleBackupInfo.from_dict(backup_data)
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"Failed to load backup info for {backup_id}: {e}"
                                    )
                ...
                logger.warning(
                    f"Backup index was corrupted, moved to {corrupted_backup} and starting fresh"
                )
                return {}
    ...
    def _save_backup_index(self) -> None:
        ...
        try:
            data = {
                ...
                for backup_id, backup in self._backup_index.items()
            }
    ...
    def create_backup(
        ...
        if not self._create_lock.acquire(blocking=False):
            logger.warning(
                "Backup already in progress; skipping concurrent create_backup request"
            )
            return None
        try:
            ...
            metadata = {
                ...
                "covers_size": self._get_directory_size(self.covers_dir)
            ...
            quiesce_enabled = os.getenv("KUZU_BACKUP_QUIESCE", "false").lower() in (
                "1",
                "true",
                "yes",
            )
            ...
            if quiesce_enabled:
                try:
                    from app.utils.safe_kuzu_manager import (
                        get_safe_kuzu_manager as _gskm,
                    )
            ...
            quiesce_ctx = (
                manager.quiesce_for_backup(reason="simple_backup")
                if manager and quiesce_enabled
            ...
            with quiesce_ctx:
                with zipfile.ZipFile(backup_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                    ...
                    logger.info(
                        f"Backed up KuzuDB directory with {db_files_count} files"
                    )
                    ...
                    if self.covers_dir.exists():
                        for file_path in self.covers_dir.rglob("*"):
                            if file_path.is_file():
                                ...
                                covers_count += 1
                                logger.debug(
                                    f"Added cover to backup: data/covers/{relative_path}"
                                )
                    ...
                    if self.uploads_dir.exists():
                        for file_path in self.uploads_dir.rglob("*"):
                            if file_path.is_file():
                                ...
                                uploads_count += 1
                                logger.debug(
                                    f"Added upload to backup: data/uploads/{relp}"
                                )
                    ...
                    try:
                        ...
                        if self.backup_settings_file.exists():
                            zipf.write(
                                self.backup_settings_file, "config/backup_settings.json"
                            )
                            settings_added += 1
                        logger.info(
                            f"Included {settings_added} config/settings files in backup"
                        )
            ...
            logger.info(
                f"Simple backup created successfully: {name} ({file_size / 1024 / 1024:.2f} MB)"
            )
        ...
        try:
            ...
            if self.kuzu_db_path.exists():
                try:
                    ...
                    moved_old_dir = (
                        self.kuzu_db_path.parent
                        / f"kuzu.pre_restore_{timestamp}_{short_id}"
                    )
                    logger.info(
                        f"Moving current Kuzu directory to {moved_old_dir} for safety snapshot..."
                    )
                    ...
                    logger.info(
                        f"Creating pre-restore backup zip at {pre_backup_path} ..."
                    )
                    with zipfile.ZipFile(
                        pre_backup_path, "w", zipfile.ZIP_DEFLATED
                    ) as zipf:
                        zipf.writestr(
                            "backup_metadata.json", json.dumps(metadata, indent=2)
                        )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed adding settings to pre-restore snapshot: {se}"
                            )
                        logger.info(
                            f"Pre-restore snapshot archived: {files_count} files"
                        )
                    ...
                    logger.warning(
                        f"Failed to create pre-restore snapshot: {e}. Proceeding cautiously."
                    )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if kuzu_source_path.exists():
                    ...
                    for file_path in temp_path.glob("*"):
                        if (
                            file_path.is_file()
                            and file_path.name != "backup_metadata.json"
                        ):
                ...
                if static_source_path.exists():
                    ...
                    if covers_source.exists():
                        ...
                        logger.info(
                            f"Restored {covers_restored} cover images from legacy backup"
                        )
                    ...
                    if uploads_source.exists():
                        ...
                        logger.info(
                            f"Restored {uploads_restored} uploaded files from legacy backup"
                        )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to remove pre-restore directory {moved_old_dir}: {e}"
                )
            ...
            logger.info(
                f"Simple restore completed successfully from backup: {backup_info.name}"
            )
        ...
        try:
            ...
            if health_status.get("database_initialized", False):
                try:
                    ...
                    cleaned_connections = safe_manager.cleanup_stale_connections(
                        max_age_minutes=0
                    )
            ...
            try:
                ...
                for attr_name in dir(app.services):
                    if not attr_name.startswith("_"):
                        ...
                        if (
                            service
                            and hasattr(service, "__class__")
                            and hasattr(service.__class__, "__name__")
                        ):
                            if "service" in service.__class__.__name__.lower():
                                try:
                                    if hasattr(service, "_db") or hasattr(
                                        service, "db"
                                    ):
    ...
    def _clear_service_cache(self) -> None:
        ...
        try:
            ...
            if hasattr(app.services, "book_service") and hasattr(
                app.services.book_service, "_service"
            ):
                app.services.book_service._service = None
            if hasattr(app.services, "user_service") and hasattr(
                app.services.user_service, "_service"
            ):
                app.services.user_service._service = None
            if hasattr(app.services, "custom_field_service") and hasattr(
                app.services.custom_field_service, "_service"
            ):
                app.services.custom_field_service._service = None
            if hasattr(app.services, "import_mapping_service") and hasattr(
                app.services.import_mapping_service, "_service"
            ):
                app.services.import_mapping_service._service = None
            if hasattr(app.services, "person_service") and hasattr(
                app.services.person_service, "_service"
            ):
                app.services.person_service._service = None
            if hasattr(app.services, "reading_log_service") and hasattr(
                app.services.reading_log_service, "_service"
            ):
            ...
            try:
                ...
                if hasattr(app.kuzu_integration, "kuzu_service"):
                    app.kuzu_integration.kuzu_service = (
                        app.kuzu_integration.KuzuIntegrationService()
                    )
        ...
        while retry_count < max_retries:
            try:
                ...
                try:
                    ...
                    logger.info(
                        "Successfully reconnected to Kuzu database via SafeKuzuManager"
                    )
                except Exception as e:
                ...
                try:
                    ...
                    if (
                        service
                        and hasattr(service, "_initialized")
                        and service._initialized
                    ):
                        ...
                        logger.info(
                            "Kuzu service available, considering reconnection successful"
                        )
                        return True
                    else:
                        logger.warning(
                            "Kuzu service returned None, but database connection successful"
                        )
                        return True  # Database connection is working, service issue is non-critical
                except Exception as service_error:
                    logger.warning(
                        f"Kuzu service check failed: {service_error}, but database connection successful"
                    )
                    return True  # Database connection is working, service issue is non-critical
                ...
                if retry_count < max_retries:
                    logger.warning(
                        f"Reconnection attempt {retry_count} failed: {e}. Retrying..."
                    )
                    import time
                    ...
                    logger.error(
                        f"Error during KuzuDB reconnection after {max_retries} attempts: {e}"
                    )
                    return False
    ...
    def _copy_file_with_retry(
        self, source: Path, target: Path, max_retries: int = 3
    ) -> None:
        ...
        while retry_count < max_retries:
            try:
                ...
                if e.errno == 35:  # Resource deadlock avoided
                    ...
                    if retry_count < max_retries:
                        logger.warning(
                            f"File copy failed for {source.name} (attempt {retry_count}), retrying in 2 seconds..."
                        )
                        import time
                        ...
                        logger.error(
                            f"Failed to copy {source.name} after {max_retries} attempts: {e}"
                        )
                        raise
                else:
    ...
    def get_backup_stats(self) -> dict[str, Any]:
        ...
        database_size = (
            self._get_directory_size(self.kuzu_db_path)
            if self.kuzu_db_path.exists()
            else 0
        )
        database_size_formatted = (
            f"{database_size / (1024 * 1024):.1f} MB" if database_size > 0 else "0 MB"
        )

        # Get current static files size
        covers_size = (
            self._get_directory_size(self.covers_dir) if self.covers_dir.exists() else 0
        )
        uploads_size = (
            self._get_directory_size(self.uploads_dir)
            if self.uploads_dir.exists()
            else 0
        )
        covers_count = (
            len(list(self.covers_dir.rglob("*"))) if self.covers_dir.exists() else 0
        )
        uploads_count = (
            len(list(self.uploads_dir.rglob("*"))) if self.uploads_dir.exists() else 0
        )

        covers_size_formatted = (
            f"{covers_size / (1024 * 1024):.1f} MB" if covers_size > 0 else "0 MB"
        )
        uploads_size_formatted = (
            f"{uploads_size / (1024 * 1024):.1f} MB" if uploads_size > 0 else "0 MB"
        )
    ...
    def _apply_retention_policy(self):
        ...
        if not to_delete:
            return
        logger.info(
            f"Retention policy: deleting {len(to_delete)} old backups (>{retention_days} days)"
        )
