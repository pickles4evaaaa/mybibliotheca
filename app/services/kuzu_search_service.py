...
class KuzuSearchService:
    ...
    def __init__(self, user_id: str | None = None):
        ...
        self.relationship_service = (
            KuzuRelationshipService()
        )  # This service may not be migrated yet
    ...
    async def search_books(
        ...
        try:
            ...
            user_books = await self.relationship_service.get_books_for_user(
                user_id, limit=1000
            )  # Get all books for filtering

            tokens = [
                token.lower() for token in re.split(r"\s+", query.strip()) if token
            ]
            if not tokens:
            ...
            for book in user_books:
                ...
                if getattr(book, "title", None):
                    ...
                    search_blobs.extend(
                        word for word in re.split(r"\s+", book.title.lower()) if word
                    )
                ...
                if subtitle_value:
                    ...
                    search_blobs.extend(
                        word for word in re.split(r"\s+", subtitle_lower) if word
                    )
                ...
                if getattr(book, "contributors", None):
                    for contribution in book.contributors:
                        ...
                        normalized_name = (
                            getattr(person, "normalized_name", None) if person else None
                        )
                        if normalized_name:
                ...
                if series_obj and getattr(series_obj, "name", None):
                    ...
                    search_blobs.extend(
                        word
                        for word in re.split(r"\s+", series_obj.name.lower())
                        if word
                    )
    ...
    async def get_books_with_sharing_users(
        ...
        try:
            ...
            for user_dict in sharing_users:
                if user_dict.get("share_reading_activity", False) and user_dict.get(
                    "is_active", True
                ):
            ...
            finished_books = [
                ...
                if hasattr(book, "finish_date")
                and book.finish_date
                and book.finish_date >= cutoff_date
            ]

            # Sort by finish date descending and limit
            finished_books.sort(
                key=lambda b: getattr(b, "finish_date", date.min) or date.min,
                reverse=True,
            )
    ...
    async def get_currently_reading_shared(self, limit: int = 20) -> list[Book]:
        ...
        try:
            ...
            for user_dict in sharing_users:
                if user_dict.get("share_current_reading", False) and user_dict.get(
                    "is_active", True
                ):
                    sharing_user_ids.append(user_dict.get("id"))
            ...
            currently_reading = [
                ...
                for book in all_books
                if (
                    hasattr(book, "start_date")
                    and book.start_date
                    and (not hasattr(book, "finish_date") or not book.finish_date)
                )
            ]

            # Sort by start date descending and limit
            currently_reading.sort(
                key=lambda b: getattr(b, "start_date", date.min) or date.min,
                reverse=True,
            )
        ...
        try:
            ...
            for book in user_books:
                ...
                if "reading_status" in filters:
                    if (
                        getattr(book, "reading_status", None)
                ...
                if "year_published" in filters and include_book:
                    ...
                    if isinstance(year_filter, dict):
                        if "min" in year_filter and (
                            not book_year or book_year < year_filter["min"]
                        ):
                            include_book = False
                        if "max" in year_filter and (
                            not book_year or book_year > year_filter["max"]
                        ):
                            include_book = False
