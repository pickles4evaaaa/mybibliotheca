...
class KuzuUserService:
        ...
        try:
            ...
            if user_data:
                ...
                return User(
                    ...
                    share_reading_activity=user_data.get(
    ...
    async def get_user_by_username_or_email(
        ...
        try:
            ...
            current_app.logger.error(
                f"Error getting user by username or email {username_or_email}: {e}"
            )
            return None
    ...
    def create_user_sync(
        ...
        try:
            ...
            print("🚀 [USER_SERVICE] Calling kuzu_service.create_user via run_async...")
            created_user_data = run_async(self.kuzu_service.create_user(user_data))
            print(
                f"🚀 [USER_SERVICE] kuzu_service.create_user returned: {created_user_data}"
            )
            ...
            if created_user_data:
                user = User(
                    ...
                    is_admin=created_user_data.get("is_admin", False),
                    is_active=created_user_data.get("is_active", True),
                    password_must_change=created_user_data.get(
        ...
        try:
            ...
            if updated_user_data:
                return User(
                    ...
                    password_must_change=updated_user_data.get(
                        "password_must_change", False
                    ),
                    failed_login_attempts=updated_user_data.get(
                        "failed_login_attempts", 0
                    ),
                    share_current_reading=updated_user_data.get(
                        "share_current_reading", True
                    ),
                    share_reading_activity=updated_user_data.get(
                        "share_reading_activity", True
                    ),
                    share_library=updated_user_data.get("share_library", False),
                    reading_streak_offset=updated_user_data.get(
    ...
    async def create_user_location(
        ...
        try:
            location = Location(
                ...
                user_id=user_id,
                name=name,
                description=description or "Default location set during onboarding",
