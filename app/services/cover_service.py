...
class CoverService:
    ...
    def fetch_and_cache(
        ...
        try:
            ...
            candidates = (
                get_cover_candidates(isbn=isbn, title=None, author=None)
                if isbn
                else get_cover_candidates(isbn=isbn, title=title, author=author)
            )
            ...
            if not cover_url and (title or author):
                try:
                    ...
                    ta_candidates = get_cover_candidates(
                        isbn=None, title=title, author=author
                    )
                    if ta_candidates:
            ...
            if cover_url:
                try:
                    ...
                    current_app.logger.warning(
                        f"[COVER][SERVICE] Download/process failed url={cover_url} err={e}"
                    )
            ...
            if cover_url:
                if chosen:
                    ...
                    quality = (
                        ...
                        if (
                            "zoom=1" in cover_url
                            or "extraLarge" in cover_url
                            or "edge=c" in cover_url
                        )
                        else "medium"
                    )
                elif sel:
            ...
            log_fn = (
                current_app.logger.info if cached_url else current_app.logger.warning
            )
            log_fn(
                f"[COVER][SERVICE] isbn={isbn} title='{(title or '')[:40]}' elapsed={elapsed:.3f}s steps={result.steps} cached={bool(cached_url)}"
            )
            return result
        except Exception as e:
            elapsed = time.perf_counter() - t0
            current_app.logger.error(
                f"[COVER][SERVICE] FATAL isbn={isbn} title='{(title or '')[:40]}' err={e} elapsed={elapsed:.3f}s"
            )
            return CoverResult(
                source="error",
                quality="none",
    ...
    def schedule_async_processing(
        ...
        cand = self.select_candidate(
            isbn=isbn, title=title, author=author, prefer_provider=prefer_provider
        )
        ...
        def _worker(app_obj=app_obj):
            ...
            try:
                try:
                    ...
                    try:
                        current_app.logger.error(
                            f"[COVER][ASYNC] Job {job_id} failed: {e}"
                        )
                    except Exception:
                        pass
            finally:
