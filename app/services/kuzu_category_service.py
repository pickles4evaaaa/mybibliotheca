...
class KuzuCategoryService:
    ...
    async def list_all_categories(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raw_result = safe_execute_kuzu_query(
                query=query, user_id=self.user_id, operation="list_all_categories"
            )
    ...
    def _build_category_with_hierarchy(self, category_id: str) -> Optional[Category]:
        ...
        try:
            ...
            if category.parent_id:
                parent_category = self._build_category_with_hierarchy(
                    category.parent_id
                )
        ...
        try:
            ...
            for child in immediate_children:
                ...
                if child_id:
                    ...
                    child_descendants = await self._get_all_descendant_categories(
                        child_id
                    )
                    descendant_ids.extend(child_descendants)

            print(
                f"🌳 [GET_DESCENDANTS] Found {len(descendant_ids)} descendant categories for {category_id}"
            )
    ...
    async def merge_categories(
        ...
        try:
            print(
                f"🔄 [MERGE_CATEGORIES] Merging {len(merge_category_ids)} categories into {primary_category_id}"
            )
            ...
            for merge_id in merge_category_ids:
                ...
                safe_execute_kuzu_query(
                    ...
                    params={"merge_id": merge_id, "primary_id": primary_category_id},
                    user_id=self.user_id,
