...
class AudiobookshelfImportService:
    ...
    def _map_abs_item_to_simplified(self, item: Dict[str, Any]) -> SimplifiedBook:
        ...
        s = (
            item.get("series")
            if isinstance(item.get("series"), (dict, list, str))
            else md.get("series")
        )
        ...
        if isinstance(s, dict):
            ...
            alt_name = (
                item.get("seriesName") or md.get("seriesName") or md.get("series_name")
            )
            ...
            cand_order = (
                ...
                or item.get("bookNumber")
                or md.get("seriesSequence")
                or md.get("seriesIndex")
                or md.get("seriesPosition")
                or md.get("sequenceNumber")
                or md.get("bookNumber")
            )
        ...
        if isinstance(duration, (int, float)):
            duration_ms = (
                int(float(duration) * 1000) if duration < 10_000_000 else int(duration)
            )
    ...
    def _needs_item_detail_for_persons(self, item: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if isinstance(a, str) and a.strip():
                return False
            if (
                isinstance(md.get("narrators"), list)
                and len(md.get("narrators") or []) > 0
            ):
                return False
            n = md.get("narrator")
        ...
        try:
            ...
            if item_id:
                candidate_urls.append(
                    self.client._url(f"/api/items/{item_id}/cover?width=800")
                )
                candidate_urls.append(
                    self.client._url(f"/api/items/{item_id}/cover?w=800")
                )
            ...
            resp = None
            insecure = os.environ.get("ABS_INSECURE", "").lower() in (
                "1",
                "true",
                "yes",
            )
            for u in candidate_urls:
                try:
                    r = requests.get(
                        u, headers=headers, timeout=12, stream=True, verify=not insecure
                    )
                    ...
                    if "image" not in ctype_try and not u.lower().endswith(
            ...
            # If content-type didn't help, try using URL suffix
                ...
                if any(
                    lower_urls.endswith(suf)
                    for suf in [".png", ".webp", ".gif", ".jpeg", ".jpg"]
                ):
    ...
    def _apply_audiobook_fields(self, book_id: str, abs_item: Dict[str, Any]):
        ...
        try:
            ...
            if (
                abs_updated_iso
                and isinstance(abs_updated_iso, str)
                and abs_updated_iso.strip() != ""
            ):
                safe_execute_kuzu_query(
                    """
                    MATCH (b:Book {id: $book_id})
    ...
    def _ensure_person_exists(self, name: str) -> Optional[str]:
        ...
        try:
            ...
            from app.simplified_book_service import (
                _convert_query_result_to_list as _legacy_convert,
            )
    ...
    def _ensure_contributor(
        self, book_id: str, person_name: str, role: str, order_index: int = 0
    ) -> None:
    ...
    def _ensure_contributors_for_book(
        self, book_id: str, book_data: SimplifiedBook
    ) -> None:
        try:
            # Primary author
            if (
                getattr(book_data, "author", None)
                and str(book_data.author).strip().lower() != "unknown"
            ):
            ...
            if rows:
                ...
                if isinstance(r0, dict):
                    return (
                        r0.get("result")
                        or r0.get("col_0")
                        or r0.get("b.audiobookshelf_updated_at")
                    )
                try:
                    return str(r0)
                except Exception:
    ...
    def _run_test_sync_job(
        ...
        try:
            ...
            if not items:
                safe_update_import_job(
                    ...
                    task_id,
                    {
                        ...
                        "total_books": 0,
                        "error_messages": (
                            [f"No items returned from ABS. Last message: {last_msg}"]
                            if last_msg
                            else ["No items returned from ABS."]
                        ),
                    },
                )
                return
            ...
            for item in items[:limit]:
                ...
                try:
                    ...
                    if not self._should_process_abs_item(item):
                        ...
                        try:
                            listener = AudiobookshelfListeningSync(
                                self.user_id, self.client
                            )
                            iid = (
                                item.get("id") or item.get("_id") or item.get("itemId")
                            )
                            if iid:
                    ...
                    try:
                        safe_update_import_job(
                            self.user_id, task_id, {"current_book": current_title}
                        )
                    except Exception:
                        pass

                    created = self.simple_books.add_book_to_user_library_sync(
                        book_data, self.user_id, media_type="audiobook"
                    )
                    if not created:
                    ...
                    if book_id:
                        ...
                        if updates:
                            try:
                                self.kuzu_book_service.update_book_sync(
                                    book_id, updates
                                )
                            except Exception:
                                pass
                        ...
                        if cats:
                            try:
                                ...
                                svc.update_book_sync(
                                    book_id, self.user_id, raw_categories=cats
                                )
                            except Exception:
                                pass
                    ...
                    safe_update_import_job(
                        ...
                        task_id,
                        {
                            ...
                            "processed_books": pb[-100:],
                            "audiobooks_synced": (
                                current_job.get("audiobooks_synced") or 0
                            )
                            + 1,
                            "recent_activity": (
                                current_job.get("recent_activity") or []
                            )[-9:]
                            + [f"Imported: {current_title}"],
                        },
                    )
                    ...
                    try:
                        ...
                        if pre_local_cover:
                            ...
                            if book_id:
                                self.kuzu_book_service.update_book_sync(
                                    book_id, {"cover_url": pre_local_cover}
                                )
                                try:
                                    existing = (
                                        self.kuzu_book_service.get_book_by_id_sync(
                                            book_id
                                        )
                                    )
                                    updates: Dict[str, Any] = {}
                                    if existing and bd:
                                        ...
                                        if (
                                            getattr(existing, "series_order", None)
                                            in (None, 0)
                                        ) and (bd.series_order is not None):
                                            updates["series_order"] = bd.series_order
                                        if (
                                            not getattr(existing, "series_volume", None)
                                        ) and (bd.series_volume):
            ...
            try:
                ...
                pb.append(
                    {
                        "title": f"ABS listening sync: processed {lsum.get('processed', 0)}, matched {lsum.get('matched', 0)}",
                        "status": "info",
                    }
                )
                safe_update_import_job(
            ...
            safe_update_import_job(
                self.user_id,
                task_id,
                {"status": "completed", "processed": processed, "total": total_local},
            )
        except Exception:
            traceback.print_exc()
            safe_update_import_job(
        ...
        try:
                    ...
                    for item in items:
                        ...
                        try:
                            ...
                            if not self._should_process_abs_item(item):
                                processed += 1
                                current_job = (
                                    safe_get_import_job(self.user_id, task_id) or {}
                                )
                                pb = list(current_job.get("processed_books") or [])
                                pb.append(
                                    {
                                ...
                                safe_update_import_job(
                                    ...
                                    task_id,
                                    {
                                        "status": "running",
                                        "processed": processed,
                                        "skipped": (current_job.get("skipped") or 0)
                                        + 1,
                                        "processed_books": pb[-100:],
                                    },
                                )
                            ...
                            if book_id:
                                ...
                                # Ensure contributors are present
                                try:
                                    self._ensure_contributors_for_book(
                                        book_id, book_data
                                    )
                                except Exception:
                                    pass
                            ...
                            try:
                                ...
                                if pre_local_cover:
                                    bd: SimplifiedBook = (
                                        book_data
                                        if "book_data" in locals() and book_data
                                        else self._map_abs_item_to_simplified(item)
                                    )
                                    book_id = self._resolve_book_id(bd)
                                    if book_id:
                                        self.kuzu_book_service.update_book_sync(
                                        ...
                                        try:
                                            ...
                                            if existing and bd:
                                                ...
                                                if (
                                                    not getattr(
                                                        existing, "series_volume", None
                                                    )
                                                ) and (bd.series_volume):
                                                    updates["series_volume"] = (
                                                        bd.series_volume
                                                    )
                                            if updates:
                                                self.kuzu_book_service.update_book_sync(
            ...
            try:
                if latest_seen_map:
                    ...
                    save_abs_settings(
                        {
                            "last_library_sync_map": new_map,
                            "last_library_sync": __import__("time").time(),
                        }
                    )
            except Exception:
                pass
            ...
            safe_update_import_job(
                self.user_id,
                task_id,
                {"status": "completed", "processed": processed, "total": grand_total},
            )
        except Exception:
            traceback.print_exc()
            safe_update_import_job(
    ...
    def _resolve_library_ids(self, library_ids: List[str]) -> List[str]:
        ...
        for lib in libs:
            lid = str(
                lib.get("id") or lib.get("_id") or lib.get("libraryId") or ""
            ).strip()
        ...
        if not resolved:
            ...
            resolved = (
                list(id_map.values())
                if id_map
                else [str(x).strip() for x in (library_ids or []) if str(x).strip()]
            )
        ...
        def worker():
            ...
            try:
                ...
                try:
                    ...
                    lsum = listener.sync(
                        page_size=100, seed_item_ids=imported_item_ids[:limit]
                    )
                    ...
                    try:
                        ...
                        pb.append(
                            {
                                "title": f"ABS listening sync error: {le}",
                                "status": "error",
                            }
                        )
                        safe_update_import_job(
                            self.user_id, task_id, {"processed_books": pb[-100:]}
                        )
                    except Exception:
                        pass
    ...
    def start_full_sync(
        ...
        def worker():
            ...
            try:
                ...
                for lib_id in libs_to_try:
                    meta = self.client.list_library_items(
                        lib_id, page=1, size=page_size
                    )
                    lib_total = int(meta.get("total") or 0)
                    grand_total += lib_total
                safe_update_import_job(
                ...
                for lib_id in libs_to_try:
                    ...
                    try:
                        cutoff_iso = (
                            settings_local.get("last_library_sync_map") or {}
                        ).get(lib_id)
                    except Exception:
                        cutoff_iso = None
                    ...
                    while True:
                        ...
                        def _prepare(item_in: Dict[str, Any]) -> Dict[str, Any]:
                            ...
                            try:
                                ...
                                item_id = (
                                    loc_item.get("id")
                                    or loc_item.get("_id")
                                    or loc_item.get("itemId")
                                )
                                if item_id and self._needs_item_detail_for_persons(
                                    loc_item
                                ):
                                    detail = self.client.get_item(
                                        item_id, expanded=True
                                    )
                                    if detail.get("ok") and detail.get("item"):
                        ...
                        for entry in prepared:
                            ...
                            if entry.get("skip"):
                                ...
                                try:
                                    upd = self._extract_abs_updated_at(
                                        entry.get("item") or {}
                                    )
                                    if upd and (
                                        (not latest_seen_for_lib)
                                        or (str(upd) > str(latest_seen_for_lib))
                                    ):
                                ...
                                pb = list(current_job.get("processed_books") or [])
                                t = (
                                    (entry.get("item") or {}).get("title")
                                    or ((entry.get("item") or {}).get("media") or {})
                                    .get("metadata", {})
                                    .get("title")
                                    or "Unknown"
                                )
                                ...
                                if cutoff_iso:
                                    try:
                                        r_iso = self._extract_abs_updated_at(
                                            entry.get("item") or {}
                                        )
                                        if r_iso and str(r_iso) <= str(cutoff_iso):
                                            stop_due_to_cutoff = True
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                if not book_data:
                                    book_data = self._map_abs_item_to_simplified(
                                        obj_item
                                    )
                                current_title = (
                                    getattr(book_data, "title", None) or "Unknown"
                                )
                                created = (
                                    self.simple_books.add_book_to_user_library_sync(
                                        book_data, self.user_id, media_type="audiobook"
                                    )
                                )
                                if not created:
                                ...
                                if book_id:
                                    ...
                                    if local_cover:
                                        ...
                                        try:
                                            local_cover2 = self._cache_cover_with_auth(
                                                obj_item
                                            )
                                            if local_cover2:
                                ...
                                try:
                                    local_cover = entry.get("local_cover")
                                    bd_dup: SimplifiedBook = (
                                        book_data
                                        or self._map_abs_item_to_simplified(obj_item)
                                    )
                                        ...
                                        try:
                                            ...
                                            if existing and bd_dup:
                                                ...
                                                if (
                                                    ...
                                                    in (None, 0)
                                                ) and (bd_dup.series_order is not None):
                                                    updates["series_order"] = (
                                                        bd_dup.series_order
                                                    )
                                                if (
                                                ...
                                                ) and (bd_dup.series_volume):
                                                    updates["series_volume"] = (
                                                        bd_dup.series_volume
                                                    )
                                            if updates:
                                                self.kuzu_book_service.update_book_sync(
                                                    book_id, updates
                        ...
                        if stop_due_to_cutoff:
                            ...
                            try:
                                if latest_seen_for_lib:
                                    ...
                                    prev = m.get(lib_id)
                                    if (not prev) or (
                                        str(latest_seen_for_lib) > str(prev)
                                    ):
                                        m[lib_id] = latest_seen_for_lib
                                    save_abs_settings(
                                        {
                                            "last_library_sync_map": m,
                                            "last_library_sync": __import__(
                                                "time"
                                            ).time(),
                                        }
                                    )
                ...
                try:
                        ...
                        try:
                            safe_update_import_job(
                                ...
                                task_id,
                                {
                                    ...
                                    "listening_matched": int(
                                        snapshot.get("matched") or 0
                                    ),
                                },
                            )
                        except Exception:
                    ...
                    pb2.append(
                        {
                            "title": f"ABS listening sync: processed {lsum2.get('processed', 0)}, matched {lsum2.get('matched', 0)}",
                            "status": "info",
                        }
                    )
                    safe_update_import_job(
