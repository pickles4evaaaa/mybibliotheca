...
class AudiobookShelfClient:
        ...
        try:
            ...
            try:
                ...
                if resp.status_code != 401:
                    ...
                    libraries = (
                        data if isinstance(data, list) else data.get("libraries") or []
                    )
    ...
    def list_library_items(
        ...
        try:
            ...
            def _parse_items(data: Any) -> tuple[list, int]:
                ...
                if isinstance(data, dict):
                    items = (
                        data.get("results")
                        or data.get("items")
                        or data.get("libraryItems")
                        or []
                    )
                    total = data.get("total") or data.get("totalItems") or len(items)
                    return items, int(total) if isinstance(
                        total, (int, float)
                    ) else len(items)
                return [], 0
            ...
            if library_id:
                attempts.append(
                    (
                        self._url(f"/api/libraries/{library_id}/items"),
                        {
                            "limit": size,
                            "page": max(0, page - 1),
                            "sort": "updatedAt",
            ...
            if library_id:
                attempts.append(
                    (
                        self._url(f"/api/libraries/{library_id}/items"),
                        {"limit": size, "page": max(0, page - 1), "minified": 1},
                    )
                )
            ...
            return {
                ...
                "items": [],
                "total": 0,
                "message": last_err or "No items returned",
            }
        except Exception as e:
    ...
    def get_item(self, item_id: str, expanded: bool = True) -> Dict[str, Any]:
        ...
        if expanded:
            attempts.append(
                (self._url(f"/api/items/{item_id}"), {"include": "media,metadata"})
            )
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        if "item" in data and isinstance(data["item"], dict):
                            item_obj = data["item"]
                        elif "libraryItem" in data and isinstance(
                            data["libraryItem"], dict
                        ):
        ...
        try:
            ...
            if not user_id:
                ...
                base_attempts.append(
                    (
                        self._url("/api/me/listening-sessions"),
                        {"itemsPerPage": limit, "page": page},
                    )
                )
                # Fallbacks (older variants)
                base_attempts.append(
                    (
                        self._url("/api/me/sessions"),
                        {"itemsPerPage": limit, "page": page},
                    )
                )
            else:
                base_attempts.append(
                    (
                        self._url(f"/api/users/{user_id}/listening-sessions"),
                        {"itemsPerPage": limit, "page": page},
                    )
                )
                ...
                base_attempts.append(
                    (
                        ...
                        {"user": user_id, "itemsPerPage": limit, "page": page},
                    )
                )
            ...
            return {
                ...
                "sessions": [],
                "total": 0,
                "message": last_err or "No sessions returned",
            }
        except Exception as e:
            return {
                "ok": False,
    ...
    def get_user_progress_for_item(
        ...
        try:
            ...
            if user_id:
                attempts.append(
                    (self._url(f"/api/users/{user_id}/progress/{item_id}"), None)
                )
                attempts.append(
                    (self._url(f"/api/users/{user_id}/items/{item_id}/progress"), None)
                )
