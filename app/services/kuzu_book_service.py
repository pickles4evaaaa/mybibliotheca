...
class KuzuBookService:
        ...
        try:
            ...
            for field, value in updates.items():
                ...
                if field == "cover_url":
                    ...
                    if (
                        ...
                        logger.info(
                            f"[COVER] Ignoring blank cover_url update to preserve existing cover {current_cover}"
                        )
                        continue
            ...
            try:
                ...
                if "quantity" in updates and "Cannot find property quantity" in str(ex):
                    try:
                        logger.warning(
                            "[BOOK][MIGRATION] Adding missing quantity column to Book node"
                        )
    ...
    def create_book_sync(self, domain_book: Book) -> Book:
        ...
        try:
            ...
            current_app.logger.info(
                f"[BOOK][CREATE_SYNC] total={end - start:.3f}s id={book.id if book else 'n/a'}"
            )
        except Exception:
            pass
        return book
