...
class KuzuReadingLogService:
    ...
    def get_recent_shared_logs_sync(
        ...
        logger.debug(
            "Reading log system not fully implemented - returning empty list for recent shared logs"
        )
        return []
    ...
    def get_user_reading_logs_sync(
        ...
        try:
            ...
            limit_clause = ""
            query_params = {"user_id": user_id, "cutoff_date": cutoff_date}
            ...
            for result in results:
                if "col_0" in result:
                    ...
                    if book_data:
                        ...
                        log_dict["book"] = (
                            None  # Explicitly set to None for bookless logs
                        )
                ...
                if isinstance(raw_value, date):
                    ...
                    try:
                        reading_dates.append(
                            date.fromisoformat(raw_value.split("T")[0])
                        )
                    except ValueError:
                        continue
    ...
    def create_reading_log_sync(
        ...
        try:
            ...
            if reading_log.book_id:
                ...
                existing_results = safe_execute_kuzu_query(
                    existing_query,
                    {
                ...
                if existing_list:
                    ...
                    existing = (
                        existing_list[0].get("result")
                        or existing_list[0].get("col_0")
                        or next(iter(existing_list[0].values()), None)
                    )
            ...
            log_result = safe_execute_kuzu_query(
                create_log_query,
                {
                    "log_id": log_id,
            ...
            if log_list:
                ...
                logger.debug(
                    f"log_list[0] keys: {log_list[0].keys() if hasattr(log_list[0], 'keys') else 'No keys method'}"
                )
            ...
            safe_execute_kuzu_query(
                user_rel_query, {"user_id": reading_log.user_id, "log_id": log_id}
            )
            ...
            if reading_log.book_id:
                ...
                safe_execute_kuzu_query(
                    book_rel_query, {"log_id": log_id, "book_id": reading_log.book_id}
                )
            ...
            created_log = {
                ...
                "updated_at": (
                    reading_log.updated_at or reading_log.created_at
                ).isoformat()
                if reading_log.updated_at or reading_log.created_at
                else None,
            }

            logger.info(
                f"Successfully created reading log {log_id} for user {reading_log.user_id}"
            )
            return created_log
    ...
    def _update_existing_log(
        ...
        try:
            ...
            if result:
                ...
                if result_list:
                    node = (
                        result_list[0].get("result")
                        or result_list[0].get("col_0")
                        or next(iter(result_list[0].values()), None)
                    )
                    if node is not None:
                        return dict(node)
    ...
    def delete_reading_log_sync(self, log_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            verify_result = safe_execute_kuzu_query(
                verify_query, {"user_id": user_id, "log_id": log_id}
            )
    ...
    def get_user_reading_stats_sync(
        ...
        try:
            ...
            result = safe_execute_kuzu_query(
                stats_query, {"user_id": user_id, "cutoff_date": cutoff_date}
            )
            ...
            if result_list:
                ...
                if stats["days_read"] > 0:
                    stats["avg_pages_per_day"] = (
                        stats["total_pages"] / stats["days_read"]
                    )
                    stats["avg_minutes_per_day"] = (
                        stats["total_minutes"] / stats["days_read"]
                    )
                else:
    ...
    def get_user_all_time_reading_stats_sync(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logs_result = safe_execute_kuzu_query(all_logs_query, {"user_id": user_id})
            ...
            if logs_list:
                ...
                for result in logs_list:
                    raw_log = (
                        result.get("col_0") or result.get("result") or result.get("rl")
                    )
                    if not raw_log:
                        continue
                    try:
            ...
            logger.error(
                f"Error getting all-time reading stats for user {user_id}: {e}"
            )
            return {
                "total_log_entries": 0,
                "total_pages": 0,
    ...
    def get_user_reading_logs_paginated_sync(
        ...
        try:
            ...
            logs_result = safe_execute_kuzu_query(
                logs_query, {"user_id": user_id, "offset": offset, "limit": per_page}
            )
            ...
            logger.error(
                f"Error getting paginated reading logs for user {user_id}: {e}"
            )
            return {
                "logs": [],
                "pagination": {
    ...
    def update_reading_log_sync(
        ...
        try:
            ...
            verify_result = safe_execute_kuzu_query(
                verify_query, {"user_id": reading_log.user_id, "log_id": log_id}
            )

            if not verify_result:
                logger.error(
                    f"Reading log {log_id} not found for user {reading_log.user_id}"
                )
                return None
        ...
        try:
            ...
            if result:
                ...
                if result_list:
                    ...
                    book_data = (
                        dict(result_list[0]["col_1"])
                        if result_list[0]["col_1"]
                        else None
                    )
