...
class KuzuPersonService:
    ...
    async def update_person(
        ...
        try:
            ...
            print(
                f"🔧 [PERSON_SERVICE] About to update person_id={person_id}, updates={updates}"
            )
            ...
            results = _convert_query_result_to_list(raw_result)
            print(
                f"🔧 [PERSON_SERVICE] Update query returned: {len(results) > 0} results"
            )

            if results:
                print("🔧 [PERSON_SERVICE] Success, fetching updated person data...")
                # Return updated person data
                updated_person = await self.get_person_by_id(person_id)
                print(
                    f"🔧 [PERSON_SERVICE] Fetched updated person: {updated_person is not None}"
                )
                return updated_person
            else:
                print("🔧 [PERSON_SERVICE] No results from update, returning None")
                return None

        except Exception as e:
            print(
                f"🔧 [PERSON_SERVICE] Exception in update_person: {type(e).__name__}: {str(e)}"
            )
            traceback.print_exc()
            return None
