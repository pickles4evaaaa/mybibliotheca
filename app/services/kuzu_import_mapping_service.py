...
class KuzuImportMappingService:
    ...
    def create_template_sync(
        ...
        try:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    "📋 [IMPORT_MAPPING] Step 1: Checking if template already exists"
                )
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    "📋 [IMPORT_MAPPING] Step 2: Executing existing template query"
                )
            existing_result = safe_execute_kuzu_query(
                existing_query, {"id": template.id}
            )
            existing_results = _convert_query_result_to_list(existing_result)
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"📋 [IMPORT_MAPPING] Step 3: Existing results: {existing_results}"
                )

            if existing_results and len(existing_results) > 0:
                if logger.isEnabledFor(logging.INFO):
                    logger.info(
                        f"📋 [IMPORT_MAPPING] Template already exists: {template.name}"
                    )
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    "📋 [IMPORT_MAPPING] Step 5: Template data before insertion:"
                )
                ...
                logger.debug(
                    f"📋 [IMPORT_MAPPING]   Description: {template.description}"
                )
                logger.debug(
                    f"📋 [IMPORT_MAPPING]   Source Type: {template.source_type}"
                )
                logger.debug(
                    f"📋 [IMPORT_MAPPING]   Sample Headers Type: {type(template.sample_headers)}, Length: {len(template.sample_headers) if template.sample_headers else 0}"
                )
                logger.debug(
                    f"📋 [IMPORT_MAPPING]   Field Mappings Type: {type(template.field_mappings)}, Keys: {list(template.field_mappings.keys()) if template.field_mappings else []}"
                )
            ...
            try:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"📋 [IMPORT_MAPPING] Serialized sample_headers successfully (length: {len(sample_headers_json)})"
                    )
            except Exception:
                sample_headers_json = "[]"
            ...
            try:
                ...
                print(
                    f"📋 [IMPORT_MAPPING] Serialized field_mappings successfully (length: {len(field_mappings_json)})"
                )
            except Exception:
                field_mappings_json = "{}"

            print("📋 [IMPORT_MAPPING] Step 7: Preparing query")
            ...
            print("📋 [IMPORT_MAPPING] Step 8: Preparing parameters")
            params = {
                "id": template.id,
            ...
            print("📋 [IMPORT_MAPPING] Step 9: About to execute query")
            print("📋 [IMPORT_MAPPING] Query prepared")
            print("📋 [IMPORT_MAPPING] Params prepared")

            print("📋 [IMPORT_MAPPING] Step 10: Executing query now...")
            query_result = safe_execute_kuzu_query(query, params)
            result = _convert_query_result_to_list(query_result)
            print("📋 [IMPORT_MAPPING] Step 11: Query executed successfully")
            print(f"📋 [IMPORT_MAPPING] Query result: {result}")

            if result and len(result) > 0:
                created_template_data = _first_column_payload(result[0])
                print(
                    f"📋 [IMPORT_MAPPING] Created template data: {created_template_data}"
                )
    ...
    def get_user_templates_sync(self, user_id: str) -> list[ImportMappingTemplate]:
        ...
        try:
            ...
            print(
                f"📋 [IMPORT_MAPPING] Found {len(templates)} templates for user {user_id}"
            )
            return templates
    ...
    def detect_template_sync(
        ...
        try:
            ...
            for template in templates:
                ...
                score = self._calculate_header_match_score(
                    headers, template.sample_headers
                )
            ...
            if best_match and best_score > 0.5:
                print(
                    f"📋 [IMPORT_MAPPING] Detected template: {best_match.name} (score: {best_score:.2f})"
                )
                return best_match

            print(
                f"📋 [IMPORT_MAPPING] No suitable template detected (best score: {best_score:.2f})"
            )
            return None
    ...
    def delete_template_sync(self, template_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            results = safe_execute_kuzu_query(
                check_query, {"template_id": template_id, "user_id": user_id}
            )
    ...
    def _dict_to_template(
        ...
        try:
            ...
            print(
                f"📋 [IMPORT_MAPPING] Raw timestamps - created_at: {created_at} ({type(created_at)}), updated_at: {updated_at} ({type(updated_at)}), last_used: {last_used} ({type(last_used)})"
            )
            ...
            print(
                f"📋 [IMPORT_MAPPING] Parsed timestamps - created_at: {created_at}, updated_at: {updated_at}, last_used: {last_used}"
            )
            ...
            print("📋 [IMPORT_MAPPING] Creating ImportMappingTemplate with data...")
