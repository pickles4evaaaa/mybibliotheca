...
class KuzuServiceFacade:
    ...
    def create_book_sync(self, domain_book: Book, user_id: str) -> Book:
        ...
        if book and book.id:
            ...
            _ = self.relationship_service.add_book_to_user_library_sync(
                user_id=user_id, book_id=book.id, reading_status=""
            )
            return book
    ...
    def update_book_sync(self, book_id: str, user_id: str, **kwargs) -> Optional[Book]:
        ...
        book_updates = {
            ...
            if k not in personal_relationship_fields
            and k not in custom_metadata_fields
            and k not in location_fields
            and k not in contributor_fields
            and k not in category_fields
        }
        personal_updates = {
            k: v for k, v in kwargs.items() if k in personal_relationship_fields
        }
        custom_metadata_updates = {
            k: v for k, v in kwargs.items() if k in custom_metadata_fields
        }
        location_updates = {k: v for k, v in kwargs.items() if k in location_fields}
        contributor_updates = {
            k: v for k, v in kwargs.items() if k in contributor_fields
        }
        ...
        if personal_updates:
            ...
            try:
                existing_meta = personal_metadata_service.get_personal_metadata(
                    user_id, book_id
                )
            except Exception:
                existing_meta = {}
            ...
            status_norm = (
                _norm_status(custom_updates["reading_status"])
                if status_explicit
                else existing_status_norm
            )

            user_set_start = "start_date" in personal_kwargs
            user_cleared_start = (
                custom_updates.get("start_date", object()) is None
                if "start_date" in custom_updates
                else False
            )
            user_set_finish = "finish_date" in personal_kwargs
            user_cleared_finish = (
                custom_updates.get("finish_date", object()) is None
                if "finish_date" in custom_updates
                else False
            )
            ...
            if status_norm == "read":
                if (
                    not user_set_finish
                    and not user_cleared_finish
                    and existing_finish is None
                ):
                    personal_kwargs["finish_date"] = now_utc()
                if (
                    not user_set_start
                    and not user_cleared_start
                    and existing_start is None
                ):
                    personal_kwargs["start_date"] = (
                        personal_kwargs.get("start_date") or now_utc()
                    )
            else:
                if (
                    not user_set_finish
                    and not user_cleared_finish
                    and existing_finish is not None
                ):
                    custom_updates["finish_date"] = None
            ...
            try:
                personal_metadata_service.update_personal_metadata(
                    user_id, book_id, **personal_kwargs
                )
            except Exception as e:
        ...
        if contributor_updates and "contributors" in contributor_updates:
            try:
                success = run_async(
                    self._update_contributors_async(
                        book_id, contributor_updates["contributors"]
                    )
                )
                if success:
                    pass  # Success logged elsewhere
        ...
        if category_updates and "raw_categories" in category_updates:
            try:
                success = run_async(
                    self._update_categories_async(
                        book_id, category_updates["raw_categories"]
                    )
                )
                if success:
                    pass  # Success logged elsewhere
        ...
        if updated_book:
            ...
            personal_updates
            or custom_metadata_updates
            or location_updates
            or contributor_updates
            or category_updates
        ):
    ...
    def delete_book_sync(self, book_id: str, user_id: str) -> bool:
        ...
        try:
            self.relationship_service.remove_book_from_user_library_sync(
                user_id, book_id
            )
            ...
            if self.book_service.delete_book_sync(book_id):
                ...
                try:
                    safe_execute_kuzu_query(
                        "MATCH ()-[o:OWNS]->(b:Book {id: $book_id}) DELETE o",
                        {"book_id": book_id},
                    )
                except Exception:
                    pass
    ...
    async def update_book_relationships(
        self, user_id: str, book_id: str, **kwargs
    ) -> bool:
        """Update relationship-specific fields for a user's book."""
        return run_async(
            self.relationship_service.update_user_book_relationship_sync(
        ...
        try:
            ...
            if contributors:
                for i, contribution in enumerate(contributors):
                    await self.book_repo._create_contributor_relationship(
    ...
    async def _update_categories_async(self, book_id: str, raw_categories: Any) -> bool:
        ...
        try:
            ...
            if raw_categories:
                await self.book_repo._create_category_relationships_from_raw(
