...
class SafeImportJobManager:
    ...
    def update_job(self, user_id: str, task_id: str, updates: dict) -> bool:
        ...
        with self._get_user_lock(user_id):
            ...
            logger.debug(
                f"Updated job {task_id} for user {user_id}: {list(updates.keys())}"
            )
    ...
    def delete_job(self, user_id: str, task_id: str) -> bool:
        ...
        with self._get_user_lock(user_id):
            ...
            logger.warning(
                f"Job {task_id} not found for user {user_id} (cannot delete)"
            )
            return False
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._global_lock:
            ...
            return {
                ...
                "jobs_by_user": {
                    user_id: len(jobs) for user_id, jobs in self._jobs_by_user.items()
                },
                "operation_stats": self._stats.copy(),
                "memory_usage_estimate_kb": total_jobs
                * 2,  # Rough estimate: 2KB per job
            }
    ...
    def get_jobs_for_admin_debug(
        ...
        with self._global_lock:
            if include_user_data:
                ...
                return {
                    "total_jobs": sum(
                        len(jobs) for jobs in self._jobs_by_user.values()
                    ),
                ...
                return {
                    ...
                    "statistics": {
                        ...
                        "uptime_hours": round(
                            (time.time() - self._creation_time) / 3600, 2
                        ),
                    },
                }
