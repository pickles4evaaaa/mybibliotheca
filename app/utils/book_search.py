...
def _purge_search_cache_locked() -> None:
    ...
    if _SEARCH_CACHE_TTL > 0:
        expired = [
            cache_key
            for cache_key, (ts, _) in _SEARCH_CACHE.items()
        ...
        candidates.append(
            {
                ...
                "size": label,
                "url": f"{base}-{suffix}.jpg?default=false",
            }
        )
    return candidates
...
def select_best_publication_date(
    ...
    date1_is_full = bool(
        re.search(
            r"\d{1,2}[-/]\d{1,2}[-/]\d{4}|\d{4}-\d{1,2}-\d{1,2}|[A-Za-z]{3,}", date1
        )
    )
    date2_is_full = bool(
        re.search(
            r"\d{1,2}[-/]\d{1,2}[-/]\d{4}|\d{4}-\d{1,2}-\d{1,2}|[A-Za-z]{3,}", date2
        )
    )
...
def search_google_books(
    ...
    try:
        response = requests.get(
            url, timeout=(_GOOGLE_CONNECT_TIMEOUT, _GOOGLE_READ_TIMEOUT)
        )
        ...
        for item in items:
            try:
                ...
                for identifier in industry_identifiers:
                    ...
                    if identifier_type == "ISBN_10":
                        ...
                        asin_match = re.search(
                            r"[A-Z0-9]{10}", str(identifier_value).upper()
                        )
                ...
                try:
                    ...
                    raw_cover = select_highest_google_image(
                        volume_info.get("imageLinks", {})
                    )
                    if raw_cover:
                        cover_url = upgrade_google_cover_url(
                            raw_cover, allow_probe=False
                        )
                ...
                if author and authors:
                    try:
                        ...
                        if sa and any(sa == rn for rn in result_names):
                            ...
                            if last and any(
                                last in rn.split(" ")[-1] for rn in result_names
                            ):
                                author_bonus = 0.08
                    except Exception:
                        author_bonus = 0.0
...
def search_openlibrary(
    ...
    try:
        response = requests.get(
            url, timeout=(_OPENLIBRARY_CONNECT_TIMEOUT, _OPENLIBRARY_READ_TIMEOUT)
        )
        response.raise_for_status()
        data = response.json()
        ...
        for doc in docs:
            try:
                ...
                publication_year = first_publish_year or (
                    publish_year[0] if publish_year else None
                )
...
def merge_and_rank_results(
    ...
    """
    print(
        f"🔀 [MERGE_RANK] Merging {len(google_results)} Google + {len(openlibrary_results)} OpenLibrary results"
    )
    ...
    for result in google_results:
        isbn_key = (
            result.get("isbn_13")
            or result.get("isbn_10")
            or f"google_{result.get('google_books_id')}"
        )
        if isbn_key:
            record = result.copy()
            if result.get("cover_candidates"):
                record["cover_candidates"] = [
                    c.copy() for c in result["cover_candidates"] if isinstance(c, dict)
                ]
    ...
    for result in openlibrary_results:
        isbn_key = (
            result.get("isbn_13")
            or result.get("isbn_10")
            or f"openlibrary_{result.get('openlibrary_id')}"
        )
        ...
        if isbn_key in merged_results:
            ...
            for key, value in result.items():
                if key not in ["published_date", "publication_year"] and (
                    key not in existing or not existing[key]
                ):
                    existing[key] = value
            ...
            existing["similarity_score"] = max(
                existing.get("similarity_score", 0), result.get("similarity_score", 0)
            )
    ...
    for i, result in enumerate(final_results[:5]):  # Log top 5
        print(
            f"  {i + 1}. '{result.get('title', '')}' (score: {result.get('similarity_score', 0):.3f}, source: {result.get('source', '')})"
        )
...
def search_books_by_title(
    ...
    """
    print(
        f"🔍 [BOOK_SEARCH] Starting search for: '{title}'"
        + (f" by '{author}'" if author else "")
    )

    if not title or not title.strip():
        print("❌ [BOOK_SEARCH] Empty title provided")
        return []
    ...
    if remaining_budget <= 0:
        print(
            "⏱️ [BOOK_SEARCH] Skipping OpenLibrary results due to global timeout budget"
        )
    else:
        try:
    ...
    final_results = merge_and_rank_results(
        title, google_results, openlibrary_results, max_results
    )
    _search_cache_set(cache_key, final_results)

    print(
        f"🎯 [BOOK_SEARCH] Search complete. Returning {len(final_results)} results for '{title}'"
        + (f" by '{author}'" if author else "")
    )
...
def search_books_with_display_fields(
    title: str,
    max_results: int = 10,
    ...
    """
    results = search_books_by_title(
        title, max_results, author
    )  # Pass author to search_books_by_title

    # Filter for ISBN if required
    if isbn_required:
        results = [
            book for book in results if book.get("isbn_10") or book.get("isbn_13")
        ]
    ...
    for result in results:
        ...
        cover_url = result.get("cover_url") or (
            cover_candidates[0].get("url") if cover_candidates else None
        )
        if not cover_url:
            try:
                ...
                cand = cover_service.select_candidate(
                    ...
                    author=(result.get("authors") or [None])[0]
                ...
                if cand:
                    ...
                    existing_candidates = [
                        c.copy() for c in cover_candidates if isinstance(c, dict)
                    ]
                    if isinstance(cand, dict):
                        if not any(
                            c.get("url") == cand.get("url") for c in existing_candidates
                        ):
                            existing_candidates.append(cand.copy())
                    if existing_candidates:
