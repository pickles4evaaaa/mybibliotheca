...
def _best_cache_set(cache_key: str, value: dict) -> None:
    ...
    _purge_ordered_dict(
        _BEST_COVER_CACHE, _BEST_CACHE_TTL_SECONDS, _BEST_CACHE_MAX_ENTRIES
    )
...
def _candidate_cache_set(
    ...
    if _CANDIDATE_CACHE_MAX_ENTRIES <= 0:
        return
    key = _candidate_cache_key(isbn, title, author)
    _COVER_CANDIDATE_CACHE[key] = (
        _time.time(),
        [candidate.copy() for candidate in candidates],
    )
    _purge_ordered_dict(
        _COVER_CANDIDATE_CACHE,
        _CANDIDATE_CACHE_TTL_SECONDS,
        _CANDIDATE_CACHE_MAX_ENTRIES,
    )
...
def upgrade_google_cover_url(
    ...
    attempt_zoom0 = (
        ("zoom=0" not in url) and ("extraLarge" not in url) and ("large" not in url)
    )
    chosen = url
    if attempt_zoom0 and allow_probe:
        ...
        try:
            ...
            if size_z0 > size_curr * 1.15 or (
                size_curr < 35_000 and size_z0 > size_curr
            ):
                chosen = z0
                if _COVER_VERBOSE:
                    try:
                        current_app.logger.info(
                            f"[COVER][UPGRADE] zoom0 adopted curr={size_curr} z0={size_z0} url={chosen}"
                        )
                    except Exception:
                        pass
            else:
                if _COVER_VERBOSE:
                    try:
                        current_app.logger.debug(
                            f"[COVER][UPGRADE] zoom0 skipped curr={size_curr} z0={size_z0}"
                        )
                    except Exception:
                        pass
        except Exception:
...
def get_best_cover_for_book(isbn=None, title=None, author=None):
    ...
    if isbn:
        ...
        phase_logs.append(
            f"google_isbn={time.perf_counter() - t_gb_start:.3f}s has={'yes' if gb_data else 'no'}"
        )
    if not gb_data and title:
        t_gb_title = time.perf_counter()
        gb_results = search_google_books_by_title_author(title, author, limit=1)
        phase_logs.append(
            f"google_title={time.perf_counter() - t_gb_title:.3f}s res={len(gb_results) if gb_results else 0}"
        )
    ...
    if gb_data and gb_data.get("cover_url"):
        ...
        if cover_url and "books.google" in cover_url:
            ...
            if "zoom=" not in cover_url and not any(
                x in cover_url
                for x in ("extraLarge", "large", "medium", "small", "thumbnail")
            ):
        ...
        if cover_url and (
            "extraLarge" in cover_url or "zoom=1" in cover_url or "large" in cover_url
        ):
            quality = "high"
        else:
            quality = "medium"
    ...
    if not cover_url or quality == "none":
        ...
        if isbn:
            ...
            try:
                ...
                ol_data = None
            phase_logs.append(
                f"openlibrary_isbn={time.perf_counter() - t_ol_start:.3f}s has={'yes' if ol_data else 'no'}"
            )
        if not ol_data and title:
            ...
            count = (
                ...
                else (1 if ol_results else 0)
            )
            phase_logs.append(
                f"openlibrary_title={time.perf_counter() - t_ol_title:.3f}s res={count}"
            )
            ol_data = (
                ol_results
                if isinstance(ol_results, dict)
                else (ol_results[0] if ol_results else None)
            )
        ...
        source = "Google Books"
    # Final fallback: None
    result_obj = {"cover_url": cover_url, "source": source, "quality": quality}
    ...
    try:
        ...
        if _COVER_VERBOSE:
            current_app.logger.info(
                f"[COVER][SELECT] total={total:.3f}s isbn={isbn} title='{(title or '')[:40]}' source={result_obj['source']} quality={result_obj['quality']} steps={' | '.join(phase_logs)}"
            )
    except Exception:
        pass
    return result_obj
...
def get_cover_candidates(isbn=None, title=None, author=None):
    ...
    def _expand_google_variants(url: str):
        ...
        for z in zoom_levels:
            sep = (
                "&" if ("?" in base_no_edge and not base_no_edge.endswith("?")) else "?"
            )
            ...
            if variant not in seen:
                variants.append(
                    {"provider": "google", "size": f"zoom{z}", "url": variant}
                )
                seen.add(variant)
    ...
    try:
        ...
        if isinstance(ol_data, dict) and ol_data.get("cover_url"):
            candidates.append(
                {"provider": "openlibrary", "size": "L", "url": ol_data["cover_url"]}
            )
    except Exception:
        pass
...
_IMPORT_VERBOSE = (os.getenv("VERBOSE") or "false").lower() == "true" or (
    os.getenv("IMPORT_VERBOSE") or "false"
).lower() == "true"
        ...
        if k in ("title", "subtitle", "description"):
            ...
            if isinstance(v, str) and isinstance(ov, str):
                ...
                def _score(u: str):
                    ...
                    s += (
                        len(u) // 50
                    )  # slight preference for longer parameterized URLs (often higher res hints)
                    return s
...
def search_author_by_name(author_name):
    ...
    try:
        ...
        for i, doc in enumerate(docs):
            ...
            key = doc.get("key", "")
            author_id = (
                key.replace("/authors/", "") if key.startswith("/authors/") else key
            )
            ...
            if name.lower().strip() == author_name.lower().strip():
                ...
                author_name.lower() in name.lower()
                or name.lower() in author_name.lower()
            ):
        ...
        if exact_matches:
            for match in exact_matches:
                print(
                    f"  - {match['name']} ({match['author_id']}): {match['score']} points"
                )
        ...
        if book_key in data:
            ...
            for source in desc_sources:
                if source in book:
                    ...
                    print(
                        f"📖 [OPENLIBRARY] Found {source}: {type(desc_data)} - {str(desc_data)[:100] if desc_data else 'None'}..."
                    )
                    ...
                    if isinstance(desc_data, dict):
                        if "value" in desc_data:
                            ...
                            print(
                                f"📖 [OPENLIBRARY] Using description from {source}.value"
                            )
                            break
                        elif "text" in desc_data:
                            description = desc_data["text"]
                            print(
                                f"📖 [OPENLIBRARY] Using description from {source}.text"
                            )
                            break
            ...
            if not description:
                print("📖 [OPENLIBRARY] No description found in any source")
            else:
            ...
            for subject in subjects[:15]:  # Limit to 15 most relevant categories
                ...
                if (
                    category_name
                    and len(category_name) > 2
                    and category_name not in exclude_categories
                    and not category_name.startswith("places--")
                    and not category_name.startswith("people--")
                ):
                    # Capitalize properly
...
def fetch_author_data(author_id):
    ...
    if not author_id:
        print("[OPENLIBRARY] No author ID provided")
        return None
    ...
    try:
        ...
        print(
            f"[OPENLIBRARY] Error fetching author data from OpenLibrary for ID {author_id}: {e}"
        )
        return None
    except Exception as e:
        print(
            f"[OPENLIBRARY] Unexpected error processing OpenLibrary author data for ID {author_id}: {e}"
        )
        return None
...
def get_google_books_cover(isbn, fetch_title_author=False):
    ...
    if not isbn:
        print("❌ [GOOGLE_BOOKS] No ISBN provided")
        return None
    ...
    try:
        ...
        if "items" in data and len(data["items"]) > 0:
            ...
            print(
                f"✅ [GOOGLE_BOOKS] Successfully retrieved data for ISBN {isbn}: {list(result.keys())}"
            )
...
def generate_month_review_image(books, month, year):
    ...
    total_pages = sum(
        book.get("page_count", 0) for book in books if book.get("page_count")
    )
...
def search_multiple_books_by_title_author(title, author=None, limit=10):
    ...
    if not title:
        print("[MULTI_API] No title provided for book search")
        return []
    ...
    for result in all_results:
        ...
        for seen_title in seen_titles:
            ...
            if (result_title in seen_title or seen_title in result_title) and len(
                result_title
            ) > 3:
                is_duplicate = True
                break
    ...
    print(
        f"[MULTI_API] Returning {len(final_results)} unique results from {len(all_results)} total results"
    )
...
def _search_openlibrary_multiple(title, author=None, limit=10):
    ...
    if not title:
        print("[OPENLIBRARY] No title provided for book search")
        return []
    ...
    print(
        f"[OPENLIBRARY] Searching for multiple books: title='{title}', author='{author}' at {url}"
    )

    try:
        response = requests.get(url, timeout=15)
        print(
            f"[OPENLIBRARY] Multiple book search response status: {response.status_code}"
        )
        ...
        for i, doc in enumerate(docs):
            ...
            print(
                f"[OPENLIBRARY] Result {i}: title='{doc_title}', authors={doc_authors}, isbn={best_isbn}"
            )
            ...
            if author:
                for doc_author in doc_authors:
                    ...
                    if (
                        doc_author
                        and author.lower().strip() == doc_author.lower().strip()
                    ):
            ...
            if doc.get("first_publish_year"):
                try:
                    ...
                    if year > 1950:  # Reasonable cutoff
                        score += min(
                            (year - 1950) // 10, 5
                        )  # Up to 5 bonus points for newer books
                except:
                    pass
        ...
        print(
            f"[OPENLIBRARY] Failed to search for multiple books '{title}' by '{author}': {e}"
        )
        return []
...
def search_book_by_title_author(title, author=None):
    ...
    print(
        f"[OPENLIBRARY] Searching for book: title='{title}', author='{author}' at {url}"
    )
    ...
    try:
        ...
        if scored_matches:
            ...
            print(
                f"[OPENLIBRARY] Best match: '{best_match['title']}' by {best_match['authors']} (score: {best_match['score']}, ISBN: {best_match['isbn']})"
            )

            # If we have an ISBN, fetch full book data using existing function
            if best_match["isbn"]:
                print(
                    f"[OPENLIBRARY] Fetching full data using ISBN: {best_match['isbn']}"
                )
            ...
            result = {
                ...
                "author": ", ".join(best_match["authors"])
...
def search_google_books_by_title_author(title, author=None, limit=10):
    ...
    if not title:
        if _VERBOSE:
            print("[GOOGLE_BOOKS] No title provided for book search")
        return []
    ...
    if _VERBOSE:
        print(
            f"[GOOGLE_BOOKS] Searching for multiple books: title='{title}', author='{author}' at {url}"
        )
    ...
    try:
        ...
        if _VERBOSE:
            print(
                f"[GOOGLE_BOOKS] Multiple book search response status: {response.status_code}"
            )

        if response.status_code != 200:
            if _VERBOSE:
                print(
                    f"[GOOGLE_BOOKS] Google Books API returned {response.status_code}"
                )
            return []
        ...
        if not items:
            if _VERBOSE:
                print(
                    f"[GOOGLE_BOOKS] No search results found for '{title}' by '{author}'"
                )
            return []
        ...
        for i, item in enumerate(items):
            try:
                ...
                if _VERBOSE:
                    print(
                        f"[GOOGLE_BOOKS] Result {i}: title='{book_title}', authors={book_authors}, isbn={isbn13 or isbn10}"
                    )
        ...
        if _VERBOSE:
            print(
                f"[GOOGLE_BOOKS] Failed to search for book '{title}' by '{author}': {e}"
            )
        return []
