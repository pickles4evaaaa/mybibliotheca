...
def _normalize_first_last(name: str) -> str:
    ...
    This does not attempt language-specific surname particles;
    it follows existing project expectations (simple, predictable behavior).
    """
    raw = (name or "").strip()
    if not raw:
        return ""
...
def author_first_sort_key_for_book(book: Any) -> Tuple[str, str]:
    ...
    Uses Person.normalized_name when available to normalize "Last, First" to "First Last".
...
def author_last_sort_key_for_book(book: Any) -> Tuple[str, str, str]:
    ...
    Returns (last, first, title) where last/first are derived from a normalized "first last" representation.
    """
    person = _primary_author_person(book)
    if person:
        normalized = _person_normalized_name(person)
