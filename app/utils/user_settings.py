...
_DEFAULT_READING_STATUS_ORDER: Tuple[Tuple[str, str], ...] = (
    ...
    (
        ...
        _READING_STATUS_LABELS[ReadingStatus.PLAN_TO_READ.value],
    ),
    ...
    (
        ReadingStatus.LIBRARY_ONLY.value,
        _READING_STATUS_LABELS[ReadingStatus.LIBRARY_ONLY.value],
    ),
)
...
_LIBRARY_STATUS_OPTIONS = {key for key, _ in _LIBRARY_STATUS_ORDER}.union(
    {"currently_reading"}
)
...
def load_user_settings(user_id: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            status_token = normalize_library_status_filter(
                data.get("library_default_status")
            )
...
def get_library_view_defaults(user_id: Optional[str]) -> Tuple[str, str]:
    ...
    status = (
        normalize_library_status_filter(settings.get("library_default_status")) or "all"
    )
    sort = (
        normalize_library_sort_option(settings.get("library_default_sort"))
        or "title_asc"
    )
    return status, sort
