    ...
    try:
        ...
        try:
            ...
            current_app.logger.debug(
                f"Falling back to detailed logs for streak calculation (user={user_id}): {date_exc}"
            )
            raw_dates = []
        ...
        if not reading_dates:
            logs = reading_log_service.get_user_reading_logs_sync(
                user_id, days_back=3650, limit=None
            )

            if not logs:
                current_app.logger.debug(
                    f"No reading logs found for user {user_id}, returning streak offset: {streak_offset}"
                )
        ...
        if not reading_dates:
            current_app.logger.debug(
                f"No valid reading dates found for user {user_id}, returning streak offset: {streak_offset}"
            )
            return streak_offset
        ...
        if (
            current_date in reading_dates
            or (current_date - timedelta(days=1)) in reading_dates
        ):
        ...
        current_app.logger.debug(
            f"Calculated reading streak for user {user_id}: {streak} days from logs + {streak_offset} offset = {total_streak}"
        )
        return total_streak
