...
# Logging controls
_QUERY_LOG_ENABLED = os.getenv("KUZU_QUERY_LOG", "false").lower() in (
    "1",
    "true",
    "on",
    "yes",
...
try:
    ...
    _SLOW_QUERY_MS = 150
_VERBOSE_INIT = os.getenv("MYBIBLIOTHECA_VERBOSE_INIT", "false").lower() in (
    "1",
    "true",
    "on",
    "yes",
) or os.getenv("KUZU_DEBUG", "false").lower() in ("1", "true", "on", "yes")
...
class SafeKuzuManager:
    ...
    def __init__(self, database_path: str | None = None):
        ...
        self._db_dir = (
            ...
            if Path(self.database_path).suffix
            else Path(self.database_path)
        )
    ...
    def _get_recovery_mode(self) -> str:
        ...
        if not mode:
            # Legacy mapping
            if os.getenv("KUZU_AUTO_RECOVER_CLEAR", "false").lower() in (
                "1",
                "true",
                "yes",
            ):
                mode = "CLEAR_REBUILD"
            elif os.getenv("KUZU_ENABLE_AUTO_RENAME_CORRUPT", "false").lower() in (
                "1",
                "true",
                "yes",
            ):
    ...
    def _consume_shutdown_marker(self) -> dict[str, Any]:
        ...
        """
        info = {"previous_clean_shutdown": False, "marker_age_seconds": None}
        try:
    ...
    def _start_integrity_probe_thread(self):
        ...
        def _loop():
            logger.info(
                f"[KUZU] Integrity probe thread started (interval={self._integrity_interval_sec}s)"
            )
        ...
        self._integrity_thread = threading.Thread(
            target=_loop, daemon=True, name="kuzu-integrity-probe"
        )
    ...
    def _run_integrity_probe(self):
        ...
        with self.get_connection(operation="integrity_probe") as conn:
            ...
            for label in core_nodes:
                try:
                    ...
                    if c < 0:
                        anomalies.append(
                            {"label": label, "count": c, "reason": "negative_count"}
                        )
                except Exception as e:
        ...
        if anomalies:
            ...
            self._log_corruption_event(
                {"type": "INTEGRITY_ANOMALY", "anomalies": anomalies, "totals": totals}
            )
        else:
        ...
        with self._lock:
            ...
            while self._connection_count > 0:
                ...
                if time.time() - wait_start > 30:
                    logger.warning(
                        "[KUZU] Quiesce wait exceeded 30s; proceeding anyway"
                    )
                    break
        ...
        if db_path.exists():
            try:
                if db_path.is_file():
                    ...
                    size_bytes = sum(
                        f.stat().st_size for f in db_path.glob("**/*") if f.is_file()
                    )
            except Exception:
                size_bytes = 0
        if size_bytes > 0 and suspected_offset > size_bytes * 2:
            logger.error(
                f"[KUZU_CORRUPTION_DETECTED] Out-of-range read offset {suspected_offset} (size={size_bytes})"
            )
            self._log_corruption_event(
                {
    ...
    def _initialize_database(self) -> None:
        ...
        try:
            ...
            if (
                hasattr(self, "_creator_pid")
                and self._creator_pid
                and self._creator_pid != current_pid
            ):
                ...
                try:
                    print(
                        f"[KUZU] ♻️ Detected fork; resetting manager state in pid {current_pid}"
                    )
                except Exception:
                    pass
        except Exception:
        ...
        if _VERBOSE_INIT:
            try:
                print(
                    f"[KUZU] ⏳ Initializing database at {self.database_path} (thread {thread_info['thread_id']})"
                )
            except Exception:
                pass
        logger.info(
            f"[THREAD-{thread_info['thread_id']}:{thread_info['thread_name']}] "
        ...
        try:
            ...
            try:
                ...
                is_io_corruption = (
                    "Cannot read from file" in err_str
                    or "bad_alloc" in err_str
                    or "Error during recovery" in err_str
                )
                recovery_mode = self._get_recovery_mode()
                if is_io_corruption:
                    ...
                    if recovery_mode == "FAIL_FAST":
                        logger.error(
                            "[KUZU] ❌ Corruption suspected (mode=FAIL_FAST). Aborting startup. Error: %s",
                            err_str,
                        )
                        raise open_err
                    try:
                        ...
                        if db_path.exists():
                            ...
                            if db_path.is_dir():
                                ...
                                logger.warning(
                                    f"[KUZU] 📦 Copied suspected corrupt DB directory to {target}"
                                )
                            else:
                                target = backup_dir / f"bibliotheca_{ts}.db"
                                shutil.copy2(str(db_path), str(target))
                                logger.warning(
                                    f"[KUZU] 📦 Copied suspected corrupt DB file to {target}"
                                )
                        if recovery_mode == "SOFT_RENAME":
                            if db_path.exists() and db_path.is_file():
                                renamed = (
                                    db_path.parent / f"bibliotheca.db.corrupt.{ts}"
                                )
                                db_path.rename(renamed)
                                logger.warning(
                                    f"[KUZU] 📛 Soft-renamed corrupt DB to {renamed.name}"
                                )
                                ...
                                db_path.rename(renamed)
                                logger.warning(
                                    f"[KUZU] 📛 Soft-renamed corrupt DB directory to {renamed.name}"
                                )
                            ...
                            if db_path.exists():
                                ...
                                logger.warning(
                                    "[KUZU] 🧹 Cleared suspected corrupt DB (CLEAR_REBUILD)"
                                )
                        # Recreate fresh DB
                        self._database = kuzu.Database(self.database_path)
                        logger.warning(
                            f"[KUZU] ✅ Recreated database (mode={recovery_mode})"
                        )
                    except Exception as rec_err:
                        logger.error(
                            f"[KUZU] ❌ Recovery mode {recovery_mode} failed: {rec_err}"
                        )
                        raise open_err
                else:
            ...
            if not startup_marker_info.get("previous_clean_shutdown"):
                try:
                    ...
                    if _VERBOSE_INIT:
                        print(
                            "[KUZU] ⚠️ Previous run ended uncleanly (no clean shutdown marker) – continuing with caution"
                        )
                except Exception:
                    pass
    ...
    def get_connection(
        ...
        with self._lock:
            ...
            while self._writes_quiesced:
                ...
                logger.debug(
                    f"[THREAD-{thread_info['thread_id']}:{thread_info['thread_name']}] Waiting for quiesce release (reason={self._pending_quiesce_reason})"
                )
                self._quiesce_condition.wait(timeout=1.0)
        ...
        try:
            ...
            is_schema_exists_error = "already exists" in error_str and (
                "customfield" in error_str
                or "has_personal_metadata" in error_str
                or "globalmetadata" in error_str
                or "has_global_metadata" in error_str
                or "catalog" in error_str
            )

            # Legacy OWNS relationship errors (expected when OWNS table doesn't exist)
            is_owns_legacy_error = (
                "owns does not exist" in error_str
                or "table owns does not exist" in error_str
            )
    ...
    def execute_query(
        ...
        def _looks_like_date_only_key(k: str) -> bool:
            ...
            return ("date" in k_lower) and not any(
                s in k_lower for s in ("time", "timestamp", "_at")
            )
    ...
    def cleanup_stale_connections(self, max_age_minutes: int = 30) -> int:
        ...
        with self._lock:
            ...
            for thread_id in stale_threads:
                logger.warning(
                    f"Cleaning up stale connection tracking for thread {thread_id}"
                )
    ...
    def force_reset(self) -> None:
        ...
        with self._lock:
            logger.warning(
                "Force resetting KuzuDB connection - this should only happen in tests or recovery!"
            )
    ...
    def _initialize_schema(self):
        ...
        try:
            ...
            logger.info(
                f"🔧 [DEBUG] Schema init - force_reset={force_reset}, debug_mode={debug_mode}"
            )
            ...
            if not force_reset:
                try:
                        ...
                        for table_name in essential_tables:
                            try:
                                ...
                                result = temp_conn.execute(
                                    f"MATCH (n:{table_name}) RETURN COUNT(n) as count LIMIT 1"
                                )
                                ...
                                if "does not exist" in str(table_e).lower():
                                    ...
                                    logger.debug(
                                        f"❌ Table {table_name} missing: {table_e}"
                                    )
                                else:
                                    # Some other error, assume table exists but has issues
                                    logger.debug(
                                        f"⚠️ Table {table_name} exists but has issues: {table_e}"
                                    )

                        if not missing_tables:
                            has_complete_schema = True
                            logger.info(
                                "✅ All essential tables exist - schema appears complete"
                            )
                            ...
                            try:
                                ...
                                if "Cannot find property updated_at" in str(rl_e):
                                    try:
                                        ...
                                        logger.info(
                                            "🛠️ Added missing updated_at column to ReadingLog table"
                                        )
                                    except Exception as alter_e:
                                        logger.debug(
                                            f"Could not add updated_at to ReadingLog: {alter_e}"
                                        )

                            # Log some stats for confirmation
                            try:
                                user_result = temp_conn.execute(
                                    "MATCH (u:User) RETURN COUNT(u) as count LIMIT 1"
                                )
                                if isinstance(user_result, list):
                                    user_result = (
                                        user_result[0] if user_result else None
                                    )
                                if user_result and user_result.has_next():
                                    try:
                                        ...
                                        user_count = 0
                                    logger.info(
                                        f"📊 Database contains {user_count} users"
                                    )
                            except Exception as e:
                                logger.debug(f"Could not get user count: {e}")
                        else:
                            logger.info(
                                f"❌ Missing essential tables: {missing_tables} - will create full schema"
                            )
                    ...
                    logger.debug(
                        f"Could not check schema completeness - will initialize schema: {e}"
                    )
            ...
            if _os_dep_owns_mgr.getenv("ENABLE_OWNS_SCHEMA", "false").lower() in (
                "1",
                "true",
                "yes",
            ):  # pragma: no cover
...
def is_safe_kuzu_initialized() -> bool:
    ...
    global _safe_kuzu_manager
    return bool(
        _safe_kuzu_manager and getattr(_safe_kuzu_manager, "_is_initialized", False)
    )
