...
class BackwardCompatibleKuzuDB:
    ...
    def execute_query(self, query: str, params: Optional[Dict[str, Any]] = None):
        ...
        return safe_execute_query(
            query=query, params=params, user_id=self.user_id, operation="legacy_query"
        )
...
class BackwardCompatibleKuzuStorage:
    ...
    def get_connection(self):
        ...
        with safe_get_connection(
            user_id=self.user_id, operation="legacy_storage_connection"
        ) as conn:
            yield conn
...
def get_kuzu_database_safe(user_id: Optional[str] = None) -> BackwardCompatibleKuzuDB:
    ...
    deprecated_warning(
        "get_kuzu_database()", "safe_execute_query() or safe_get_connection()"
    )
...
def get_graph_storage_safe(
    ...
    deprecated_warning(
        "get_graph_storage()", "safe_execute_query() or safe_get_connection()"
    )
...
def with_safe_kuzu(operation_name: str = "unknown"):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            logger.debug(
                f"Executing safe KuzuDB operation: {operation_name} (user: {user_id})"
            )
...
def check_migration_status() -> Dict[str, Any]:
    ...
    return {
        "safe_manager_status": {
            ...
            "total_connections": health["connection_metrics"][
...
def log_migration_status():
    ...
    logger.info(
        f"  Active Connections: {status['safe_manager_status']['active_connections']}"
    )
    logger.info(
        f"  Total Connections Created: {status['safe_manager_status']['total_connections']}"
    )
    logger.info(
        f"  Legacy Database Instance: {status['legacy_compatibility']['database_instance_created']}"
    )
    logger.info(
        f"  Legacy Storage Instance: {status['legacy_compatibility']['storage_instance_created']}"
    )
...
def force_reset_all_connections():
    ...
    logger.warning(
        "🚨 Force resetting all KuzuDB connections - this should only happen in tests!"
    )
