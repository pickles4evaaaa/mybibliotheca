...
class SQLiteMigrationService:
    ...
    def migrate_sqlite_database(
        ...
        """
        print("🔄 [SQLITE_MIGRATION] Starting SQLite migration")
        ...
        if db_version == "v1":
            ...
            return self._migrate_v1_5_database(
                sqlite_file_path, target_user_id, create_default_user
            )
        else:
    ...
    def _migrate_v1_database(
        ...
        try:
            ...
            print(
                f"📋 [V1_MIGRATION] Found {len(reading_log_rows)} reading log entries"
            )
            ...
            for row in book_rows:
                simplified_book = self._convert_v1_row_to_simplified_book(
                    dict(row), reading_logs_by_book
                )
        ...
        return self._process_books_with_batch_pipeline(
            all_books_data, all_isbns, all_authors, target_user_id
        )

    def _migrate_v1_5_database(
        self,
        sqlite_file_path: str,
        target_user_id: str,
        create_default_user: bool = False,
        ...
        try:
            ...
            cursor.execute(
                "SELECT * FROM user WHERE is_admin = 1 ORDER BY created_at ASC LIMIT 1"
            )
            admin_user = cursor.fetchone()

            if admin_user and create_default_user:
                print(
                    f"📋 [V1.5_MIGRATION] Found admin user: {admin_user['username']} (ID: {admin_user['id']})"
                )
            ...
            if sqlite_user_id:
                cursor.execute(
                    "SELECT * FROM book WHERE user_id = ?", (sqlite_user_id,)
                )
            else:
                cursor.execute("SELECT * FROM book")
            ...
            reading_log_rows = cursor.fetchall()
            print(
                f"📋 [V1.5_MIGRATION] Found {len(reading_log_rows)} reading log entries"
            )
            ...
            for row in book_rows:
                simplified_book = self._convert_v1_5_row_to_simplified_book(
                    dict(row), reading_logs_by_book
                )
        ...
        # Use batch processing pipeline
        result = self._process_books_with_batch_pipeline(
            all_books_data, all_isbns, all_authors, target_user_id
        )
    ...
    def _process_books_with_batch_pipeline(
        ...
        print(
            f"📚 [BATCH_MIGRATION] Batch fetching book metadata for {len(all_isbns)} ISBNs..."
        )
        ...
        print(
            f"👥 [BATCH_MIGRATION] Using {len(all_authors)} authors from SQLite data..."
        )
        ...
        for book_data in all_books_data:
            try:
                ...
                domain_book = self._convert_simplified_book_to_domain(
                    book_data, book_api_data, author_api_data
                )
                ...
                if book_data.isbn13:
                    existing_book = book_service.get_book_by_isbn_for_user_sync(
                        book_data.isbn13, target_user_id
                    )
                if not existing_book and book_data.isbn10:
                    existing_book = book_service.get_book_by_isbn_for_user_sync(
                        book_data.isbn10, target_user_id
                    )
                ...
                if existing_book:
                    ...
                    created_book = book_service.create_book_sync(
                        domain_book, target_user_id
                    )

                if created_book:
                    # Get book ID (handle both Book objects and book IDs)
                    book_id = (
                        getattr(created_book, "id", None)
                        if hasattr(created_book, "id")
                        else str(created_book)
                    )

                    if book_id:
                        # Update with personal information from SQLite
                        self._update_personal_information(
                            user_book_repo, target_user_id, book_id, book_data
                        )
                ...
                error_count += 1
                migration_details.append(
                    {"title": book_data.title, "status": "error", "error": str(e)}
                )
    ...
    def _convert_simplified_book_to_domain(
        self, book_data: SimplifiedBook, book_api_data: dict, author_api_data: dict
    ):
    ...
    def _update_personal_information(
        self, user_book_repo, user_id: str, book_id: str, book_data: SimplifiedBook
    ):
        ...
        try:
            ...
            if hasattr(book_data, "reading_logs") and book_data.reading_logs:
                log_dates = [
                    str(log_date) for log_date in book_data.reading_logs[:5]
                ]  # First 5 dates
            ...
            run_async(
                user_book_repo.update_reading_status(
            ...
            try:
                ...
                personal_metadata_service.update_personal_metadata(
                    ...
                    custom_updates=custom_updates or None,
                    merge=True,
                )
            except Exception as _pm_err:  # pragma: no cover
                print(
                    f"⚠️ [MIGRATION] Personal metadata update fallback failed: {_pm_err}"
                )
        ...
        if isinstance(categories_str, str):
            ...
            for delimiter in [";", ",", "|", "\n"]:
                if delimiter in categories_str:
                    categories = [
                        cat.strip() for cat in categories_str.split(delimiter)
                    ]
