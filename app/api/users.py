...
def get_current_user():
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "Error retrieving user profile"}
        ), 500
    ...
    try:
        ...
        if missing_attrs:
            current_app.logger.warning(
                f"User object missing attributes: {missing_attrs}"
            )
        ...
        try:
            ...
            try:
                if (
                    hasattr(current_user, "share_current_reading")
                    and "share_current_reading" in data
                ):
                    current_user.share_current_reading = bool(
                        data["share_current_reading"]
                    )
                if (
                    hasattr(current_user, "share_reading_activity")
                    and "share_reading_activity" in data
                ):
                    current_user.share_reading_activity = bool(
                        data["share_reading_activity"]
                    )
                ...
                current_app.logger.error(
                    f"Fallback user update also failed: {fallback_error}"
                )
                return jsonify(
                    {"status": "error", "message": "Failed to update user settings"}
                ), 500
...
def get_user_library(user_id):
    ...
    try:
        ...
        if not getattr(user, "share_library", False) and user.id != current_user.id:
            return jsonify(
                {"status": "error", "message": "User library is private"}
            ), 403
        ...
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "Error retrieving user library"}
        ), 500
...
def get_users():
    ...
    try:
        ...
        users_data = [
            serialize_user_profile(user, include_private=False) for user in users
        ]

        return jsonify(
            {"status": "success", "data": users_data, "count": len(users_data)}
        ), 200
