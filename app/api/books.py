...
def serialize_book(domain_book):
    ...
    if hasattr(domain_book, "__dict__"):
        ...
        return {
            ...
            "authors": [author.name for author in getattr(domain_book, "authors", [])]
            if hasattr(domain_book, "authors")
            else [],
            "publisher": getattr(domain_book.publisher, "name", None)
            if getattr(domain_book, "publisher", None)
            else None,
        ...
        return {
            ...
            "authors": [
                ...
                for author in domain_book.get("authors", [])
            ],
            "publisher": domain_book.get("publisher", {}).get("name")
            if isinstance(domain_book.get("publisher"), dict)
            else domain_book.get("publisher"),
        ...
        if isinstance(data["authors"], list):
            for i, name in enumerate(data["authors"]):
                if name.strip():
                    ...
                    person = Publisher(
                        name=name.strip()
                    )  # Temporary use of Publisher as Person-like
...
def get_books():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "data": books_data, "count": len(books_data)}
        ), 200
        ...
        return jsonify(
            {"status": "error", "message": "Failed to retrieve books", "error": str(e)}
        ), 500
...
def get_book(book_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "data": serialize_book(domain_book)}), 200

    except Exception as e:
        current_app.logger.error(f"Error getting book {book_id}: {e}")
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "Failed to retrieve book", "error": str(e)}
        ), 500
...
def create_book():
    ...
    try:
        ...
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "Failed to create book", "error": str(e)}
        ), 500
...
def update_book(book_id):
    ...
    try:
        ...
        updated_book = kuzu_book_service.update_book_sync(
            book_id, current_user.id, **updates
        )

        if not updated_book:
            return jsonify(
                {"status": "error", "message": "Book not found or update failed"}
            ), 404
        ...
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "Failed to update book", "error": str(e)}
        ), 500
...
def delete_book(book_id):
    ...
    try:
        ...
        if not success:
            return jsonify(
                {"status": "error", "message": "Book not found or deletion failed"}
            ), 404
        ...
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "Failed to delete book", "error": str(e)}
        ), 500
...
def search_books():
    ...
    try:
        ...
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "Search failed", "error": str(e)}
        ), 500
...
def search_user_books():
    ...
    try:
        ...
        if not query or len(query) < 2:
            return jsonify({"status": "success", "books": [], "count": 0})
        ...
        return jsonify(
            {"status": "success", "books": books, "count": len(books), "query": query}
        )
...
def search_external_books():
    ...
    try:
        ...
        search_results = search_books_with_display_fields(
            title, limit, isbn_required=isbn_required, author=author or None
        )
        ...
        current_app.logger.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": "External search failed", "error": str(e)}
        ), 500
...
def unified_metadata_lookup():
    ...
    try:
        ...
        if not isbn and not title:
            return jsonify(
                {"status": "error", "message": "Provide either isbn or title"}
            ), 400
