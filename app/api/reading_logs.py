...
reading_logs_api = Blueprint(
    "reading_logs_api", __name__, url_prefix="/api/v1/reading-logs"
)
...
def serialize_reading_log(log):
    ...
    if isinstance(log, dict):
        ...
        return {
            ...
            "date": _iso(getattr(log, "date", None))
...
def create_reading_log():
    ...
    try:
        ...
        if not data.get("book_id"):
            return jsonify({"status": "error", "message": "book_id is required"}), 400
...
def check_existing_log():
    ...
    try:
        ...
        existing_log = reading_log_service.get_existing_log_sync(
            book_id=str(data["book_id"]), user_id=current_user.id, log_date=log_date
        )
...
def delete_reading_log(log_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": "Reading log deleted successfully"}
        ), 200
