...
def inject_site_config():
    ...
    try:
        ...
        site_name = system_config.get(
            "site_name", os.getenv("SITE_NAME", "MyBibliotheca")
        )
        server_timezone = system_config.get(
    ...
    return {
        ...
        "get_genre_term": lambda: "Genre"
        if terminology_preference == "genre"
        else "Category",
        "get_genre_term_lower": lambda: "genre"
        if terminology_preference == "genre"
        else "category",
        "get_genre_term_plural": lambda: "Genres"
        if terminology_preference == "genre"
        else "Categories",
        "get_genre_term_plural_lower": lambda: "genres"
        if terminology_preference == "genre"
        else "categories",
        # URL helper for genre routes
        "get_genre_url_prefix": lambda: "genres"
        if terminology_preference == "genre"
        else "categories",
        # Dynamic URL generator for genre routes
        "genre_url_for": lambda endpoint, **values: _genre_url_for(
            terminology_preference, endpoint, **values
        ),
    }
...
def inject_reading_streak():
    ...
    try:
        ...
        from app.utils.user_utils import (
            calculate_reading_streak as _calculate_reading_streak,
        )
        ...
        try:
            ...
            personal_best_streak = max(
                personal_best_streak, best_run + streak_offset, streak
            )
