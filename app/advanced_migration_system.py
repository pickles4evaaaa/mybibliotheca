...
class AdvancedMigrationSystem:
    ...
    def detect_database_version(self, db_path: Path) -> tuple[str, dict]:
        ...
        try:
            ...
            if "reading_log" in tables:
                ...
                analysis["has_user_id_in_reading_logs"] = (
                    "user_id" in reading_log_columns
                )
    ...
    def _backup_kuzu_data(self, backup_path: Path) -> None:
        ...
        backup_data = {"timestamp": datetime.now().isoformat(), "stats": {}}
        ...
        try:
            ...
            # Count users
            try:
                user_result = self.safe_manager.execute_query(
            ...
            # Count books
            try:
                book_result = self.safe_manager.execute_query(
            ...
            try:
                ...
                reading_result = self.safe_manager.execute_query(
                    "MATCH ()-[r:READS]->() RETURN COUNT(r) as count"
                )
                rows = (
                    _convert_query_result_to_list(reading_result)
                    if reading_result
                    else []
                )
                if rows:
    ...
    def create_first_admin_user(
        ...
        try:
            ...
            if admin_user and admin_user_id:
                ...
                self._log_action(
                    f"First admin user created successfully: {admin_user_id}"
                )
                ...
                logger.error("❌ Failed to create first admin user - no ID returned")
                self._log_error("Admin user creation failed - no ID returned")
                return None
    ...
    def migrate_v1_database(self, db_path: Path, admin_user_id: str) -> bool:
        ...
        try:
            ...
            if not default_location:
                self._log_action(
                    "No default location found. Creating default location..."
                )
                ...
                if created_locations:
                    ...
                    if default_location:
                        ...
                        self._log_action(
                            f"✅ Created and using default location: {loc_name} (ID: {default_location_id})"
                        )
                    else:
                ...
                default_location_id = getattr(default_location, "id", None)
                self._log_action(
                    f"Using existing default location: {loc_name} (ID: {default_location_id})"
                )
            ...
            for book_row in books:
                try:
                    ...
                    created_book = self.book_service.create_book_sync(domain_book=book)

                    # Add book to admin user's library with appropriate status
                    created_book_id = (
                        getattr(created_book, "id", None) if created_book else None
                    )
                    if created_book and created_book_id:
                        ...
                        if default_location_id:
                            try:
                                location_success = (
                                    self.location_service.add_book_to_location(
                                        created_book_id,
                                        default_location_id,
                                        admin_user_id,
                                    )
                                ...
                                if location_success:
                                    self._log_action(
                                        f"✅ Assigned book '{book.title}' to admin user's default location"
                                    )
                                    self.stats["books_migrated"] += 1
                                else:
                                    self._log_error(
                                        f"Failed to assign book '{book.title}' to admin user's default location"
                                    )
                            except Exception as e:
                                self._log_error(
                                    f"Error assigning book '{book.title}' to admin user's location: {e}"
                                )
                        else:
                            self._log_error(
                                f"No default location found for admin user when trying to assign book '{book.title}'"
                            )
                            self.stats["books_migrated"] += (
                                1  # Still count as migrated even if location assignment failed
                            )
                    ...
                    title = (
                        book_row["title"] if "title" in book_row.keys() else "Unknown"
                    )
                    ...
                    try:
                        ...
                        if old_book_id and old_book_id in book_id_mapping and log_date:
                            ...
                            reading_log = ReadingLog(
                                ...
                                date=log_date,
                                pages_read=safe_get(
                                    log_row, "pages_read", 1
                                ),  # Default 1 page if not in SQLite
                                minutes_read=safe_get(
                                    log_row, "minutes_read", 1
                                ),  # Default 1 minute if not in SQLite
                                notes=safe_get(
                                    log_row, "notes", ""
                                ),  # Empty notes if not in SQLite
                            )

                            # Create the reading log in Kuzu
                            created_log = reading_log_service.create_reading_log_sync(
                                reading_log
                            )
                            ...
                            if created_log:
                                ...
                                logger.debug(
                                    f"✅ Migrated reading log for book {new_book_id} on {log_date}"
                                )
                            else:
                                logger.warning(
                                    f"⚠️ Failed to create reading log for book {new_book_id} on {log_date}"
                                )
                        else:
                            logger.warning(
                                f"⚠️ Skipping reading log - missing data: book_id={old_book_id}, date={log_date}"
                            )
            ...
            logger.info(
                f"✅ V1 migration completed: {self.stats['books_migrated']} books migrated"
            )
    ...
    def migrate_v2_database(
        ...
        try:
            ...
            if not default_location:
                ...
                if created_locations:
                    ...
                    if default_location:
                        ...
                        loc_name = getattr(default_location, "name", "Unknown")
                        self._log_action(
                            f"✅ Created default location: {loc_name} (ID: {default_location_id})"
                        )
                    ...
                    try:
                        ...
                        new_user = self.user_service.create_user_sync(
                            ...
                            password_must_change=bool(
                                safe_get(user_row, "password_must_change", False)
                            ),
                        )
                        new_user_id = (
                            getattr(new_user, "id", None) if new_user else None
                        )
                        if new_user and new_user_id:
                            ...
                            self._log_error(
                                f"Failed to create user {user_row['username']} - no ID returned"
                            )

                    except Exception as e:
                        self._log_error(
                            f"Error migrating user {user_row['username']}: {e}"
                        )
                        continue
            ...
            created_books = (
                set()
            )  # Track books we've already created (avoid duplicates)
            ...
            for book_row in books:
                try:
                    ...
                    book_identifier = (
                        safe_get(book_row, "isbn")
                        or f"{safe_get(book_row, 'title', '')}|{safe_get(book_row, 'author', '')}"
                    )

                    if book_identifier not in created_books:
                        # Convert SQLite row to Book object with optional API enhancement
                        book = self._sqlite_row_to_book(
                            book_row, fetch_api_metadata=fetch_api_metadata
                        )
                        ...
                        if created_book and created_book_id:
                            ...
                            self._log_error(
                                f"Failed to create global book '{book.title}'"
                            )
                    else:
            ...
            for book_row in books_with_user_data:
                try:
                    ...
                    if (
                        old_user_id
                        and old_user_id in user_mapping
                        and old_book_id in book_id_mapping
                    ):
                        ...
                        user_default_location_id = user_default_locations.get(
                            new_user_id
                        )
                        ...
                        if user_default_location_id:
                            try:
                                ...
                                if location_success:
                                    self._log_action(
                                        f"✅ Assigned book '{safe_get(book_row, 'title', 'Unknown')}' to user {new_user_id}'s default location"
                                    )
                                    self.stats["relationships_created"] += 1
                                else:
                                    self._log_error(
                                        f"Failed to assign book '{safe_get(book_row, 'title', 'Unknown')}' to user {new_user_id}'s default location"
                                    )
                            except Exception as e:
                                self._log_error(
                                    f"Error assigning book '{safe_get(book_row, 'title', 'Unknown')}' to user {new_user_id}'s location: {e}"
                                )
                        else:
                            # No default location - leave book unassigned to any location (but still owned by user)
                            self._log_action(
                                f"📝 Book '{safe_get(book_row, 'title', 'Unknown')}' assigned to user {new_user_id} but no default location set - book remains location-free"
                            )
                            self.stats["relationships_created"] += 1
            ...
            if self._table_exists(cursor, "reading_log"):
                ...
                for log_row in reading_logs:
                    try:
                        ...
                        old_book_id = safe_get(log_row, "book_id")
                        old_user_id = (
                            safe_get(log_row, "user_id") if has_user_id_column else None
                        )
                        log_date = safe_get(log_row, "date")
                        ...
                        if (
                            ...
                            if created_log:
                                self.stats["reading_logs_migrated"] += 1
                                logger.debug(
                                    f"✅ Migrated reading log for user {target_user_id}, book {new_book_id} on {log_date}"
                                )
                            else:
                                logger.warning(
                                    f"⚠️ Failed to create reading log for user {target_user_id}, book {new_book_id} on {log_date}"
                                )
                        else:
                            ...
                            if old_book_id not in book_id_mapping:
                                missing_parts.append(
                                    f"book_id={old_book_id} not in mapping"
                                )
                            if not target_user_id:
                                if has_user_id_column:
                                    missing_parts.append(
                                        f"user_id={old_user_id} not in mapping"
                                    )
                                else:
                                    missing_parts.append(
                                        "no admin_user_id for V1-style reading log"
                                    )
                            if not log_date:
                                missing_parts.append("missing date")

                            logger.warning(
                                f"⚠️ Skipping reading log - {', '.join(missing_parts)}"
                            )
            ...
            logger.info(
                f"✅ V2 migration completed: {self.stats['users_migrated']} users, {self.stats['books_migrated']} books"
            )
    ...
    def _sqlite_row_to_book(self, row, fetch_api_metadata: bool = True) -> Book:
        ...
        def safe_get(row, key, default=None):
            try:
                return (
                    row[key] if key in row.keys() and row[key] is not None else default
                )
            except (KeyError, IndexError):
                return default
        ...
        if isbn and fetch_api_metadata:
            ...
            has_existing_metadata = bool(
                safe_get(row, "categories") or safe_get(row, "description")
            )
            ...
            if not has_existing_metadata or not safe_get(row, "categories"):
                ...
                logger.info(
                    f"📡 Fetching API metadata for ISBN {isbn} (V1 migration or missing categories)"
                )
            elif fetch_api_metadata:
                # Has existing metadata but user chose to fetch fresh API data (v2 option)
                should_fetch_api = True
                logger.info(
                    f"📡 User chose fresh API metadata for ISBN {isbn} - fetching from APIs"
                )
            ...
            try:
                ...
                if google_data or openlibrary_data:
                    ...
                    if openlibrary_data:
                        if (
                            google_data
                            and google_data.get("categories")
                            and openlibrary_data.get("categories")
                        ):
                        ...
                        elif openlibrary_data.get("categories") and not api_data.get(
                            "categories"
                        ):
                            api_data["categories"] = openlibrary_data.get(
                                "categories", []
                            )
                        for field in [
                            "description",
                        ...
                        if openlibrary_data.get("openlibrary_id"):
                            api_data["openlibrary_id"] = openlibrary_data[
                                "openlibrary_id"
                            ]
                        logger.info(f"✅ Merged OpenLibrary data for {isbn}")
                    if api_data.get("categories"):
                        logger.info(
                            f"📚 Categories for {isbn}: {api_data['categories']}"
                        )
                ...
                try:
                    ...
                    if best_cover and best_cover.get("cover_url"):
                        ...
                        logger.info(
                            f"🖼️ Selected cover ({best_cover.get('source')}) for {isbn}"
                        )
                except Exception as e:
        ...
        book = Book(
            ...
            or (api_data.get("title") if api_data else "")
            or "",
            ...
            or (api_data.get("cover") if api_data else None),
            description=safe_get(row, "description")
            or (api_data.get("description") if api_data else None),
            published_date=safe_get(row, "published_date")
            or (api_data.get("published_date") if api_data else None),
            page_count=safe_get(row, "page_count")
            or (api_data.get("page_count") if api_data else None),
            language=safe_get(row, "language")
            or (api_data.get("language") if api_data else "en")
            or "en",
            average_rating=safe_get(row, "average_rating")
            or (api_data.get("average_rating") if api_data else None),
            rating_count=safe_get(row, "rating_count")
            or (api_data.get("rating_count") if api_data else None),
            ...
            created_at=self._parse_datetime(safe_get(row, "created_at"))
            or datetime.now(),
        ...
        if api_authors:
            ...
            logger.info(
                f"📚 Using API authors for {safe_get(row, 'title', 'Unknown')}: {api_authors}"
            )
            ...
            if "," in raw_author:
                authors_to_process = [
                    name.strip() for name in raw_author.split(",") if name.strip()
                ]
            elif ";" in raw_author:
                authors_to_process = [
                    name.strip() for name in raw_author.split(";") if name.strip()
                ...
                authors_to_process = [
                    name.strip()
                    for name in re.split(r"\s+and\s+", raw_author, flags=re.IGNORECASE)
                    if name.strip()
                ]
            elif " & " in raw_author:
                authors_to_process = [
            ...
            logger.info(
                f"📚 Parsed SQLite authors for {safe_get(row, 'title', 'Unknown')}: {authors_to_process}"
            )
        ...
        for i, author in enumerate(authors_to_process):
            if author and author.strip():
                ...
                try:
                    ...
                    logger.info(
                        f"📡 Fetching OpenLibrary metadata for author: {author.strip()}"
                    )
                    ...
                    if search_result and search_result.get("openlibrary_id"):
                        ...
                        if detailed_author_data:
                            ...
                            from app.routes.people_routes import (
                            ...
                            logger.info(
                                f"✅ Enriched author '{author.strip()}' with comprehensive OpenLibrary data: {author_id}"
                            )
                        ...
                        logger.info(
                            f"⚠️ No OpenLibrary data found for author: {author.strip()}"
                        )

                except Exception as e:
                    logger.warning(
                        f"⚠️ Failed to fetch OpenLibrary data for author '{author.strip()}': {e}"
                    )
        ...
        categories_str = safe_get(
            row, "categories"
        )  # From SQLite (v2 databases may have this)
        ...
        if api_categories:
            ...
            if categories_str:
                logger.info(
                    f"📚 Using API categories over existing SQLite categories for {book.title}"
                )
            else:
                logger.info(
                    f"📚 Using API categories for {book.title}: {categories_to_use}"
                )
        elif categories_str:
            ...
            for sep in [",", ";", "|"]:
                if sep in categories_str:
                    categories_to_use = [
                        name.strip() for name in categories_str.split(sep)
                    ]
                    break
            else:
                # Single category
            ...
            logger.info(
                f"📚 Using existing SQLite categories (no API data) for {book.title}: {categories_to_use}"
            )
        else:
        ...
        if categories_to_use:
            ...
            logger.info(
                f"📚 Set raw_categories for {book.title}: {book.raw_categories}"
            )

        # Handle publisher with intelligent prioritization
        publisher_name = safe_get(row, "publisher") or (
            api_data.get("publisher") if api_data else None
        )
        if publisher_name:
            publisher = Publisher(
                name=publisher_name.strip(), created_at=datetime.now()
            )
