    ...
    if form.validate_on_submit():
        try:
            ...
            username = form.username.data or (
                setup_form_data and setup_form_data.get("username")
            )
            email = form.email.data or (
                setup_form_data and setup_form_data.get("email")
            )
            password = form.password.data or (
                setup_form_data and setup_form_data.get("password")
            )

            if not all([username, email, password]):
                flash("All fields are required.", "error")
                return render_template(
                    "migration/first_time_setup.html", form=form, databases=db_analysis
                )
...
def choose_migration():
    ...
    return render_template(
        "migration/choose_migration.html",
            ...
            try:
                ...
                if success:
                    ...
                    session["migration_results"] = (
                        migration_system.get_migration_summary()
                    )
...
def setup_v2_migration():
    ...
    if request.method == "POST":
        ...
        if admin_mapping:
            ...
            flash(
                "Please select which old user corresponds to your admin account.",
                "error",
            )

    return render_template(
        "migration/setup_v2_migration.html",
        database=selected_db,
        analysis=analysis,
    ...
    if request.method == "POST":
        if request.form.get("action") == "migrate":
            try:
                ...
                fetch_api_metadata = (
                    request.form.get("api_metadata", "false").lower() == "true"
                )
                ...
                success = migration_system.migrate_v2_database(
                    db_path, int_user_mapping, fetch_api_metadata=fetch_api_metadata
                )
