...
def validate_strong_password(form, field):
    ...
    if not User.is_password_strong(field.data):
        ...
        raise ValidationError(
            f"Password must meet the following requirements: {'; '.join(requirements)}"
        )
...
class ChangePasswordForm(FlaskForm):
    ...
    new_password = PasswordField(
        "New Password", validators=[DataRequired(), validate_strong_password]
    )
...
class AdminPasswordResetForm(FlaskForm):
    ...
    force_change = BooleanField(
        "Require user to change password on next login", default=True
    )
...
class SetupForm(FlaskForm):
    ...
    def validate_email(self, email):
        ...
        if email.data and email.data.endswith(".local"):
            raise ValidationError(
                "Email addresses ending in .local are not supported. Please use a standard email domain like @gmail.com, @example.com, etc."
            )
...
class ReadingStreakForm(FlaskForm):
    ...
    submit = SubmitField(
        "Update Streak Settings", render_kw={"class": "btn btn-primary"}
    )
...
class CategoryForm(FlaskForm):
        ...
        try:
            ...
            for category in categories:
                ...
                category_id = (
                    ...
                    else getattr(category, "id", None)
                )
                category_name = (
                    category.get("name")
                    if isinstance(category, dict)
                    else getattr(category, "name", "Unknown")
                )
                category_level = (
                    category.get("level", 0)
                    if isinstance(category, dict)
                    else getattr(category, "level", 0)
                )
                ...
                if self.current_category_id and self.is_descendant(
                    category, self.current_category_id
                ):
                    continue
    ...
    def is_descendant(self, category, ancestor_id):
        ...
        parent_id = (
            ...
            if isinstance(category, dict)
            else getattr(category, "parent_id", None)
        )
        ...
        try:
            ...
            for category in existing_categories:
                ...
                category_name = (
                    category.get("name")
                    if isinstance(category, dict)
                    else getattr(category, "name", None)
                )
                category_parent_id = (
                    category.get("parent_id")
                    if isinstance(category, dict)
                ...
                if (
                    category_name
                    and category_name.lower() == name.data.lower()
                    and category_parent_id == parent_id
                    and (
                        not self.current_category_id
                        or str(category_id) != str(self.current_category_id)
                    )
                ):
                    if parent_id:
                        raise ValidationError(
                            f'A category named "{name.data}" already exists under the selected parent.'
                        )
                    else:
                        raise ValidationError(
                            f'A root category named "{name.data}" already exists.'
                        )
        except Exception as e:
...
class MergeCategoriesForm(FlaskForm):
    source_id = SelectField(
        "Source Category (will be deleted)", validators=[DataRequired()], coerce=str
    )
    target_id = SelectField(
        "Target Category (will receive content)",
        validators=[DataRequired()],
        coerce=str,
    )
    merge_aliases = BooleanField("Merge aliases from source to target", default=True)
    merge_description = BooleanField(
        "Append source description to target", default=False
    )
    preserve_hierarchy = BooleanField(
        "Move subcategories to target category", default=True
    )
    ...
    def populate_category_choices(self):
        ...
        try:
            ...
            for category in categories:
                ...
                category_book_count = (
                    category.get("book_count", 0)
                    if isinstance(category, dict)
                    else getattr(category, "book_count", 0)
                )

                # Create display name with hierarchy
                indent = "  " * category_level
                book_info = (
                    f" ({category_book_count} books)" if category_book_count > 0 else ""
                )
...
class ReadingLogEntryForm(FlaskForm):
    ...
    minutes_read = IntegerField(
        "Minutes Read",
        validators=[Optional(), NumberRange(min=1)],
        render_kw={"class": "form-control"},
    )
    notes = TextAreaField(
        "Notes", validators=[Optional()], render_kw={"class": "form-control", "rows": 3}
    )
