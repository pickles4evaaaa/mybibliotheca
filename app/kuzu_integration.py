...
class KuzuIntegrationService:
    ...
    def _ensure_initialized(self) -> bool:
        ...
        if not all(
            [self.user_repo, self.book_repo, self.user_book_repo, self.location_repo]
        ):
    ...
    def _get_user_book_repo(self) -> KuzuUserBookRepository:
        ...
        assert self.user_book_repo is not None, (
            "User book repository must be initialized"
        )
    ...
    async def get_user_by_username_or_email(
        ...
        try:
            ...
            logger.error(
                f"Failed to get user by username or email {username_or_email}: {e}"
            )
            return None
    ...
    async def _book_to_dict(self, book) -> dict[str, Any]:
        ...
        try:
            if self._ensure_initialized():
                book_repo = self._get_book_repo()
                authors = (
                    await book_repo.get_book_authors(book.id)
                    if hasattr(book_repo, "get_book_authors")
                    else []
                )
            else:
                authors = []
        ...
        try:
            if self._ensure_initialized():
                book_repo = self._get_book_repo()
                categories = (
                    await book_repo.get_book_categories(book.id)
                    if hasattr(book_repo, "get_book_categories")
                    else []
                )
            else:
                categories = []
    ...
    async def _book_to_dict_from_data(
        ...
        book_dict = {
            ...
            "uid": book_data.get(
                "id"
            ),  # uid is an alias for id for backward compatibility
            "title": book_data.get("title"),
            "isbn13": book_data.get("isbn13"),
            "isbn10": book_data.get("isbn10"),
            "description": book_data.get("description"),
        ...
        try:
            ...
            if book_id and self._ensure_initialized():
                book_repo = self._get_book_repo()
                authors = (
                    await book_repo.get_book_authors(book_id)
                    if hasattr(book_repo, "get_book_authors")
                    else []
                )
            else:
        ...
        book_dict["authors"] = [
            {
                ...
                else getattr(author, "id", ""),
                "name": author.get("name")
                if isinstance(author, dict)
                else getattr(author, "name", ""),
                "role": author.get("role")
                if isinstance(author, dict)
                else getattr(author, "role", "author"),
                "order_index": author.get("order_index")
                if isinstance(author, dict)
                else getattr(author, "order_index", 0),
            }
            for author in authors
        ]
        ...
        try:
            ...
            if book_id and self._ensure_initialized():
                book_repo = self._get_book_repo()
                categories = (
                    await book_repo.get_book_categories(book_id)
                    if hasattr(book_repo, "get_book_categories")
                    else []
                )
            else:
        ...
        book_dict["categories"] = [
            {
                ...
                "normalized_name": cat.get("normalized_name")
                if isinstance(cat, dict)
                else getattr(cat, "normalized_name", ""),
            }
            for cat in categories
        ]
        ...
        try:
            ...
            if date_added_value is None:
                ...
                try:
                    ...
                    final_date_added = datetime.fromisoformat(
            ...
            success = await user_book_repo.create_ownership(
                ...
                ownership_status=OwnershipStatus(
                    ownership_data.get("ownership_status", "owned")
                ),
    ...
    async def remove_book_from_library(self, user_id: str, book_uid: str) -> bool:
        ...
        try:
            ...
            book_id = (
                book_data.get("id") if isinstance(book_data, dict) else book_data.id
            )
            if not book_id:
            ...
            if success:
                ...
                logger.warning(
                    f"Failed to remove book {book_uid} from user {user_id} library"
                )
    ...
    async def get_user_library(
        ...
        try:
            ...
            logger.info(
                f"🔍 Getting user library for {user_id}, status_filter: {status_filter}"
            )
                ...
                if location_id:
                    try:
                        ...
                        if location_data:
                            ...
                            logger.debug(
                                f"Successfully resolved location {location_id} to name: {location_name}"
                            )
                        else:
                            # If location not found, try to get from user's locations list
                            user_locations = await location_repo.get_user_locations(
                                user_id
                            )
                            location_name = None
                            ...
                            if location_name:
                                locations = [location_name]
                                logger.debug(
                                    f"Resolved location {location_id} from user locations: {location_name}"
                                )
                            else:
                                # Last resort: use a placeholder name
                                locations = [f"Location {location_id}"]
                                logger.warning(
                                    f"Could not resolve location {location_id}, using placeholder"
                                )
                    except Exception as e:
    ...
    async def update_reading_status(
        self, user_id: str, book_id: str, status: str
    ) -> bool:
