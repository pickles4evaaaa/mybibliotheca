...
def _sanitize_for_logging(
    value: str, extra_secrets: Iterable[str] | None = None
) -> str:
...
def save_system_config(config):
    ...
    try:
        ...
        existing_config["terminology_preference"] = config.get(
        ...
        if "reading_log_defaults" in config:
            ...
            try:
                ...
                dp_i = (
                    rld.get("default_pages_per_log")
                    if isinstance(rld.get("default_pages_per_log"), int)
                    else None
                )
                dm_i = (
                    rld.get("default_minutes_per_log")
                    if isinstance(rld.get("default_minutes_per_log"), int)
                    else None
                )
        ...
        if "library_defaults" in config:
            ...
            existing_library_defaults = existing_config.get(
                "library_defaults", {}
            ).copy()
            try:
                ...
                dr_i = (
                    lib.get("default_rows_per_page")
                    if isinstance(lib.get("default_rows_per_page"), int)
                    else None
                )
        ...
        if "import_settings" in config:
            ...
            try:
                if metadata_concurrency_val in (
                    None,
                    "",
                ):
            ...
            if metadata_concurrency_int is None:
                ...
                existing_import_settings["metadata_concurrency"] = (
                    metadata_concurrency_int
                )
        ...
        if "security_settings" in config:
            ...
            min_length_value = coerce_min_password_length(
                security_settings.get("min_password_length")
            )
...
def load_system_config():
    ...
    return {
        ...
        "import_settings": {"metadata_concurrency": None},
        "security_settings": {"min_password_length": default_min_length},
    }
...
def save_smtp_config(config):
    ...
    try:
        ...
        default_from_name = (
            f"{site_name} (MyBibliotheca)"
            if site_name != "MyBibliotheca"
            else "MyBibliotheca"
        )
        ...
        if raw_security not in allowed_security_values:
            ...
            legacy_str = (
                str(legacy_tls).lower() if isinstance(legacy_tls, (str, bool)) else ""
            )
            raw_security = (
                "starttls"
                if legacy_tls is True or legacy_str in {"true", "1", "on", "yes"}
                else "none"
            )
...
def load_backup_config():
    ...
    return system_config.get("backup_config", {"backup_directory": "data/backups"})
...
def save_ai_config(config):
    ...
    ai_keys = [
        ...
        "AI_MAX_TOKENS",
        "AI_TEMPERATURE",
        "AI_BOOK_EXTRACTION_ENABLED",
        "AI_BOOK_EXTRACTION_AUTO_SEARCH",
    ]
    # If user provided any provider/model key and did not explicitly set extraction flag, auto-enable extraction
    try:
        if (
            "AI_BOOK_EXTRACTION_ENABLED" not in config
            or str(config.get("AI_BOOK_EXTRACTION_ENABLED")).strip() == ""
        ):
...
def users():
    ...
    try:
        ...
        all_users = (
            user_service.get_all_users_sync()
            if hasattr(user_service, "get_all_users_sync")
            else []
        )
        ...
        users_paginated = PaginationResult(
            page_users, page, per_page, len(filtered_users)
        )
        ...
        return render_template(
            "admin/users.html", title="User Management", users=None, search=search
        )
...
def user_detail(user_id):
    ...
    try:
        ...
        reading_count = (
            reading_log_service.get_user_logs_count_sync(user_id)
            if hasattr(reading_log_service, "get_user_logs_count_sync")
            else 0
        )  # type: ignore
        ...
        recent_books = sorted(
            user_books, key=lambda x: x.get("created_at") or datetime.min, reverse=True
        )[:5]
...
def toggle_admin(user_id):
    ...
    try:
        ...
        if user.is_admin:
            ...
            if admin_count <= 1:
                flash(
                    "Cannot remove admin privileges from the last admin user.", "error"
                )
...
def settings():
    ...
    if request.method == "POST":
            ...
            if file and file.filename:
                ...
                if (
                    ...
                    try:
                        ...
                        flash(
                            "Error uploading background image. Please try again.",
                            "error",
                        )
                else:
                    flash(
        ...
        configured_min = coerce_min_password_length(
            current_security_settings.get("min_password_length")
        )
        min_password_length_raw = (
            request.form.get("min_password_length") or ""
        ).strip()
        ...
        config = {
            ...
            "security_settings": {"min_password_length": requested_min},
        }

        if save_system_config(config):
            if (
                "background_image_file" not in request.files
                or not request.files["background_image_file"].filename
            ):
                flash(
                    "System settings saved successfully! Changes are now active.",
                    "success",
                )
        else:
            flash(
                "Error saving system settings. Please check permissions and try again.",
                "error",
            )
...
def api_delete_user(user_id):
    ...
    try:
        ...
        admin_pwd = (
            request.form.get("admin_password") or request.json.get("admin_password")
            if request.is_json
            else None
        )  # type: ignore
        ...
        try:
            ...
            if repo and hasattr(repo, "safe_manager"):
                res = repo.safe_manager.execute_query(
                    "MATCH (u:User {id: $uid}) RETURN COUNT(u) as c", {"uid": user_id}
                )
        ...
        return jsonify(
            {
                "ok": deleted and not exists_flag,
                "deleted": deleted,
                "exists_after": exists_flag,
            }
        )
    except Exception as e:
...
def get_admin_settings_context():
    ...
    system_config = load_system_config()
    current_site_name = system_config.get(
        "site_name", os.getenv("SITE_NAME", "MyBibliotheca")
    )
    current_timezone = system_config.get(
    ...
    media_type_options = [
        {"value": mt.value, "label": _format_media_type_label(mt.value)}
        for mt in MediaType
    ]
    ...
    return {
        ...
        "background_config": system_config.get(
            "background_config",
            {
                "type": "default",
                "solid_color": "#667eea",
        ...
        "reading_log_defaults": system_config.get(
            "reading_log_defaults",
            {"default_pages_per_log": None, "default_minutes_per_log": None},
        ),
        "library_defaults": library_defaults,
        "media_type_options": media_type_options,
        "media_type_labels": media_type_labels,
        "import_settings": system_config.get(
            "import_settings", {"metadata_concurrency": None}
        ),
...
def update_ai_settings():
    ...
    try:
        ...
        config["OPENAI_BASE_URL"] = request.form.get(
            "openai_base_url", "https://api.openai.com/v1"
        )
        config["OPENAI_MODEL"] = request.form.get("openai_model", "gpt-4o")
        config["OLLAMA_BASE_URL"] = request.form.get(
        ...
        config["AI_BOOK_EXTRACTION_ENABLED"] = (
            "true" if request.form.get("ai_book_extraction_enabled") else "false"
        )
        config["AI_BOOK_EXTRACTION_AUTO_SEARCH"] = (
            "true" if request.form.get("ai_book_extraction_auto_search") else "false"
        )
...
def update_smtp_settings():
    ...
    try:
        ...
        config["smtp_from_name"] = request.form.get(
            "smtp_from_name", "MyBibliotheca"
        ).strip()
        allowed_security_values = {"starttls", "ssl", "none"}
        ...
        _log(
            "error",
            f"Error updating SMTP settings: {e}",
...
def test_smtp_connection():
    ...
    _log_force(
        "info",
        f"[SMTP] Configuration - Security: {raw_security.upper()}, Username: {smtp_username if smtp_username else '(none)'}",
        extra_secrets=secret_values,
    )
    ...
    try:
        ...
        _log_force(
            "error",
            f"[SMTP] SMTP error: {type(e).__name__}: {str(e)}",
            extra_secrets=secret_values,
        )
        ...
        _log_force(
            "error",
            f"[SMTP] Unexpected error: {type(e).__name__}: {str(e)}",
            extra_secrets=secret_values,
            exc_info=True,
        )
        return jsonify(
...
def update_backup_settings():
    ...
    try:
        ...
        # Get form data
        config["backup_directory"] = request.form.get(
    ...
    try:
        ...
        if form.validate_on_submit():
            try:
                ...
                flash(
                    f"Password has been reset for user {user.username}.{force_msg}",
                    "success",
                )
...
def get_system_stats():
    ...
    try:
        ...
        return {
            ...
            "top_users": [
                {"username": user[0], "book_count": user[1]} for user in top_users
            ],
            "system": system_info,
        }
    except Exception as e:
