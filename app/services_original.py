...
class KuzuBookService:
    ...
    async def add_book_to_user_library(self, user_id: str, book_id: str, 
        ...
        try:            
            ...
            # Create user-book relationship (simplified for new schema)
            ...
            print(f"📚 [ADD_TO_LIBRARY] Created simplified relationship (locations handled separately)")
            # Store relationship in Kuzu
            await self.kuzu_user_book_repo.create_relationship(relationship)
            
            # Handle locations separately using LOCATED_AT relationships
            if locations:
                # Get primary location (first in list or specified)
                primary_location_id = locations[0] if locations else None
                self.kuzu_book_repo.create_book_location_relationships(
                    book_id, str(user_id), locations, primary_location_id
                )
                print(f"📚 [ADD_TO_LIBRARY] Created LOCATED_AT relationships for locations: {locations}")
    ...
    async def get_books_by_category(self, category_id: str, user_id: Optional[str] = None, include_subcategories: bool = True):
        ...
        try:
            ...
            for cat_id in category_ids_to_check:
                ...
                # Find books that have CATEGORIZED_AS relationships to this category
    ...
    async def get_book_categories(self, book_id: str) -> List[Category]:
        ...
        try:
            ...
            # Get all CATEGORIZED_AS relationships for this book
            relationships = self.storage.get_relationships('book', book_id, 'CATEGORIZED_AS')
        ...
        try:
            ...
            for category_name in categories_list:
                ...
                try:
                    ...
                    if category:
                        # Check if relationship already exists
                        existing_relationships = self.storage.get_relationships('book', book_id, 'CATEGORIZED_AS')
                        ...
                        if not relationship_exists:
                            ...
                            relationship_created = self.storage.create_relationship(
                                'book', book_id, 'CATEGORIZED_AS', 'category', category.id
                            )
    ...
    async def get_book_locations_for_user(self, book_id: str, user_id: str) -> List[Dict[str, Any]]:
        """Get all locations where a book is stored for a specific user."""
        query = """
        MATCH (b:Book)-[l:LOCATED_AT]->(loc:Location)
        WHERE b.id = $book_id AND l.user_id = $user_id
        RETURN loc, l.is_primary as is_primary
        """
        results = self.kuzu_book_repo.storage.execute_cypher(query, {
            "book_id": book_id,
            "user_id": user_id
        })
        ...
        for result in results:
            ...
            is_primary = result.get('col_1', False)
            locations.append({
