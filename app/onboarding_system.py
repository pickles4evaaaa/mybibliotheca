...
def update_onboarding_data(data: Dict):
    ...
    logger.info(
        f"🔍 SESSION DEBUG: onboarding_data in session: {session.get('onboarding_data', 'NOT_FOUND')}"
    )
...
def set_onboarding_step(step: int):
    ...
    logger.info(
        f"🔍 SESSION DEBUG: onboarding_step set to: {session.get('onboarding_step', 'NOT_FOUND')}"
    )
...
def clear_onboarding_session():
    ...
    session.modified = True
    logger.info("🔍 SESSION DEBUG: Onboarding session cleared")
...
def start():
    ...
    if current_user.is_authenticated:
        logger.info(
            "🔍 ONBOARDING DEBUG: Authenticated user hit /onboarding/start; redirecting without flash banner"
        )
...
def step(step_num: int):
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: User accessing step {step_num}, current_step={current_step}"
    )
    ...
    if current_user.is_authenticated:
        ...
        if current_step == 1 and not onboarding_data:
            logger.info(
                "🔍 ONBOARDING DEBUG: Authenticated user accessing onboarding; silent redirect to library"
            )
            return redirect(url_for("main.library"))
    ...
    if step_num > current_step:
        ...
        logger.info(
            f"🔍 ONBOARDING DEBUG: Forward navigation check - onboarding_data keys: {list(onboarding_data.keys())}"
        )
        ...
        if step_num >= 2 and "admin" not in onboarding_data:
            ...
            if "admin" in backup:
                ...
                logger.warning(
                    f"🔍 ONBOARDING DEBUG: Missing admin data for step {step_num}, redirecting to step 1"
                )
        ...
        if step_num >= 3 and "site_config" not in onboarding_data:
            logger.warning(
                f"🔍 ONBOARDING DEBUG: Missing site_config data for step {step_num}, redirecting to step 2"
            )
        ...
        if step_num >= 4 and "data_options" not in onboarding_data:
            logger.warning(
                f"🔍 ONBOARDING DEBUG: Missing data_options for step {step_num}, redirecting to step 3"
            )
        ...
        if step_num >= 5:
            ...
            if data_options.get("option") == "import":
                ...
                if "import_file" not in data_options or not data_options.get(
                    "import_file"
                ):
                    logger.warning(
                        f"🔍 ONBOARDING DEBUG: Import option selected but no file chosen for step {step_num}"
                    )
        ...
        logger.info(
            f"🔍 ONBOARDING DEBUG: Backward navigation to step {step_num} from {current_step}"
        )
    ...
    if step_num == 1:
        ...
        logger.warning(
            f"🔍 ONBOARDING DEBUG: Invalid step {step_num}, redirecting to start"
        )
...
def admin_setup_step():
    ...
    logger.info("🔍 ONBOARDING DEBUG: admin_setup_step called")
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: Form created, is_submitted={form.is_submitted()}"
    )
    ...
    if request.method == "POST":
        ...
        if form_valid or (username and email and password and password == password2):
            try:
                logger.info(
                    "🔍 ONBOARDING DEBUG: Form processing, using form data directly"
                )
                ...
                logger.info(
                    f"🔍 ONBOARDING DEBUG: Saving admin data: {admin_data['username']}, {admin_data['email']}"
                )
                ...
                # Move to next step
                logger.info("🔍 ONBOARDING DEBUG: Moving to step 2")
                session["onboarding_step"] = 2
                session.modified = True

                logger.info(
                    f"🔍 ONBOARDING DEBUG: Session after update: {dict(session)}"
                )
    ...
    logger.info("🔍 ONBOARDING DEBUG: Rendering step 1 template")

    return render_template(
        "onboarding/step1_admin_setup.html", form=form, step=1, total_steps=5
    )
...
def site_config_step():
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: site_config_step called, method={request.method}"
    )
    ...
    if request.method == "POST":
        try:
            logger.info(
                f"🔍 ONBOARDING DEBUG: Processing POST data: {dict(request.form)}"
            )
            ...
            site_config = {
                ...
                "terminology_preference": request.form.get(
                ...
                "people_metadata_mode": request.form.get(
                    "people_metadata_mode", "openlibrary"
                ),
                # Optional reading log defaults (may be omitted or blank)
                "default_pages_per_log": (
                    request.form.get("default_pages_per_log") or ""
                ).strip(),
                "default_minutes_per_log": (
                    request.form.get("default_minutes_per_log") or ""
                ).strip(),
                # Library pagination default rows per page (optional)
                "default_rows_per_page": (
                    request.form.get("default_rows_per_page") or ""
                ).strip(),
            }
            ...
            try:
                ...
                scheduled_minute = int(
                    request.form.get("backup_scheduled_minute") or 30
                )
                ...
                logger.info(
                    f"🔍 ONBOARDING DEBUG: Captured backup settings from step 2: {backup_settings}"
                )

                # Persist immediately via backup service
                try:
                    from .services.simple_backup_service import (
                    ...
                    if hasattr(svc, "_settings"):
                        ...
                        if svc._settings.get("enabled"):
                            ...
                            svc.stop_scheduler()
                        logger.info(
                            "🔍 ONBOARDING DEBUG: Applied backup settings to backup service from step 2"
                        )
                except Exception as e:
                    logger.warning(
                        f"⚠️ ONBOARDING DEBUG: Failed applying backup settings during step 2: {e}"
                    )
            except Exception as e:
                logger.warning(
                    f"⚠️ ONBOARDING DEBUG: Error capturing backup settings in step 2: {e}"
                )
            ...
            try:
                ...
                abs_library_ids_raw = (
                    request.form.get("abs_library_ids") or ""
                ).strip()
                save_abs_settings(
                    {
                        "enabled": abs_enabled,
                ...
                logger.info(
                    "🔍 ONBOARDING DEBUG: Applied Audiobookshelf settings from onboarding step 2"
                )
            except Exception as abs_err:
                logger.warning(
                    f"⚠️ ONBOARDING DEBUG: Failed to apply ABS settings during onboarding: {abs_err}"
                )
            ...
            # Move to next step
            logger.info("🔍 ONBOARDING DEBUG: Moving to step 3")
            ...
            try:
                ...
                if cache.path.exists():
                    logger.info(
                        "🔍 ONBOARDING DEBUG: Metadata settings file already exists; skipping seed"
                    )
                else:
                    logger.info(
                        "🔍 ONBOARDING DEBUG: Seeding initial metadata settings file"
                    )
                book_mode = site_config.get("book_metadata_mode", "both")
                people_mode = site_config.get("people_metadata_mode", "openlibrary")
                if book_mode not in ("google", "openlibrary", "both"):
                    book_mode = "both"
                if people_mode not in ("openlibrary", "none"):
                    people_mode = "openlibrary"
                ...
                for f in DEFAULT_PERSON_FIELDS:
                    ...
                    logger.info(
                        "🔍 ONBOARDING DEBUG: Seeded metadata provider settings from onboarding step 2"
                    )
            except Exception as e:
                logger.warning(
                    f"⚠️ ONBOARDING DEBUG: Failed to seed metadata settings: {e}"
                )
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: Rendering step 2 template with current_config: {display_config}"
    )
    ...
    try:
        ...
        logger.warning(
            f"⚠️ ONBOARDING DEBUG: Could not load ABS settings for template: {e}"
        )

    return render_template(
        "onboarding/step2_site_config.html",
        timezones=timezones,
...
def data_options_step():
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: data_options_step called, method={request.method}"
    )
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: Found {len(databases)} databases: {[str(db) for db in databases]}"
    )
        ...
        try:
            ...
            try:
                ...
                if backup_enabled_raw is not None:
                    ...
                    backup_enabled = str(backup_enabled_raw).lower() in (
                        "1",
                        "true",
                        "yes",
                        "on",
                    ...
                    retention_days = int(
                        request.form.get("backup_retention_days") or 14
                    )
                    scheduled_hour = int(request.form.get("backup_scheduled_hour") or 2)
                    scheduled_minute = int(
                        request.form.get("backup_scheduled_minute") or 30
                    ...
                    logger.info(
                        f"🔍 ONBOARDING DEBUG: Captured backup settings from onboarding: {data_options['backup_settings']}"
                    )
                    ...
                    try:
                        ...
                        if hasattr(svc, "_settings"):
                            ...
                            if svc._settings.get("enabled"):
                                ...
                                svc.stop_scheduler()
                            logger.info(
                                "🔍 ONBOARDING DEBUG: Applied onboarding backup settings to backup service"
                            )
                    except Exception as e:
                        logger.warning(
                            f"⚠️ ONBOARDING DEBUG: Failed applying backup settings during onboarding: {e}"
                        )
            except Exception as e:
                logger.warning(
                    f"⚠️ ONBOARDING DEBUG: Error capturing backup settings: {e}"
                )
            ...
            if data_option == "migrate":
                ...
                if selected_db:
                    ...
                    logger.info(
                        f"🔍 ONBOARDING DEBUG: Selected pre-detected database: {selected_db}"
                    )
                    ...
                    for db in db_analysis:
                        if db["path"] == selected_db:
                            ...
                            logger.info(
                                f"🔍 ONBOARDING DEBUG: Found database analysis: {db}"
                            )
                            break
                    ...
                    try:
                        ...
                        logger.info(
                            f"🔍 ONBOARDING DEBUG: Custom database file saved: {custom_db_file.filename} -> {temp_path}"
                        )

                        # Analyze the uploaded database
                        migration_system = AdvancedMigrationSystem()
                        version, analysis = migration_system.detect_database_version(
                            Path(temp_path)
                        )
                        ...
                        logger.info(
                            f"🔍 ONBOARDING DEBUG: Custom database analysis: version={version}, analysis={analysis}"
                        )
                        ...
                        return render_template(
                            "onboarding/step3_data_options.html",
                    ...
                    logger.warning(
                        "🔍 ONBOARDING DEBUG: No database selected for migration"
                    )
                    flash(
                        "Please select a database to migrate from before proceeding.",
                        "error",
                    )
                    return render_template(
                        "onboarding/step3_data_options.html",
                ...
                if not uploaded_file or not uploaded_file.filename:
                    logger.warning("🔍 ONBOARDING DEBUG: No import file provided")
                    flash(
                        "Please select a CSV file to import before proceeding.", "error"
                    )
                ...
                if uploaded_file and uploaded_file.filename:
                    ...
                    try:
                        ...
                        logger.info(
                            f"🔍 ONBOARDING DEBUG: Import file saved: {uploaded_file.filename} -> {temp_path}"
                        )
            ...
            # Move to next step
            logger.info("🔍 ONBOARDING DEBUG: Moving to step 4")
    ...
    logger.info("🔍 ONBOARDING DEBUG: Rendering step 3 template")
...
def data_config_step():
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: data_config_step called, method={request.method}"
    )
    ...
    if option == "migrate":
        logger.info("🔍 ONBOARDING DEBUG: Calling migration_config_step")
        return migration_config_step(data_options)
    elif option == "import":
        logger.info("🔍 ONBOARDING DEBUG: Calling import_config_step")
        return import_config_step(data_options)
    else:
        # Skip configuration for fresh start
        logger.info("🔍 ONBOARDING DEBUG: Fresh start, skipping to step 5")
...
def migration_config_step(data_options: Dict):
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: database_analysis_data: {database_analysis_data}"
    )
    ...
    if request.method == "POST":
        try:
            ...
            if version == DatabaseVersion.V2_MULTI_USER:
                ...
                try:
                    migration_config["admin_user_mapping"] = str(
                        int(admin_user_mapping)
                    )
                ...
                logger.info(
                    f"🔍 ONBOARDING DEBUG: V2 migration with admin_user_mapping: {admin_user_mapping}"
                )

            logger.info(
                f"🔍 ONBOARDING DEBUG: Saving migration_config: {migration_config}"
            )
            update_onboarding_data({"migration_config": migration_config})

            # Move to confirmation step
            logger.info("🔍 ONBOARDING DEBUG: Moving to step 5")
            set_onboarding_step(5)
    ...
    logger.info("🔍 ONBOARDING DEBUG: Rendering step 4 migration template")

    return render_template(
        "onboarding/step4_migration_config.html",
...
def import_config_step(data_options: Dict):
        ...
        try:
            ...
            try:
                ...
                if any(sig in first_line for sig in goodreads_signatures):
                    ...
                    if (
                        isbn_like_lines >= len(lines) * 0.8
                    ):  # 80% of lines look like ISBNs
                        detected_type = "isbn_only"
                        field_mappings = {
                            "isbn": "isbn"
                        }  # Simple mapping for ISBN-only files
                        logger.info("✅ Detected ISBN-only CSV format")
                    else:
                        logger.warning("❌ Could not detect CSV format")
                        flash(
                            "Could not detect the format of your CSV file. Please ensure it is a Goodreads export, StoryGraph export, or a simple list of ISBNs.",
                            "error",
                        )
                        return redirect(url_for("onboarding.step", step_num=3))
            ...
            logger.info(
                f"🔧 Created import config: detected_type={detected_type}, mappings={len(field_mappings)} fields"
            )
    ...
    try:
        ...
        logger.info(
            f"🔧 Auto-created import config: detected_type={detected_type}, mappings={len(field_mappings)} fields"
        )
        ...
        flash(
            "Import configuration will be automatically detected based on your file.",
            "info",
        )
        set_onboarding_step(5)
        return redirect(url_for("onboarding.step", step_num=5))
...
def confirmation_step():
    ...
    logger.info(
        f"🔍 ONBOARDING DEBUG: confirmation_step called, method={request.method}"
    )
    ...
    if request.method == "POST":
        print("🚨🚨🚨 STEP5 DEBUG: POST REQUEST RECEIVED! 🚨🚨🚨")
        ...
        if action == "execute":
            try:
                logger.info(
                    f"🔍 ONBOARDING DEBUG: Executing onboarding with data keys: {list(onboarding_data.keys())}"
                )
                ...
                if not onboarding_data.get("admin"):
                    ...
                    flash(
                        "Missing admin account information. Please start over.", "error"
                    )
                    return redirect(url_for("onboarding.step", step_num=1))
                ...
                if not onboarding_data.get("site_config"):
                    ...
                    flash(
                        "Missing site configuration. Please complete site setup.",
                        "error",
                    )
                    return redirect(url_for("onboarding.step", step_num=2))
                ...
                if not onboarding_data.get("data_options"):
                    ...
                    flash(
                        "Missing data configuration. Please complete data setup.",
                        "error",
                    )
                    return redirect(url_for("onboarding.step", step_num=3))
                ...
                if data_option == "import":
                    ...
                    logger.info(
                        "Import option selected - executing synchronous import like migration"
                    )
                    ...
                    if success:
                        ...
                        logger.error("🔍 ONBOARDING DEBUG: Import failed")
                        flash(
                    ...
                    logger.info("Migration option selected - executing full migration")
                    success = execute_onboarding(onboarding_data)

                    if success:
                        return handle_onboarding_completion(onboarding_data)
                    else:
                        logger.error("🔍 ONBOARDING DEBUG: Migration failed")
                        flash(
                            "Migration failed. Please check the error message and try again.",
                            "error",
                        )
                        # Ensure session data is preserved
                        update_onboarding_data(onboarding_data)
                    ...
                    logger.info(
                        "🔍 ONBOARDING DEBUG: Fresh start - executing basic setup only"
                    )
                    success = execute_onboarding_setup_only(onboarding_data)

                    if success:
                        logger.info(
                            "🔍 ONBOARDING DEBUG: Fresh start setup completed, proceeding to completion"
                        )
                        return handle_onboarding_completion(onboarding_data)
                    else:
                        logger.error("🔍 ONBOARDING DEBUG: Fresh start setup failed")
                        flash(
    ...
    logger.info("🔍 ONBOARDING DEBUG: Rendering step 5 confirmation template")
...
def complete():
    ...
    print("🎉 COMPLETION: Showing completion page")
    logger.info("🔍 ONBOARDING DEBUG: Showing completion page")
    ...
    return render_template("onboarding/complete.html", import_summary=import_summary)
...
def import_progress(task_id: str):
    ...
    if current_user.is_authenticated:
        logger.info(
            "🔍 ONBOARDING DEBUG: User is authenticated, redirecting to library"
        )
    ...
    try:
        ...
        logger.info(
            f"🔍 ONBOARDING DEBUG: Rendering splash screen with context: {template_context}"
        )
...
def import_progress_json(task_id: str):
    ...
    try:
        ...
        if current_user.is_authenticated:
            ...
            logger.info(
                "✅ User is authenticated - assuming import completed successfully"
            )
            return jsonify(
                {
                    "status": "completed",
                    ...
                    "error_messages": [],
                    "recent_activity": [
                        "Setup completed successfully - please continue to your library"
                    ],
                }
            )
        ...
        if task_id == "simple-onboarding-import":
            ...
            logger.info("✅ Simple onboarding import - returning completed status")
            return jsonify(
                {
                    "status": "completed",
                    "processed": 1,
                    ...
                    "error_messages": [],
                    "recent_activity": [
                        "Import completed successfully - redirecting to library"
                    ],
                }
            )
        ...
        logger.warning(
            f"⚠️ Job {task_id} not found in memory - checking if onboarding is complete"
        )
        ...
        if current_step is None or current_step == 1:  # Default state or cleared
            logger.info(
                "✅ No active onboarding step found - assuming import completed"
            )
            return jsonify(
                {
                    "status": "completed",
...
def execute_onboarding(onboarding_data: Dict) -> bool:
    ...
    try:
        ...
        admin_user = user_service.create_user_sync(
            ...
            timezone=site_config.get(
                "timezone", "UTC"
            ),  # Set timezone from site config
        ...
        if admin_user:
            print(
                f"🚀 [EXECUTE] Admin user details: ID={admin_user.id}, username={admin_user.username}, email={admin_user.email}"
            )
        else:
            print("🚀 [EXECUTE] Failed to create admin user!")
            return False
        ...
        if location_name:
            try:
                ...
                location = location_service.create_location(
                    name=location_name,
                    description="Default location set during onboarding",
        ...
        try:
            ...
            system_config = {
                ...
                "terminology_preference": site_config.get(
                    "terminology_preference", "genre"
                ),
            }
            ...
            try:
                ...
                dr_val = None
            system_config["library_defaults"] = {"default_rows_per_page": dr_val}
            if save_system_config(system_config):
                logger.info(
                    f"✅ Applied site configuration to system settings: {system_config}"
                )
            else:
                logger.warning("⚠️ Failed to save site configuration to system settings")
        except Exception as e:
        ...
        if data_options.get("option") == "migrate":
            ...
            if selected_db:
                ...
                if database_analysis.get("version") == DatabaseVersion.V1_SINGLE_USER:
                    ...
                    if not admin_user.id:
                        logger.error(
                            "❌ Admin user has no ID - cannot migrate V1 database"
                        )
                        return False
                    success = migration_system.migrate_v1_database(
                        db_path, admin_user.id
                    )
                    ...
                    success = migration_system.migrate_v2_database(
                        db_path, user_mapping, fetch_api_metadata=True
                    )
                else:
                    success = False
                ...
                try:
                    ...
                    for csv_field, field_config in custom_fields.items():
                        ...
                        field_type = field_type_map.get(
                            field_type_str, CustomFieldType.TEXT
                        )

                        # Create CustomFieldDefinition object
                        if not admin_user.id:
                            logger.error(
                                "❌ Admin user has no ID - cannot create custom fields"
                            )
                            continue
                        ...
                        field_data = {
                            ...
                            if hasattr(field_definition.field_type, "value")
                            else str(field_definition.field_type),
                            "description": field_definition.description,
                            "is_global": field_definition.is_global,
                            "is_shareable": field_definition.is_shareable,
                        }
                        custom_field = custom_field_service.create_field_sync(
                            admin_user.id, field_data
                        )

                        logger.info(
                            f"✅ Created custom field: {field_name} ({field_type}) from CSV column: {csv_field}"
                        )
            ...
            try:
                ...
                logger.info(
                    "🚀 Starting CSV import via SimplifiedBookService (unified pipeline)"
                )
                ...
                with open(csv_file_path, "r", encoding="utf-8") as csvfile:
                    ...
                    for row in reader:
                        try:
                            ...
                            title = (
                                mapped.get("title") or mapped.get("isbn") or "Untitled"
                            )
                            ...
                            try:
                                if (
                                    mapped.get("page_count")
                                    and str(mapped["page_count"]).isdigit()
                                ):
                            ...
                            cleaned_isbn = (
                                "".join(filter(str.isdigit, isbn_raw))
                                if isbn_raw
                                else ""
                            )
                            ...
                            if "categories" in mapped and mapped["categories"]:
                                categories = [
                                    c.strip()
                                    for c in mapped["categories"].split(",")
                                    if c.strip()
                                ]
                            ...
                            if cleaned_isbn:
                                try:
                                    ...
                                    if unified:
                                        ...
                                        description = description or unified.get(
                                            "description"
                                        )
                                        publisher = publisher or unified.get(
                                            "publisher"
                                        )
                                        published_date = published_date or unified.get(
                                            "published_date"
                                        )
                                        language = (
                                            language or unified.get("language") or "en"
                                        )
                                        cover_url = cover_url or unified.get(
                                            "cover_url"
                                        )
                                        if (
                                            not page_count
                                            and unified.get("page_count") is not None
                                        ):
                                            try:
                                        ...
                                        # Provider IDs
                                        google_books_id = (
                                            unified.get("google_books_id")
                                            or google_books_id
                                        )
                                        openlibrary_id = (
                                            unified.get("openlibrary_id")
                                            or openlibrary_id
                                        )
                                        ...
                                        uni_cats = [
                                            c
                                            for c in (unified.get("categories") or [])
                                    ...
                                    logger.warning(
                                        f"⚠️ Unified metadata fetch failed for ISBN {cleaned_isbn}: {uerr}"
                                    )
                            ...
                            success = service.add_book_to_user_library_sync(
                                book_data=new_book, user_id=admin_user.id
                            )
                            ...
                            errors += 1
                            logger.error(
                                f"❌ Error importing book from row: {book_error}"
                            )
                            continue

                logger.info(
                    f"✅ CSV import completed via SimplifiedBookService: {books_imported} books imported, {errors} errors"
                )

            except Exception as e:
                logger.error(
                    f"❌ Failed to execute CSV import (SimplifiedBookService): {e}"
                )
                return False
...
def execute_onboarding_setup_only(onboarding_data: Dict) -> bool:
    ...
    try:
        ...
        logger.info(
            f"🚀 Starting onboarding setup with data keys: {list(onboarding_data.keys())}"
        )
        ...
        print("🚀 [SETUP] ============ STEP 1: CREATING ADMIN USER ============")
        ...
        logger.info(
            f"Creating admin user: {admin_data.get('username')} with email: {admin_data.get('email')}"
        )

        try:
            password_hash = generate_password_hash(admin_data["password"])
            print("🚀 [SETUP] Password hash generated successfully")
        except Exception as hash_error:
            logger.error(f"Failed to generate password hash: {hash_error}")
            return False

        try:
            print("🚀 [SETUP] Calling user_service.create_user_sync...")
            display_name = admin_data.get("display_name") or admin_data["username"]
            try:
                import re
        ...
        logger.info(
            f"✅ Admin user created: {admin_user.username} (ID: {admin_user.id})"
        )

        # Log in the user immediately after creation
        try:
            print("🚀 [SETUP] Attempting to log in admin user...")
        ...
        print("🚀 [SETUP] ============ STEP 2: CREATING LOCATION ============")
        # Step 2: Apply site configuration and create location
        site_config = onboarding_data.get("site_config", {})
        ...
        if location_name:
            try:
                ...
                print("🏠 [SETUP] Getting safe Kuzu connection...")

                # Create location service
                admin_user_id = getattr(admin_user, "id", None)
                print(
                    f"🏠 [SETUP] Admin user ID for location creation: {admin_user_id}"
                )
                if not admin_user_id:
                    logger.error("❌ Admin user has no ID for location creation")
                    raise Exception("Admin user has no ID")

                print("🏠 [SETUP] Creating LocationService...")
                location_service = LocationService()
                print(f"🏠 [SETUP] LocationService created: {location_service}")

                # Create the location
                print("🏠 [SETUP] Calling location_service.create_location...")
                location = location_service.create_location(
                    name=location_name,
                    description="Default location set during onboarding",
                    location_type="home",  # Default to home type
                ...
                if location:
                    logger.info(
                        f"✅ Created location: {location.name} (ID: {location.id})"
                    )
                else:
                    logger.error("❌ Location creation returned None")
            ...
            print(
                f"🏠 [SETUP] No location specified (location='{location_name}'), skipping location creation"
            )
            logger.info("No location specified, skipping location creation")

        # Apply site configuration settings to system settings
        print("🚀 [SETUP] ============ APPLYING SITE CONFIGURATION ============")
        try:
            from .admin import save_system_config
            ...
            if save_system_config(system_config):
                ...
                print("🚀 [SETUP] Site configuration applied successfully")
            else:
                logger.warning("⚠️ Failed to save site configuration to system settings")
                print("🚀 [SETUP] Failed to apply site configuration")
        except Exception as e:
            logger.error(f"❌ Error applying site configuration: {e}")
            print(f"🚀 [SETUP] Error applying site configuration: {e}")
            # Don't fail onboarding if system config save fails

        print("🚀 [SETUP] ============ STEP 3: CREATING CUSTOM FIELDS ============")
        ...
        if custom_fields:
            try:
                print(
                    f"🏷️ [SETUP] Starting creation of {len(custom_fields)} custom fields"
                )
                ...
                for csv_field, field_config in custom_fields.items():
                    print(
                        f"🏷️ [SETUP] Processing custom field: {csv_field} -> {field_config}"
                    )
                    ...
                    print(
                        f"🏷️ [SETUP] Mapped field type: {field_type_str} -> {field_type}"
                    )

                    # Check if admin user has an ID
                    admin_user_id = getattr(admin_user, "id", None)
                    print(
                        f"🏷️ [SETUP] Admin user ID for custom field creation: {admin_user_id}"
                    )
                    if not admin_user_id:
                        logger.error(
                            "❌ Admin user has no ID for custom field creation"
                        )
                        continue  # Skip this custom field

                    try:
                        print("🏷️ [SETUP] Creating CustomFieldDefinition object...")
                        # Create CustomFieldDefinition object
                        field_definition = CustomFieldDefinition(
                            name=field_name,
                            display_name=csv_field,  # Use original CSV header as display name
                        ...
                        print(
                            f"🏷️ [SETUP] CustomFieldDefinition created: {field_definition}"
                        )

                        # Create the custom field
                        print("🏷️ [SETUP] Preparing field data for service...")
                        field_data = {
                            "name": field_definition.name,
                            "display_name": field_definition.display_name,
                            "field_type": field_definition.field_type.value
                        ...
                        print(
                            "🏷️ [SETUP] Calling custom_field_service.create_field_sync..."
                        )
                        custom_field = custom_field_service.create_field_sync(
                            admin_user_id, field_data
                        )  # Use the checked ID
                        print(
                            f"🏷️ [SETUP] Custom field service returned: {custom_field}"
                        )
                        ...
                        if custom_field:
                            ...
                            logger.error(
                                f"❌ Custom field creation returned None for: {field_name}"
                            )

                    except Exception as field_error:
                        logger.error(
                            f"❌ Exception creating custom field {field_name}: {field_error}"
                        )
                        import traceback
            ...
            print("🏷️ [SETUP] No custom fields to create (custom_fields is empty)")
            logger.info("No custom fields to create")

        print("🚀 [SETUP] ============ STEP 4: CSV IMPORT JOB ============")
        ...
        if data_option == "import":
            print("📂 [SETUP] Import option selected, starting CSV import job")
            ...
            try:
                print("📂 [SETUP] Calling start_onboarding_import_job...")
                print(f"📂 [SETUP] Import config: {import_config}")
                import_task_id = start_onboarding_import_job(
                    admin_user_id, import_config
                )
                print(
                    f"📂 [SETUP] start_onboarding_import_job returned: {import_task_id}"
                )
            ...
            print(
                f"📂 [SETUP] No import selected (option='{data_option}'), skipping CSV import"
            )
...
def handle_onboarding_completion(onboarding_data: Dict):
    ...
    try:
        ...
        logger.info("🔍 ONBOARDING DEBUG: Starting completion process")

        # Check if the user is already logged in from the setup process
        from flask_login import current_user

        print(
            f"🎉 [COMPLETION] Current user status: authenticated={current_user.is_authenticated}"
        )
        if hasattr(current_user, "id"):
            print(f"🎉 [COMPLETION] Current user ID: {current_user.id}")
            print(
                f"🎉 [COMPLETION] Current user username: {getattr(current_user, 'username', 'NO_USERNAME')}"
            )

        if current_user.is_authenticated:
            print("🎉 [COMPLETION] User already logged in from setup process!")
            logger.info(
                f"User already logged in: {getattr(current_user, 'username', 'UNKNOWN')}"
            )

            # Clear onboarding session and redirect
            print("🎉 [COMPLETION] Clearing onboarding session")
            clear_onboarding_session()
            logger.info("🔍 ONBOARDING DEBUG: Onboarding session cleared")

            print("🎉 [COMPLETION] Success! Redirecting to main index")
            logger.info("🔍 ONBOARDING DEBUG: Success! Redirecting to main index")
        ...
        print(
            "🎉 [COMPLETION] User not logged in, attempting to find and log in admin user"
        )
        ...
        try:
            ...
            if kuzu_service.db is not None:
                all_users = kuzu_service.db.query(
                    "MATCH (u:User) RETURN u.username, u.id, u.email LIMIT 10"
                )
        ...
        if not admin_user:
            logger.error(
                f"🔍 ONBOARDING DEBUG: Could not find admin user: {admin_username}"
            )
        ...
        login_user(admin_user)
        logger.info(
            f"🔍 ONBOARDING DEBUG: Admin user logged in successfully: {admin_user.username}"
        )
        ...
        print(
            f"🎉 [COMPLETION] Current user after login: {current_user.is_authenticated}, ID: {getattr(current_user, 'id', 'NO_ID')}"
        )
        ...
        if not current_user.is_authenticated:
            ...
            flash(
                "Setup completed but login failed. Please log in manually.", "warning"
            )
            return redirect(url_for("auth.login"))
        ...
        print("🎉 [COMPLETION] Success! Redirecting to library")
        logger.info("🔍 ONBOARDING DEBUG: Success! Redirecting to library")
        flash("🎉 Welcome to MyBibliotheca! Your library is ready.", "success")
        ...
        traceback.print_exc()
        flash(
            f"Setup completed but login failed: {e}. Please log in manually.", "warning"
        )
        return redirect(url_for("auth.login"))
...
def start_onboarding_import_job(user_id: str, import_config: Dict) -> Optional[str]:
    ...
    try:
        ...
        logger.info(
            f"🚀 Starting simplified onboarding import job {task_id} for user {user_id}"
        )
        ...
        try:
            print("🚀 [IMPORT_JOB] Counting CSV rows...")
            import csv
        ...
        field_mappings = import_config.get("field_mappings", {})
        print(
            f"🚀 [IMPORT_JOB] Auto-creating custom fields for mappings: {field_mappings}"
        )
        auto_create_custom_fields(field_mappings, user_id)
        print("🚀 [IMPORT_JOB] Custom fields auto-creation complete")

        # Start the import using the proven working import system
        def run_import():
            try:
                print(
                    f"🚀 [IMPORT_JOB] Starting background import thread for task {task_id}"
                )
                print(f"🚀 [IMPORT_JOB] Import config: {import_config}")
                ...
                print("🚀 [IMPORT_JOB] Updated job status to running")
        ...
        print("🚀 [IMPORT_JOB] Creating background thread...")
        import threading

        thread = threading.Thread(target=run_import)
        thread.daemon = True
        print("🚀 [IMPORT_JOB] Starting background thread...")
        thread.start()
        ...
        if verification_job:
            print(
                f"🚀 [IMPORT_JOB] Job {task_id} confirmed in safe storage after thread start"
            )
            logger.info(f"Job {task_id} confirmed in safe storage after thread start")
        else:
            print(
                f"❌ [IMPORT_JOB] Job {task_id} missing from safe storage after thread start"
            )
            logger.warning(
                f"Job {task_id} missing from safe storage after thread start"
            )
    ...
    try:
        ...
        if not default_locations:
            ...
            if not default_location:
                logger.info("🏠 Creating default location for onboarding import")
                ...
                if created_locations:
                    ...
                    if default_location and default_location.id:
                        ...
                        logger.info(
                            f"✅ Created and using default location: {default_location.name} (ID: {default_location.id})"
                        )
                    else:
                        logger.error("❌ Failed to get default location after creation")
                        default_locations = []
                else:
                    logger.error("❌ Failed to create default locations")
                    default_locations = []
            else:
                if default_location.id:
                    default_locations = [default_location.id]
                    logger.info(
                        f"✅ Using existing default location: {default_location.name} (ID: {default_location.id})"
                    )
                else:
                    logger.error("❌ Default location has no ID")
                    default_locations = []
            ...
            for row_num, row in enumerate(reader, 1):
                try:
                        ...
                        for csv_field, book_field in field_mappings.items():
                            ...
                            if value:  # Only process non-empty values
                                ...
                                if book_field.startswith("custom_"):
                                    ...
                                    custom_field_name = book_field.replace(
                                        "custom_", ""
                                    )
                                    personal_custom_metadata[custom_field_name] = value
                                    logger.info(
                                        f"📝 Added custom metadata: {custom_field_name} = {value}"
                                    )
                                else:
                    ...
                    if not book_data.get("title") and not book_data.get("isbn"):
                        ...
                        job["recent_activity"].append(
                            f"Row {row_num}: Skipped - no title or ISBN"
                        )
                        continue
                    ...
                    success = process_single_book_import(
                        book_data, user_id, default_locations, personal_custom_metadata
                    )

                    if success:
                        job["success"] += 1
                        job["recent_activity"].append(
                            f"Row {row_num}: Successfully imported '{title}'"
                        )
                        logger.info(
                            f"✅ Row {row_num}: Successfully imported '{title}'"
                        )
                        ...
                        job["errors"] += 1
                        job["recent_activity"].append(
                            f"Row {row_num}: Failed to import"
                        )
        ...
        logger.info(
            f"📊 CSV processing completed. Success: {job['success']}, Errors: {job['errors']}"
        )
...
def process_single_book_import(
    ...
    try:
        ...
        description = (
            book_data.get("description") or book_data.get("summary") or ""
        ) or unified.get("description")
        publisher = (book_data.get("publisher") or "") or unified.get("publisher")
        published_date = (book_data.get("published_date") or "") or unified.get(
            "published_date"
        )
        ...
        if not author:
            ...
            if auth_list:
                ...
                if len(auth_list) > 1:
                    additional_authors = (
                        ", ".join(
                            a for a in auth_list[1:] if isinstance(a, str) and a.strip()
                        )
                        or None
                    )
        ...
        if book_data.get("categories"):
            if isinstance(book_data["categories"], list):
                categories.extend(
                    [
                        c
                        for c in book_data["categories"]
                        if isinstance(c, str) and c.strip()
                    ]
                )
            elif isinstance(book_data["categories"], str):
                categories.extend(
                    [c.strip() for c in book_data["categories"].split(",") if c.strip()]
                )
        uni_cats = [
            c
            for c in (unified.get("categories") or [])
        ...
        service = SimplifiedBookService()
        success = service.add_book_to_user_library_sync(
            book_data=new_book, user_id=str(user_id)
        )
