...
def fetch_book_data(isbn):
    ...
    try:
        ...
        if book_key in data:
            ...
            # Enhanced metadata
            openlibrary_id = (
                book.get("key", "").replace("/books/", "") if book.get("key") else ""
            )
        ...
        current_app.logger.warning(
            f"Failed to fetch OpenLibrary book data for ISBN {isbn}: {e}"
        )
        return None
...
def search_author_by_name(author_name):
    ...
    try:
        ...
        current_app.logger.info(
            f"[OPENLIBRARY] Choosing author '{best_match['name']}' (ID: {author_id}, score: {best_match['score']}) from {len(candidates)} candidates"
        )
...
def get_google_books_cover(isbn, fetch_title_author=False):
    ...
    try:
                    ...
                    if isinstance(cat, str):
                        ...
                        for part in parts:
                            ...
                            if (
                                ...
                                and clean_part not in exclude_categories
                                and not clean_part.startswith("general")
                            ):
                                # Capitalize properly
        ...
        current_app.logger.error(
            f"Failed to fetch Google Books data for ISBN {isbn}: {e}"
        )
        return None
    if fetch_title_author:
        return None
...
def calculate_reading_streak(user_id, streak_offset=0):
    ...
    try:
        ...
        current_app.logger.error(
            f"Error calculating reading streak for user {user_id}: {e}"
        )
...
def generate_month_review_image(books, month, year):
    ...
    bg_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "static", "bookshelf.png")
    )
    try:
        bg = (
            Image.open(bg_path)
    ...
    shadow_offset = 4
    draw.text(
        ((img_size - w) // 2 + shadow_offset, 40 + shadow_offset),
        month_name,
        fill=(0, 0, 0, 128),
        font=font,
    )
    draw.text(
        ((img_size - w) // 2, 40), month_name, fill=(255, 255, 255, 255), font=font
    )
...
def normalize_goodreads_value(value, field_type="text"):
    ...
    if field_type == "isbn":
        ...
        if value and len(value) not in (10, 13):
            print(
                f"WARNING: Unexpected ISBN length after cleaning '{raw_before}' -> '{value}'"
            )
