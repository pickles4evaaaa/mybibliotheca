...
def index():
    ...
    try:
        ...
        user_fields = custom_field_service.get_user_fields_with_calculated_usage_sync(
            current_user.id
        )
    ...
    if request.method == "POST":
        try:
            ...
            current_app.logger.info(
                f"ℹ️ [METADATA_ROUTES] Received request to create custom field '{name}' for user {current_user.id}"
            )
            ...
            current_app.logger.info(
                f"ℹ️ [METADATA_ROUTES] Validation passed for custom field '{name}'. Creating definition."
            )
            ...
            if field_type in ["rating_5", "rating_10"]:
                ...
                if predefined_options:
                    field_def.predefined_options = [
                        opt.strip()
                        for opt in predefined_options.split(",")
                        if opt.strip()
                    ]
                field_def.allow_custom_options = (
                    request.form.get("allow_custom_options") == "on"
                )
            ...
            current_app.logger.info(
                f"ℹ️ [METADATA_ROUTES] Custom field definition for '{name}' created. Saving..."
            )

            # Check if custom field service is available (not a stub)
            if hasattr(custom_field_service, "__class__") and "StubService" in str(
                custom_field_service.__class__
            ):
                flash(
                    "Custom fields functionality is not yet available in this version.",
                    "warning",
                )
                current_app.logger.warning(
                    "⚠️ [METADATA_ROUTES] Custom field service is not implemented (stub service)"
                )
            ...
            field_data = {
                ...
                if hasattr(field_def.field_type, "value")
                else str(field_def.field_type),
                ...
                "allow_custom_options": getattr(
                    field_def, "allow_custom_options", False
                ),
            }

            success = custom_field_service.create_field_sync(
            ...
            if success:
                ...
                current_app.logger.info(
                    f"✅ [METADATA_ROUTES] Successfully saved custom field '{name}' for user {current_user.id}"
                )
            else:
                flash(
                    "Error creating custom field. Please check logs for details.",
                    "error",
                )
                current_app.logger.error(
                    f"❌ [METADATA_ROUTES] Failed to save custom field '{name}' for user {current_user.id}"
                )
            ...
            current_app.logger.error(
                f"❌ [METADATA_ROUTES] Unhandled exception in create_field: {e}",
                exc_info=True,
            )
...
def edit_field(field_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            updated_field = custom_field_service.update_field_sync(
                field_id, current_user.id, field_data
            )

            if updated_field:
                flash(
                    f'Custom field "{field_data["display_name"]}" updated successfully!',
                    "success",
                )
                return redirect(url_for("metadata.index"))
            else:
...
def delete_field(field_id):
    ...
    try:
        ...
        if success:
            flash(
                f'Custom field "{field_def.get("display_name", field_def.get("name"))}" deleted successfully!',
                "success",
            )
        else:
...
def templates():
    ...
    try:
        ...
        return render_template("metadata/templates.html", user_templates=user_templates)
    except Exception as e:
