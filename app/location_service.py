...
class LocationService:
    ...
    def __init__(self, kuzu_db_path: str | None = None):
        ...
        debug_log(
            "LocationService initialized with global connection management", "LOCATION"
        )
    ...
    def create_location(
        ...
        debug_log(
            f"Creating location: name='{name}', type='{location_type}', is_default={is_default}",
            "LOCATION",
            {"location_name": name, "location_type": location_type},
        )
        ...
        if is_default:
            debug_log("Clearing other default locations", "LOCATION")
            clear_defaults_query = """
            MATCH (l:Location) 
            SET l.is_default = false
            """
            safe_execute_kuzu_query(
                clear_defaults_query, {}, operation="clear_default_locations"
            )
            debug_log("Setting as default location", "LOCATION")
        ...
        safe_execute_kuzu_query(
            create_query, location_data, operation="location_operation"
        )
    ...
    def get_location(self, location_id: str) -> Location | None:
        ...
        results = safe_execute_kuzu_query(
            query, {"location_id": location_id}, operation="location_operation"
        )
    ...
    def update_location(self, location_id: str, **updates) -> Location | None:
        ...
        if updates.get("is_default") and not location.is_default:
            print(
                f"🏠 [UPDATE_LOCATION] Setting location {location_id} as default, clearing others"
            )
        ...
        if set_clauses:
            ...
            safe_execute_kuzu_query(
                update_query, params, operation="location_operation"
            )
    ...
    def delete_location(self, location_id: str) -> bool:
        ...
        safe_execute_kuzu_query(
            delete_query, {"location_id": location_id}, operation="location_operation"
        )
    ...
    def get_default_location(self) -> Location | None:
        ...
        result = safe_execute_kuzu_query(
            default_query, {}, operation="location_operation"
        )
        rows = _convert_query_result_to_list(result)
        if rows:
        ...
        """
        result = safe_execute_kuzu_query(
            all_locations_query, {}, operation="location_operation"
        )
        rows = _convert_query_result_to_list(result)
        if rows:
            node = rows[0].get("result") or rows[0].get("col_0")
    ...
    def get_location_book_count(
        ...
        """
        debug_log(
            f"Getting book count for location {location_id}, user filter: {user_id}",
            "LOCATION",
        ...
        try:
            ...
            debug_log(
                f"Location {location_id} has {count} books (user filter: {user_id})",
                "LOCATION",
            ...
            debug_log(
                f"Error getting book count for location {location_id}: {e}",
                "ERROR",
    ...
    def get_all_location_book_counts(
        ...
        """
        debug_log(
            f"Getting book counts for all locations (user filter: {user_id})",
            "LOCATION",
            {"user_id": user_id},
        )
    ...
    def get_books_at_location(
        ...
        """
        debug_log(
            f"Getting books at location {location_id} (user filter: {user_id})",
            "LOCATION",
            {"location_id": location_id, "user_id": user_id},
        )
        ...
        try:
            ...
            for r in rows:
                ...
                if book_id:
                    ...
                    debug_log(
                        f"Found book {book_id} at location",
                        "LOCATION",
                        {"book_id": book_id, "location_id": location_id},
                    )

            debug_log(
                f"Found {len(book_ids)} books at location {location_id}",
                "LOCATION",
                {
                    "location_id": location_id,
                    "user_id": user_id,
            ...
            debug_log(
                f"Error getting books at location {location_id}: {e}",
                "ERROR",
                {"location_id": location_id, "user_id": user_id, "error": str(e)},
            )
            import traceback
    ...
    def add_book_to_location(
        self, book_id: str, location_id: str, user_id: str
    ) -> bool:
        ...
        """
        debug_log(
            f"Adding book {book_id} to location {location_id} for user {user_id}",
            "LOCATION",
        )
        ...
        try:
            ...
            if int(val or 0) > 0:
                debug_log(
                    f"Book {book_id} already stored at location {location_id}",
                    "LOCATION",
                )
            ...
            debug_log(
                f"✅ Book {book_id} added to location {location_id} for user {user_id}",
                "LOCATION",
            )
            return True
    ...
    def remove_book_from_location(
        ...
        try:
            ...
            debug_log(
                f"✅ Book {book_id} removed from location {location_id}", "LOCATION"
            )
            return True
    ...
    def get_book_locations(
        ...
        try:
            ...
            debug_log(
                f"Found {len(locations)} locations for book {book_id}", "LOCATION"
            )
            return locations
    ...
    def set_book_location(
        ...
        """
        debug_log(
            f"Setting book {book_id} location to {location_id} for user {user_id}",
            "LOCATION",
        )
        ...
        try:
            ...
            if location_id:
                ...
                debug_log(
                    f"✅ Book {book_id} removed from all locations for user {user_id}",
                    "LOCATION",
                )
                return True
    ...
    def migrate_user_books_to_default_location(self, user_id: str) -> int:
        ...
        if not default_location:
            debug_log(
                f"No default location found for user {user_id}, cannot migrate",
                "LOCATION",
            )
            return 0
        ...
        try:
            ...
            result = safe_execute_kuzu_query(
                query, {"user_id": user_id}, operation="location_operation"
            )
            rows = _convert_query_result_to_list(result)
            migration_count = 0
            for r in rows:
                ...
                if (
                    book_id
                    and default_location.id
                    and self.add_book_to_location(
                        str(book_id), default_location.id, user_id
                    )
                ):
                    migration_count += 1
                    debug_log(
                        f"Migrated book {book_id} to default location {default_location.name}",
                        "LOCATION",
                    )

            debug_log(
                f"Successfully migrated {migration_count} books to default location for user {user_id}",
                "LOCATION",
            )
