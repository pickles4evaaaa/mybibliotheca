        ...
        if self._connection is None:
            try:
                ...
                if db_path.exists():
                    ...
                    if debug_mode:
                        for file in files[:10]:  # Limit to first 10 files
                            print(
                                f"📁 DB file: {file.name} ({file.stat().st_size} bytes)"
                            )
                        if len(files) > 10:
                ...
                if "std::bad_alloc" in str(e) or "Error during recovery" in str(e):
                    logger.error(
                        "Database recovery failed - this usually indicates corrupted or incomplete database files"
                    )

                    # Do NOT clear the database by default. Require explicit opt-in.
                    allow_clear = (
                        os.getenv("KUZU_AUTO_RECOVER_CLEAR", "false").lower() == "true"
                    )
                    if not allow_clear:
                        logger.error(
                            "Safety lock engaged: KUZU_AUTO_RECOVER_CLEAR is not true. Skipping destructive recovery to protect data."
                        )
                        logger.error(
                            "To force a rebuild, set KUZU_AUTO_RECOVER_CLEAR=true (optional: KUZU_DEBUG=true) and restart. Back up /app/data/kuzu first."
                        )
                        raise

                    logger.warning(
                        "Attempting destructive recovery by clearing database path (opt-in enabled)..."
                    )
                    try:
                        db_path = Path(self.database_path)
                        if db_path.exists():
                            ...
                            backup_dir = Path(
                                os.getenv("KUZU_BACKUP_DIR", "/app/data/backups")
                            )
                            ...
                            try:
                                ...
                                if db_path.is_dir():
                                    shutil.make_archive(
                                        str(backup_target), "gztar", root_dir=db_path
                                    )
                                else:
                                    # Single-file DB: copy to backups with timestamp
                                    shutil.copy2(
                                        str(db_path),
                                        str(backup_target.with_suffix(".db")),
                                    )
                            ...
                            try:
                                ...
                                logger.info(
                                    "Database path cleared, creating fresh database..."
                                )
                        ...
                        self._initialize_schema()
                        logger.info(
                            f"Kuzu connected at {self.database_path} (recovered)"
                        )
                    except Exception as recovery_error:
                        logger.error(f"Database recovery failed: {recovery_error}")
                        print("🔧 Make sure KuzuDB is running and accessible")
                        print(
                            "💡 If this is a fresh deployment, the database will be created automatically"
                        )
                        print(
                            "⚠️  If this persists, check that the data directory has proper permissions"
                        )
        ...
        try:
                ...
                try:
                    ...
                    if result and result.has_next():
                        try:
                            ...
                            _vals = (
                                _row if isinstance(_row, (list, tuple)) else list(_row)
                            )
                            try:
                        ...
                        if int(user_count or 0) > 0:
                            ...
                            logger.info(
                                f"Database contains {user_count} users - will ensure all tables exist"
                            )

                            # Check for books too
                            try:
                                book_result = self._execute_query(
                                    "MATCH (b:Book) RETURN COUNT(b) as count LIMIT 1"
                                )
                                if book_result and book_result.has_next():
                                    try:
                                        ...
                                        _valsb = (
                                            _rowb
                                            if isinstance(_rowb, (list, tuple))
                                            else list(_rowb)
                                        )
                                        ...
                                        book_count = 0
                                    logger.info(
                                        f"Database also contains {book_count} books"
                                    )
                            except Exception as book_e:
                                logger.debug(f"Error checking book count: {book_e}")
                        else:
                            logger.info(
                                "Database exists but is empty - will initialize schema"
                            )
                    else:
                    ...
                    logger.debug(
                        f"User table doesn't exist - will initialize schema: {e}"
                    )
            else:
                logger.warning(
                    "KUZU_FORCE_RESET=true - forcing schema reset (all data will be lost)"
                )
            ...
            if force_reset:
                ...
                for table in drop_tables:
                    try:
                        ...
                        logger.debug(
                            f"Table {table} doesn't exist or couldn't be dropped: {e}"
                        )
            elif has_existing_users:
                logger.info(
                    "🔧 Ensuring all tables exist (preserving existing data)..."
                )
                ...
                try:
                    ...
                    if self._connection:
                        self._connection.execute(
                            "MATCH (p:Person) RETURN p.openlibrary_id LIMIT 1"
                        )
                except Exception as e:
                    if "Cannot find property openlibrary_id" in str(e):
                        ...
                        for field_name, field_type in person_fields_to_add:
                            try:
                                if self._connection:
                                    self._connection.execute(
                                        f"ALTER TABLE Person ADD {field_name} {field_type}"
                                    )
                                    logger.debug(
                                        f"Added {field_name} column to Person table"
                                    )
                            except Exception as alter_e:
                                print(
                                    f"Note: Could not add {field_name} to Person table: {alter_e}"
                                )
                else:
                ...
                try:
                    if self._connection:
                        self._connection.execute(
                            "MATCH (rl:ReadingLog) RETURN rl.updated_at LIMIT 1"
                        )
                except Exception as e:
                    if "Cannot find property updated_at" in str(e):
                        try:
                            if self._connection:
                                self._connection.execute(
                                    "ALTER TABLE ReadingLog ADD updated_at TIMESTAMP"
                                )
                                logger.info(
                                    "Added updated_at column to ReadingLog table"
                                )
                        except Exception as alter_e:
                            print(
                                f"Note: Could not add updated_at to ReadingLog table: {alter_e}"
                            )
                ...
                try:
                    if self._connection:
                        self._connection.execute(
                            "MATCH (b:Book) RETURN b.opds_source_id LIMIT 1"
                        )
                except Exception as e:
                    if "Cannot find property opds_source_id" in str(e):
                        try:
                            if self._connection:
                                self._connection.execute(
                                    "ALTER TABLE Book ADD opds_source_id STRING"
                                )
                                logger.info("Added opds_source_id column to Book table")
                        except Exception as alter_e:
                            print(
                                f"Note: Could not add opds_source_id to Book table: {alter_e}"
                            )
                ...
                for column_name in ("opds_source_updated_at", "opds_source_entry_hash"):
                    try:
                        if self._connection:
                            self._connection.execute(
                                f"MATCH (b:Book) RETURN b.{column_name} LIMIT 1"
                            )
                    except Exception as e:
                        if "Cannot find property" in str(e) and column_name in str(e):
                            try:
                                if self._connection:
                                    self._connection.execute(
                                        f"ALTER TABLE Book ADD {column_name} STRING"
                                    )
                                    logger.info(
                                        "Added %s column to Book table", column_name
                                    )
                            except Exception as alter_e:
                                print(
                                    f"Note: Could not add {column_name} to Book table: {alter_e}"
                                )
            ...
            for i, query in enumerate(all_queries):
                try:
                    ...
                    tables_created += 1
                    logger.debug(
                        f"Successfully created table/relationship {i + 1}/{len(all_queries)}"
                    )
                except Exception as e:
            ...
            logger.info(
                f"✅ Kuzu schema ensured: {tables_created} created, {tables_existed} already existed"
            )
    ...
    def disconnect(self):
        ...
        if self._connection:
            ...
            if db_path.exists():
                ...
                try:
                    user_result = self._connection.execute(
                    ...
                    if user_result and user_result.has_next():  # type: ignore
                        try:
                            ...
                            try:
                                ...
                                user_count = (
                                    int(str(_urv[0]))
                                    if _urv and _urv[0] is not None
                                    else 0
                                )
                        except Exception:
                            user_count = 0  # type: ignore

                    book_result = self._connection.execute(
                    ...
                    if book_result and book_result.has_next():  # type: ignore
                        try:
                            ...
                            try:
                                ...
                                book_count = (
                                    int(str(_brv[0]))
                                    if _brv and _brv[0] is not None
                                    else 0
                                )
                        except Exception:
                    ...
                    try:
                        owns_enabled = os.getenv(
                            "ENABLE_OWNS_SCHEMA", "false"
                        ).lower() in ("1", "true", "yes")
                    except Exception:
                        owns_enabled = False
                    if owns_enabled:
                        owns_result = self._connection.execute(
                        ...
                        if owns_result and owns_result.has_next():  # type: ignore
                            try:
                                ...
                                _orv = (
                                    _or if isinstance(_or, (list, tuple)) else list(_or)
                                )
                                try:
                                    ...
                                    owns_count = (
                                        int(str(_orv[0]))
                                        if _orv and _orv[0] is not None
                                        else 0
                                    )
                            except Exception:
            ...
            if "already exists" in str(e).lower() or "duplicate" in str(e).lower():
                ...
                try:
                    ...
                    if handle_connection_error(str(e)):
                        logger.info(
                            "🔄 Connection recovered, but original query still failed"
                        )
                except Exception as recovery_error:
                    logger.debug(
                        f"Connection recovery attempt failed: {recovery_error}"
                    )
...
class KuzuGraphStorage:
    ...
    def store_node(self, node_type: str, node_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for ts_field in ["created_at", "updated_at"]:
                if ts_field in data:
                    print(
                        f"[KUZU_GRAPH][DEBUG] {ts_field} before serialization: {data[ts_field]} (type: {type(data[ts_field])})"
                    )
            ...
            for ts_field in ["created_at", "updated_at"]:
                if ts_field in serialized_data and isinstance(
                    serialized_data[ts_field], str
                ):
                    try:
                        serialized_data[ts_field] = datetime.fromisoformat(
                            serialized_data[ts_field]
                        )
                    except ValueError:
                        logger.warning(
                            f"Could not parse {ts_field}: {serialized_data[ts_field]}, leaving original value"
                        )
            ...
            if "custom_metadata" in serialized_data and isinstance(
                serialized_data["custom_metadata"], dict
            ):
                try:
                    serialized_data["custom_metadata"] = json.dumps(
                        serialized_data["custom_metadata"]
                    )
                except Exception:
            ...
            for k, v in serialized_data.items():
                if k == "parent_id":
                    ...
                    logger.debug(
                        f"[KUZU_GRAPH][DEBUG] Filtering out NULL/empty field {k}={v} for {node_type}"
                    )
            ...
            logger.info(
                f"[KUZU_GRAPH][DEBUG] Storing {node_type} with fields: {list(serialized_data.keys())}"
            )
            print(
                f"[KUZU_STORAGE] 🔧 Final fields to store: {list(serialized_data.keys())}"
            )

            # Handle published_date specifically - convert to date if needed
            if (
                "published_date" in serialized_data
                and serialized_data["published_date"] is not None
            ):
                ...
                if isinstance(pub_date, str):
                    try:
                        ...
                        if "T" in pub_date:
                            ...
                            # Just date string
                            serialized_data["published_date"] = datetime.fromisoformat(
                                pub_date
                            ).date()
                    except (ValueError, TypeError):
                        print(
                            f"[KUZU_GRAPH][WARNING] Could not parse published_date: {pub_date}, setting to None"
                        )
            ...
            for ts_field in ["created_at", "updated_at"]:
                if ts_field in serialized_data:
                    print(
                        f"[KUZU_GRAPH][DEBUG] {ts_field} for insertion: {serialized_data[ts_field]} (type: {type(serialized_data[ts_field])})"
                    )
            ...
            verification_query = (
                f"MATCH (n:{node_type}) WHERE n.id = $node_id RETURN COUNT(n) as count"
            )
            verify_result = self._execute_query(
                verification_query, {"node_id": node_id}
            )
            if verify_result and verify_result.has_next():
                ...
                if int(count) > 0:
                    print(
                        f"[KUZU_STORAGE] ✅ Verification: {node_type} node {node_id} exists in database"
                    )
                else:
                    print(
                        f"[KUZU_STORAGE] ❌ Verification failed: {node_type} node {node_id} not found after creation!"
                    )
            ...
            print(
                f"[KUZU_STORAGE] 🔍 Database path: {getattr(self.connection, 'database_path', 'Unknown')}"
            )
            import traceback
        ...
        try:
            ...
            if result and result.has_next():
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not convert {node_type} node to dict: {conv_error}"
                    )
    ...
    def update_node(
        ...
        try:
            print(
                f"🔧 [UPDATE_NODE] Called with node_type={node_type}, node_id={node_id}"
            )
            print(f"🔧 [UPDATE_NODE] Raw updates: {updates}")

            # Serialize updates
            # Convert custom_metadata dict to JSON string so it persists
            if "custom_metadata" in updates and isinstance(
                updates["custom_metadata"], dict
            ):
                try:
            ...
            try:
                print("🔧 [UPDATE_NODE] About to execute query...")
                ...
                print(
                    f"🔧 [UPDATE_NODE] Query execution failed: {type(exec_e).__name__}: {str(exec_e)}"
                )
                raise exec_e
    ...
    def create_relationship(
        ...
        try:
            ...
            print(
                f"[KUZU_STORAGE] 🔧 Relationship properties: {list(properties.keys()) if properties else 'None'}"
            )

            # Debug: Print incoming properties
            logger.info(
                f"[KUZU_GRAPH][DEBUG] Creating {rel_type} relationship with properties: {properties}"
            )
            ...
            for key, value in properties.items():
                ...
                is_datetime_field = (
                    key.endswith(("_at", "_date", "_time"))
                    or key.startswith(("date_", "time_"))
                    or key
                    in [
                        "date_added",
                        "date_finished",
                ...
                if (
                    ...
                    and (is_datetime_field or is_date_only_field)
                    and value
                ):
                    try:
                        ...
                        if "T" in value or value.count("-") >= 2:
                            ...
                            if is_date_only_field:
                                ...
                                logger.info(
                                    f"[KUZU_GRAPH][DEBUG] Converted {key} from '{value}' to date: {parsed_datetime.date()}"
                                )
                            else:
                                processed_props[key] = parsed_datetime
                                logger.info(
                                    f"[KUZU_GRAPH][DEBUG] Converted {key} from '{value}' to datetime: {parsed_datetime}"
                                )
                        else:
                        ...
                        processed_props[key] = value
                        logger.warning(
                            f"[KUZU_GRAPH][DEBUG] Failed to parse {key}='{value}' as datetime: {e}"
                        )
                else:
                    ...
                    elif key in ["locations", "user_tags", "moods"] and isinstance(
                        value, list
                    ):
            ...
            processed_props = {
                k: v for k, v in processed_props.items() if v is not None
            }
                ...
                try:
                    if processed_props["created_at"].endswith("Z"):
                        processed_props["created_at"] = (
                            processed_props["created_at"][:-1] + "+00:00"
                        )
                    processed_props["created_at"] = datetime.fromisoformat(
                        processed_props["created_at"]
                    )
                except (ValueError, TypeError):
                    processed_props["created_at"] = datetime.now(timezone.utc)
            ...
            print("[KUZU_STORAGE] 🚀 Executing CREATE RELATIONSHIP query")
            ...
            """
            verify_result = self._execute_query(
                verify_query, {"from_id": from_id, "to_id": to_id}
            )
            if verify_result and verify_result.has_next():
                _row = verify_result.get_next()
                _vals = self._row_values(_row)
                ...
                if int(count) > 0:
                    print(
                        f"[KUZU_STORAGE] ✅ Verification: {rel_type} relationship exists"
                    )
                else:
                    print(
                        f"[KUZU_STORAGE] ❌ Verification failed: {rel_type} relationship not found after creation!"
                    )
            ...
            print(
                f"[KUZU_STORAGE] ❌ Failed to create relationship {from_id} -{rel_type}-> {to_id}: {e}"
            )
            print(
                f"[KUZU_STORAGE] 🔍 Database path: {getattr(self.connection, 'database_path', 'Unknown')}"
            )
        ...
        try:
            ...
            if result:
                while result.has_next():
                    ...
                    relationships.append({"relationship": rel_data, "target": to_data})
    ...
    def delete_relationship(
        ...
        try:
            ...
            logger.error(
                f"Failed to delete relationship {from_id} -{rel_type}-> {to_id}: {e}"
            )
            return False

    def store_custom_metadata(
        self,
        user_id: str,
        book_id: str,
        field_name: str,
        ...
        try:
            ...
            owns_enabled = os.getenv("ENABLE_OWNS_SCHEMA", "false").lower() in (
                "1",
                "true",
                "yes",
            )
            if owns_enabled:
                try:
                    ...
                    """
                    result = self._execute_query(
                        query_get_owns, {"user_id": user_id, "book_id": book_id}
                    )
                    current_metadata = {}
                    if result and result.has_next():
                        _row = result.get_next()
            ...
            if result and result.has_next():
                ...
                self.kuzu_conn.execute(
                    query_increment, {"field_id": field_definition_id}
                )
                ...
                if not rel_exists:
                    ...
                    if field_definition_id is None:
                        logger.error(
                            "Cannot create HAS_CUSTOM_FIELD relationship: field_definition_id is None"
                        )
                    else:
                        success = self.create_relationship(
                            ...
                            "HAS_CUSTOM_FIELD",
                            "CustomField",
                            str(field_definition_id),
                            rel_props,
                        )
                        if success:
                            logger.info(
                                f"Created custom field relationship for field {field_definition_id}"
                            )
                        else:
                            logger.error(
                                f"Failed to create custom field relationship for field {field_definition_id}"
                            )
                else:
                ...
                print(
                    f"ℹ️ [STORE_CUSTOM_METADATA] No field definition found for '{field_name}' - stored in personal metadata only (OWNS skipped)"
                )
    ...
    def _force_checkpoint(self):
        ...
        try:
            ...
            print("🔄 [KUZU_CHECKPOINT] Forcing checkpoint to flush WAL to disk...")
    ...
    def safe_checkpoint(self):
        ...
        try:
            print("💾 [KUZU_SAFE] Performing safe checkpoint...")
            self.kuzu_conn.execute("CHECKPOINT;")
            print("✅ [KUZU_SAFE] Safe checkpoint completed successfully")
            return True
        except Exception as e:
...
def get_graph_storage() -> "KuzuGraphStorage":
    ...
    if _graph_storage is None:
        logger.warning(
            "🚨 DANGEROUS: Using thread-unsafe global KuzuDB storage! "
...
def safe_execute_kuzu_query(
    ...
    try:
        ...
        logger.error(
            "🚨 CRITICAL: safe_execute_query not available! Using dangerous fallback."
        )
...
def safe_get_kuzu_connection(
    ...
    try:
        ...
        logger.error(
            "🚨 CRITICAL: safe_get_connection not available! Using dangerous fallback."
        )
        # Fallback to dangerous global (temporary during migration)
        db = get_kuzu_database()
        return db.connect()
