    ...
    def create_node(self, node_type: str, node_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            for key, value in node_data.items():
                if key.endswith("_str") and (
                    "created_at" in key or "updated_at" in key
                ):
                    # Handle timestamp fields specially with empty-string guard
                    timestamp_field = key.replace("_str", "")
                    props.append(
                        f"{timestamp_field}: CASE WHEN ${key} IS NULL OR ${key} = '' THEN NULL ELSE timestamp(${key}) END"
                    )
                else:
...
class KuzuUserRepository:
    ...
    async def create(self, user: Any) -> Any | None:
        ...
        try:
            ...
            if result_data:
                logger.info(
                    f"✅ Created user: {getattr(user, 'username', 'unknown')} (ID: {getattr(user, 'id', 'unknown')})"
                )
                return user
            return None
    ...
    async def get_all(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in result_data:
                user_data = row["u"]
                users.append(
                    dict(user_data) if hasattr(user_data, "__dict__") else user_data
                )
    ...
    async def delete(self, user_id: str) -> bool:
        ...
        try:
            ...
            if not data:
                logger.info(
                    f"[USER_DELETE_DEBUG] Repo.delete user_id={user_id} NOT FOUND (nothing to delete)"
                )
                return False
            ...
            logger.error(
                f"[USER_DELETE_DEBUG] Repo.delete exception user_id={user_id} error={e}"
            )
            return False
...
class KuzuPersonRepository:
    ...
    def create(self, person: Any) -> Any | None:
        ...
        try:
            ...
            logger.info(
                f"🔍 [DEBUG] CREATE: Checking auto-fetch conditions for {person_name}: openlibrary_id={openlibrary_id}, bio='{bio}', birth_year={birth_year}, image_url={image_url}"
            )
                ...
                try:
                    ...
                    if search_result and search_result.get("openlibrary_id"):
                        ...
                        if detailed_author_data:
                            ...
                            from ..routes.people_routes import (
                                parse_comprehensive_openlibrary_data,
                            )

                            # Parse comprehensive data
                            updates = parse_comprehensive_openlibrary_data(
                                detailed_author_data
                            )
                            ...
                            logger.info(
                                f"✅ Auto-fetched comprehensive metadata for {person_name}: OpenLibrary ID {openlibrary_id}"
                            )
                            logger.info(f"📚 Applied fields: {list(updates.keys())}")
                        else:
                            logger.warning(
                                f"⚠️ Could not fetch detailed data for OpenLibrary ID: {author_id}"
                            )
                    else:
                    ...
                    logger.warning(
                        f"⚠️ Failed to auto-fetch metadata for {person_name}: {e}"
                    )
    ...
    async def get_by_id(self, person_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if results and len(results) > 0:
                print(
                    f"🔧 [PERSON_REPO] First result keys: {list(results[0].keys()) if results[0] else 'None'}"
                )
                ...
                if "result" in results[0]:
                    ...
                    print("🔧 [PERSON_REPO] No recognized key format found")
                    return None
            else:
                print("🔧 [PERSON_REPO] No results found")
                return None

        except Exception as e:
            print(
                f"🔧 [PERSON_REPO] Exception in get_by_id: {type(e).__name__}: {str(e)}"
            )
    ...
    async def get_by_name(self, name: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            results = self.db.query(
                query, {"normalized_name": normalized_name, "name": name}
            )
...
class KuzuBookRepository:
    ...
    async def create(self, book: Any) -> Any | None:
        ...
        try:
            ...
            for i, contrib in enumerate(contributors):
                ...
                person_name = (
                    getattr(person, "name", "unknown") if person else "no person"
                )
            ...
            book_data = {
                ...
                "normalized_title": getattr(book, "normalized_title", None)
                or getattr(book, "title", "").lower(),
            ...
            if not success:
                logger.error(
                    f"❌ Failed to create book node: {getattr(book, 'title', 'unknown')}"
                )
                return None
            ...
            if contributors:
                logger.info(
                    f"🔗 Creating {len(contributors)} contributor relationships"
                )
                for i, contribution in enumerate(contributors):
                    await self._create_contributor_relationship(
                        book_id, contribution, i
                    )
            ...
            if raw_categories:
                await self._create_category_relationships_from_raw(
                    book_id, raw_categories
                )
            ...
            logger.info(
                f"✅ Created book with all relationships: {getattr(book, 'title', 'unknown')}"
            )
            return book
    ...
    async def _create_contributor_relationship(
        self, book_id: str, contribution: Any, order_index: int = 0
    ):
        ...
        try:
            ...
            person_id = await self._ensure_person_exists(
                person, auto_fetch=auto_fetch_metadata
            )
            if not person_id:
                logger.warning(
                    f"⚠️ Could not create/find person: {getattr(person, 'name', 'unknown')}"
                )
                return
            ...
            if contribution_type and hasattr(contribution_type, "value"):
                ...
                contribution_str = (
                    str(contribution_type).upper() if contribution_type else "AUTHORED"
                )
            ...
            logger.info(
                f"🔍 [DEBUG] Creating relationship with role: {role}, contribution_str: {contribution_str}"
            )
            ...
            if success:
                logger.info(
                    f"✅ Created {rel_type} relationship: {getattr(person, 'name', 'unknown')} -> {book_id}"
                )
            else:
                logger.error(
                    f"❌ Failed to create {rel_type} relationship: {getattr(person, 'name', 'unknown')} -> {book_id}"
                )
    ...
    async def _ensure_person_exists(
        ...
        try:
            ...
            results = self.db.query(
                query, {"normalized_name": normalized_name, "name": person_name}
            )
            ...
            # Auto-fetch OpenLibrary metadata if not already provided
            logger.info(
                f"🔍 [DEBUG] Checking auto-fetch conditions for {person_name}: openlibrary_id={openlibrary_id}, bio='{bio}', birth_year={birth_year}, image_url={image_url}"
            )
            if (
                auto_fetch
                and not openlibrary_id
                and not bio
                and not birth_year
                and not image_url
            ):
    ...
    async def _create_category_relationships_from_raw(
        self, book_id: str, raw_categories: Any
    ):
        ...
        try:
            ...
            if isinstance(raw_categories, str):
                ...
                category_names = [
                    cat.strip() for cat in raw_categories.split(",") if cat.strip()
                ]
            elif isinstance(raw_categories, list):
                category_names = [
                    str(cat).strip() for cat in raw_categories if str(cat).strip()
                ]
            else:
                logger.warning(
                    f"⚠️ Unknown raw_categories format: {type(raw_categories)}"
                )
                return

            logger.info(
                f"🔗 Processing {len(category_names)} categories from raw data: {category_names}"
            )
            ...
            for category_name in category_names:
                ...
                if ("/" in category_name) or (">" in category_name):
                    try:
                        ...
                        parts = [
                            p.strip()
                            for p in re.split(r"[>/]", category_name)
                            if p.strip()
                        ]
                        if not parts:
                            continue
                        leaf_category_id = await self._ensure_category_path_exists(
                            parts
                        )
                        if leaf_category_id:
                            ...
                            if success:
                                logger.info(
                                    f"✅ Linked book {book_id} to leaf category path '{category_name}' (leaf id: {leaf_category_id})"
                                )
                            else:
                                logger.error(
                                    f"❌ Failed to link book {book_id} to leaf category id {leaf_category_id}"
                                )
                        else:
                            logger.warning(
                                f"⚠️ Could not resolve category path for: {category_name}"
                            )
                    except Exception as e:
                        logger.error(
                            f"❌ Failed processing hierarchical category '{category_name}': {e}"
                        )
                        # Fallback to flat handling
                        await self._create_category_relationship_by_name(
        ...
        try:
            ...
            for name in parts:
                ...
                if results and (results[0].get("result") or results[0].get("col_0")):
                    ...
                    if not created:
                        logger.error(
                            f"❌ Failed to create category in path: {name} (level {level})"
                        )
                        return None
                    ...
                    logger.info(
                        f"📁 Created category '{name}' (id={category_id}) under parent_id={parent_id}"
                    )
    ...
    async def _create_category_relationship_by_name(
        self, book_id: str, category_name: str
    ):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Created CATEGORIZED_AS relationship: {book_id} -> {category_name}"
                )
            else:
                logger.error(
                    f"❌ Failed to create CATEGORIZED_AS relationship: {book_id} -> {category_name}"
                )
    ...
    async def _ensure_category_exists(self, category_name: str) -> str | None:
        ...
        try:
            ...
            results = self.db.query(
                query, {"normalized_name": normalized_name, "name": category_name}
            )

            if results and (results[0].get("result") or results[0].get("col_0")):
                category_id = results[0].get("result") or results[0]["col_0"]
                logger.debug(
                    f"Found existing category: {category_name} (ID: {category_id})"
                )
                return category_id
            ...
            if success:
                logger.info(
                    f"✅ Created new category: {category_name} (ID: {category_id})"
                )
                return category_id
            else:
    ...
    async def _create_publisher_relationship(self, book_id: str, publisher: Any):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Created PUBLISHED_BY relationship: {book_id} -> {getattr(publisher, 'name', publisher)}"
                )
            else:
                logger.error(
                    f"❌ Failed to create PUBLISHED_BY relationship: {book_id} -> {getattr(publisher, 'name', publisher)}"
                )
    ...
    async def _ensure_publisher_exists(self, publisher: Any) -> str | None:
        ...
        try:
            ...
            if results and (results[0].get("result") or results[0].get("col_0")):
                ...
                logger.debug(
                    f"Found existing publisher: {publisher_name} (ID: {publisher_id})"
                )
                return publisher_id
            ...
            if success:
                logger.info(
                    f"✅ Created new publisher: {publisher_name} (ID: {publisher_id})"
                )
                return publisher_id
            else:
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                if "result" in result:
                    ...
                    logger.warning(
                        f"⚠️ Unexpected result structure for book {book_id}: {result}"
                    )
                    return result
                ...
                if book_data is not None:
                    if hasattr(book_data, "__dict__"):
                        ...
                        logger.info(
                            f"✅ Converted book object to dict with keys: {list(book_dict.keys())}"
                        )
                        return book_dict
                    elif isinstance(book_data, dict):
                        logger.info(
                            f"✅ Book data is already dict with keys: {list(book_data.keys())}"
                        )
                        return book_data
                    else:
                        logger.warning(
                            f"⚠️ Book data is unexpected type: {type(book_data)}"
                        )
    ...
    async def search(self, query_text: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        try:
            ...
            results = self.db.query(
                query, {"query_text": query_text.lower(), "limit": limit}
            )
                    ...
                    contributors.append(
                        {
                            ...
                            "role": result.get(
                                "col_2", "authored"
                            ),  # Default to 'authored' if no role
    ...
    async def get_all_persons(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("🔍 [DEBUG] Executing get_all_persons query")
            results = self.db.query(query)
            ...
            for i, result in enumerate(results):
                ...
                if "col_0" in result and "col_1" in result:
                    ...
                    logger.info(
                        f"🔍 [DEBUG] Using col_0/col_1 format: {person_data['name'] if 'name' in person_data else 'unknown'} with {book_count} books"
                    )
                    ...
                    book_count = 0
                    logger.info(
                        f"🔍 [DEBUG] Using result format: {person_data['name'] if 'name' in person_data else 'unknown'}"
                    )
    ...
    async def get_all_categories(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("🔍 [DEBUG] Executing get_all_categories query")
            results = self.db.query(query)
            ...
            for i, result in enumerate(results):
                ...
                if "result" in result:
                    ...
                    logger.info(
                        f"🔍 [DEBUG] Using result format: {category_data['name'] if 'name' in category_data else 'unknown'}"
                    )
                    categories.append(category_data)
                elif "col_0" in result:
                    category_data = dict(result["col_0"])
                    logger.info(
                        f"🔍 [DEBUG] Using col_0 format: {category_data['name'] if 'name' in category_data else 'unknown'}"
                    )
                    categories.append(category_data)
    ...
    async def delete(self, book_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully deleted book and all relationships: {book_id}"
            )
...
class KuzuUserBookRepository:
    ...
    async def add_book_to_library(
        ...
        try:
            ...
            logger.info(
                f"✅ Added (personal overlay) book {book_id} for user {user_id}"
            )
            return True
        except Exception as e:
            logger.error(
                f"❌ Failed to add book to library (personal metadata path): {e}"
            )
            return False
    ...
    async def get_user_books(
        ...
        try:
            ...
            for result in results:
                ...
                pm = personal_metadata_service.get_personal_metadata(
                    user_id, book_node.get("id", "")
                )
    ...
    async def update_reading_status(
        self, user_id: str, book_id: str, new_status: str
    ) -> bool:
        ...
        try:
            ...
            personal_metadata_service.update_personal_metadata(
                user_id, book_id, custom_updates={"reading_status": new_status}
            )
            return True
        except Exception as e:
    ...
    async def create_ownership(
        ...
        try:
            ...
            ownership_status_str = getattr(
                ownership_status, "value", str(ownership_status)
            )
            ...
            logger.info(
                f"✅ (Deprecated OWNS) stored personal metadata for user {user_id} book {book_id}"
            )
            return True
        except Exception as e:
    ...
    async def remove_ownership(self, user_id: str, book_id: str) -> bool:
        ...
        try:
            ...
            if result:
                logger.info(
                    f"✅ Removed personal metadata (legacy ownership) for user {user_id} book {book_id}"
                )
                return True
            logger.warning(
                f"⚠️ No personal metadata to remove for user {user_id} book {book_id}"
            )
            return False
        except Exception as e:
            logger.error(
                f"❌ Failed to remove personal metadata (legacy ownership): {e}"
            )
            return False
    ...
    async def get_reading_timeline(
        ...
        try:
            ...
            for row in results:
                ...
                blob_raw = (
                    rel_dict.get("personal_custom_fields")
                ...
                timeline.append(
                    {
                        ...
                        "activity_date": rel_dict.get("updated_at")
                        if isinstance(rel_dict, dict)
                        else None or book_data.get("created_at"),
                        "reading_status": meta.get("reading_status"),
                        "ownership_status": meta.get("ownership_status"),
                        "media_type": meta.get("media_type"),
                        "notes": (
                            rel_dict.get("personal_notes")
                            if isinstance(rel_dict, dict)
                            else None
                        )
                        or meta.get("personal_notes")
                        or "",
                    }
                )
            return timeline
...
class KuzuLocationRepository:
    ...
    async def create(self, location: Any, user_id: str) -> Any | None:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Created location: {getattr(location, 'name', 'unknown')}"
                )
                return location
...
class KuzuCategoryRepository:
    ...
    async def create(self, category: Any) -> Any | None:
        ...
        try:
            ...
            logger.info(
                f"📁 [CREATE_CATEGORY] Creating category '{category_name}' with parent_id: {parent_id} (type: {type(parent_id)})"
            )
    ...
    async def update(self, category: Any) -> Any | None:
        ...
        try:
            ...
            if results:
                logger.info(
                    f"✅ Updated category: {getattr(category, 'name', 'unknown')} (ID: {category_id})"
                )
                return category
            else:
                logger.error(
                    f"❌ Failed to update category: {getattr(category, 'name', 'unknown')}"
                )
                return None
...
class KuzuCustomFieldRepository:
    ...
    async def create(self, field_def: Any) -> Any | None:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Created custom field: {getattr(field_def, 'name', 'unknown')}"
                )
                return field_def
            return None
    ...
    async def search(self, query: str, user_id: str | None = None) -> list[Any]:
        ...
        try:
            ...
            query_conditions.append(
                "(cf.name CONTAINS $search_query OR cf.description CONTAINS $search_query)"
            )

            if user_id:
                query_conditions.append(
                    "(cf.created_by_user_id = $user_id OR cf.is_global = true)"
                )
                params["user_id"] = user_id
            else:
...
class KuzuImportMappingRepository:
    ...
    async def create(self, template: Any) -> Any | None:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Created import mapping template: {getattr(template, 'name', 'unknown')}"
                )
                return template
            return None
    ...
    async def get_by_user(self, user_id: str) -> list[Any]:
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to get import mapping templates for user {user_id}: {e}"
            )
            return []
    ...
    async def delete(self, template_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to delete import mapping template {template_id}: {e}"
            )
            return False
    ...
    async def increment_usage(self, template_id: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Could not increment usage count for template {template_id}: {e}"
            )
