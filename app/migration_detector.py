...
class MigrationDetector:
    ...
    def find_sqlite_databases(self) -> List[Path]:
        ...
        logger.info(
            f"📚 Found {len(db_files)} Bibliotheca database(s): {[f.name for f in db_files]}"
        )
        return db_files
    ...
    def _is_bibliotheca_database(self, db_path: Path) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"   Database {db_path.name}: tables={tables}, has_book_table={has_book_table}"
            )
    ...
    def analyze_database(self, db_path: Path) -> Dict:
        ...
        try:
            ...
            logger.debug(
                f"   Database {db_path.name}: {total_books} books, {total_users} users, {total_logs} logs"
            )
...
def check_migration_at_startup() -> Optional[Dict]:
    ...
    if migration_info:
        logger.info(
            f"🔍 Found {migration_info['total_databases']} SQLite database(s) with {migration_info['total_books']} books"
        )
...
def auto_migrate_if_safe(migration_info: Dict, default_user_id: str = "admin") -> bool:
    ...
    try:
        ...
        if needs_user_id and not has_user_id:
            logger.warning(
                "⚠️  Cannot auto-migrate: v1 databases found but no default user ID provided"
            )
        ...
        logger.info(
            f"📊 Migrating {migration_info['total_databases']} database(s) with {migration_info['total_books']} books"
        )
        ...
        for db_info in migration_info["databases"]:
            try:
                ...
                logger.info(
                    f"🔄 Migrating {db_info['path'].name} (v{db_info['schema_version']}, {db_info['total_books']} books)..."
                )
        ...
        if migrated_count == migration_info["total_databases"]:
            ...
            logger.warning(
                f"⚠️  Partial migration: {migrated_count}/{migration_info['total_databases']} databases migrated"
            )
            return False
