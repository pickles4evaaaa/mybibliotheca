...
def check_migration_status():
    ...
    try:
        ...
        if not databases:
            return jsonify(
                {"migration_needed": False, "message": "No SQLite databases found"}
            )
        ...
        return jsonify({"migration_needed": False, "error": str(e)}), 500
...
def configure_migration():
    ...
    try:
        ...
        logger.info(
            f"🚀 Migration configured for user: {display_user_name} (ID: {current_user.id})"
        )
...
def run_migration(migration_id):
    ...
    try:
        ...
        logger.info(
            f"🚀 Starting migration for user: {current_user.username} (ID: {current_user_id})"
        )
                    ...
                    results.append(
                        {
                            ...
                            "reading_logs_migrated": migrator.stats[
                                "reading_logs_migrated"
                            ],
                            "relationships_created": migrator.stats[
...
def dismiss_migration():
    ...
    session["migration_dismissed"] = True
    flash(
        "Migration reminder dismissed. You can always access migration from the admin panel.",
        "info",
    )
    return redirect(url_for("main.index"))
