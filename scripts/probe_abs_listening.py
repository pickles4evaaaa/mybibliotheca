...
def main():
    ...
    for page in range(max(1, args.pages)):
        ...
        attempts = [
            (
                f"{base_url.rstrip('/')}/api/me/listening-sessions",
                {"itemsPerPage": args.limit, "page": page},
            ),
            (
                f"{base_url.rstrip('/')}/api/me/sessions",
                {"itemsPerPage": args.limit, "page": page},
            ),
            (
        ...
        for url, params in attempts:
            ...
            if isinstance(payload, dict):
                ...
                total = (
                    payload.get("total") or payload.get("totalItems") or len(sessions)
                )
                try:
                    total = int(total)
                except Exception:
                ...
                break
        print(
            f"Page {page}: ok={(len(sessions) > 0)} total={total} returned={len(sessions)} msg={last_msg}"
        )
        if not sessions:
            break
        ...
        for s in sessions[:5]:
            dur = (
                s.get("duration") or s.get("msPlayed") or s.get("secondsListened") or 0
            )
            ...
            print(
                {
                    ...
                    "bookId": s.get("bookId"),
                    "startedAt": iso_from_epoch(
                        s.get("startedAt") or s.get("startTime") or s.get("createdAt")
                    ),
                    "updatedAt": iso_from_epoch(
                        s.get("updatedAt") or s.get("lastUpdate")
                    ),
                    "positionMs": s.get("positionMs")
                    or s.get("currentTimeMs")
                    or s.get("position"),
                    "msPlayed": s.get("msPlayed")
                    or s.get("timeListenedMs")
                    or s.get("playedMs"),
                    "secondsListened": s.get("secondsListened")
                    or s.get("timeListened")
                    or s.get("playedSeconds"),
                    "isFinished?": s.get("isFinished")
                    or s.get("finished")
                    or s.get("completed")
                    or s.get("isCompleted")
                    or s.get("isComplete"),
