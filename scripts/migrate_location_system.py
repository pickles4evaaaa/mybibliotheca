...
def migrate_user_locations(user_id: str, username: str):
    ...
    if not locations:
        print("  ⚠️  User has no locations, setting up defaults...")
    ...
    if old_system_books:
        print(
            f"  🔄 Found {len(old_system_books)} books with old location_id properties"
        )
        ...
        for book_id, old_location_id in old_system_books:
            ...
            if location and location.user_id == user_id:
                ...
                if location_service.add_book_to_location(
                    book_id, old_location_id, user_id
                ):
                ...
                if default_location and default_location.id:
                    if location_service.add_book_to_location(
                        ...
                        print(
                            f"    ✅ Migrated book {book_id[:8]}... to default location: {default_location.name}"
                        )
                        migrated_count += 1
                    else:
                        print(
                            f"    ❌ Failed to migrate book {book_id[:8]}... to default location"
                        )
    ...
    if unlocated_count > 0:
        print(
            f"  🏠 Migrated {unlocated_count} books without location to default location"
        )
...
def clean_old_location_properties(user_id: str):
    ...
    print("  🧹 Cleaning up old location_id properties...")
    ...
    try:
        ...
        print("  ✅ Cleaned up old location properties")
...
def verify_migration(user_id: str, username: str):
    ...
    if total_books_new_system == total_books:
        ...
        print("  ⚠️  Migration verification failed - some books may not have locations")
        return False
...
def main():
    ...
    print(
        "This will migrate from OWNS.location_id properties to STORED_AT relationships"
    )
    print()
