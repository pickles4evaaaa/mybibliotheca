    ...
    with app.app_context():
        ...
        if args.identifier:
            ...
            while not selected_user:
                choice = input(
                    "Select a user by number or enter username/email: "
                ).strip()
                if not choice:
                    continue
                if choice.isdigit():
...
def list_users(args):
    ...
    with app.app_context():
        ...
        try:
            ...
            print("-" * 80)
            print(
                f"{'Username':<20} {'Email':<30} {'Admin':<8} {'Active':<8} {'Created'}"
            )
            print("-" * 80)
            ...
            for user in sorted_users:
                ...
                print(
                    f"{user.username:<20} {user.email:<30} {admin_status:<8} {active_status:<8} {created_date}"
                )
...
def system_stats(args):
    ...
    with app.app_context():
        ...
        print()
        print("📚 Data:")
        print("   Books: (Redis service not implemented)")
        print("   Reading Logs: (Redis service not implemented)")
        print()
        ...
        if os.path.exists(db_path):
            ...
            print("💾 SQLite Database (for migration):")
...
Examples:
  ...
  python3 admin_tools.py system-stats
    ...
    reset_parser = subparsers.add_parser(
        "reset-admin-password", help="Reset admin user password"
    )
    reset_parser.add_argument(
        "--password", help="New password (if not provided, will prompt securely)"
    )

    # Reset any user password
    reset_user_parser = subparsers.add_parser(
        "reset-user-password",
        help="Reset password for any user (interactive by default)",
    )
    reset_user_parser.add_argument(
        "--identifier", help="Username or email to reset (optional)"
    )
    reset_user_parser.add_argument(
        "--password", help="New password (if not provided, will prompt securely)"
    )
    ...
    create_parser = subparsers.add_parser(
        "create-admin", help="Create a new admin user"
    )
    create_parser.add_argument("--username", help="Admin username")
    create_parser.add_argument("--email", help="Admin email")
    create_parser.add_argument(
        "--password", help="Admin password (if not provided, will prompt securely)"
    )
    create_parser.add_argument(
        "--force", action="store_true", help="Create admin even if one exists"
    )

    # Promote user
    promote_parser = subparsers.add_parser(
        "promote-user", help="Grant admin privileges to user"
    )
    ...
    stats_parser = subparsers.add_parser(
        "system-stats", help="Display system statistics"
    )
