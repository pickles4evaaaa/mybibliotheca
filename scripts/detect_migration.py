...
def main():
    ...
    has_v1 = any(
        detector.analyze_database(db)["schema_version"] == 1 for db in databases
    )
    has_v2 = any(
        detector.analyze_database(db)["schema_version"] == 2 for db in databases
    )
    ...
    if has_v1 and not has_v2:
        ...
        for db_path in databases:
            ...
            if info["schema_version"] == 1:
                print(
                    f"   python3 scripts/migrate_sqlite_to_redis.py --db-path {db_path} --user-id <your-admin-username>"
                )
            else:
                print(
                    f"   python3 scripts/migrate_sqlite_to_redis.py --db-path {db_path}"
                )

    print()
    print("💾 Note: All databases will be backed up before migration")
    print(
        "🌐 Web-based migration: Log into http://localhost:5054 and use the migration wizard"
    )
