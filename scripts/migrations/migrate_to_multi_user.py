...
def migrate_books_to_user(user):
    ...
    try:
        ...
        result = db.session.execute(
            "SELECT COUNT(*) FROM book WHERE user_id IS NULL"
        ).fetchone()
...
def migrate_reading_logs_to_user(user):
    ...
    try:
        ...
        result = db.session.execute(
            "SELECT COUNT(*) FROM reading_log WHERE user_id IS NULL"
        ).fetchone()
        orphaned_count = result[0] if result else 0
        ...
        print(
            f"📖 Migrating {orphaned_count} reading logs to user '{user.username}'..."
        )
...
def run_migration():
    ...
    with app.app_context():
        ...
        try:
            orphaned_books_result = db.session.execute(
                "SELECT COUNT(*) FROM book WHERE user_id IS NULL"
            ).fetchone()
        ...
        try:
            orphaned_logs_result = db.session.execute(
                "SELECT COUNT(*) FROM reading_log WHERE user_id IS NULL"
            ).fetchone()
        ...
        if total_users == 0:
            ...
            if not admin_user:
                ...
                db.session.commit()
                print(
                    f"✅ Granted admin privileges to existing user: {admin_user.username}"
                )
