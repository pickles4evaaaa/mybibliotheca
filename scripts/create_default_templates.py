...
def create_goodreads_template():
    ...
    template = ImportMappingTemplate(
        ...
        field_mappings={
            "Book Id": {
                "action": "map_existing",
                "target_field": "custom_global_goodreads_book_id",
            },
            "Title": {"action": "map_existing", "target_field": "title"},
            "Author": {"action": "map_existing", "target_field": "author"},
            "Author l-f": {
                "action": "skip"
            },  # Skip alternate author format - we'll use primary Author field
            "Additional Authors": {
                "action": "map_existing",
            ...
            "Average Rating": {
                "action": "map_existing",
                "target_field": "custom_global_average_rating",
            },
            "Publisher": {"action": "map_existing", "target_field": "publisher"},
            "Binding": {
                "action": "map_existing",
                "target_field": "custom_global_binding",
            },
            ...
            "Original Publication Year": {
                "action": "map_existing",
                "target_field": "custom_global_original_publication_year",
            },
            ...
            },  # Fixed: Bookshelves are categories, not reading status
            "Bookshelves with positions": {
                "action": "map_existing",
                "target_field": "categories",
            },  # Fixed: These are also categories
            "Exclusive Shelf": {
                "action": "map_existing",
                "target_field": "reading_status",
            },  # Correct: This is the reading status
            "My Review": {"action": "map_existing", "target_field": "notes"},
            "Spoiler": {
                "action": "map_existing",
                "target_field": "custom_global_spoiler_review",
            },
            "Private Notes": {
                "action": "map_existing",
                "target_field": "custom_personal_private_notes",
            },
            "Read Count": {
                "action": "map_existing",
                "target_field": "custom_global_read_count",
            },
            "Owned Copies": {
                "action": "map_existing",
                "target_field": "custom_personal_owned_copies",
            },
        },
        times_used=0,
        last_used=None,
...
def create_storygraph_template():
    ...
    template = ImportMappingTemplate(
        ...
        field_mappings={
            ...
            "Format": {
                "action": "map_existing",
                "target_field": "custom_global_format",
            },
            ...
            "Dates Read": {
                "action": "skip"
            },  # Skip - complex format that's hard to parse
            "Read Count": {
                "action": "map_existing",
                "target_field": "custom_global_read_count",
            },
            ...
            "Moods": {
                "action": "map_existing",
                "target_field": "custom_global_moods",
            },  # Fixed: Create separate field for moods
            "Pace": {"action": "map_existing", "target_field": "custom_global_pace"},
            "Character- or Plot-Driven?": {
                "action": "map_existing",
                "target_field": "custom_global_character_plot_driven",
            },
            "Strong Character Development?": {
                "action": "map_existing",
                "target_field": "custom_global_strong_character_development",
            },
            "Loveable Characters?": {
                "action": "map_existing",
                "target_field": "custom_global_loveable_characters",
            },
            "Diverse Characters?": {
                "action": "map_existing",
                "target_field": "custom_global_diverse_characters",
            },
            "Flawed Characters?": {
                "action": "map_existing",
                "target_field": "custom_global_flawed_characters",
            },
            "Content Warnings": {
                "action": "map_existing",
                "target_field": "custom_global_content_warnings",
            },
            "Content Warning Description": {
                "action": "map_existing",
                "target_field": "custom_global_content_warning_description",
            },
            "Owned?": {
                "action": "map_existing",
                "target_field": "custom_personal_owned",
            },
        },
        times_used=0,
        last_used=None,
...
def main():
    ...
    def _create_or_update_template(create_func, template_id, template_name):
        ...
        try:
            ...
            existing_template = import_mapping_service.get_template_by_id_sync(
                template_id
            )

            if existing_template:
                print(
                    f"Found existing {template_name} template - updating with new mappings..."
                )
            ...
            print(
                f"Note: Could not check for existing {template_name} template ({e}), creating new one..."
            )
            template = create_func()
            saved_template = import_mapping_service.create_template_sync(template)
            print(f"✅ Created {template_name} template: {saved_template.name}")
    ...
    try:
        ...
        with app.app_context():
            _create_or_update_template(
                create_goodreads_template, "default_goodreads", "Goodreads"
            )
            _create_or_update_template(
                create_storygraph_template, "default_storygraph", "StoryGraph"
            )
